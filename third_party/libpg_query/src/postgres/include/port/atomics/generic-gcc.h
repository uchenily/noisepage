...
#if !defined(PG_HAVE_ATOMIC_FLAG_SUPPORT) && (defined(HAVE_GCC__SYNC_INT32_TAS) || defined(HAVE_GCC__SYNC_CHAR_TAS))
...
#if !defined(PG_HAVE_ATOMIC_U32_SUPPORT) && (defined(HAVE_GCC__ATOMIC_INT32_CAS) || defined(HAVE_GCC__SYNC_INT32_CAS))
...
#if !defined(PG_HAVE_ATOMIC_U64_SUPPORT) && !defined(PG_DISABLE_64_BIT_ATOMICS)                                        \
...
pg_atomic_compare_exchange_u32_impl(volatile pg_atomic_uint32 *ptr, uint32 *expected, uint32 newval) {
    ...
    return __atomic_compare_exchange_n(&ptr->value, expected, newval, false, __ATOMIC_SEQ_CST, __ATOMIC_SEQ_CST);
}
#endif
