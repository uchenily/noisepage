...
typedef enum PgStat_Shared_Reset_Target { RESET_ARCHIVER, RESET_BGWRITER } PgStat_Shared_Reset_Target;
...
#define PGSTAT_NUM_TABENTRIES                                                                                          \
    ((PGSTAT_MSG_PAYLOAD - sizeof(Oid) - 3 * sizeof(int) - 2 * sizeof(PgStat_Counter)) / sizeof(PgStat_TableEntry))
...
extern void pgstat_report_vacuum(Oid tableoid, bool shared, PgStat_Counter livetuples, PgStat_Counter deadtuples);
...
extern const char *pgstat_get_crashed_backend_activity(int pid, char *buffer, int buflen);
