...
#define REGBUF_WILL_INIT                                                                                               \
    (0x04 | 0x02) /* page will be re-initialized                                                                       \
                   * at replay (implies                                                                                \
                   * NO_IMAGE) */
#define REGBUF_STANDARD                                                                                                \
    0x08 /* page follows "standard" page layout, (data                                                                 \
          * between pd_lower and pd_upper will be                                                                      \
          * skipped) */
#define REGBUF_KEEP_DATA                                                                                               \
    0x10 /* include data even if a full-page image is                                                                  \
          * taken */
...
extern void
XLogRegisterBlock(uint8 block_id, RelFileNode *rnode, ForkNumber forknum, BlockNumber blknum, char *page, uint8 flags);
extern void XLogRegisterBufData(uint8 block_id, char *data, int len);
extern void XLogResetInsertion(void);
extern bool XLogCheckBufferNeedsBackup(Buffer buffer);

extern XLogRecPtr log_newpage(RelFileNode *rnode, ForkNumber forkNum, BlockNumber blk, char *page, bool page_std);
