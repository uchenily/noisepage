...
#define WRITE_ENUM_FIELD(fldname) appendStringInfo(str, "\"" CppAsString(fldname) "\": %d, ", (int) node->fldname)

/* Write a float field */
#define WRITE_FLOAT_FIELD(fldname) appendStringInfo(str, "\"" CppAsString(fldname) "\": %f, ", node->fldname)
...
static void removeTrailingDelimiter(StringInfo str) {
...
static void _outToken(StringInfo buf, const char *str) {
    if (str == NULL) {
