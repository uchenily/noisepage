...
static bool create_function_stmts_walker(Node *node, createFunctionStmts *state) {
    ...
    if (IsA(node, CreateFunctionStmt)) {
        if (state->stmts_count >= state->stmts_buf_size) {
            ...
            state->stmts
                = (CreateFunctionStmt **) repalloc(state->stmts, state->stmts_buf_size * sizeof(CreateFunctionStmt *));
        }
...
PgQueryPlpgsqlParseResult pg_query_parse_plpgsql(const char *input) {
    ...
    for (i = 0; i < statements.stmts_count; i++) {
        ...
        // These are all malloc-ed and will survive exiting the memory context, the caller is responsible to free them
        // now
