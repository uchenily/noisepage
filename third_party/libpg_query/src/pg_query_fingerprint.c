...
static void _fingerprintNode(FingerprintContext *ctx,
                             const void         *obj,
                             const void         *parent,
...
static void _fingerprintString(FingerprintContext *ctx, const char *str) {
    if (ctx->sha1 != NULL) {
...
static void _fingerprintInteger(FingerprintContext *ctx, const Value *node) {
    if (node->val.ival != 0) {
...
static void _fingerprintFloat(FingerprintContext *ctx, const Value *node) {
...
static void _fingerprintBitString(FingerprintContext *ctx, const Value *node) {
    if (node->val.str != NULL) {
...
_fingerprintList(FingerprintContext *ctx, const List *node, const void *parent, char *field_name, unsigned int depth) {
    if (field_name != NULL
        && (strcmp(field_name, "fromClause") == 0 || strcmp(field_name, "targetList") == 0
...
static void _fingerprintCopyTokens(FingerprintContext *source, FingerprintContext *target, char *field_name) {
    ...
    if (dlist_is_empty(&source->tokens))
        return;
...
void _fingerprintNode(FingerprintContext *ctx,
                      const void         *obj,
                      const void         *parent,
                      char               *field_name,
                      unsigned int        depth) {
...
#include "pg_query_fingerprint_conds.c"
        ...
        default:
            elog(WARNING, "could not fingerprint unrecognized node type: %d", (int) nodeTag(obj));
