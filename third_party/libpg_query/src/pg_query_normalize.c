 ...
 */
static void fill_in_constant_lengths(pgssConstLocations *jstate, const char *query) {
        ...
        for (;;) {
            ...
            if (yylloc >= loc) {
                ...
                if (locs[i].length > 4 && /* U&'' */
                    (yyextra.scanbuf[loc] == 'u' || yyextra.scanbuf[loc] == 'U') && yyextra.scanbuf[loc + 1] == '&'
                    && yyextra.scanbuf[loc + 2] == '\'') {
 ...
 */
static char *generate_normalized_query(pgssConstLocations *jstate, const char *query, int *query_len_p, int encoding) {
    char *norm_query;
...
static bool const_record_walker(Node *node, pgssConstLocations *jstate) {
    ...
    if ((IsA(node, A_Const) && ((A_Const *) node)->location >= 0)
        || (IsA(node, DefElem) && ((DefElem *) node)->location >= 0)) {
        /* enlarge array if needed */
        if (jstate->clocations_count >= jstate->clocations_buf_size) {
            jstate->clocations_buf_size *= 2;
            jstate->clocations = (pgssLocationLen *) repalloc(jstate->clocations,
                                                              jstate->clocations_buf_size * sizeof(pgssLocationLen));
        }
        jstate->clocations[jstate->clocations_count].location
            = IsA(node, DefElem) ? ((DefElem *) node)->location : ((A_Const *) node)->location;
