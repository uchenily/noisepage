...
public:
    ...
    void swap(cuckoohash_map &other) noexcept {
        ...
        other.maximum_hashpower_.store(
            maximum_hashpower_.exchange(other.maximum_hashpower(), std::memory_order_release),
            std::memory_order_release);
    }
    ...
    bool erase(const K &key) {
        return erase_fn(key, [](mapped_type &) {
            return true;
        });
    }
...
private:
    ...
    public:
        ...
        counter_type &elem_counter() noexcept {
            return elem_counter_;
        }
        counter_type elem_counter() const noexcept {
            return elem_counter_;
        }

        bool &is_migrated() noexcept {
            return is_migrated_;
        }
    ...
    struct LockDeleter {
        void operator()(spinlock *l) const {
            l->unlock();
        }
    };
    ...
    lock_three(size_type, size_type i1, size_type i2, size_type, locked_table_mode) const {
        return std::make_pair(TwoBuckets(i1, i2, locked_table_mode()), LockManager());
    }

    std::pair<TwoBuckets, LockManager>
    lock_three(size_type hp, size_type i1, size_type i2, size_type i3, normal_mode) const {
        ...
        return std::make_pair(TwoBuckets(locks, i1, i2, normal_mode()),
                              LockManager((lock_ind(i3) == lock_ind(i1) || lock_ind(i3) == lock_ind(i2))
                                              ? nullptr
                                              : &locks[lock_ind(i3)]));
    }
    ...
    template <typename K, typename... Args>
    void
    add_to_bucket(const size_type bucket_ind, const size_type slot, const partial_t partial, K &&key, Args &&...val) {
    ...
    void move_bucket(buckets_t &old_buckets, buckets_t &new_buckets, size_type old_bucket_ind) const noexcept {
        ...
        for (size_type old_bucket_slot = 0; old_bucket_slot < slot_per_bucket(); ++old_bucket_slot) {
            ...
            new_buckets.setKV(dst_bucket_ind,
                              dst_bucket_slot++,
