...
void SelfDrivingRecordingUtil::RecordQueryMetadata(
    ...
    std::vector<execution::sql::SqlTypeId>                    param_types = {execution::sql::SqlTypeId::Integer,
    ...
    for (auto &data : qmetadata) {
                                                    ...
                                                    execution::sql::Integer(data.first.UnderlyingValue()));
        {
            ...
            params[2] = parser::ConstantValueExpression(execution::sql::SqlTypeId::Varchar,
                                                        string_val.first,
...

        {
            ...
            auto       string_val = execution::sql::ValueUtil::CreateStringVal(string);
            params[3] = parser::ConstantValueExpression(execution::sql::SqlTypeId::Varchar,
                                                        string_val.first,
                                                        std::move(string_val.second));
        }
    ...
    if (!params_vec.empty()) {
        ...
        task_manager->AddTask(std::make_unique<task::TaskDML>(catalog::INVALID_DATABASE_OID,
                                                              query,
...
void SelfDrivingRecordingUtil::RecordQueryParameters(
    ...
    for (auto &data : (*params)) {
        ...
        for (auto &sample : samples) {
            ...
            auto       string_val = execution::sql::ValueUtil::CreateStringVal(string);
            param_vec[2] = parser::ConstantValueExpression(execution::sql::SqlTypeId::Varchar,
                                                           string_val.first,
                                                           std::move(string_val.second));
...
void SelfDrivingRecordingUtil::RecordForecastClusters(uint64_t                             timestamp_to_record,
    ...
    for (auto &cluster : prediction) {
        for (auto &qid_info : cluster.second) {
            ...
            // Cluster ID
            clusters_params[1] = parser::ConstantValueExpression(execution::sql::SqlTypeId::Integer,
                                                                 execution::sql::Integer(cluster.first));

            // Query ID
            clusters_params[2] = parser::ConstantValueExpression(execution::sql::SqlTypeId::Integer,
    ...
    if (!clusters_params_vec.empty()) {
        // Clusters
        std::vector<execution::sql::SqlTypeId> param_types = {execution::sql::SqlTypeId::BigInt,
                                                              execution::sql::SqlTypeId::Integer,
                                                              execution::sql::SqlTypeId::Integer,
                                                              execution::sql::SqlTypeId::Integer};
...
void SelfDrivingRecordingUtil::RecordForecastQueryFrequencies(uint64_t                             timestamp_to_record,
    ...
    for (auto &cluster : prediction) {
        for (auto &qid_info : cluster.second) {
            ...
            for (size_t interval = 0; interval < qid_info.second.size(); interval++) {
                ...
                forecasts_params[2] = parser::ConstantValueExpression(execution::sql::SqlTypeId::Integer,
                                                                      execution::sql::Integer(interval));
    ...
    if (!forecast_params_vec.empty()) {
        ...
        task_manager->AddTask(std::make_unique<task::TaskDML>(catalog::INVALID_DATABASE_OID,
                                                              ...
                                                              std::make_unique<optimizer::TrivialCostModel>(),
                                                              false,
                                                              std::move(forecast_params_vec),
                                                              std::move(param_types)));
    }
}

void SelfDrivingRecordingUtil::RecordAppliedAction(uint64_t                                  timestamp_to_record,
                                                   selfdriving::pilot::action_id_t           action_id,
                                                   double                                    cost,
                                                   catalog::db_oid_t                         db_id,
...

    {
        ...
        auto       string_val = execution::sql::ValueUtil::CreateStringVal(string);
        param[4] = parser::ConstantValueExpression(execution::sql::SqlTypeId::Varchar,
                                                   string_val.first,
                                                   std::move(string_val.second));
    }
    std::vector<std::vector<parser::ConstantValueExpression>> params_vec;
...
void SelfDrivingRecordingUtil::RecordBestActions(
    ...
    for (size_t i = 0; i < actions.size(); i++) {
        for (const selfdriving::pilot::ActionTreeNode &node : actions[i]) {
            ...
            param[7] = parser::ConstantValueExpression(execution::sql::SqlTypeId::Double,
                                                       execution::sql::Real(node.GetCost()));
            ...
            auto       string_val = execution::sql::ValueUtil::CreateStringVal(string);
            param[9] = parser::ConstantValueExpression(execution::sql::SqlTypeId::Varchar,
                                                       string_val.first,
                                                       std::move(string_val.second));
