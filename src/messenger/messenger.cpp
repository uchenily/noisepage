...
ZmqMessage ZmqMessage::Build(message_id_t       message_id,
    ...
    return ZmqMessage{routing_id,
                      fmt::format("{}-{}-{}-{}",
                                  ...
                                  source_cb_id.UnderlyingValue(),
                                  dest_cb_id.UnderlyingValue(),
                                  message)};
}
...
public:
    ...
    PollItems &GetPollItems() {
        ...
        if (!writer_.items_.empty()) {
            ...
            read_.server_callbacks_.insert(read_.server_callbacks_.cend(),
                                           writer_.server_callbacks_.cbegin(),
...
ConnectionId::ConnectionId(common::ManagedPointer<Messenger> messenger,
...
ConnectionRouter::ConnectionRouter(common::ManagedPointer<Messenger> messenger,
                                   const ConnectionDestination      &target,
                                   const std::string                &identity,
                                   CallbackFn                        callback)
...
Messenger::Messenger(const uint16_t port, std::string identity)
    : port_(port)
...
void Messenger::SendMessage(const connection_id_t connection_id,
    ...
    common::ManagedPointer<zmq::socket_t> socket = common::ManagedPointer(connection->socket_);
    {
        ...
        pending_messages_.emplace(
            msg_id,
            PendingMessage{socket,
                           connection->target_name_,
                           ZmqMessage::Build(msg_id, sender_cb_id, remote_cb_id, connection->routing_id_, message),
                           false});
    }
}

void Messenger::SendMessage(const router_id_t  router_id,
    ...
    common::ManagedPointer<zmq::socket_t> socket = common::ManagedPointer(router->socket_);
    {
        ...
        pending_messages_.emplace(
            ...
            PendingMessage{socket,
                           recv_id,
                           ZmqMessage::Build(msg_id, send_cb_id, remote_cb_id, router->identity_, message),
                           true});
    }
}
...
void Messenger::ServerLoopAddRouters() {
    // Note that the stale read of empty() is probably undefined behavior. If wonky behavior is observed, watch out
    // here.
    if (!routers_to_be_added_.empty()) {
        std::lock_guard lock(routers_add_mutex_);
        for (auto &item : routers_to_be_added_) {
            auto router = std::make_unique<ConnectionRouter>(common::ManagedPointer(this),
                                                             item.target_,
                                                             item.identity_,
                                                             item.callback_);
...
void Messenger::ServerLoopSendMessages() {
    ...
    if (!pending_messages_.empty()) {
        ...
        for (auto &item : pending_messages_) {
            ...
            if (msg.is_router_socket_) {
                MESSENGER_LOG_TRACE(fmt::format("[PID={}] Messenger ({}) SENT-TO {}: {} ",
                                                ::getpid(),
                                                msg.msg_.routing_id_,
                                                destination,
                                                msg.msg_.GetRawPayload()));
            } else {
                MESSENGER_LOG_TRACE(fmt::format("[PID={}] Messenger SENT-TO {}: {} ",
                                                ::getpid(),
                                                destination,
    ...
    for (size_t i = 0; i < poll_items.items_.size(); ++i) {
        ...
        if (socket_has_data) {
            ...
            if (msg.GetDestinationCallbackId().UnderlyingValue() != static_cast<uint8_t>(BuiltinCallback::ACK)) {
                ...
                ZmqMessage ack = ZmqMessage::Build(msg.GetMessageId(),
                                                   GetBuiltinCallback(BuiltinCallback::NOOP),
                                                   GetBuiltinCallback(BuiltinCallback::ACK),
                                                   identity_,
                                                   "");
            ...
            MESSENGER_LOG_TRACE("[PID={}] Messenger RECV-FR {} (custom serverloop: {}): {}",
                                ::getpid(),
                                msg.GetRoutingId(),
                                has_custom_serverloop,
                                msg.GetRawPayload());
            // See the ProcessMessage() docstring.
            // ProcessMessage() must always be invoked so that the callback that was passed in with SendMessage() is
            // invoked.
            ProcessMessage(msg);
            if (msg.GetDestinationCallbackId().UnderlyingValue() != static_cast<uint8_t>(BuiltinCallback::ACK)) {
...
void Messenger::ProcessMessage(const ZmqMessage &msg) {
    ...
    case static_cast<uint8_t>(BuiltinCallback::ECHO): {
        ...
        if (zmq_default_socket_->send(router_data, zmq::send_flags::sndmore).has_value()) {
            ZmqMessage reply = ZmqMessage::Build(next_message_id_++,
                                                 GetBuiltinCallback(BuiltinCallback::NOOP),
                                                 msg.GetSourceCallbackId(),
                                                 identity_,
                                                 msg.GetMessage());
...
void Messenger::ServerLoop() {
    while (is_messenger_running_) {
        ...
        //            Right now, messages are only sent every MESSENGER_POLL_TIMER instead of being sent whenever
        //            available.
