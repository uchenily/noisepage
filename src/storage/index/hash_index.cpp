...
size_t HashIndex<KeyType>::EstimateHeapUsage() const {
    ...
    // wrapper level, however, then we don't know anything about the over-provisioning taking place at the underlying
    // data structure to maintain its load factor. In theory we could take max(wrapper elements * (sizeof(key) +
    // sizeof(value)), underlying capacity * (sizeof(key) + sizeof(value)) if we think the current calculation is off
    // too far.
    ...
    [=]() {                                                                                                            \
        ...
        auto key_found_fn = [location](ValueType &value) -> bool {                                                     \
            if (std::holds_alternative<TupleSlot>(value)) {                                                            \
                /* It's just a TupleSlot, functor should return true for cuckoohash_map's uprase_fn to erase key/value \
                 * pair */                                                                                             \
                return true;                                                                                           \
            }                                                                                                          \
            ...
            if (value_map.size() == 2) {                                                                               \
                ...
                for (const auto i : value_map) {                                                                       \
                    if (i != location) {                                                                               \
                        value = i; /* Assigning TupleSlot type will change the std::variant to TupleSlot and free the  \
                                      ValueMap */                                                                      \
                        NOISEPAGE_ASSERT(std::holds_alternative<TupleSlot>(value),                                     \
                                         "value should now be a TupleSlot.");                                          \
...
bool HashIndex<KeyType>::InsertUnique(const common::ManagedPointer<transaction::TransactionContext> txn,
    ...
    if (overall_result) {
        // TODO(wuwenw): transaction context is not thread safe for now, and a latch is used here to protect it, may
        // need a better way
        common::SpinLatch::ScopedSpinLatch guard(&transaction_context_latch_);
        txn->RegisterAbortAction(ERASE_KEY_ACTION);
    } else {
        // Presumably you've already made modifications to a DataTable (the source of the TupleSlot argument to this
        // function) however, the index found a constraint violation and cannot allow that operation to succeed. For
        // MVCC correctness, this txn must now abort for the GC to clean up the version chain in the DataTable
        // correctly.
        txn->SetMustAbort();
    }
...
template <typename KeyType>
void HashIndex<KeyType>::ScanKey(const transaction::TransactionContext &txn,
