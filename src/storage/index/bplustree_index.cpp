...
BPlusTreeIndex<KeyType>::BPlusTreeIndex(IndexMetadata &&metadata)
    ...
    , bplustree_{new BPlusTree<KeyType, TupleSlot>} {}
...
template <typename KeyType>
void BPlusTreeIndex<KeyType>::ScanKey(const transaction::TransactionContext &txn,
...
template <typename KeyType>
void BPlusTreeIndex<KeyType>::ScanAscending(const transaction::TransactionContext &txn,
                                            ScanType                               scan_type,
                                            uint32_t                               num_attrs,
    ...
    while (!scan_completed) {
        value_list->clear();
        scan_completed = bplustree_->ScanAscending(index_low_key,
                                                   index_high_key,
                                                   low_key_exists,
                                                   num_attrs,
                                                   high_key_exists,
...
template <typename KeyType>
void BPlusTreeIndex<KeyType>::ScanDescending(const transaction::TransactionContext &txn,
