...
std::pair<uint32_t, uint32_t> GarbageCollector::PerformGarbageCollection() {
    ...
    if (txns_unlinked > 0) {
        // Only update this field if we actually unlinked anything, otherwise we're being too conservative about when
        // it's safe to deallocate the transactions in our queue.
    ...
    if ((txns_deallocated > 0 || txns_unlinked > 0) && gc_metrics_enabled) {
        if (common::thread_context.resource_tracker_.IsRunning()) {
            ...
            common::thread_context.metrics_store_->RecordGCData(txns_deallocated,
                                                                txns_unlinked,
                                                                buffer_unlinked,
                                                                readonly_unlinked,
                                                                gc_interval_,
...
uint32_t GarbageCollector::ProcessDeallocateQueue(transaction::timestamp_t oldest_txn) {
    ...
    if (transaction::TransactionUtil::NewerThan(oldest_txn, last_unlinked_)) {
        ...
        // and have been serialized by the log manager. We are now safe to deallocate these txns because no running
...
void GarbageCollector::TruncateVersionChain(DataTable *const               table,
