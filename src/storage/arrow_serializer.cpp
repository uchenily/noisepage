...
void ArrowSerializer::AssembleMetadataBuffer(std::ofstream                  &outfile,
...
void ArrowSerializer::WriteSchemaMessage(std::ofstream                          &outfile,
...
void ArrowSerializer::WriteDictionaryMessage(std::ofstream                  &outfile,
    ...
    AssembleMetadataBuffer(outfile,
                           ...
                           aligned_offset,
                           flatbuf_builder);
    WriteDataBlock(outfile,
                   reinterpret_cast<const char *>(varlen_col.Offsets()),
    ...
    for (auto it : data_table_.GetBlocks()) { // NOLINT
        ...
        for (size_t i = 0; i < column_id_size; ++i) {
            ...
            if (layout.IsVarlen(col_id) && !(col_info.Type() == ArrowColumnType::FIXED_LENGTH)) {
                ...
                // Calculate the length of the data region of current column. For the columns except the last one, we
                // calculate their length by using the start of next column's bit map - the start of current column's
                // data. For the last column, we calculate the length by using the beginning address of the next block -
                // the start of current column data.
        ...
        for (size_t i = 0; i < column_id_size; ++i) {
            ...
            WriteDataBlock(outfile,
                           reinterpret_cast<const char *>(column_bitmap),
                           reinterpret_cast<uintptr_t>(column_start) - reinterpret_cast<uintptr_t>(column_bitmap));
            ...
            if (layout.IsVarlen(col_id) && !(col_info.Type() == ArrowColumnType::FIXED_LENGTH)) {
                ...
                case ArrowColumnType::GATHERED_VARLEN: {
                    ...
                    WriteDataBlock(outfile,
                                   ...
                                   varlen_col.OffsetsLength() * sizeof(uint64_t));
                    WriteDataBlock(outfile,
                                   reinterpret_cast<const char *>(varlen_col.Values()),
                                   varlen_col.ValuesLength());
                    break;
                }
                case ArrowColumnType::DICTIONARY_COMPRESSED: {
