...
TupleAccessStrategy::TupleAccessStrategy(BlockLayout layout)
    ...
    , column_offsets_(layout_.NumColumns()) {
...
void TupleAccessStrategy::InitializeRawBlock(storage::DataTable *const data_table,
                                             RawBlock *const           raw,
    ...
    // Also need to clean up any potential dangling version pointers (in cases where GC is off, or when a table is
    // deleted and individual tuples in it are not)
...
bool TupleAccessStrategy::Allocate(RawBlock *const block, TupleSlot *const slot) const {
    ...
    [[maybe_unused]] bool flip_res = bitmap->Flip(pos, false);
