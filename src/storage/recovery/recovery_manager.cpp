...
void RecoveryManager::RecoverFromLogs(const common::ManagedPointer<AbstractLogProvider> log_provider) {
    ...
    while (true) {
        ...
        default:
            NOISEPAGE_ASSERT(log_record->RecordType() == LogRecordType::REDO
                                 || log_record->RecordType() == LogRecordType::DELETE,
...
uint32_t RecoveryManager::ProcessCommittedTransaction(noisepage::transaction::timestamp_t txn_id) {
    ...
    for (uint32_t idx = 0; idx < buffered_changes_map_[txn_id].size(); idx++) {
        ...
        NOISEPAGE_ASSERT(buffered_record->RecordType() == LogRecordType::REDO
...
void RecoveryManager::ReplayRedoRecord(transaction::TransactionContext *txn, LogRecord *record) {
    ...
    if (IsInsertRecord(redo_record)) {
        ...
        UpdateIndexesOnTable(txn,
                             staged_record->GetDatabaseOid(),
                             staged_record->GetTableOid(),
                             sql_table_ptr,
                             new_tuple_slot,
                             staged_record->Delta(),
                             true /* insert */);
...
void RecoveryManager::ReplayDeleteRecord(transaction::TransactionContext *txn, LogRecord *record) {
    ...
    UpdateIndexesOnTable(txn,
                         delete_record->GetDatabaseOid(),
                         delete_record->GetTableOid(),
                         sql_table_ptr,
                         new_tuple_slot,
                         pr,
                         false /* delete */);
...
void RecoveryManager::UpdateIndexesOnTable(transaction::TransactionContext          *txn,
    ...
    case (catalog::postgres::PgDatabase::DATABASE_TABLE_OID.UnderlyingValue()): {
        ...
        index_objects.emplace_back(catalog_->databases_oid_index_,
                                   catalog_->databases_oid_index_->metadata_.GetSchema());
        break;
    }
    ...
    case (catalog::postgres::PgConstraint::CONSTRAINT_TABLE_OID.UnderlyingValue()): {
        ...
        index_objects.emplace_back(
            db_catalog_ptr->pg_constraint_.constraints_foreigntable_index_,
        ...
        for (uint32_t col_idx = 0; col_idx < num_index_cols; col_idx++) {
            ...
            if (table_pr->IsNull(pr_map[table_col_oid])) {
                ...
                std::memcpy(index_pr->AccessForceNotNull(index->GetKeyOidToOffsetMap().at(index_col_oid)),
                            table_pr->AccessWithNullCheck(pr_map[table_col_oid]),
                            size);
            }
        }
...
uint32_t RecoveryManager::ProcessSpecialCaseCatalogRecord(
    ...
    case (catalog::postgres::PgAttribute::COLUMN_TABLE_OID.UnderlyingValue()): {
        ...
        //  2. Cascading delete from drop table: In this case, we don't process the record because the DeleteTable
        //  catalog function will clean up the columns
...
uint32_t RecoveryManager::ProcessSpecialCasePGDatabaseRecord(
    ...
    if (curr_record->RecordType() == LogRecordType::REDO) {
        ...
        // Step 3: Update metadata. We need to use the indexes on pg_database to find what tuple slot we just inserted
        // into. We get the new tuple slot using the oid index.
    ...
    if (start_idx + 1 < buffered_changes->size()) { // there is one more record
        ...
        if (next_record->RecordType() == LogRecordType::REDO) { // next record is a redo record
            ...
            if (next_redo_record->GetDatabaseOid() == delete_record->GetDatabaseOid()
                ...
                auto next_db_oid
                    = *(reinterpret_cast<catalog::db_oid_t *>(next_redo_record->Delta()->AccessWithNullCheck(
                        pr_map[catalog::postgres::PgDatabase::DATOID.oid_])));
                ...
                if (db_oid == next_db_oid) {
                    ...
                    VarlenEntry name_varlen
                        = *(reinterpret_cast<VarlenEntry *>(next_redo_record->Delta()->AccessWithNullCheck(
                            pr_map[catalog::postgres::PgDatabase::DATNAME.oid_])));
                    std::string name_string(name_varlen.StringView());
                    ...
                    // pg_database to find what tuple slot we just inserted into. We get the new tuple slot using the
                    // oid index.
                    auto pg_database_oid_index = catalog_->databases_oid_index_;
...
uint32_t RecoveryManager::ProcessSpecialCasePGClassRecord(
        ...
        case (catalog::postgres::PgClass::REL_PTR.oid_.UnderlyingValue()): { // Case 3
            ...
            case (catalog::postgres::PgClass::RelKind::REGULAR_TABLE): {
                ...
                if (class_oid < catalog::START_OID) { // All catalog tables/indexes have OIDS less than START_OID
                    // Use of the -> operator is ok here, since we are the ones who wrapped the table with the
                    // ManagedPointer
                    sql_table
                ...
                result = db_catalog->SetTablePointer(common::ManagedPointer(txn),
                                                     catalog::table_oid_t(class_oid),
                                                     sql_table);
            ...
            case (catalog::postgres::PgClass::RelKind::INDEX): {
                ...
                auto index_cols
                    = db_catalog
                          ->GetColumns<catalog::IndexSchema::Column, catalog::index_oid_t, catalog::indexkeycol_oid_t>(
                              common::ManagedPointer(txn),
                              catalog::index_oid_t(class_oid));
                ...
                *(reinterpret_cast<uint32_t *>(pr->AccessForceNotNull(0))) = class_oid;
                ...
                // NOLINTNEXTLINE
                col_oids.clear();
                col_oids = {catalog::postgres::PgIndex::INDISUNIQUE.oid_,
                            catalog::postgres::PgIndex::INDISPRIMARY.oid_,
                            catalog::postgres::PgIndex::INDISEXCLUSION.oid_,
                            catalog::postgres::PgIndex::INDIMMEDIATE.oid_,
                ...
                auto *index_schema = new catalog::IndexSchema(index_cols,
                                                              index_type,
                                                              is_unique,
                                                              is_primary,
                                                              is_exclusion,
                ...
                result = db_catalog->SetIndexPointer(common::ManagedPointer(txn),
                                                     catalog::index_oid_t(class_oid),
                                                     index.Get());
    ...
    std::vector<catalog::col_oid_t> col_oids
        = {catalog::postgres::PgClass::RELOID.oid_, catalog::postgres::PgClass::RELKIND.oid_};
    ...
    if (start_idx + 1 < buffered_changes->size()) { // Condition 1: there is one more record
        ...
        if (next_record->RecordType() == LogRecordType::REDO) { // Condition 2: next record is a redo record
            ...
            if (next_redo_record->GetDatabaseOid() == delete_record->GetDatabaseOid()
                ...
                if (class_oid
                    == next_class_oid) { // Condition 4: If the oid matches on the next record, this is a renaming
                    NOISEPAGE_ASSERT(class_kind == catalog::postgres::PgClass::RelKind::REGULAR_TABLE
                                         && class_kind == next_class_kind,
                                     "We only allow renaming of tables");
                    // Step 4: Extract out the new name
                    VarlenEntry name_varlen
                        = *(reinterpret_cast<VarlenEntry *>(next_redo_record->Delta()->AccessWithNullCheck(
                            pr_map[catalog::postgres::PgClass::RELNAME.oid_])));
                    std::string name_string(name_varlen.StringView());
                    ...
                    // Step 6: Update metadata and clean up additional record processed. We need to use the indexes on
                    // pg_class to find what tuple slot we just inserted into. We get the new tuple slot using the oid
                    // index.
