...
DataTable::DataTable(common::ManagedPointer<BlockStore> store,
...
void DataTable::Scan(const common::ManagedPointer<transaction::TransactionContext> txn,
...
auto DataTable::Update(const common::ManagedPointer<transaction::TransactionContext> txn,
    ...
    for (uint16_t i = 0; i < redo.NumColumns(); i++) {
        ...
        // TODO(Matt): It would be nice to check that a ProjectedRow that modifies the logical delete column only
        // originated from the DataTable calling Update() within Delete(), rather than an outside soure modifying this
...
void DataTable::InsertInto(const common::ManagedPointer<transaction::TransactionContext> txn,
...
template bool
DataTable::SelectIntoBuffer<ProjectedRow>(const common::ManagedPointer<transaction::TransactionContext> txn,
                                          const TupleSlot                                               slot,
...
auto DataTable::IsVisible(const transaction::TransactionContext &txn, const TupleSlot slot) const -> bool {
    ...
    do {
        version_ptr = AtomicallyReadVersionPtr(slot, accessor_);
        // Here we will need to check that the version pointer did not change during our read. If it did, the visibility
        // of this tuple might have changed and we should check again.
