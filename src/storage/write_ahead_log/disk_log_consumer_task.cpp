...
void DiskLogConsumerTask::WriteBuffersToLogFile() {
    ...
    while (!filled_buffer_queue_->Empty()) {
        ...
        if (logs.first != nullptr) {
            // Need the nullptr check because read-only txns don't serialize any buffers, but generate callbacks to be
            // invoked
...
void DiskLogConsumerTask::DiskLogConsumerTaskLoop() {
    ...
    // Disk log consumer task thread spins in this loop. When notified or periodically, we wake up and process
    // serialized buffers
    do {
        ...
        curr_sleep = next_sleep;
        {
            ...
            bool signaled = disk_log_writer_thread_cv_.wait_for(lock, curr_sleep, [&] {
        ...
        if (num_buffers > 0) {
            if (common::thread_context.resource_tracker_.IsRunning()) {
                ...
                common::thread_context.metrics_store_->RecordConsumerData(num_bytes,
                                                                          num_buffers,
