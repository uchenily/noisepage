...
void LogSerializerTask::LogSerializerTaskLoop() {
    ...
    do {
        ...
        // Serializing is now on the "critical txn path" because txns wait to commit until their logs are serialized.
        // Thus, a sleep is not fast enough. We perform exponential back-off, doubling the sleep duration if we don't
        // process any buffers in our call to Process. Calls to Process will process as long as new buffers are
        // available. We only sleep as part of this exponential backoff when there are logs that need to be processed
        // and we wake up when there are new logs to be processed.
        if (empty_) {
        ...
        // If Process did not find any new buffers, we perform exponential back-off to reduce our rate of polling for
        // new buffers. We cap the maximum back-off, since in the case of large gaps of no txns, we don't want to
        // unboundedly sleep
        ...
        if (num_records > 0) {
            if (common::thread_context.resource_tracker_.IsRunning()) {
                ...
                common::thread_context.metrics_store_->RecordSerializerData(num_bytes,
                                                                            num_records,
                                                                            num_txns,
                                                                            serialization_interval_.count(),
                                                                            resource_metrics);
            }
...
std::tuple<uint64_t, uint64_t, uint64_t> LogSerializerTask::Process() {
...

    {
        ...
        // We continually grab all the buffers until we find there are no new buffers. This way we serialize buffers
        // that came in during the previous serialization loop

        // Continually loop, break out if there's no new buffers
        while (true) {
            // In a short critical section, get all buffers to serialize. We move them to a temp queue to reduce
            // contention on the queue transactions interact with
            {
                std::unique_lock<std::mutex> guard(flush_queue_latch_);
            ...
            while (!temp_flush_queue_.empty()) {
                ...
                // Check if the buffer's policy is compatible with the current filled buffer.
                {
                    ...
                    // Currently, the invariant is maintained that the filled buffer policy will ALWAYS have a value
                    // once buffers have started to be serialized.
...
void LogSerializerTask::HandFilledBufferToWriter() {
    ...
    if (txn_policy.replication_ != transaction::ReplicationPolicy::DISABLE) {
        ...
        primary_replication_manager_->ReplicateBatchOfRecords(filled_buffer_,
                                                              commits_in_buffer_,
                                                              txn_policy.replication_,
                                                              newest_buffer_txn_);
        oat_replicas_ = true;
    }
...
uint64_t LogSerializerTask::SerializeRecord(const noisepage::storage::LogRecord &record) {
    ...
    // LogRecordType is REDO. On recovery, the goal is to turn the serialized format back into an in-memory log record
    // of this size.
    ...
    case LogRecordType::REDO: {
        ...
        for (uint16_t i = 0; i < delta->NumColumns(); i++) {
            ...
            if (column_value_address == nullptr) {
                // If the column in this REDO record is null, then there's nothing to serialize out. The bitmap contains
            ...
            if (block_layout.IsVarlen(col_id)) {
                ...
                // Note that by writing out AttrSize(col_id) bytes instead of just the difference between successive
                // offsets of the delta record, we avoid serializing out any potential padding.
