...
ProjectedColumnsInitializer::ProjectedColumnsInitializer(const BlockLayout    &layout,
    ...
    size_
        = StorageUtil::PadUpToSize(sizeof(uint32_t), size_ + static_cast<uint32_t>(col_ids_.size() * sizeof(uint16_t)));
    // space needed to store value offsets, pad up to 8 bytes to store tuple slots
    size_ = StorageUtil::PadUpToSize(sizeof(TupleSlot),
