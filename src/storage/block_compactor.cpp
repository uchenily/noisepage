...
bool BlockCompactor::MoveTuple(CompactionGroup *cg, TupleSlot from, TupleSlot to) {
    ...
    // own special operators that takes physical locations instead of predicates, but it should share logic with the
    // rest of the system when it comes to index updates and such.
...
bool BlockCompactor::CheckForVersionsAndGaps(const TupleAccessStrategy &accessor, RawBlock *block) {
    ...
    for (uint32_t offset = 0; offset < layout.NumSlots(); offset++) {
        ...
        // Not contiguous. If the code reaches here the slot must be allocated, and we have seen an unallocated slot
        // before
        if (unallocated_region_start) {
            return false;
        }
...
void BlockCompactor::CopyToArrowVarlen(std::vector<const byte *>   *loose_ptrs,
...
void BlockCompactor::BuildDictionary(std::vector<const byte *>   *loose_ptrs,
                                     ArrowBlockMetadata          *metadata,
                                     col_id_t                     col_id,
                                     common::RawConcurrentBitmap *column_bitmap,
                                     ArrowColumnInfo             *col,
