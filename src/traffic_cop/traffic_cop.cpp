...
void TrafficCop::ExecuteTransactionStatement(const common::ManagedPointer<network::ConnectionContext>    connection_ctx,
    ...
    case network::QueryType::QUERY_BEGIN: {
        NOISEPAGE_ASSERT(
            connection_ctx->TransactionState() != network::NetworkTransactionStateType::FAIL,
            "We're in an aborted state. This should have been caught already before calling this function.");
        if (explicit_txn_block) {
            out->WriteError({common::ErrorSeverity::WARNING,
...
TrafficCop::OptimizeBoundQuery(const common::ManagedPointer<network::ConnectionContext>             connection_ctx,
    ...
    return TrafficCopUtil::Optimize(connection_ctx->Transaction(),
...
TrafficCopResult
TrafficCop::ExecuteCreateStatement(const common::ManagedPointer<network::ConnectionContext> connection_ctx,
    ...
    // TODO(Matt): get more verbose failure info out of the catalog and DDL executors. I think at this point, if it's
    // past binding, the only thing you could fail on is a conflict with a DDL change you already made in this txn or
    // failure to acquire DDL lock?
    return {ResultType::ERROR,
            common::ErrorData(common::ErrorSeverity::ERROR,
...
TrafficCopResult
TrafficCop::ExecuteDropStatement(const common::ManagedPointer<network::ConnectionContext> connection_ctx,
                                 const common::ManagedPointer<planner::AbstractPlanNode>  physical_plan,
                                 const noisepage::network::QueryType                      query_type) const {
    NOISEPAGE_ASSERT(connection_ctx->TransactionState() == network::NetworkTransactionStateType::BLOCK,
                     "Not in a valid txn. This should have been caught before calling this function.");
    NOISEPAGE_ASSERT(
        query_type == network::QueryType::QUERY_DROP_TABLE || query_type == network::QueryType::QUERY_DROP_SCHEMA
            ...
            || query_type == network::QueryType::QUERY_DROP_VIEW
            || query_type == network::QueryType::QUERY_DROP_TRIGGER,
    ...
    return {ResultType::ERROR,
            common::ErrorData(common::ErrorSeverity::ERROR,
                              "failed to execute DROP",
                              common::ErrorCode::ERRCODE_DATA_EXCEPTION)};
}

TrafficCopResult
TrafficCop::ExecuteExplainStatement(const common::ManagedPointer<network::ConnectionContext>    connection_ctx,
                                    const common::ManagedPointer<network::PostgresPacketWriter> out,
    ...
    out->WriteDataRow(reinterpret_cast<const byte *const>(&plan_string_val),
                      output_columns,
...
TrafficCop::ParseQuery(const std::string                                       &query,
    ...
    try {
        ...
        common::ErrorData error(common::ErrorSeverity::ERROR,
                                std::string(e.what()),
...
TrafficCopResult
TrafficCop::BindQuery(const common::ManagedPointer<network::ConnectionContext>                   connection_ctx,
        ...
        if (statement->OptimizeResult() == nullptr || !UseQueryCache()) {
            ...
            if (parameters != nullptr && !parameters->empty()) {
                ...
                visitor.BindNameToNode(statement->ParseResult(),
                                       parameters,
                                       common::ManagedPointer(&desired_param_types));
...
TrafficCopResult
TrafficCop::CodegenPhysicalPlan(const common::ManagedPointer<network::ConnectionContext>    connection_ctx,
                                const common::ManagedPointer<network::PostgresPacketWriter> out,
                                const common::ManagedPointer<network::Portal>               portal) const {
    NOISEPAGE_ASSERT(connection_ctx->TransactionState() == network::NetworkTransactionStateType::BLOCK,
                     "Not in a valid txn. This should have been caught before calling this function.");
    ...
    if (portal->GetStatement()->GetExecutableQuery() == nullptr) {
        ...
        if (query_trace_metrics_enabled) {
            common::thread_context.metrics_store_->RecordQueryText(connection_ctx->GetDatabaseOid(),
                                                                   exec_query->GetQueryId(),
...
TrafficCopResult TrafficCop::RunExecutableQuery(const common::ManagedPointer<network::ConnectionContext> connection_ctx,
    ...
    if (query_cache_timestamp_ > portal->GetStatement()->GetExecutableQuery()->GetTimestamp()) {
        ...
        auto optimize_result
            = OptimizeBoundQuery(connection_ctx, statement->ParseResult(), portal->ModifiableParameters());
    ...
    execution::exec::OutputCallback callback
        = [capture_writer](byte *tuples, uint32_t num_tuples, uint32_t tuple_size) {
    ...
    auto exec_ctx = std::make_unique<execution::exec::ExecutionContext>(connection_ctx->GetDatabaseOid(),
    ...
    if (query_trace_metrics_enabled) {
        common::thread_context.metrics_store_->RecordQueryTrace(connection_ctx->GetDatabaseOid(),
                                                                exec_query->GetQueryId(),
                                                                metrics::MetricsUtil::Now(),
                                                                portal->Parameters());
    }
...
TrafficCop::CreateTempNamespace(const network::connection_id_t connection_id, const std::string &database_name) {
...
bool TrafficCop::DropTempNamespace(const catalog::db_oid_t db_oid, const catalog::namespace_oid_t ns_oid) {
    ...
    NOISEPAGE_ASSERT(db_accessor != nullptr,
                     "Catalog failed to provide a CatalogAccessor. Was the db_oid still valid?");
...
void TrafficCop::UpdateQueryCacheTimestamp() {
    query_cache_timestamp_ = txn_manager_->GetCurrentTimestamp();
}
