...
public:
    ...
    callback_id_t GetSourceCallbackId() const {
        return source_cb_id_;
    }

    /** @return The callback to invoke on the destination. */
    callback_id_t GetDestinationCallbackId() const {
        return dest_cb_id_;
    }

    /** @return The routing ID of this message. */
    std::string_view GetRoutingId() const {
        return std::string_view(routing_id_);
    }
    ...
    std::string_view GetRawPayload() const {
...
private:
     ...
     */
    static ZmqMessage Build(message_id_t       message_id,
...
public:
     ...
     */
    explicit ConnectionId(common::ManagedPointer<Messenger> messenger,
                          const ConnectionDestination      &target,
...
public:
     ...
     */
    explicit ConnectionRouter(common::ManagedPointer<Messenger> messenger,
                              const ConnectionDestination      &target,
                              const std::string                &identity,
                              CallbackFn                        callback);
...
public:
    ...
    static callback_id_t GetBuiltinCallback(BuiltinCallback cb) {
        return callback_id_t{static_cast<uint64_t>(cb)};
    }
     ...
     */
    router_id_t
    ListenForConnection(const ConnectionDestination &target, const std::string &identity, CallbackFn callback);
     ...
     *
     * @warning           The default behavior of ZMQ sockets is to allow connections to any target and to queue
     * messages. Obtaining a connection_id does NOT guarantee that a successful connection has been made. If this is
     * necessary, test the connection explicitly by sending a message through. Currently, connection_id is setup so that
     * messages are only queued for successful connections. But note that it should not be necessary as your code should
     * handle the case of the target going away permanently anyway, in particular consider this ordering of events:
     ...
     */
    void SendMessage(connection_id_t    connection_id,
     ...
     *                        To invoke preregistered functions, use static_cast<uint8_t>(Messenger::BuiltinCallback).
     */
    void SendMessage(router_id_t        router_id,
                     const std::string &recv_id,
                     const std::string &message,
                     CallbackFn         callback,
                     callback_id_t      remote_cb_id);
...
private:
     ...
     * Note that these callbacks are distinct from the ServerLoop callbacks common to custom Messenger endpoints, such
     * as replication and the model server messenger.
     *
     ...
     * to periodically resend messages that have not been acknowledged as part of bolting guaranteed delivery onto
     * ZeroMQ.
     *
...
public:
     ...
     */
    explicit MessengerManager(common::ManagedPointer<common::DedicatedThreadRegistry> thread_registry,
                              uint16_t                                                port,
                              const std::string                                      &identity);

    /** @return The Messenger being managed. */
    common::ManagedPointer<Messenger> GetMessenger() const {
        return messenger_;
    }
