...
public:
     ...
     * returns. This function will free the task if it is stopped, thus, it is up to the requester (as well as the
     * task's Terminate method) to do any necessary cleanup to the task before calling StopTask. StopTask will call the
     * owners OnThreadRemoval method in order to allow the owner to clean up the task.
