...
public:
    ...
    /**
     * Helper method to convert timeval to microseconds. Might eventually live elsewhere, but timeval only used here
     * right now
     * @param t timeval to convert
     * @return timeval converted to microseconds
     */
    static int64_t TimevalToMicroseconds(const timeval &t) {
...
private:
    ...
    /**
     * Platform-specific representation of current rusage. On macOS it can only get process info. Thread-specific info
     * is non-POSIX and added in later Linux kernels.
     */
