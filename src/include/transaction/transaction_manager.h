...
public:
    ...
    TransactionManager(const common::ManagedPointer<TimestampManager>                 timestamp_manager,
        ...
        NOISEPAGE_ASSERT(timestamp_manager_ != DISABLED,
                         "transaction manager cannot function without a timestamp manager");
    ...
    void SetDefaultTransactionReplicationPolicy(const ReplicationPolicy &policy) {
        ...
        NOISEPAGE_ASSERT(
            !(default_txn_policy_.durability_ == DurabilityPolicy::ASYNC && policy == ReplicationPolicy::SYNC),
    ...
    const TransactionPolicy &GetDefaultTransactionPolicy() const {
        return default_txn_policy_;
    }

    /** @return current transaction timestamp without advancing the tick */
    timestamp_t GetCurrentTimestamp() const {
        return timestamp_manager_->CurrentTime();
    }
