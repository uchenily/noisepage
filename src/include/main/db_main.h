...
public:
    ...
    public:
         ...
         * @param wal_async_commit_enable true if commit callbacks should be invoked by TransactionManager at commit
         * time rather than waiting until durable on disk and being invoked by the WAL worker. Doesn't make sense to set
         * to true if WAL is not enabled.
        ...
        TransactionLayer(const common::ManagedPointer<storage::RecordBufferSegmentPool> buffer_segment_pool,
                         ...
                         const bool                                                     wal_async_commit_enable,
            ...
            txn_manager_
                = std::make_unique<transaction::TransactionManager>(common::ManagedPointer(timestamp_manager_),
    ...
    public:
         ...
         */
        StorageLayer(const common::ManagedPointer<TransactionLayer>    txn_layer,
                     const uint64_t                                    block_store_size_limit,
                     const uint64_t                                    block_store_reuse_limit,
                     const bool                                        use_gc,
                     const common::ManagedPointer<storage::LogManager> log_manager,
    ...
    public:
        ...
        CatalogLayer(const common::ManagedPointer<TransactionLayer>    txn_layer,
                     ...
                     const common::ManagedPointer<storage::LogManager> log_manager,
                     const bool                                        create_default_database)
            : deferred_action_manager_(txn_layer->GetDeferredActionManager())
            ...
            catalog_ = std::make_unique<catalog::Catalog>(txn_layer->GetTransactionManager(),
                                                          storage_layer->GetBlockStore(),
                                                          garbage_collector_);
            ...
            if (create_default_database) {
                ...
                txn_layer->GetTransactionManager()->Commit(bootstrap_txn,
        ...
        ~CatalogLayer() {
            ...
            // Specifically, this will generate the following (Note: these transactions are read-only and there are no
            // logical deletes of the data as that would cause replicas to replay :
            //
            // CatalogTearDown:
            //   BEGIN
            //     For each database:
            ...
            //   COMMIT
            //
            // TODO(John): We could eagerly execute the database.TearDown() and delete calls, but this would force us
            // into sequential execution of TearDown when we could potentially multithread the table scans of each
            // database catalog using DAF.
            catalog_->TearDown();
    ...
    public:
        ...
        std::unique_ptr<DBMain> Build() {
            ...
            if (use_replication_) {
                ...
                if (network_identity_ == "primary") {
                    replication_manager = std::make_unique<replication::PrimaryReplicationManager>(
                        messenger_layer->GetMessenger(),
                        network_identity_,
                        replication_port_,
                        replication_hosts_path_,
            ...
            if (use_logging_) {
                ...
                log_manager
                    = std::make_unique<storage::LogManager>(wal_file_path_,
                                                            wal_num_buffers_,
                                                            std::chrono::microseconds{wal_serialization_interval_},
                                                            std::chrono::microseconds{wal_persist_interval_},
                                                            wal_persist_threshold_,
                                                            common::ManagedPointer(buffer_segment_pool),
                                                            common::ManagedPointer(empty_buffer_queue),
                                                            rep_manager_ptr,
            ...
            auto txn_layer = std::make_unique<TransactionLayer>(common::ManagedPointer(buffer_segment_pool),
                                                                use_gc_,
                                                                wal_async_commit_enable_,
                                                                common::ManagedPointer(log_manager));

            auto storage_layer = std::make_unique<StorageLayer>(common::ManagedPointer(txn_layer),
                                                                block_store_size_,
                                                                block_store_reuse_,
                                                                use_gc_,
                                                                common::ManagedPointer(log_manager),
                                                                std::move(empty_buffer_queue));
            ...
            if (use_catalog_) {
                ...
                catalog_layer = std::make_unique<CatalogLayer>(common::ManagedPointer(txn_layer),
                                                               common::ManagedPointer(storage_layer),
                                                               common::ManagedPointer(log_manager),
                                                               create_default_database_);
            }
            ...
            if (thread_registry && txn_layer && use_catalog_ && use_settings_manager_) {
                ...
                query_exec_util = std::make_unique<util::QueryExecUtil>(txn_layer->GetTransactionManager(),
                ...
                task_manager = std::make_unique<task::TaskManager>(
                    common::ManagedPointer(thread_registry),
                    util::QueryExecUtil::ConstructThreadLocal(common::ManagedPointer(query_exec_util)),
                    task_pool_size_);
            }
            ...
            if (use_model_server_) {
                ...
                model_server_manager
                    = std::make_unique<modelserver::ModelServerManager>(model_server_path_,
                                                                        messenger_layer->GetMessenger(),
            ...
            if (use_pilot_thread_) {
                ...
                pilot_thread = std::make_unique<selfdriving::pilot::PilotThread>(
                    common::ManagedPointer(pilot),
                    std::chrono::microseconds{pilot_interval_},
                    std::chrono::microseconds{forecast_train_interval_},
                    pilot_planning_);
            }

            NOISEPAGE_ASSERT(
                !(async_replication_enable_ && !use_replication_),
            ...
            if (use_replication_) {
                if (replication_manager->IsPrimary()) {
                    // On the primary, replicate buffers synchronously by default, but can be made async via command
                    // line.
                    transaction::ReplicationPolicy policy = async_replication_enable_
    ...
    private:
        ...
        // default values here. Larger scale tests and benchmarks may need to to use setters on the Builder to adjust
        // these before building DBMain. The real system should use the SettingsManager.
        ...
        BootstrapSettingsManager(const common::ManagedPointer<DBMain> db_main) {
            ...
            optimizer_timeout_
                = static_cast<uint64_t>(settings_manager->GetInt(settings::Param::task_execution_timeout));
