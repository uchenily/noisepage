...
public:
     ...
     */
    namespace_oid_t GetDefaultNamespace() const {
     ...
     * @param table the OID of the table to drop, this must be a valid oid from GetTableOid. Invalid input will trigger
     * an assert
     ...
     * @warning If the caller needs to reference the schema object after this call, they should use the GetSchema
     * function to obtain the authoritative schema for this table.
     */
     ...
     * @param table table to get index objects for, this must be a valid oid from GetTableOid. Invalid input will
     * trigger an assert
     ...
     */
    common::ManagedPointer<transaction::TransactionContext> GetTxn() const {
        return txn_;
    }
     ...
     * @param schema The schema of the temp table. //TODO(Matt): this is owned by the CTEScanIterator. Are there
     * lifecycle issues there? Maybe The CatalogAccessor should own temp objects to guarantee availability?
     */
    void RegisterTempTable(table_oid_t                                   table_oid,
     ...
     */
    uint32_t GetNewTempOid() {
        return TEMP_OID_MASK | (++temp_oid_counter_);
    }
     ...
     */
    CatalogAccessor(const common::ManagedPointer<Catalog>                         catalog,
