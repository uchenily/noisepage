...
public:
     ...
     */
    void
    RecordQueryText(execution::query_id_t qid, catalog::db_oid_t db_oid, std::string text, std::string param_type) {
        // Assume qid is unique, don't re-record if already recorded
        NOISEPAGE_ASSERT(qmetadata_.find(qid) == qmetadata_.end(),
                         "Expected query_text recording to be unique for qid");
...
public:
    ...
    static constexpr char QUERY_OBSERVED_INSERT_STMT[]
        = "INSERT INTO noisepage_forecast_frequencies VALUES ($1, $2, $3)";
     ...
     */
    MetricsComponent GetMetricType() const override {
        return MetricsComponent::QUERY_TRACE;
    }
    ...
    void ToCSV(std::vector<std::ofstream> *const outfiles) final {
        ...
        NOISEPAGE_ASSERT(std::count_if(outfiles->cbegin(),
                                       outfiles->cend(),
...
private:
     ...
     */
    void SubmitFrequencyRecordJob(uint64_t                                         timestamp,
    ...
    struct QueryText {
        QueryText(catalog::db_oid_t           db_oid,
                  const execution::query_id_t query_id,
    ...
    struct QueryTrace {
        QueryTrace(catalog::db_oid_t           db_oid,
                   const execution::query_id_t query_id,
                   const uint64_t              timestamp,
