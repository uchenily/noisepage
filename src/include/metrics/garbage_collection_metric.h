...
public:
    ...
    MetricsComponent GetMetricType() const override {
        return MetricsComponent::GARBAGECOLLECTION;
    }
...
private:
    ...
    void RecordGCData(uint64_t                                txns_deallocated,
        ...
        gc_data_.emplace_back(txns_deallocated,
                              txns_unlinked,
                              buffer_unlinked,
                              readonly_unlinked,
                              interval,
    ...
    struct GCData {
        GCData(uint64_t                                txns_deallocated,
               uint64_t                                txns_unlinked,
               uint64_t                                buffer_unlinked,
               uint64_t                                readonly_unlinked,
               const uint64_t                          interval,
...
private:
    ...
    void RecordGCData(uint64_t                                txns_deallocated,
                      ...
                      uint64_t                                interval,
                      const common::ResourceTracker::Metrics &resource_metrics) {
        GetRawData()->RecordGCData(txns_deallocated,
                                   txns_unlinked,
                                   buffer_unlinked,
                                   readonly_unlinked,
                                   interval,
