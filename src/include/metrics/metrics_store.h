...
public:
     ...
     */
    void RecordSerializerData(const uint64_t                          num_bytes,
                              const uint64_t                          num_records,
                              const uint64_t                          num_txns,
                              const uint64_t                          interval,
                              const common::ResourceTracker::Metrics &resource_metrics) {
        if (!ComponentEnabled(MetricsComponent::LOGGING))
            METRICS_LOG_WARN("RecordSerializerData() called without logging metrics enabled. Was it recently disabled "
                             "and the component "
                             "is just lagging?");
    ...
    void RecordConsumerData(const uint64_t                          num_bytes,
                            ...
                            const common::ResourceTracker::Metrics &resource_metrics) {
        if (!ComponentEnabled(MetricsComponent::LOGGING))
            METRICS_LOG_WARN("RecordConsumerData() called without logging metrics enabled. Was it recently disabled "
     ...
     * @param resource_metrics third entry of metrics datapoint
     */
    void RecordRecoveryData(const uint64_t                          num_records,
                            const uint64_t                          num_txns,
                            const common::ResourceTracker::Metrics &resource_metrics) {
        if (!ComponentEnabled(MetricsComponent::LOGGING))
            METRICS_LOG_WARN("RecordRecoveryData() called without logging metrics enabled. Was it recently disabled "
                             "and the component is "
                             "just lagging?");
        NOISEPAGE_ASSERT(logging_metric_ != nullptr, "LoggingMetric not allocated. Check MetricsStore constructor.");
     ...
     */
    void RecordGCData(uint64_t                                txns_deallocated,
                      uint64_t                                txns_unlinked,
                      uint64_t                                buffer_unlinked,
                      uint64_t                                readonly_unlinked,
                      uint64_t                                interval,
                      const common::ResourceTracker::Metrics &resource_metrics) {
        if (!ComponentEnabled(MetricsComponent::GARBAGECOLLECTION))
            METRICS_LOG_WARN("RecordUnlinkData() called without GC metrics enabled. Was it recently disabled and the "
                             "component is just "
                             "lagging?");
        NOISEPAGE_ASSERT(gc_metric_ != nullptr,
                         "GarbageCollectionMetric not allocated. Check MetricsStore constructor.");
        gc_metric_->RecordGCData(txns_deallocated,
                                 txns_unlinked,
                                 buffer_unlinked,
                                 readonly_unlinked,
                                 interval,
    ...
    void RecordBeginData(const common::ResourceTracker::Metrics &resource_metrics) {
        if (!ComponentEnabled(MetricsComponent::TRANSACTION))
            METRICS_LOG_WARN("RecordBeginData() called without transaction metrics enabled. Was it recently disabled "
                             "and the component is "
                             "just lagging?");
    ...
    void RecordCommitData(const uint64_t is_readonly, const common::ResourceTracker::Metrics &resource_metrics) {
        if (!ComponentEnabled(MetricsComponent::TRANSACTION))
            METRICS_LOG_WARN("RecordCommitData() called without transaction metrics enabled. Was it recently disabled "
                             "and the component "
                             "is just lagging?");
        NOISEPAGE_ASSERT(txn_metric_ != nullptr, "TransactionMetric not allocated. Check MetricsStore constructor.");
     ...
     */
    void RecordExecutionData(const char                             *feature,
                             uint32_t                                len,
        ...
        NOISEPAGE_ASSERT(execution_metric_ != nullptr,
                         "ExecutionMetric not allocated. Check MetricsStore constructor.");
     ...
     * @param resource_metrics Metrics
     */
    void RecordPipelineData(execution::query_id_t                                     query_id,
        ...
        pipeline_metric_->RecordPipelineData(query_id,
                                             pipeline_id,
                                             execution_mode,
                                             std::move(features),
                                             resource_metrics);
     ...
     * @param resource_metrics Metrics
     */
    void RecordBindCommandData(uint64_t                                param_num,
                               uint64_t                                query_text_size,
                               const common::ResourceTracker::Metrics &resource_metrics) {
     ...
     */
    void RecordQueryText(catalog::db_oid_t                                                          db_oid,
                         const execution::query_id_t                                                query_id,
        ...
        NOISEPAGE_ASSERT(query_trace_metric_ != nullptr,
     ...
     * @param param parameter associated with this query
     */
    void RecordQueryTrace(catalog::db_oid_t                                                          db_oid,
                          const execution::query_id_t                                                query_id,
                          const uint64_t                                                             timestamp,
                          common::ManagedPointer<const std::vector<parser::ConstantValueExpression>> param) {
        NOISEPAGE_ASSERT(ComponentEnabled(MetricsComponent::QUERY_TRACE), "QueryTraceMetric not enabled.");
    ...
    bool ComponentToRecord(const MetricsComponent component) {
        ...
        if (!enabled_metrics_.test(component_index))
            return false;
     ...
     */
    common::ManagedPointer<metrics::MetricsManager> MetricsManager() const {
