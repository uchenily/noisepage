...
public:
     ...
     */
    bool IsGreaterThanOrEqualToMaxValue(KeyType value) const {
        return ConvertToPoint(value) >= maximum_;
    }
     ...
     */
    bool IsLessThanMinValue(KeyType value) const {
        return ConvertToPoint(value) < minimum_;
    }

    /**
     * @return the total number of values that are recorded in this histogram
     */
    double GetTotalValueCount() const {
        return std::floor(total_);
    }

    /**
     * @return the maximum number of bins that this histogram supports
     */
    uint8_t GetMaxBinSize() const {
        return max_bins_;
    }
...
private:
     ...
     */
    static double ConvertToPoint(const decltype(execution::sql::BoolVal::val_) &key) {
        return static_cast<double>(key);
    }

    static double ConvertToPoint(const decltype(execution::sql::Integer::val_) &key) {
        return static_cast<double>(key);
    }

    static double ConvertToPoint(const int &key) {
        return static_cast<double>(key);
    }

    static double ConvertToPoint(const decltype(execution::sql::Real::val_) &key) {
        return static_cast<double>(key);
    }
