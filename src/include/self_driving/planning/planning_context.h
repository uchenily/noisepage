...
public:
     ...
     */
    PlanningContext(std::string                                             ou_model_save_path,
                    std::string                                             interference_model_save_path,
    ...
    const std::string &GetOuModelSavePath() const {
        return ou_model_save_path_;
    }

    /** @brief Getter for interference_model_save_path_*/
    const std::string &GetInterferenceModelSavePath() const {
        return interference_model_save_path_;
    }
    ...
    common::ManagedPointer<optimizer::StatsStorage> GetStatsStorage() const {
        return stats_storage_;
    }

    /** @brief Getter for txn_manager_ */
    common::ManagedPointer<transaction::TransactionManager> GetTxnManager() const {
        return txn_manager_;
    }

    /** @brief Getter for query_exec_util_ */
    const std::unique_ptr<util::QueryExecUtil> &GetQueryExecUtil() const {
        return query_exec_util_;
    }
    ...
    const std::set<catalog::db_oid_t> &GetDBOids() const {
        return db_oids_;
    }
     ...
     */
    void
    AddOUInference(ExecutionOperatingUnitType ou_type, const std::vector<int> &feature, std::vector<double> label) {
    ...
    bool HasOUInference(ExecutionOperatingUnitType ou_type, const std::vector<int> &feature) const {
        if (ou_inference_cache_.find(ou_type) == ou_inference_cache_.end())
            return false;
        if (ou_inference_cache_.at(ou_type).find(feature) == ou_inference_cache_.at(ou_type).end())
            return false;
        return true;
    }
     ...
     */
    const std::vector<double> *GetOUInference(ExecutionOperatingUnitType ou_type,
                                              const std::vector<int>    &feature) const {
        NOISEPAGE_ASSERT(ou_inference_cache_.find(ou_type) != ou_inference_cache_.end(), "Cannot find OU type");
        NOISEPAGE_ASSERT(ou_inference_cache_.find(ou_type)->second.find(feature)
                             != ou_inference_cache_.find(ou_type)->second.end(),
                         "Cannot find feature");
    ...
    /**
     * Acquire labels from interference inference cache (assuming the cache exists. Use HasInterferenceInference()
     * first)
     * @param feature Input feature
     * @return cached labels
     */
