...
namespace parser {
    ...
    struct ColumnDefinition {
         ...
         */
        ColumnDefinition(std::string                                name,
                         DataType                                   type,
                         bool                                       is_primary,
                         bool                                       is_not_null,
                         bool                                       is_unique,
         ...
         * @return type ID
         */
        static execution::sql::SqlTypeId
        StrToValueType(char *str) { // TODO(Matt): compare with execution::sql::SqlTypeIdFromString to check redundancy
        ...
        common::ManagedPointer<TableInfo> GetTableInfo() {
            return common::ManagedPointer(table_info_);
        }
        ...
        int32_t GetTypeModifier() {
            return type_modifier_;
        }

        /** @return foreign key sources */
        std::vector<std::string> GetForeignKeySources() {
            return fk_sources_;
        }

        /** @return foreign key sinks */
        std::vector<std::string> GetForeignKeySinks() {
            return fk_sinks_;
        }

        /** @return foreign key sink table name */
        std::string GetForeignKeySinkTableName() {
            return fk_sink_table_name_;
        }

        /** @return foreign key delete action */
        FKConstrActionType GetForeignKeyDeleteAction() {
            return fk_delete_action_;
        }

        /** @return foreign key update action */
        FKConstrActionType GetForeignKeyUpdateAction() {
            return fk_update_action_;
        }

        /** @return foreign key match type */
        FKConstrMatchType GetForeignKeyMatchType() {
            return fk_match_type_;
        }
    ...
    private:
        ...
        bool           is_primary_
            = false; // not const because of how the parser returns us columns and primary key info separately
    ...
    public:
        ...
        explicit IndexAttr(common::ManagedPointer<AbstractExpression> expr)
            : has_expr_(true)
            , name_("")
            , expr_(expr) {}
    ...
    public:
        ...
        CreateStatement(std::unique_ptr<TableInfo>   table_info,
                        ...
                        const catalog::IndexOptions &index_options)
            : TableRefStatement(StatementType::CREATE, std::move(table_info))
            , create_type_(kIndex)
        ...
        CreateStatement(std::unique_ptr<TableInfo>                 table_info,
                        ...
                        common::ManagedPointer<AbstractExpression> trigger_when,
        ...
        catalog::IndexOptions &&MoveIndexOptions() {
            return std::move(index_options_);
        }
        ...
        common::ManagedPointer<SelectStatement> GetViewQuery() {
    ...
    private:
        ...
        const common::ManagedPointer<AbstractExpression> trigger_when_
            = common::ManagedPointer<AbstractExpression>(nullptr);
