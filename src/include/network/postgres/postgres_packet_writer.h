...
public:
     ...
     */
    explicit PostgresPacketWriter(const common::ManagedPointer<WriteQueue> write_queue)
        : PacketWriter(write_queue) {}
     ...
     */
    void
    WriteParseCommand(const std::string &destinationStmt, const std::string &query, const std::vector<int32_t> &params);
     ...
     * paramFormatCodes
     */
    void WriteBindCommand(const std::string                         &destinationPortal,
     ...
     * @param field_formats vector formats for the attributes to write
     */
    void WriteDataRow(const byte                                       *tuple,
