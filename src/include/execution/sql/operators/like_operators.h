...
struct Like {
    ...
    static bool Impl(const char *str,
                     std::size_t str_len,
                     const char *pattern,
    ...
    bool operator()(const storage::VarlenEntry &str,
                    const storage::VarlenEntry &pattern,
                    char                        escape = DEFAULT_ESCAPE) const {
        return Impl(reinterpret_cast<const char *>(str.Content()),
                    str.Size(),
                    reinterpret_cast<const char *>(pattern.Content()),
                    pattern.Size(),
                    escape);
    }
};
