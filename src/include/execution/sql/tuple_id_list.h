...
public:
    ...
    private:
        ...
        explicit ConstIterator(const BitVectorType &bv)
            : ConstIterator(bv, bv.FindFirst()) {}
     ...
     */
    explicit TupleIdList(uint32_t num_tuples)
        : bit_vector_(num_tuples) {}
     ...
     */
    void Resize(const uint32_t num_tuples) {
        bit_vector_.Resize(num_tuples);
    }

    /**
     * @return True if the given TID @em tid is in the list; false otherwise.
     */
    bool Contains(const uint32_t tid) const {
        return bit_vector_.Test(tid);
    }
     ...
     */
    void Enable(const uint32_t tid, const bool enable) {
        bit_vector_.Set(tid, enable);
    }
     ...
     * @param tid The ID of the tuple.
     */
    void Remove(const uint32_t tid) {
        bit_vector_.Unset(tid);
    }
    ...
    void AssignFrom(const TupleIdList &other) {
        bit_vector_.Copy(other.bit_vector_);
    }
    ...
    float ComputeSelectivity() const {
        return bit_vector_.ComputeDensity();
    }
     ...
     */
    [[nodiscard]]
    uint32_t ToSelectionVector(sel_t *sel_vec) const;
    ...
    TupleIdList &operator=(std::initializer_list<uint32_t> tids) {
        if (!std::all_of(tids.begin(), tids.end(), [this](auto tid) {
                return tid < GetCapacity();
            })) {
     ...
     */
    ConstIterator Begin() {
        return ConstIterator(bit_vector_);
    }
     ...
     */
    ConstIterator End() {
        return ConstIterator(bit_vector_, BitVectorType::INVALID_POS);
    }
...
public:
     ...
     */
    sel_t GetCurrentTupleId() const {
        return sel_vector_[curr_idx_];
    }
