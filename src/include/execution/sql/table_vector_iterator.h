...
public:
     ...
     */
    explicit TableVectorIterator(exec::ExecutionContext *exec_ctx,
                                 uint32_t                table_oid,
                                 uint32_t               *col_oids,
                                 uint32_t                num_oids);
    ...
    uint64_t GetVectorProjectionIteratorNumTuples() const {
        return vector_projection_iterator_.GetTotalTupleCount();
    }
     ...
     */
    static bool ParallelScan(uint32_t                table_oid,
                             uint32_t               *col_oids,
                             uint32_t                num_oids,
                             void                   *query_state,
...
private:
    ...
    bool Init(common::ManagedPointer<storage::SqlTable> table,
