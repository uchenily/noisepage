...
public:
    ...
    static void
    Execute(const exec::ExecutionSettings &exec_settings, const Vector &left, const Vector &right, Vector *result) {
     ...
     */
    template <typename LeftType, typename RightType, typename ResultType, typename Op, bool IgnoreNull = false>
    static void Execute(const exec::ExecutionSettings &exec_settings,
                        const Vector                  &left,
                        const Vector                  &right,
                        Vector                        *result,
                        Op                           &&op) {
        ...
        if (left.IsConstant()) {
            ExecuteImplConstantVector<LeftType, RightType, ResultType, Op, IgnoreNull>(exec_settings,
                                                                                       left,
                                                                                       right,
                                                                                       result,
                                                                                       std::forward<Op>(op));
        } else if (right.IsConstant()) {
            ExecuteImplVectorConstant<LeftType, RightType, ResultType, Op, IgnoreNull>(exec_settings,
                                                                                       left,
                                                                                       right,
                                                                                       result,
                                                                                       std::forward<Op>(op));
        } else {
            ExecuteImplVectorVector<LeftType, RightType, ResultType, Op, IgnoreNull>(exec_settings,
                                                                                     left,
                                                                                     right,
                                                                                     result,
                                                                                     std::forward<Op>(op));
...
private:
    ...
    template <typename LeftType, typename RightType, typename ResultType, typename Op, bool IgnoreNull>
    static void ExecuteImplVectorVector(const exec::ExecutionSettings &exec_settings,
                                        const Vector                  &left,
                                        const Vector                  &right,
                                        Vector                        *result,
                                        Op                           &&op) {
