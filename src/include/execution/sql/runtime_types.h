...
private:
    ...
    explicit Date(NativeType value)
        : value_(value) {}
...
public:
     ...
     */
    void ExtractComponents(int32_t *year,
                           int32_t *month,
                           int32_t *day,
                           int32_t *hour,
                           int32_t *min,
     ...
     * @return The constructed timestamp.
     */
    static Timestamp FromMicroseconds(uint64_t usec) {
     ...
     */
    static Timestamp AdjustTimeZone(char        c,
                                    int32_t     year,
                                    int32_t     month,
                                    int32_t     day,
                                    int32_t     hour,
     ...
     * @return The constructed timestamp if valid.
     */
    static Timestamp FromYMDHMSMU(int32_t year,
                                  int32_t month,
                                  int32_t day,
                                  int32_t hour,
                                  int32_t min,
...
private:
    ...
    explicit Timestamp(NativeType value)
        : value_(value) {}
...
public:
     ...
     */
    explicit Decimal(const T &value)
        : value_(value) {}
     ...
     */
    hash_t Hash(const hash_t seed) const {
        return common::HashUtil::HashCrc(value_);
    }
...
public:
     ...
     */
    char *PreAllocate(std::size_t len) {
        return heap_.Allocate(len);
    }
     ...
     * @return A varlen.
     */
    storage::VarlenEntry AddVarlen(const std::string &string) {
...
public:
     ...
     */
    Blob(byte *data, std::size_t size) noexcept
        : data_(data)
        , size_(size) {}
...
inline Timestamp Date::ConvertToTimestamp() const {
    return Timestamp(value_ * K_MICRO_SECONDS_PER_DAY);
}
/** Converts the provided timestamp into a date. */
inline Date Timestamp::ConvertToDate() const {
    return Date(value_ / K_MICRO_SECONDS_PER_DAY);
}
...
struct hash<noisepage::execution::sql::Date> {
     ...
     */
    size_t operator()(const noisepage::execution::sql::Date &date) const {
        return date.Hash();
    }
};
...
struct hash<noisepage::execution::sql::Timestamp> {
     ...
     */
    size_t operator()(const noisepage::execution::sql::Timestamp &timestamp) const {
...
struct hash<noisepage::execution::sql::Decimal<T>> {
     ...
     */
    size_t operator()(const noisepage::execution::sql::Decimal<T> &decimal) const {
        return decimal.Hash();
    }
};
} // namespace std
