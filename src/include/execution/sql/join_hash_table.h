...
public:
     ...
     */
    explicit JoinHashTable(const exec::ExecutionSettings &exec_settings,
     ...
     *         the join index.
     */
    uint64_t GetBufferedTupleMemoryUsage() const {
        return entries_.size() * entries_.ElementSize();
    }
     ...
     */
    bool UsingConciseHashTable() const {
        return use_concise_ht_;
    }
