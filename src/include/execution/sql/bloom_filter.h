...
public:
    ...
    uint64_t GetSizeInBytes() const {
        return sizeof(Block) * GetNumBlocks();
    }
    ...
    uint64_t GetSizeInBits() const {
        return GetSizeInBytes() * common::Constants::K_BITS_PER_BYTE;
    }
     ...
     */
    bool IsEmpty() const {
        return num_additions_ == 0;
    }

    /**
     * @return The number of elements that have been added to the filter.
     */
    uint32_t GetNumAdditions() const {
        return num_additions_;
    }
