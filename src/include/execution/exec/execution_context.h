...
public:
     ...
     */
    ExecutionContext(catalog::db_oid_t                                       db_oid,
                     common::ManagedPointer<transaction::TransactionContext> txn,
                     const OutputCallback                                   &callback,
                     const planner::OutputSchema                            *schema,
     ...
     */
    common::ManagedPointer<transaction::TransactionContext> GetTxn() {
        return txn_;
    }
     ...
     */
    sql::ThreadStateContainer *GetThreadStateContainer() {
        return thread_state_container_.get();
    }

    /**
     * @return the memory pool
     */
    sql::MemoryPool *GetMemoryPool() {
     ...
     */
    sql::VarlenHeap *GetStringAllocator() {
     ...
     */
    void SetExecutionMode(uint8_t mode) {
     ...
     */
    void SetAccessor(const common::ManagedPointer<catalog::CatalogAccessor> accessor) {
        accessor_ = accessor;
    }
    ...
    uint32_t GetRowsAffected() const {
        return rows_affected_;
    }

    /** Increment or decrement the number of rows affected. */
    void AddRowsAffected(int64_t num_rows) {
        rows_affected_ += num_rows;
    }
     ...
     */
    common::ManagedPointer<metrics::MetricsManager> GetMetricsManager() {
     ...
     */
    execution::query_id_t GetQueryId() {
        return query_id_;
    }
     ...
     */
    void SetQueryId(execution::query_id_t query_id) {
        query_id_ = query_id;
    }
     ...
     */
    void SetQueryState(void *query_state) {
     ...
     */
    void SetNumConcurrentEstimate(uint32_t estimate) {
        num_concurrent_estimate_ = estimate;
    }
