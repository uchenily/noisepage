...
namespace vm {
    ...
    public:
         ...
         */
        LocalVar()
            : LocalVar(K_INVALID_OFFSET, AddressMode::Address) {}
        ...
        uint32_t GetOffset() const {
            return LocalOffsetField::Decode(bitfield_);
        }
         ...
         */
        static LocalVar Decode(uint32_t encoded_var) {
            return LocalVar(encoded_var);
        }
        ...
        bool IsInvalid() const {
            return GetOffset() == K_INVALID_OFFSET;
        }
    ...
    private:
        explicit LocalVar(uint32_t bitfield)
            : bitfield_(bitfield) {}
    ...
    public:
         ...
         */
        FunctionId GetId() const noexcept {
            return id_;
        }
         ...
         */
        std::pair<std::size_t, std::size_t> GetBytecodeRange() const noexcept {
         ...
         */
        const std::vector<LocalInfo> &GetLocals() const noexcept {
            return locals_;
        }
         ...
         */
        std::size_t GetFrameSize() const noexcept {
            return frame_size_;
        }
         ...
         */
        std::size_t GetParamsStartPos() const noexcept {
            return params_start_pos_;
        }

        /**
         * @return The size (in bytes) of all input arguments, including alignment.
         */
        std::size_t GetParamsSize() const noexcept {
            return params_size_;
        }
         ...
         */
        uint32_t GetParamsCount() const noexcept {
            return num_params_;
        }
