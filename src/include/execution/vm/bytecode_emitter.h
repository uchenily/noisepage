...
public:
     ...
     */
    explicit BytecodeEmitter(std::vector<uint8_t> *bytecode)
        : bytecode_(bytecode) {
    ...
    void EmitThreadStateContainerReset(LocalVar   tls,
                                       LocalVar   state_size,
                                       FunctionId init_fn,
    ...
    void EmitTableIterInit(Bytecode bytecode,
                           LocalVar iter,
                           LocalVar exec_ctx,
                           LocalVar table_oid,
                           LocalVar col_oids,
    ...
    void
    EmitTempTableIterInit(Bytecode bytecode, LocalVar iter, LocalVar exec_ctx, LocalVar col_oids, uint32_t num_oids);

    /** Emit a parallel table scan. */
    void EmitParallelTableScan(LocalVar   table_oid,
                               LocalVar   col_oids,
                               uint32_t   num_oids,
    ...
    void EmitAggHashTableProcessBatch(LocalVar   agg_ht,
                                      LocalVar   vpi,
                                      uint32_t   num_keys,
                                      LocalVar   key_cols,
    ...
    void
    EmitAggHashTableParallelPartitionedScan(LocalVar agg_ht, LocalVar context, LocalVar tls, FunctionId scan_part_fn);
    ...
    /** ONLY FOR TESTING! */
    void EmitTestCatalogLookup(LocalVar oid_var,
                               LocalVar exec_ctx,
                               LocalVar table_name,
     ...
     */
    void EmitCteScanIteratorInit(Bytecode bytecode,
                                 LocalVar iter,
                                 LocalVar exec_ctx,
                                 LocalVar table_oid,
                                 LocalVar col_oids,
     ...
     */
    void EmitIndCteScanIteratorInit(Bytecode bytecode,
                                    LocalVar iter,
                                    LocalVar exec_ctx,
                                    LocalVar table_oid,
                                    LocalVar col_oids,
     ...
     */
    void
    EmitCteScanIteratorInit(Bytecode bytecode, LocalVar iter, LocalVar exec_ctx, LocalVar col_oids, uint32_t num_oids);
     ...
     * @param num_oids length of the array
     */
    void EmitIndexIteratorInit(Bytecode bytecode,
                               LocalVar iter,
                               LocalVar exec_ctx,
                               uint32_t num_attrs,
                               LocalVar table_oid,
     ...
     */
    void EmitStorageInterfaceInit(Bytecode bytecode,
...
private:
    ...
    void EmitImpl(const LocalVar local) {
        EmitScalarValue(local.Encode());
    }
