...
public:
     ...
     */
    BytecodeModule(std::string                 name,
     ...
     */
    const std::vector<FunctionInfo> &GetFunctionsInfo() const {
        return functions_;
    }
     ...
     */
    const std::vector<LocalInfo> &GetStaticLocalsInfo() const {
        return static_locals_;
    }
     ...
     */
    uint32_t GetStaticLocalsCount() const {
        return static_locals_.size();
    }
...
private:
    ...
    const uint8_t *AccessStaticLocalDataRaw(const uint32_t offset) const {
        ...
        NOISEPAGE_ASSERT(std::find_if(static_locals_.begin(),
                                      static_locals_.end(),
                                      [&](const LocalInfo &info) {
                                          return info.GetOffset() == offset;
                                      })
                             != static_locals_.end(),
