...
private:
    ...
    bool IsBuildPipeline(const Pipeline &pipeline) const {
        return &build_pipeline_ == &pipeline;
    }
    bool IsProducePipeline(const Pipeline &pipeline) const {
    ...
    ast::Identifier PerformLookup(FunctionBuilder *function,
                                  ast::Expr       *agg_ht,
    ...
    void            AdvanceAggregate(WorkContext     *ctx,
    ...
    // For minirunners.
    ast::StructDecl *GetStructDecl() const {
        return struct_decl_;
    }
