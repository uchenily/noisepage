...
public:
    ...
    /**
     * Intersect the sorted selection vectors @em sel_vector_1 with @em sel_vector_2 with lengths @em sel_vector_1_len
     * and
     ...
     */
    [[nodiscard]]
    static uint32_t IntersectSelected(const sel_t *sel_vector_1,
                                      uint32_t     sel_vector_1_len,
                                      const sel_t *sel_vector_2,
                                      uint32_t     sel_vector_2_len,
     ...
     */
    [[nodiscard]]
    static uint32_t IntersectSelected(const sel_t    *sel_vector,
                                      uint32_t        sel_vector_len,
                                      const uint64_t *bit_vector,
                                      uint32_t        bit_vector_len,
                                      sel_t          *out_sel_vector);

    /**
     * Populate the output selection vector @em out_sel_vector with all indexes that do not appear in the input
     * selection vector @em sel_vector.
     *
     ...
     */
    [[nodiscard]]
    static uint32_t DiffSelected(uint32_t n, const sel_t *sel_vector, uint32_t sel_vector_len, sel_t *out_sel_vector);
     ...
     */
    [[nodiscard]]
    static uint32_t ByteVectorToSelectionVector(const uint8_t *byte_vector, uint32_t num_bytes, sel_t *sel_vector);

    /**
     * Convert a byte vector to a bit vector. Set the corresponding bit in the bit vector @em bit_vector of all
     * positions in the byte vector containing a saturated byte value 0xFF.
     *
    ...
    /**
     * Convert a bit vector into a byte vector. For all set bits in the input bit vector, set the corresponding byte to
     * a saturated 8-bit integer.
     *
    ...
    /**
     * Convert a bit vector into a densely packed selection vector. Extract the indexes of all set (1) bits and store
     * into the output selection vector. The resulting selection vector is guaranteed to be sorted ascending.
     *
     * NOTE: Use this if you do not know the density of the bit vector. Otherwise, use the sparse or dense
     * implementations below which are optimized as appropriate.
     *
     * @param bit_vector The input bit vector.
     ...
     */
    [[nodiscard]]
    static uint32_t BitVectorToSelectionVector(const uint64_t *bit_vector, uint32_t num_bits, sel_t *sel_vector);
...
private:
    ...
    [[nodiscard]]
    static uint32_t BitVectorToSelectionVectorSparse(const uint64_t *bit_vector, uint32_t num_bits, sel_t *sel_vector);

    [[nodiscard]]
    static uint32_t BitVectorToSelectionVectorDense(const uint64_t *bit_vector, uint32_t num_bits, sel_t *sel_vector);

    [[nodiscard]]
    static uint32_t
    BitVectorToSelectionVectorDenseAvX2(const uint64_t *bit_vector, uint32_t num_bits, sel_t *sel_vector);

    [[nodiscard]]
    static uint32_t
    BitVectorToSelectionVectorDenseAVX512(const uint64_t *bit_vector, uint32_t num_bits, sel_t *sel_vector);

    /** A sorted-set difference implementation using purely scalar operations. */
    [[nodiscard]]
    static uint32_t DiffSelectedScalar(uint32_t n, const sel_t *sel_vector, uint32_t m, sel_t *out_sel_vector);
     ...
     */
    [[nodiscard]]
    static uint32_t DiffSelectedWithScratchPad(uint32_t     n,
