...
public:
     ...
     */
    explicit Vec512b(const __m512i &reg)
        : reg_(reg) {}

    /**
     * Type-cast operator so that Vec*'s can be used directly with intrinsics.
     */
    ALWAYS_INLINE operator __m512i() const {
        return reg_;
    } // NOLINT
...
public:
     ...
     * input pointer array.
     */
    Vec8 &Load(const uint32_t *ptr) {
        return Load(reinterpret_cast<const int32_t *>(ptr));
    }
     ...
     * input pointer array.
     */
    Vec8 &Load(const uint64_t *ptr) {
        return Load(reinterpret_cast<const int64_t *>(ptr));
    }
     ...
     * result into the output array.
     */
    void Store(uint8_t *arr) const {
        Store(reinterpret_cast<int8_t *>(arr));
    }
     ...
     * result into the output array.
     */
    void Store(uint16_t *arr) const {
        Store(reinterpret_cast<int16_t *>(arr));
    }
     ...
     * result into the output array.
     */
    void Store(uint32_t *arr) const {
        Store(reinterpret_cast<int32_t *>(arr));
    }
     ...
     * Store the eight 64-bit integers in this vector into the output array.
     */
    void Store(uint64_t *arr) const {
        Store(reinterpret_cast<int64_t *>(arr));
    }
     ...
     * Store the eight 64-bit integers in this vector into the output array.
     */
    void Store(unsigned long *arr) const {
        Store(reinterpret_cast<int64_t *>(arr));
    } // NOLINT (runtime/int)
#endif
...
ALWAYS_INLINE inline void Vec8::Store(int64_t *arr) const {
...
public:
     ...
     * @param val initial value for entire vector
     */
    explicit Vec16(int32_t val) {
        reg_ = _mm512_set1_epi32(val);
    }
    /**
     * Create a vector whose contents are the 512-bit register reg.
     * @param reg initial contents of the vector
     */
    explicit Vec16(const __m512i &reg)
        : Vec512b(reg) {}
    /**
     ...
     * @return number of elements that can be stored in this vector
     */
    ALWAYS_INLINE static constexpr int Size() {
        return 16;
    }
     ...
     * pointer array.
     */
    Vec16 &Load(const uint8_t *ptr) {
        return Load(reinterpret_cast<const int8_t *>(ptr));
    }
     ...
     * pointer array.
     */
    Vec16 &Load(const uint16_t *ptr) {
        return Load(reinterpret_cast<const int16_t *>(ptr));
    }
...
ALWAYS_INLINE inline Vec16 &Vec16::Gather<int8_t>(const int8_t *ptr, const Vec16 &pos) {
    alignas(64) int32_t x[Size()];
    pos.Store(x);
    reg_ = _mm512_setr_epi32(ptr[x[0]],
                             ptr[x[1]],
                             ptr[x[2]],
                             ptr[x[3]],
                             ptr[x[4]],
...
ALWAYS_INLINE inline void Vec16::Store(int32_t *arr) const {
    Vec512b::StoreUnaligned(reinterpret_cast<void *>(arr));
}
...
public:
     ...
     */
    explicit Vec8Mask(const __mmask8 &mask)
        : mask_(mask) {}
     ...
     */
    ALWAYS_INLINE bool Extract(uint32_t index) const {
        return (static_cast<uint32_t>(mask_) >> index) & 1;
    }
     ...
     * Type-cast operator so that Vec*'s can be used directly with intrinsics.
     */
    ALWAYS_INLINE operator __mmask8() const {
        return mask_;
    }
...
public:
     ...
     */
    explicit Vec16Mask(const __mmask16 &mask)
        : mask_(mask) {}
    ...
    bool Extract(uint32_t index) const {
        return static_cast<bool>((static_cast<uint32_t>(mask_) >> index) & 1);
    }
...
ALWAYS_INLINE inline Vec8 operator>>(const Vec8 &a, const uint32_t shift) {
...
static inline uint32_t FilterVectorByVal(const T *RESTRICT        in,
                                         uint32_t                 in_count,
                                         T                        val,
...
template <typename T, template <typename> typename Compare>
static inline uint32_t FilterVectorByVector(const T *RESTRICT        in_1,
