...
#undef F
     ...
     */
#define F(TypeClass)                                                                                                   \
    bool Is##TypeClass() const {                                                                                       \
        return llvm::isa<TypeClass>(this);                                                                             \
    }
    TYPE_LIST(F)
#undef F
...
#undef F
     ...
     */
    const char *GetTplName() const {
        return tpl_names[static_cast<uint16_t>(kind_)];
    }
    ...
    const char *GetCppName() const {
        return cpp_names[static_cast<uint16_t>(kind_)];
    }
    ...
    uint64_t GetSize() const {
        return SIZES[static_cast<uint16_t>(kind_)];
    }
    ...
    uint64_t GetAlignment() const {
        return ALIGNMENTS[static_cast<uint16_t>(kind_)];
    }
    ...
    bool IsPrimitive() const {
        return PRIMITIVE_FLAGS[static_cast<uint16_t>(kind_)];
    }
    ...
    bool IsFloatingPoint() const {
        return FLOATING_POINT_FLAGS[static_cast<uint16_t>(kind_)];
    }
    ...
    bool IsSqlValue() const {
        return Kind::Boolean <= GetKind() && GetKind() <= Kind::Timestamp;
    }
    ...
    static bool classof(const Type *type) {
        return type->GetTypeId() == TypeId::BuiltinType;
    } // NOLINT

private:
    friend class Context;
    BuiltinType(Context *ctx, uint32_t size, uint32_t alignment, Kind kind)
        : Type(ctx, size, alignment, TypeId::BuiltinType)
        , kind_(kind) {}
...
public:
     ...
     */
    static bool classof(const Type *type) {
        return type->GetTypeId() == TypeId::StringType;
    } // NOLINT
...
public:
     ...
     */
    static bool classof(const Type *type) {
        return type->GetTypeId() == TypeId::PointerType;
    } // NOLINT

private:
    explicit PointerType(Type *base)
        : Type(base->GetContext(), sizeof(int8_t *), alignof(int8_t *), TypeId::PointerType)
        , base_(base) {}
...
public:
     ...
     */
    static bool classof(const Type *type) {
        return type->GetTypeId() == TypeId::ArrayType;
    } // NOLINT
...
private:
    explicit ArrayType(uint64_t length, Type *elem_type)
        : Type(elem_type->GetContext(),
               ...
               (length == 0 ? alignof(uint8_t *) : elem_type->GetAlignment()),
               TypeId::ArrayType)
        , length_(length)
...
struct Field {
     ...
     */
    Field(const Identifier &name, Type *type)
        : name_(name)
        , type_(type) {}
...
public:
     ...
     */
    const util::RegionVector<Field> &GetParams() const {
        return params_;
    }
    ...
    uint32_t GetNumParams() const {
        return static_cast<uint32_t>(GetParams().size());
    }
     ...
     */
    static bool classof(const Type *type) {
        return type->GetTypeId() == TypeId::FunctionType;
    } // NOLINT
...
public:
     ...
     */
    static bool classof(const Type *type) {
        return type->GetTypeId() == TypeId::MapType;
    } // NOLINT
...
public:
     ...
     */
    const util::RegionVector<Field> &GetFieldsWithoutPadding() const {
        return unpadded_fields_;
    }

    /**
     * @returns A const reference to all fields in the struct including padding fields.
     */
    const util::RegionVector<Field> &GetAllFields() const {
        return fields_;
    }
     ...
     */
    static bool classof(const Type *type) {
        return type->GetTypeId() == TypeId::StructType;
    } // NOLINT

private:
    explicit StructType(Context                       *ctx,
                        uint32_t                       size,
                        uint32_t                       alignment,
...
inline bool Type::IsSqlBooleanType() const {
    return IsSpecificBuiltin(BuiltinType::Boolean);
}
