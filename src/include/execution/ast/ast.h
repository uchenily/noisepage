...
#undef F
    ...
    protected:
         ...
         */
        AstNode(Kind kind, const SourcePosition &pos)
            : kind_(kind)
            , pos_(pos) {}
    ...
    public:
        ...
        File(const SourcePosition &pos, util::RegionVector<Decl *> &&decls)
            : AstNode(Kind::File, pos)
            , decls_(std::move(decls)) {}

        /**
         * @return A const-view of the declarations making up the file.
         */
        const util::RegionVector<Decl *> &Declarations() const {
            return decls_;
        }
    ...
    public:
         ...
         */
        FunctionLitExpr *Function() const {
            return func_;
        }
    ...
    public:
        ...
        VariableDecl(const SourcePosition &pos, Identifier name, Expr *type_repr, Expr *init)
            : Decl(Kind::VariableDecl, pos, name, type_repr)
            , init_(init) {}
         ...
         */
        bool HasTypeDecl() const {
            return TypeRepr() != nullptr;
        }
    ...
    private:
        ...
        void SetInitial(ast::Expr *initial) {
            init_ = initial;
        }
    ...
    public:
         ...
         * @param pos source position
         */
        Stmt(Kind kind, const SourcePosition &pos)
            : AstNode(kind, pos) {}
    ...
    public:
        ...
        AssignmentStmt(const SourcePosition &pos, Expr *dest, Expr *src)
            : Stmt(AstNode::Kind::AssignmentStmt, pos)
            , dest_(dest)
            , src_(src) {}
    ...
    public:
         ...
         */
        const util::RegionVector<Stmt *> &Statements() const {
            return statements_;
        }
         ...
         */
        void AppendStatement(Stmt *stmt) {
         ...
         */
        const SourcePosition &RightBracePosition() const {
            return rbrace_pos_;
        }
         ...
         */
        void SetRightBracePosition(const SourcePosition &pos) {
            rbrace_pos_ = pos;
        }
         ...
         */
        Stmt *GetLast() const {
            return (IsEmpty() ? nullptr : statements_.back());
        }
    ...
    public:
        ...
        explicit DeclStmt(Decl *decl)
            : Stmt(Kind::DeclStmt, decl->Position())
            , decl_(decl) {}
    ...
    public:
         ...
         */
        const util::RegionVector<Expr *> &Arguments() const {
            return args_;
        }
         ...
         */
        bool IsBuiltinCall() const {
            return call_kind_ == CallKind::Builtin;
        }
    ...
    private:
        ...
        void SetCallKind(CallKind call_kind) {
            call_kind_ = call_kind;
        }
    ...
    public:
         ...
         */
        FunctionTypeRepr *TypeRepr() const {
            return type_repr_;
        }
    ...
    /**
     * Expressions for array or map accesses, e.g., x[i]. The object ('x' in the example) can either be an array or a
     * map. The index ('i' in the example) must evaluate to an integer for array access and the map's associated key
    ...
    public:
         ...
         */
        LitExpr(const SourcePosition &pos, int64_t num)
            : Expr(Kind::LitExpr, pos)
            , lit_kind_(LitKind::Int)
            , int64_(num) {}
         ...
         */
        LitExpr(const SourcePosition &pos, double num)
            : Expr(Kind::LitExpr, pos)
            , lit_kind_(LitKind::Float)
            , float64_(num) {}

        /**
         * @return The kind of literal this expression represents.
         */
        LitExpr::LitKind GetLiteralKind() const {
            return lit_kind_;
        }
    ...
    public:
        ...
        StructTypeRepr(const SourcePosition &pos, util::RegionVector<FieldDecl *> &&fields)
            : Expr(Kind::StructTypeRepr, pos)
            , fields_(std::move(fields)) {}

        /**
         * @return The fields of the struct.
         */
        const util::RegionVector<FieldDecl *> &Fields() const {
            return fields_;
        }
