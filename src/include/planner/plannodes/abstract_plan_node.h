...
public:
     ...
     */
    plan_node_id_t GetPlanNodeId() const {
     ...
     * node operator
     */
    common::ManagedPointer<OutputSchema> GetOutputSchema() const {
        return common::ManagedPointer(output_schema_);
    }
     ...
     */
    bool operator!=(const AbstractPlanNode &rhs) const {
        return !(*this == rhs);
    }
...
struct hash<std::unique_ptr<noisepage::planner::AbstractPlanNode>> {
     ...
     */
    size_t operator()(const std::unique_ptr<noisepage::planner::AbstractPlanNode> &plan) const {
        return plan->Hash();
    }
};
