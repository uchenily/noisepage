...
public:
    ...
    LogProviderType GetType() const override {
        return LogProviderType::REPLICATION;
    }
    ...
    /**
     * Block until a replication event is available; either replication ended, logs are received, or an OAT is
     * available.
    ...
    ReplicationEvent WaitUntilEvent() {
        std::unique_lock<std::mutex> lock(replication_latch_);
        while (true) {
            // The order here matters. Due to the AbstractLogProvider framework that was inherited, returning LOGS
            // without having all of the replication logs currently available may cause the log consumer to block.
            ...
            if (!replication_active_)
                return ReplicationEvent::END;
            if (OATReady())
                return ReplicationEvent::OAT;
            if (NonBlockingHasMoreRecords() || NextBatchReady())
                return ReplicationEvent::LOGS;
            replication_cv_.wait(lock, [&] {
                return !replication_active_ || NonBlockingHasMoreRecords() || NextBatchReady() || OATReady();
            });
        }
    }
     ...
     *
     * The OAT is used to signal to a replication log consumer, e.g., recovery, that all transactions up to and
     * including OAT are now safe to be applied, as long as all replication batches up to batch ID have been processed.
     *
...
private:
    ...
    struct OATPair {
        ...
        replication::record_batch_id_t
            batch_id_; ///< The last batch inclusive that must be processed before applying OAT.
    };

    /** @return True if left > right. False otherwise. */
    static bool CompareOATs(const OATPair &left, const OATPair &right) {
        return left.batch_id_ > right.batch_id_;
    }
    ...
    bool Read(void *dest, uint32_t size) override {
        if (curr_buffer_ == nullptr || !curr_buffer_->HasMore()) {
            std::unique_lock<std::mutex> lock(replication_latch_);
            replication_cv_.wait(lock, [&] {
                return !replication_active_ || NextBatchReady();
            });
            ...
            // Pop the next batch of records off into curr_buffer_.
            {
                ...
                NOISEPAGE_ASSERT(
                    (last_batch_popped_ == replication::INVALID_RECORD_BATCH_ID)
    ...
    std::priority_queue<replication::RecordsBatchMsg,
                        std::vector<replication::RecordsBatchMsg>,
