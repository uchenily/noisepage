...
constexpr uint16_t AttrSizeBytes(const uint16_t size) {
    return static_cast<uint16_t>((size) &INT16_MAX);
}
...
public:
     ...
     */
    const std::vector<col_id_t> &Varlens() const {
        return varlens_;
    }

    // TODO(Tianyu): Can probably store this like varlens to avoid computing every time.
    // TODO(Tianyu): The old test code has a util function that does this. Now that we are including this in the
    // codebase itself, we should replace the calls in test with this and delete that.
    /**
