...
public:
     ...
     */
    const std::atomic<UndoRecord *> &Next() const {
        return next_;
    }
     ...
     */
    std::atomic<transaction::timestamp_t> &Timestamp() {
        return timestamp_;
    }
    ...
    /**
     * Populates the UndoRecord's members to hold an update based on the redo changes that this UndoRecord is supposed
     * to log.
     *
     * @param head pointer to the byte buffer to initialize as a UndoRecord
     * @param timestamp timestamp of the transaction that generated this UndoRecord
