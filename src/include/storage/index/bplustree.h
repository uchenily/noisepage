...
public:
     ...
     */
    int GetLeafNodeSizeUpperThreshold() const {
        return leaf_node_size_upper_threshold_;
    }
    /**
     * @return leaf_node_size_lower_threshold
     */
    int GetLeafNodeSizeLowerThreshold() const {
...
public:
     ...
     */
    bool KeyCmpGreaterEqual(const KeyType &key1, const KeyType &key2) const {
     ...
     */
    bool KeyCmpGreater(const KeyType &key1, const KeyType &key2) const {
     ...
     */
    bool KeyCmpLessEqual(const KeyType &key1, const KeyType &key2) const {
     ...
     */
    bool ValueCmpEqual(const ValueType &v1, const ValueType &v2) {
    ...
    public:
         ...
         * Constructor
         */
        NodeMetaData(const KeyNodePointerPair *p_low_key_p,
                     const KeyNodePointerPair *p_high_key_p,
                     NodeType                  p_type,
                     int                       p_depth,
                     int                       p_item_count)
    ...
    public:
        ...
        BaseNode(NodeType                  p_type,
                 const KeyNodePointerPair *p_low_key_p,
                 const KeyNodePointerPair *p_high_key_p,
                 int                       p_depth,
                 int                       p_item_count)
         ...
         *
         */
        const NodeMetaData &GetNodeMetaData() const {
            return metadata_;
        }
         ...
         */
        const KeyType &GetLowKey() const {
            return metadata_.low_key_p->first;
        }
         ...
         * and inner nodes.
         */
        const KeyType &GetHighKey() const {
            return metadata_.high_key_p_->first;
        }

        /**
         * GetHighKeyPair() - Returns the pointer to high key node ptr pair
         */
        const KeyNodePointerPair &GetHighKeyPair() const {
            return *metadata_.high_key_p_;
        }

        /**
         * GetLowKeyPair() - Returns the pointer to low key node ptr pair
         */
        const KeyNodePointerPair &GetLowKeyPair() const {
            return *metadata_.low_key_p_;
        }
        ...
        int GetItemCount() const {
            return metadata_.item_count_;
        }

        /**
         * GetNodeExclusiveLatch() - Obtain the exclusive lock to the current node
         */
        void GetNodeExclusiveLatch() {
            metadata_.node_latch_.LockExclusive();
        }

        /**
         * GetNodeSharedLatch() - Obtain the Shared lock to the current node
         */
        void GetNodeSharedLatch() {
            metadata_.node_latch_.LockShared();
        }

        /**
         * ReleaseNodeLatch() - Release the lock to the current node
         */
        void ReleaseNodeLatch() {
            metadata_.node_latch_.UnlockExclusive();
        }

        /**
         * ReleaseNodeSharedLatch() - Release shared access to the current node
         */
        void ReleaseNodeSharedLatch() {
            metadata_.node_latch_.UnlockShared();
        }

        /**
         * GetLatchPointer() - Get the Latch Pointer of current node's latch
         */
        common::SharedLatch *GetLatchPointer() {
            return &(metadata_.node_latch_);
        }
        ...
        bool TryExclusiveLock() {
            return metadata_.node_latch_.TryExclusiveLock();
        }
         ...
         */
        bool TrySharedLock() {
            return metadata_.node_latch_.TryLockShared();
        }

        /**
         * SetLowKeyPair() - Sets the low key pair of metadata
         */
        void SetLowKeyPair(const KeyNodePointerPair *p_low_key_p) {
            metadata_.low_key_p_ = p_low_key_p;
        }

        /**
         * SetHighKeyPair() - Sets the high key pair of metdtata
         */
        void SetHighKeyPair(const KeyNodePointerPair *p_high_key_p) {
            metadata_.high_key_p_ = p_high_key_p;
        }

        /**
         * SetType() - Sets the type of metadata
         */
        void SetType(const NodeType input_type) {
            metadata_.type_ = input_type;
        }

        /**
         * SetDepth() - Sets the depth of metadata
         */
        void SetDepth(const int input_depth) {
            metadata_.depth_ = input_depth;
        }

        /**
         * SetItemCount() - Sets the item_count of metadata
         */
        void SetItemCount(const int input_item_count) {
            metadata_.item_count_ = input_item_count;
        }
    };
    ...
    public:
        ...
        ElasticNode(NodeType                  p_type,
                    int                       p_depth,
                    int                       p_item_count,
                    const KeyNodePointerPair *p_low_key,
    ...
    public:
        ...
        bool InsertElementIfPossible(const ElementType &element, ElementType *location) {
            if (this->GetSize() >= this->GetItemCount())
                return false;
            if (end_ - location > 0)
                std::memmove(reinterpret_cast<void *>(location + 1),
        ...
        ElasticNode *SplitNode() {
            if (this->GetSize() < this->GetItemCount())
                return nullptr;
            ElasticNode *new_node = this->Get(this->GetSize(),
                                              this->GetType(),
                                              this->GetDepth(),
                                              this->GetItemCount(),
                                              *this->GetElasticLowKeyPair(),
                                              *this->GetElasticHighKeyPair());
            ElementType *copy_from_location = Begin() + ((this->GetSize()) / 2);

            std::memcpy(reinterpret_cast<void *>(new_node->Begin()),
        ...
        bool Erase(int i) {
            ...
            std::memmove(reinterpret_cast<void *>(start_ + i),
                         reinterpret_cast<void *>(start_ + i + 1),
         ...
         */
        void SetElasticLowKeyPair(const KeyNodePointerPair &p_low_key) {
            low_key_ = p_low_key;
        }
         ...
         */
        void SetElasticHighKeyPair(const KeyNodePointerPair &p_high_key) {
            high_key_ = p_high_key;
        }
         ...
         */
        KeyNodePointerPair *GetElasticLowKeyPair() {
            return &low_key_;
        }
         ...
         */
        KeyNodePointerPair *GetElasticHighKeyPair() {
            return &high_key_;
        }
    ...
    public:
        ...
        KeyNodePointerPair *FindLocation(const KeyType &element, BPlusTree *tree) {
            ...
            KeyNodePointerPair *iter
                = std::lower_bound(this->Begin(), this->End(), dummy_element, [tree](const auto &a, const auto &b) {
                      return !tree->KeyCmpGreater(a.first, b.first);
                  });
            return iter;
        }
    };
...
public:
    ...
    void FreeTree() {
        ...
        while (!bfs_queue.empty()) {
            ...
            if (node->GetType() != NodeType::LeafType) {
                ...
                for (KeyNodePointerPair *element_p = current_node->Begin(); element_p != current_node->End();
                     element_p++) {
     ...
     */
    bool StructuralIntegrityVerification(KeyType            low_key,
                                         KeyType            high_key,
        ...
        if (current_node->GetType() == NodeType::LeafType) {
            ...
            return_answer &= this->StructuralIntegrityVerification(low_key,
                                                                   node->Begin()->first,
                                                                   keys,
     ...
     */
    bool ScanAscending(KeyType                              index_low_key,
                       KeyType                              index_high_key,
                       bool                                 low_key_exists,
                       uint32_t                             num_attrs,
                       bool                                 high_key_exists,
                       uint32_t                             limit,
        ...
        while ((limit == 0 || value_list->size() < limit) && (iterator != End()) && (iterator != Retry())
            ...
            if (!(limit == 0 || value_list->size() < limit))
                break;
            ++iterator;
        }
     ...
     * @return true on success, false on failure
     */
    bool ScanLimitDescending(KeyType                              index_low_key,
                             KeyType                              index_high_key,
                             std::vector<TupleSlot>              *value_list,
                             uint32_t                             limit,
                             std::function<bool(const ValueType)> predicate) {
    ...
    bool Insert(const KeyElementPair element, std::function<bool(const ValueType)> predicate) {
        ...
        if (root_ == nullptr) {
            ...
            root_ = ElasticNode<KeyValuePair>::Get(leaf_node_size_upper_threshold_,
                                                   NodeType::LeafType,
                                                   0,
        ...
        */
            ...
            if (node->InsertElementIfPossible(key_list_value,
                ...
                if (KeyCmpGreater(splitted_node_begin->first, element.first)) {
                    ...
                    splitted_node->InsertElementIfPossible(
                        key_list_value,
                        static_cast<LeafNode *>(splitted_node)->FindLocation(element.first, this));
                }
                ...
                if (node->GetElasticHighKeyPair()->second != nullptr) {
                    ...
                    auto node_next
                        = reinterpret_cast<ElasticNode<KeyNodePointerPair> *>(node->GetElasticHighKeyPair()->second);
        ...
        if (!finished_insertion) {
            ...
            root_ = ElasticNode<KeyNodePointerPair>::Get(inner_node_size_upper_threshold_,
                                                         NodeType::InnerType,
                                                         root_->GetDepth() + 1,
    ...
    bool Delete(BaseNode *current_node, const KeyElementPair &element, std::vector<common::SharedLatch *> *lock_list) {
        ...
        if (current_node != root_ && current_node->GetType() == NodeType::LeafType
            && (reinterpret_cast<ElasticNode<KeyValuePair> *>(current_node))->GetSize()
                   > GetLeafNodeSizeLowerThreshold()) {
            condition_for_underflow = false;
        }
            ...
            if (is_deleted) {
                if (!lock_list->empty()) {
                    if (child_pointer->GetType() == NodeType::LeafType) {
                        ...
                        DeleteRebalance<KeyNodePointerPair>(node,
                                                            child_pointer,
                                                            index,
                                                            GetInnerNodeSizeLowerThreshold());
                    }
    ...
    size_t EstimateHeapUsage() {
        ...
        size_t heap_usage = (depth * GetInnerNodeSizeLowerThreshold() * sizeof(KeyNodePointerPair))
                            + // InnerNode size (assuming half full)
                            (num_keys_ * sizeof(KeyType)) + (num_values_ * sizeof(ValueType)); // LeafNode size
