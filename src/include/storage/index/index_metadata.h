...
public:
     ...
     */
    const catalog::IndexSchema &GetSchema() const {
        return key_schema_;
    }
     ...
     */
    const std::vector<uint16_t> &GetAttributeSizes() const {
        return attr_sizes_;
    }
     ...
     */
    bool MustInlineVarlen() const {
        return must_inline_varlen_;
    }
     ...
     */
    void SetKeyKind(const IndexKeyKind key_kind) {
        key_kind_ = key_kind;
    }
...
private:
    ...
    static std::vector<uint16_t> ComputeInlinedAttributeSizes(const catalog::IndexSchema &key_schema) {
        ...
        for (const auto &key : key_cols) {
            ...
            case execution::sql::SqlTypeId::Varchar: {
                ...
                auto varlen_size = std::max(static_cast<uint16_t>(key.TypeModifier() + 4),
    ...
    static std::vector<uint16_t> ComputePROffsets(const std::vector<uint16_t> &attr_sizes) {
        ...
        // sort by the sizes
        std::stable_sort(size_idx.begin(), size_idx.end(), [](const auto &u, const auto &v) -> bool {
     ...
     */
    static std::unordered_map<catalog::indexkeycol_oid_t, uint16_t>
    ComputeKeyOidToOffset(const catalog::IndexSchema &key_schema, const std::vector<uint16_t> &pr_offsets) {
    ...
    static std::vector<uint16_t> GetRealAttrSizes(std::vector<uint16_t> attr_sizes) {
        std::transform(attr_sizes.begin(), attr_sizes.end(), attr_sizes.begin(), [](uint16_t elem) -> uint16_t {
            return AttrSizeBytes(elem);
        });
        return attr_sizes;
    }
};
