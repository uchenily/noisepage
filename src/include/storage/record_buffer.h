...
public:
     ...
     */
    void Reuse(RecordBufferSegment *const reused) {
        reused->Reset();
    }
     ...
     */
    void Delete(RecordBufferSegment *const ptr) {
        delete ptr;
    }
};
...
public:
     ...
     * @param buffer_pool The buffer pool to draw buffer segments from. Must be the same buffer pool the log manager
     * uses.
     */
    ...
    /**
     * Reserve a redo record with the given size, in bytes. The returned pointer is guaranteed to be valid until
     * NewEntry is called again, or when the buffer is explicitly flushed by the call Finish().
