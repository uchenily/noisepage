...
void TransactionManager::LogCommit(TransactionContext *const txn,
                                   ...
                                   const callback_fn         commit_callback,
                                   void *const               commit_callback_arg,
    ...
    if (log_manager_ != DISABLED && txn->GetDurabilityPolicy() != DurabilityPolicy::DISABLE) {
        if (txn->GetDurabilityPolicy() == DurabilityPolicy::SYNC) {
            ...
            storage::CommitRecord::Initialize(commit_record,
                                              txn->StartTime(),
                                              commit_time,
                                              commit_callback,
                                              commit_callback_arg,
            ...
            if (txn->IsReadOnly()) {
                ...
                // We still want to send this record to the LogManager, but we'll swap in the EmptyCallback to send with
                // the LogRecord, and invoke the provided callback immediately. The WAL worker will still be responsible
                // for removing it from the running transactions table once the record has been serialized. Otherwise
                // the serializer might dereference bad memory if the GC prunes any varlens this transaction might be
                // holding.
                byte *const commit_record
                    = txn->redo_buffer_.NewEntry(storage::CommitRecord::Size(), txn->GetTransactionPolicy());
                storage::CommitRecord::Initialize(commit_record,
        ...
        // Otherwise, logging is disabled. We should pretend to have serialized and flushed the record so the rest of
        // the system proceeds correctly
        timestamp_manager_->RemoveTransaction(txn->StartTime());
        commit_callback(commit_callback_arg);
    }
...
TransactionManager::Commit(TransactionContext *const txn, transaction::callback_fn callback, void *callback_arg) {
    ...
    if (log_manager_ != DISABLED && !txn->IsReadOnly()) {
        ...
        //
        // The OldestActiveTransaction (OAT) was included with a commit record so that the replica would know when it
        // was safe to apply the deferred records. However, this leads to a deadlock with the new "active txn"
        // semantics, where:
        ...
        // the commit record OAT is still desirable to kick off deferred records, but that is as a performance
        // optimization, the commit record OAT is strictly speaking is no longer related to the overall correctness of
        // replication. See docs/design_replication.md for more details.
    ...
    if (txn_metrics_enabled) {
        common::thread_context.resource_tracker_.Stop();
        auto &resource_metrics = common::thread_context.resource_tracker_.GetMetrics();
        common::thread_context.metrics_store_->RecordCommitData(static_cast<uint64_t>(txn->IsReadOnly()),
                                                                resource_metrics);
    }
...
void TransactionManager::LogAbort(TransactionContext *const txn) {
    ...
    if (log_manager_ != DISABLED && txn->redo_buffer_.HasFlushed()) {
        ...
        byte *const abort_record
            = txn->redo_buffer_.NewEntry(storage::AbortRecord::Size(), txn->GetTransactionPolicy());
        ...
        // pretend to have flushed the record so the rest of the system proceeds correctly Discard the redo buffer that
        // is not yet logged out
...
void TransactionManager::DeallocateInsertedTupleIfVarlen(TransactionContext                 *txn,
                                                         storage::UndoRecord                *undo,
                                                         const storage::TupleAccessStrategy &accessor) const {
    const storage::BlockLayout &layout = accessor.GetBlockLayout();
