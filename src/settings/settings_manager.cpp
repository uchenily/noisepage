...
void SettingsManager::ValidateSetting(Param                                  param,
    ...
    if (!ValidateValue(info.value_, min_value, max_value)) {
        throw SETTINGS_EXCEPTION(fmt::format("{} is outside the valid range for parameter \"{}\" ({} .. {})",
                                             info.value_.ToString(),
                                             info.name_,
                                             min_value.ToString(),
                                             max_value.ToString()),
...
#define DEFINE_SETTINGS_MANAGER_SET(Name, CppType, FrontendType, ExecutionType, ShouldCheckMinMaxBounds)               \
    void SettingsManager::Set##Name(Param                                 param,                                       \
                                    CppType                               value,                                       \
        ...
        if (action_state == ActionState::FAILURE) {                                                                    \
            ...
            /* TODO(WAN): I don't know what the right ErrorCode is here. */                                            \
            throw SETTINGS_EXCEPTION(                                                                                  \
                fmt::format("Callback failed after setting \"{}\" from \"{}\" to \"{}\", revert {}.",                  \
                            param_info.name_,                                                                          \
                            old_value,                                                                                 \
                            value,                                                                                     \
                            revert_success ? "succeeded" : "failed"),                                                  \
                common::ErrorCode::ERRCODE_INTERNAL_ERROR);                                                            \
        }                                                                                                              \
                                                                                                                       \
        setter_callback(action_context);                                                                               \
...
void SettingsManager::SetString(Param                                 param,
    ...
    if (!SetValue(param, {execution::sql::SqlTypeId::Varchar, string_val.first, std::move(string_val.second)})) {
        ...
        setter_callback(action_context);
        /* TODO(WAN): I don't know what the right ErrorCode is here. */
        throw SETTINGS_EXCEPTION(fmt::format("Unable to set \"{}\" from \"{}\" to \"{}\".",
                                             param_info.name_,
                                             old_cve->GetStringVal().StringView(),
                                             string_val.first.StringView()),
                                 common::ErrorCode::ERRCODE_INTERNAL_ERROR);
    }
...
void SettingsManager::SetParameter(const std::string                                                     &name,
    ...
    NOISEPAGE_ASSERT(std::all_of(values.begin(),
                                 values.end(),
                                 [](const auto &expr) {
    ...
    if (value->GetReturnValueType() != param_type) {
        try {
            ...
            throw SETTINGS_EXCEPTION(
                fmt::format("invalid value for parameter \"{}\": \"{}\"", info.name_, value->ToString()),
                common::ErrorCode::ERRCODE_INVALID_PARAMETER_VALUE);
        }
    }
