...
void IndexCreateTranslator::InitializeStorageInterface(FunctionBuilder *function,
    ...
    std::vector<ast::Expr *> args{storage_interface_ptr,
                                  GetExecutionContext(),
                                  table_oid_expr,
                                  col_oids_expr,
                                  need_indexes_expr};
...
void IndexCreateTranslator::LaunchWork(FunctionBuilder *function, ast::Identifier work_func) const {
    ...
    ast::Expr *iter_table_parallel = codegen_->CallBuiltin(ast::Builtin::TableIterParallel,
                                                           {codegen_->Const32(table_oid_.UnderlyingValue()),
                                                            global_col_oids_.Get(codegen_),
                                                            GetQueryStatePtr(),
                                                            GetExecutionContext(),
                                                            codegen_->ConstU32(num_threads_override),
                                                            codegen_->MakeExpr(work_func)});
...
void IndexCreateTranslator::PerformPipelineWork(WorkContext *context, FunctionBuilder *function) const {
    ...
    if (declare_local_tvi) {
        ...
        if (IsPipelineMetricsEnabled()) {
            ...
            auto *record = codegen->CallBuiltin(ast::Builtin::ExecutionContextSetMemoryUseOverride,
                                                {GetExecutionContext(), get_mem});
...
void IndexCreateTranslator::SetGlobalOids(FunctionBuilder *function, ast::Expr *global_col_oids) const {
    for (uint64_t i = 0; i < all_oids_.size(); i++) {
        ...
        ast::Expr *lhs = codegen_->GetAstContext()->GetNodeFactory()->NewIndexExpr(codegen_->GetPosition(),
                                                                                   global_col_oids,
                                                                                   codegen_->Const64(i));
...
void IndexCreateTranslator::DeclareTVI(FunctionBuilder *function) const {
    ...
    ast::Expr *table_iter_init = codegen_->CallBuiltin(ast::Builtin::TableIterInit,
                                                       {codegen->MakeExpr(tvi_var_),
                                                        GetExecutionContext(),
                                                        codegen_->Const32(table_oid_.UnderlyingValue()),
                                                        global_col_oids_.Get(codegen_)});
...
void IndexCreateTranslator::ScanTable(WorkContext *ctx, FunctionBuilder *function) const {
    ...
    Loop tvi_loop(function, codegen_->TableIterAdvance(codegen_->MakeExpr(tvi_var_)));
    {
        ...
        function->Append(
            codegen_->DeclareVarWithInit(vpi_var_, codegen_->TableIterGetVPI(codegen_->MakeExpr(tvi_var_))));
...
void IndexCreateTranslator::ScanVPI(WorkContext *ctx, FunctionBuilder *function, ast::Expr *vpi) const {
    auto gen_vpi_loop = [&](bool is_filtered) {
        Loop vpi_loop(function,
                      nullptr,
                      codegen_->VPIHasNext(vpi, is_filtered),
...
void IndexCreateTranslator::IndexInsert(WorkContext *ctx, FunctionBuilder *function) const {
    ...
    for (const auto &index_col : index_schema.GetColumns()) {
        ...
        const auto &col_expr
            = codegen_->VPIGet(codegen_->MakeExpr(vpi_var_), sql_type, tbl_col.Nullable(), scan_offset);
    ...
    auto *index_insert_call = codegen_->CallBuiltin(ast::Builtin::IndexInsertWithSlot,
                                                    {local_storage_interface_.GetPtr(codegen_),
                                                     local_tuple_slot_.GetPtr(codegen_),
...
auto IndexCreateTranslator::GenerateEndHookFunction() const -> ast::FunctionDecl * {
    ...
    FunctionBuilder builder(codegen, parallel_build_post_hook_fn_, std::move(params), ret_type);
    {
        ...
        builder.Append(codegen->CallBuiltin(ast::Builtin::ExecutionContextSetMemoryUseOverride,
                                            {exec_ctx, codegen->MakeExpr(heap)}));
