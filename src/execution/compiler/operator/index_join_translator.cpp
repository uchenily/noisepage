...
void IndexJoinTranslator::FinishPipelineWork(const Pipeline &pipeline, FunctionBuilder *function) const {
    ...
    if (IsCountersEnabled()) {
        ...
        auto *num_loops_pos
            = codegen->Compare(parsing::Token::Type::GREATER, CounterVal(num_loops_), codegen->Const32(0));
        If check(function, num_loops_pos);
        {
            ast::Expr *div
                = codegen->BinaryOp(parsing::Token::Type::SLASH, per_loop_num_scans_expr, CounterVal(num_loops_));
    ...
    FeatureRecord(function,
                  ...
                  pipeline,
                  CounterVal(index_size_));
    FeatureRecord(function,
                  selfdriving::ExecutionOperatingUnitType::IDX_SCAN,
                  selfdriving::ExecutionOperatingUnitFeatureAttribute::NUM_LOOPS,
                  pipeline,
                  CounterVal(num_loops_));
...
void IndexJoinTranslator::DeclareIterator(FunctionBuilder *builder) const {
    ...
    ast::Expr *init_call
        = GetCodeGen()->IndexIteratorInit(index_iter_,
                                          ...
                                          op.GetIndexOid().UnderlyingValue(),
                                          col_oids_);
...
void IndexJoinTranslator::FillKey(
    ...
    for (const auto &key : index_exprs) {
        ...
        auto                     *set_key_call = GetCodeGen()->PRSet(GetCodeGen()->MakeExpr(pr),
                                                 attr_type,
                                                 nullable,
                                                 attr_offset,
