...
ast::Expr *OperatorTranslator::GetOutput(WorkContext *context, uint32_t attr_idx) const {
    ...
    if (attr_idx >= output_schema->NumColumns()) {
        throw EXECUTION_EXCEPTION(fmt::format("Codegen: Cannot read column {} from '{}' with output schema {}",
                                              attr_idx,
...
void OperatorTranslator::GetAllChildOutputFields(const uint32_t                        child_index,
...
void OperatorTranslator::CounterSet(FunctionBuilder              *function,
                                    const StateDescriptor::Entry &counter,
                                    int64_t                       val) const {
    auto *codegen = GetCodeGen();
...
void OperatorTranslator::CounterSetExpr(FunctionBuilder              *function,
                                        const StateDescriptor::Entry &counter,
                                        ast::Expr                    *val) const {
    auto *codegen = GetCodeGen();
...
void OperatorTranslator::CounterAdd(FunctionBuilder              *function,
                                    const StateDescriptor::Entry &counter,
                                    int64_t                       val) const {
    auto *codegen = GetCodeGen();
...
void OperatorTranslator::FeatureRecord(FunctionBuilder                                    *function,
                                       selfdriving::ExecutionOperatingUnitType             feature_type,
    ...
    if (IsCountersEnabled() && IsPipelineMetricsEnabled()) {
        ...
        ast::Expr  *record
            = codegen->ExecOUFeatureVectorRecordFeature(pipeline.OUFeatureVecPtr(),
                                                        pipeline_id,
                                                        feature.GetFeatureId(),
                                                        attrib,
                                                        selfdriving::ExecutionOperatingUnitFeatureUpdateMode::SET,
                                                        val);
        function->Append(record);
    }
}

void OperatorTranslator::FeatureArithmeticRecordSet(FunctionBuilder           *function,
                                                    const Pipeline            &pipeline,
...
void OperatorTranslator::FeatureArithmeticRecordMul(FunctionBuilder           *function,
                                                    const Pipeline            &pipeline,
                                                    execution::translator_id_t translator_id,
                                                    ast::Expr                 *val) const {
    auto *codegen = GetCodeGen();
            ...
            if (is_same_translator && is_arith) {
                for (const auto &attrib : {selfdriving::ExecutionOperatingUnitFeatureAttribute::NUM_ROWS,
                    ...
                    ast::Expr *record = codegen->ExecOUFeatureVectorRecordFeature(
                        ...
                        feature.GetFeatureId(),
                        attrib,
                        selfdriving::ExecutionOperatingUnitFeatureUpdateMode::MULT,
                        val);
                    function->Append(record);
                }
            }
