...
InsertTranslator::InsertTranslator(const planner::InsertPlanNode &plan,
...
void InsertTranslator::DeclareInserter(noisepage::execution::compiler::FunctionBuilder *builder) const {
    ...
    ast::Expr *inserter_setup
        = GetCodeGen()->StorageInterfaceInit(si_inserter_.GetPtr(GetCodeGen()),
                                             GetExecutionContext(),
                                             GetPlanAs<planner::InsertPlanNode>().GetTableOid().UnderlyingValue(),
                                             col_oids_,
                                             true);
...
void InsertTranslator::GenValueSetTablePR(FunctionBuilder *builder, WorkContext *context, uint32_t idx) const {
    ...
    for (size_t i = 0; i < node_vals.size(); i++) {
        ...
        const auto &pr_set_call = GetCodeGen()->PRSet(GetCodeGen()->MakeExpr(insert_pr_),
                                                      table_col.Type(),
                                                      table_col.Nullable(),
...
void InsertTranslator::GenIndexInsert(WorkContext                *context,
