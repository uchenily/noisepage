...
auto HashJoinTranslator::GenerateEndHookFunction() const -> ast::FunctionDecl * {
    ...
    FunctionBuilder builder(codegen, parallel_build_post_hook_fn_, std::move(params), ret_type);
    {
        ...
        FeatureRecord(&builder,
                      selfdriving::ExecutionOperatingUnitType::PARALLEL_MERGE_HASHJOIN,
...
void HashJoinTranslator::InsertIntoJoinHashTable(WorkContext *ctx, FunctionBuilder *function) const {
    ...
    function->Append(
        codegen->DeclareVarWithInit(build_row_var_,
...
void HashJoinTranslator::CollectUnmatchedLeftRows(FunctionBuilder *function) const {
    ...
    Loop loop(function,
              codegen->MakeStmt(codegen->JoinHTIteratorInit(jht_iter_expr, jht)),
