...
void CteScanLeaderTranslator::DeclareCteScanIterator(FunctionBuilder *builder) const {
    ...
    ast::Expr *cte_scan_iterator_setup
        = codegen->CteScanIteratorInit(cte_scan_val_entry_.GetPtr(codegen),
...
void CteScanLeaderTranslator::FillPRFromChild(WorkContext *context, FunctionBuilder *builder) const {
    ...
    for (const auto &col : cols) {
        ...
        auto set_decl
            = codegen->DeclareVar(insertion_val, codegen->TplType(execution::sql::GetTypeId(col.Type())), val);
        builder->Append(set_decl);

        auto pr_set_call = codegen->PRSet(codegen->MakeExpr(insert_pr_),
                                          col.Type(),
                                          false,
                                          table_pm_.find(table_col_oid)->second.UnderlyingValue(),
                                          codegen->MakeExpr(insertion_val),
                                          true);
