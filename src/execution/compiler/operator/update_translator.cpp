...
UpdateTranslator::UpdateTranslator(const planner::UpdatePlanNode &plan,
...
void UpdateTranslator::DeclareUpdater(noisepage::execution::compiler::FunctionBuilder *builder) const {
    ...
    ast::Expr *updater_setup
        = GetCodeGen()->StorageInterfaceInit(si_updater_.GetPtr(GetCodeGen()),
                                             GetExecutionContext(),
                                             GetPlanAs<planner::UpdatePlanNode>().GetTableOid().UnderlyingValue(),
                                             col_oids_,
                                             true);
...
void UpdateTranslator::GenSetTablePR(FunctionBuilder *builder, WorkContext *context) const {
    ...
    for (const auto &clause : clauses) {
        ...
        auto       *pr_set_call = GetCodeGen()->PRSet(GetCodeGen()->MakeExpr(update_pr_),
                                                table_col.Type(),
                                                table_col.Nullable(),
                                                table_pm_.find(table_col_oid)->second,
                                                clause_expr,
                                                true);
    ...
    for (const auto oid : all_oids_) {
        if (set_oids.find(oid) == set_oids.end()) {
            ...
            ast::Expr  *set_pr = GetCodeGen()->PRSet(GetCodeGen()->MakeExpr(update_pr_),
                                                    col.Type(),
                                                    col.Nullable(),
                                                    idx,
                                                    child_expr,
...
void UpdateTranslator::GenIndexDelete(FunctionBuilder            *builder,
                                      WorkContext                *context,
                                      const catalog::index_oid_t &index_oid) const {
