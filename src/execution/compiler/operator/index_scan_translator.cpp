...
auto IndexScanTranslator::GetTableColumn(catalog::col_oid_t col_oid) const -> ast::Expr * {
...
void IndexScanTranslator::DeclareIterator(FunctionBuilder *builder) const {
    ...
    ast::Expr *init_call
        = GetCodeGen()->IndexIteratorInit(index_iter_.GetPtr(GetCodeGen()),
...
auto IndexScanTranslator::GetSlotAddress() const -> ast::Expr * {
    // &slot
...
void IndexScanTranslator::FreeIterator(FunctionBuilder *builder) const {
    // @indexIteratorFree(&pipelineState.indexIterator)
    ast::Expr *free_call
        = GetCodeGen()->CallBuiltin(ast::Builtin::IndexIteratorFree, {index_iter_.GetPtr(GetCodeGen())});
