...
HashAggregationTranslator::HashAggregationTranslator(const planner::AggregatePlanNode &plan,
    ...
    for (size_t agg_term_idx = 0; agg_term_idx < plan.GetAggregateTerms().size(); agg_term_idx++) {
        ...
        if (agg_term->IsDistinct()) {
            distinct_filters_.emplace(std::make_pair(agg_term_idx,
                                                     DistinctAggregationFilter(agg_term_idx,
                                                                               agg_term,
                                                                               plan.GetGroupByTerms().size(),
                                                                               compilation_context,
                                                                               pipeline,
                                                                               GetCodeGen())));
        }
    ...
    pipeline->RegisterSource(this,
                             build_pipeline_.IsParallel() ? Pipeline::Parallelism::Parallel
...
auto HashAggregationTranslator::GeneratePayloadStruct() -> ast::StructDecl * {
    ...
    for (const auto &term : GetAggPlan().GetAggregateTerms()) {
        ...
        auto type = codegen->AggregateType(term->GetExpressionType(),
                                           sql::GetTypeId(term->GetReturnValueType()),
...
void HashAggregationTranslator::MergeOverflowPartitions(FunctionBuilder *function, ast::Expr *agg_ht, ast::Expr *iter) {
    ...
    Loop loop(function,
              nullptr,
              codegen->AggPartitionIteratorHasNext(iter),
              codegen->MakeStmt(codegen->AggPartitionIteratorNext(iter)));
    {
        ...
        function->Append(codegen->DeclareVarWithInit(lookup_result,
                                                     codegen->AggHashTableLookup(agg_ht,
                                                                                 codegen->MakeExpr(hash_val),
                                                                                 key_check_partial_fn_,
                                                                                 codegen->MakeExpr(partial_row),
                                                                                 agg_payload_type_)));
...
auto HashAggregationTranslator::GenerateEndHookFunction() const -> ast::FunctionDecl * {
    ...
    FunctionBuilder builder(codegen, parallel_build_post_hook_fn_, std::move(params), ret_type);
    {
        ...
        FeatureRecord(&builder,
                      selfdriving::ExecutionOperatingUnitType::PARALLEL_MERGE_AGGBUILD,
...
void HashAggregationTranslator::TearDownPipelineState(const Pipeline &pipeline, FunctionBuilder *function) const {
    if (IsBuildPipeline(pipeline)) {
        ...
        // If any of the aggregators are required to be cleaned up, then we need to go through the hash table and free
        // them
        auto *codegen = GetCodeGen();
...
void HashAggregationTranslator::ConstructNewAggregate(FunctionBuilder *function,
                                                      ast::Expr       *agg_ht,
...
void HashAggregationTranslator::AdvanceAggregate(WorkContext     *ctx,
    ...
    for (uint32_t term_idx = 0; term_idx < agg_terms.size(); term_idx++) {
        ...
        if (agg_term->IsDistinct()) {
            ...
            distinct_filters_.at(term_idx).AggregateDistinct(codegen,
                                                             function,
                                                             agg_advance_call,
                                                             agg_val,
                                                             groupby_exprs);
...
void HashAggregationTranslator::UpdateAggregates(WorkContext     *context,
                                                 FunctionBuilder *function,
...
void HashAggregationTranslator::PerformPipelineWork(WorkContext *context, FunctionBuilder *function) const {
    ...
    if (IsBuildPipeline(context->GetPipeline())) {
        ...
        NOISEPAGE_ASSERT(IsProducePipeline(context->GetPipeline()),
...
void HashAggregationTranslator::FinishPipelineWork(const Pipeline &pipeline, FunctionBuilder *function) const {
    if (IsBuildPipeline(pipeline)) {
        if (build_pipeline_.IsParallel()) {
            ...
            function->Append(codegen->AggHashTableMovePartitions(global_agg_ht,
...
auto HashAggregationTranslator::GetChildOutput(WorkContext *context, uint32_t child_idx, uint32_t attr_idx) const
    ...
    if (IsProducePipeline(context->GetPipeline())) {
        ...
        return GetCodeGen()->AggregatorResult(GetExecutionContext(),
                                              GetAggregateTermPtr(agg_row_var_, attr_idx),
