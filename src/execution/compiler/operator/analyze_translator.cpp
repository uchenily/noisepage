...
AnalyzeTranslator::AnalyzeTranslator(const planner::AnalyzePlanNode &plan,
    ...
    for (const auto &col_info : catalog::postgres::PgStatisticImpl::ANALYZE_AGGREGATES) {
        auto agg_var
            = codegen->MakeFreshIdentifier("stat_col_" + std::to_string(col_info.column_oid_.UnderlyingValue()));
...
void AnalyzeTranslator::PerformPipelineWork(WorkContext *context, FunctionBuilder *function) const {
    ...
    NOISEPAGE_ASSERT(plan.GetChild(0)->GetOutputSchema()->NumColumns()
                         == catalog::postgres::PgStatisticImpl::NUM_ANALYZE_AGGREGATES * plan.GetColumnOids().size()
                                + 1,
    ...
    for (size_t column_offset = 0; column_offset < plan.GetColumnOids().size(); column_offset++) {
        ...
        // for (@indexIteratorScanKey(&pg_statistic_index_iterator);
...
void AnalyzeTranslator::SetPgStatisticColOids(FunctionBuilder *function) const {
    ...
    // var pg_statistic_col_oids: [num_cols]uint32
    function->Append(codegen->DeclareVarNoInit(pg_statistic_col_oids_,
...
void AnalyzeTranslator::DeclarePgStatisticIndexPR(FunctionBuilder *function) const {
    ...
    // var pg_statistic_index_pr: *ProjectedRow
    function->Append(
        codegen->DeclareVarNoInit(pg_statistic_index_pr_,
                                  codegen->PointerType(codegen->BuiltinType(ast::BuiltinType::Kind::ProjectedRow))));
}

void AnalyzeTranslator::AssignColumnStatistics(WorkContext     *context,
...
void AnalyzeTranslator::InitPgStatisticIterator(FunctionBuilder *function) const {
    ...
    // pg_statistic_col_oids)
    auto *init_call
        = codegen->IndexIteratorInit(pg_statistic_index_iterator_.GetPtr(codegen),
                                     ...
                                     catalog::postgres::PgStatistic::STATISTIC_OID_INDEX_OID.UnderlyingValue(),
                                     pg_statistic_col_oids_);
...
void AnalyzeTranslator::DeclareAndInitPgStatisticUpdater(FunctionBuilder *function) const {
    ...
    auto *updater_setup
        = codegen->StorageInterfaceInit(pg_statistic_updater_.GetPtr(codegen),
                                        GetExecutionContext(),
                                        catalog::postgres::PgStatistic::STATISTIC_TABLE_OID.UnderlyingValue(),
                                        pg_statistic_col_oids_,
                                        true);
...
void AnalyzeTranslator::SetPgStatisticTablePr(FunctionBuilder *function) const {
    ...
    for (size_t i = 0; i < pg_statistic_table_schema_.GetColumns().size(); i++) {
        ...
        auto       *pr_set_call = codegen->PRSet(codegen->MakeExpr(pg_statistic_update_pr_),
                                           col.Type(),
                                           col.Nullable(),
                                           pg_statistic_table_pm_.find(col.Oid())->second,
                                           var_expr,
                                           true);
...
void AnalyzeTranslator::DeleteFromPgStatisticIndex(FunctionBuilder     *function,
                                                   ast::Identifier      slot,
...
void AnalyzeTranslator::FillIndexPrKey(FunctionBuilder *function, ast::Identifier index_projected_row, bool own) const {
    ...
    for (const auto &col : pg_statistic_index_schema_.GetColumns()) {
        ...
        auto *set_key_call = codegen->PRSet(codegen->MakeExpr(index_projected_row),
                                            attr_type,
                                            nullable,
                                            pg_statistic_index_pm_.at(col.Oid()),
                                            var_expr,
                                            own);
