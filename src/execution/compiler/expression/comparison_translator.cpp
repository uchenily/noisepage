...
ComparisonTranslator::ComparisonTranslator(const parser::ComparisonExpression &expr,
    ...
    NOISEPAGE_ASSERT(
        (expr.GetExpressionType() != parser::ExpressionType::COMPARE_IN)
...
auto ComparisonTranslator::DeriveValue(WorkContext *ctx, const ColumnValueProvider *provider) const -> ast::Expr * {
    ...
    case parser::ExpressionType::COMPARE_IN: {
        ...
        // Convention: child 0 is "foo", child 1 is the first listed value, and these are handled separately at the
        // start.
