...
ExecutableQueryFragmentBuilder::ExecutableQueryFragmentBuilder(ast::Context *ctx)
    : ctx_(ctx) {}
...
ExecutableQueryFragmentBuilder::Compile(const execution::compiler::CompilerSettings &settings) {
    ...
    EXECUTION_LOG_DEBUG("Type-check: {:.2f} ms, Bytecode Gen: {:.2f} ms, Module Gen: {:.2f} ms",
                        timer.GetSemaTimeMs(),
                        timer.GetBytecodeGenTimeMs(),
                        timer.GetModuleGenTimeMs());
