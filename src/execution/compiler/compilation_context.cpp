...
namespace {
    // A unique ID generator used to generate globally unique TPL function names and keep track of query ID for
    // minirunners.
...
CompilationContext::CompilationContext(ExecutableQuery               *query,
                                       ...
                                       const exec::ExecutionSettings &settings)
    ...
    , query_state_(query_state_type_,
                   [this](CodeGen *codegen) {
                       return codegen->MakeExpr(query_state_var_);
                   })
...
void CompilationContext::GeneratePlan(const planner::AbstractPlanNode              &plan,
    ...
    exec_ctx_ = query_state_.DeclareStateEntry(GetCodeGen(),
                                               "execCtx",
                                               codegen_.PointerType(ast::BuiltinType::ExecutionContext));
