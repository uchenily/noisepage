...
Pipeline::Pipeline(CompilationContext *ctx)
    ...
    , state_(codegen_->MakeIdentifier(fmt::format("P{}_State", id_)),
             [this](CodeGen *codegen) {
                 return codegen_->MakeExpr(state_var_);
             })
    , driver_(nullptr)
...
void Pipeline::SetParallelCheck(bool check) {
    check_parallelism_ = check;
}
...
void Pipeline::Prepare(const exec::ExecutionSettings &exec_settings) {
    ...
    // Pretty print.
    {
        ...
        EXECUTION_LOG_TRACE("Pipeline-{}: parallel={}, vectorized={}, steps=[{}]",
                            id_,
                            IsParallel(),
                            IsVectorized(),
                            result);
...
ast::FunctionDecl *Pipeline::GenerateInitPipelineFunction() const {
    ...
    FunctionBuilder builder(codegen_, name, std::move(params), codegen_->Nil());
    {
        ...
        if (!nested_) {
            ...
            builder.Append(codegen_->TLSReset(codegen_->MakeExpr(tls),
                                              state.GetTypeName(),
                                              GetSetupPipelineStateFunctionName(),
                                              GetTearDownPipelineStateFunctionName(),
                                              state_ptr));
        } else {
...
void Pipeline::CallNestedRunPipelineFunction(WorkContext              *ctx,
                                             const OperatorTranslator *op,
    ...
    function->Append(
        codegen_->DeclareVarNoInit(p_state, codegen_->MakeExpr(GetPipelineStateDescriptor().GetTypeName())));
