...
private:
    ...
    void DumpIdentifier(Identifier str) {
        out_.write(str.GetData(), str.GetLength());
    }
    ...
    void DumpDecl(Decl *decl) {
        DumpChild([=] {
            AstVisitor<AstDumperImpl>::Visit(decl);
        });
    }

    void DumpExpr(Expr *expr) {
        DumpChild([=] {
            AstVisitor<AstDumperImpl>::Visit(expr);
        });
    }

    void DumpStmt(Stmt *stmt) {
        DumpChild([=] {
            AstVisitor<AstDumperImpl>::Visit(stmt);
        });
    }
...
auto AstDump::Dump(AstNode *node) -> std::string {
