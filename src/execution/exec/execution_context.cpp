...
void ExecutionContext::StartPipelineTracker(pipeline_id_t pipeline_id) {
    ...
    if (common::thread_context.metrics_store_ != nullptr
        ...
        if (common::thread_context.resource_tracker_.IsRunning()) {
            ...
            NOISEPAGE_ASSERT(!common::thread_context.resource_tracker_.IsRunning(),
                             "ResourceTrackers cannot be nested");
            common::thread_context.resource_tracker_.Start();
            mem_tracker_->Reset();
        }
...
void ExecutionContext::EndPipelineTracker(query_id_t                        query_id,
    ...
    if (common::thread_context.metrics_store_ != nullptr && common::thread_context.resource_tracker_.IsRunning()) {
        ...
        common::thread_context.metrics_store_->RecordPipelineData(query_id,
                                                                  pipeline_id,
                                                                  execution_mode_,
                                                                  std::move(features),
                                                                  resource_metrics);
...
void ExecutionContext::InitializeOUFeatureVector(selfdriving::ExecOUFeatureVector *ouvec, pipeline_id_t pipeline_id) {
    ...
    vec->pipeline_features_
        = std::make_unique<execution::sql::MemPoolVector<selfdriving::ExecutionOperatingUnitFeature>>(features.begin(),
                                                                                                      features.end(),
                                                                                                      GetMemoryPool());
...
void ExecutionContext::InitializeParallelOUFeatureVector(selfdriving::ExecOUFeatureVector *ouvec,
    ...
    case selfdriving::ExecutionOperatingUnitType::HASHJOIN_BUILD:
        vec->pipeline_features_->emplace_back(selfdriving::ExecutionOperatingUnitType::PARALLEL_MERGE_HASHJOIN,
                                              feature);
        break;
    case selfdriving::ExecutionOperatingUnitType::AGGREGATE_BUILD:
        vec->pipeline_features_->emplace_back(selfdriving::ExecutionOperatingUnitType::PARALLEL_MERGE_AGGBUILD,
                                              feature);
        break;
    case selfdriving::ExecutionOperatingUnitType::SORT_BUILD:
        ...
        vec->pipeline_features_->emplace_back(selfdriving::ExecutionOperatingUnitType::PARALLEL_SORT_MERGE_STEP,
                                              feature);
        break;
    case selfdriving::ExecutionOperatingUnitType::SORT_TOPK_BUILD:
        vec->pipeline_features_->emplace_back(selfdriving::ExecutionOperatingUnitType::PARALLEL_SORT_TOPK_STEP,
                                              feature);
