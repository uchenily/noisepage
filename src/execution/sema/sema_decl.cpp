...
void Sema::VisitVariableDecl(ast::VariableDecl *node) {
    ...
    if (declared_type != nullptr && initializer_type != nullptr) {
        ...
        if (!CheckAssignmentConstraints(declared_type, &init)) {
            GetErrorReporter()->Report(node->Position(),
                                       ErrorMessages::kInvalidAssignment,
                                       declared_type,
                                       initializer_type);
            return;
        }
...
void Sema::VisitFieldDecl(ast::FieldDecl *node) {
    Visit(node->TypeRepr());
}
...
void Sema::VisitStructDecl(ast::StructDecl *node) {
    ...
    if (const ast::FieldDecl *dup = nullptr;
        ...
        GetErrorReporter()->Report(node->Position(),
                                   ErrorMessages::kDuplicateStructFieldName,
                                   dup->Name(),
                                   node->Name());
        return;
    }
