...
File::File(const std::string_view &path, uint32_t flags)
    : error_(Error::OK) {
...
void File::Open(const std::string_view &path, uint32_t flags) {
    Initialize(path, flags);
}

void File::Create(const std::string_view &path) {
