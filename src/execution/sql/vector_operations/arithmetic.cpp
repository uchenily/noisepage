...
namespace {
    ...
    void CheckBinaryOperation(const Vector &left, const Vector &right, Vector *result) {
        ...
        if (!left.IsConstant() && !right.IsConstant() && left.GetCount() != right.GetCount()) {
            throw EXECUTION_EXCEPTION(
                fmt::format(
                    "Left and right input vectors to binary operation must have the same size, left {} right {}.",
                    left.GetCount(),
                    right.GetCount()),
                common::ErrorCode::ERRCODE_INTERNAL_ERROR);
        }
    ...
    template <typename T, template <typename> typename Op>
    void TemplatedBinaryArithmeticOperation(const exec::ExecutionSettings &exec_settings,
                                            const Vector                  &left,
                                            const Vector                  &right,
                                            Vector                        *result) {
    ...
    template <template <typename> typename Op>
    void BinaryArithmeticOperation(const exec::ExecutionSettings &exec_settings,
                                   const Vector                  &left,
                                   const Vector                  &right,
                                   Vector                        *result) {
        // Sanity check
...
void VectorOps::Add(const exec::ExecutionSettings &exec_settings,
                    const Vector                  &left,
                    const Vector                  &right,
                    Vector                        *result) {
...
void VectorOps::Subtract(const exec::ExecutionSettings &exec_settings,
                         const Vector                  &right,
                         Vector                        *result,
                         const Vector                  &left) {
...
void VectorOps::Multiply(const exec::ExecutionSettings &exec_settings,
                         const Vector                  &left,
                         const Vector                  &right,
                         Vector                        *result) {
...
void VectorOps::Divide(const exec::ExecutionSettings &exec_settings,
                       const Vector                  &left,
                       const Vector                  &right,
                       Vector                        *result) {
