    ...
    void TemplatedLikeOperation(const Vector &a, const Vector &b, TupleIdList *tid_list) {
        if (a.GetTypeId() != TypeId::Varchar || b.GetTypeId() != TypeId::Varchar) {
            throw EXECUTION_EXCEPTION(fmt::format("Inputs to (NOT) LIKE must be VARCHAR, left {} right {}.",
                                                  TypeIdToString(a.GetTypeId()),
                                                  TypeIdToString(b.GetTypeId())),
...
void VectorOps::SelectLike(const exec::ExecutionSettings &exec_settings,
                           const Vector                  &a,
                           const Vector                  &b,
...
void VectorOps::SelectNotLike(const exec::ExecutionSettings &exec_settings,
                              const Vector                  &a,
                              const Vector                  &b,
                              TupleIdList                   *tid_list) {
