...
namespace {
    ...
    void CheckGatherAndSelect(const Vector    &input,
        ...
        if (result->GetCapacity() != input.GetSize()) {
            throw EXECUTION_EXCEPTION(fmt::format("Result list not large enough to store all TIDs in input vector, "
                                                  "input size {} results capacity {}.",
                                                  input.GetSize(),
                                                  result->GetCapacity()),
                                      common::ErrorCode::ERRCODE_INTERNAL_ERROR);
        }
    ...
    void TemplatedGatherAndSelectOperationConstant(const Vector     &input,
                                                   const Vector     &pointers,
    ...
    template <typename T, typename Op>
    void TemplatedGatherAndSelectOperationVector(const Vector     &input,
                                                 const Vector     &pointers,
                                                 const std::size_t offset,
                                                 TupleIdList      *tid_list) {
    ...
    void TemplatedGatherAndSelectOperation(const Vector     &input,
                                           const Vector     &pointers,
                                           const std::size_t offset,
                                           TupleIdList      *tid_list) {
        if (input.IsConstant()) {
    ...
    void GatherAndSelectOperation(const Vector     &input,
                                  const Vector     &pointers,
                                  const std::size_t offset,
                                  TupleIdList      *tid_list) {
        // Sanity check.
...
void VectorOps::GatherAndSelectEqual(const Vector     &input,
                                     const Vector     &pointers,
                                     const std::size_t offset,
                                     TupleIdList      *tid_list) {
...
void VectorOps::GatherAndSelectGreaterThan(const Vector     &input,
                                           const Vector     &pointers,
                                           const std::size_t offset,
                                           TupleIdList      *tid_list) {
...
void VectorOps::GatherAndSelectGreaterThanEqual(const Vector     &input,
                                                const Vector     &pointers,
                                                const std::size_t offset,
                                                TupleIdList      *tid_list) {
...
void VectorOps::GatherAndSelectLessThan(const Vector     &input,
                                        const Vector     &pointers,
                                        const std::size_t offset,
                                        TupleIdList      *tid_list) {
...
void VectorOps::GatherAndSelectLessThanEqual(const Vector     &input,
                                             const Vector     &pointers,
                                             const std::size_t offset,
                                             TupleIdList      *tid_list) {
...
void VectorOps::GatherAndSelectNotEqual(const Vector     &input,
                                        const Vector     &pointers,
                                        const std::size_t offset,
                                        TupleIdList      *tid_list) {
