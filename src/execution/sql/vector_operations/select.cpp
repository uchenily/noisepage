...
namespace {
    ...
    template <typename T>
    struct IsSafeForFullCompute<T,
                                std::enable_if_t<std::is_fundamental_v<T> || std::is_same_v<T, Date>
    ...
    void CheckSelection(const Vector &left, const Vector &right, TupleIdList *result) {
        if (left.GetTypeId() != right.GetTypeId()) {
            throw EXECUTION_EXCEPTION(fmt::format("Input vector types must match for selections, left {} right {}.",
                                                  ...
                                                  TypeIdToString(right.GetTypeId())),
                                      common::ErrorCode::ERRCODE_INTERNAL_ERROR);
        }
        if (!left.IsConstant() && !right.IsConstant()) {
            if (left.GetSize() != right.GetSize()) {
                throw EXECUTION_EXCEPTION(
                    fmt::format("Left and right vectors to comparison have different sizes, left {} right {}.",
                                left.GetSize(),
                                right.GetSize()),
                    common::ErrorCode::ERRCODE_INTERNAL_ERROR);
            }
            if (left.GetCount() != right.GetCount()) {
                throw EXECUTION_EXCEPTION(
                    fmt::format("Left and right vectors to comparison have different counts, left {} right {}.",
                                left.GetCount(),
                                right.GetCount()),
                    common::ErrorCode::ERRCODE_INTERNAL_ERROR);
            }
        ...
        if constexpr (IsSafeForFullCompute<T>::VALUE) { // NOLINT
            ...
            if (full_compute_threshold <= tid_list->ComputeSelectivity()) {
                ...
                bit_vector->UpdateFull([&](uint64_t i) {
                    return Op{}(left_data[i], constant);
                });
    ...
    void TemplatedSelectOperationVectorVector(const exec::ExecutionSettings &exec_settings,
        ...
        if constexpr (IsSafeForFullCompute<T>::VALUE) { // NOLINT
            ...
            if (full_compute_threshold <= tid_list->ComputeSelectivity()) {
                TupleIdList::BitVectorType *bit_vector = tid_list->GetMutableBits();
                bit_vector->UpdateFull([&](uint64_t i) {
                    return Op{}(left_data[i], right_data[i]);
                });
    ...
    void TemplatedSelectOperation(const exec::ExecutionSettings &exec_settings,
                                  const Vector                  &left,
                                  const Vector                  &right,
                                  TupleIdList                   *tid_list) {
        if (right.IsConstant()) {
            ...
            TemplatedSelectOperationVectorConstant<T, typename Op<T>::SymmetricOp>(exec_settings,
                                                                                   right,
                                                                                   left,
                                                                                   tid_list);
        } else {
    ...
    void SelectOperation(const exec::ExecutionSettings &exec_settings,
                         const Vector                  &left,
                         const Vector                  &right,
                         TupleIdList                   *tid_list) {
        // Sanity check
...
void VectorOps::SelectEqual(const exec::ExecutionSettings &exec_settings,
                            const Vector                  &left,
                            const Vector                  &right,
                            TupleIdList                   *tid_list) {
...
void VectorOps::SelectGreaterThan(const exec::ExecutionSettings &exec_settings,
                                  const Vector                  &left,
                                  const Vector                  &right,
                                  TupleIdList                   *tid_list) {
...
void VectorOps::SelectLessThan(const exec::ExecutionSettings &exec_settings,
                               const Vector                  &left,
                               const Vector                  &right,
                               TupleIdList                   *tid_list) {
...
void VectorOps::SelectLessThanEqual(const exec::ExecutionSettings &exec_settings,
                                    const Vector                  &left,
                                    const Vector                  &right,
                                    TupleIdList                   *tid_list) {
...
void VectorOps::SelectNotEqual(const exec::ExecutionSettings &exec_settings,
                               const Vector                  &left,
                               const Vector                  &right,
                               TupleIdList                   *tid_list) {
