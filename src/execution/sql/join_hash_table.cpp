...
JoinHashTable::JoinHashTable(const exec::ExecutionSettings &exec_settings,
...
namespace {
    ...
    public:
        ...
        void SetProcessed(HashTableEntry *entry) const {
            entry->cht_slot_ |= PROCESSED_BIT;
        }
        ...
        void SetBuffered(HashTableEntry *entry) const {
            entry->cht_slot_ |= BUFFERED_BIT;
        }
        ...
        void Reset(const uint64_t new_buf_idx) {
            buf_idx_ = new_buf_idx;
        }
...
void JoinHashTable::ReorderOverflowEntries() {
    ...
    for (uint64_t start = overflow_start_idx; start < num_entries; start += common::Constants::K_DEFAULT_VECTOR_SIZE) {
        ...
        for (uint64_t idx = start, write_idx = 0, prefetch_idx = idx + common::Constants::K_PREFETCH_DISTANCE;
             idx < end;
...
void JoinHashTable::BuildConciseHashTableInternal() {
    ...
    EXECUTION_LOG_TRACE("Concise Table Stats: {} entries, {} overflow ({} % overflow)",
                        entries_.size(),
...
void JoinHashTable::LookupBatchInChainingHashTable(const Vector &hashes, Vector *results) const {
    UnaryOperationExecutor::Execute<hash_t, const HashTableEntry *>(exec_settings_,
                                                                    hashes,
                                                                    results,
                                                                    [&](const hash_t hash_val) noexcept {
                                                                        return chaining_hash_table_.FindChainHead(
                                                                            hash_val);
                                                                    });
}
...
void JoinHashTable::LookupBatchInConciseHashTable(const Vector &hashes, Vector *results) const {
    UnaryOperationExecutor::Execute<hash_t, const HashTableEntry *>(exec_settings_,
                                                                    ...
                                                                    results,
                                                                    [&](const hash_t hash_val) noexcept {
                                                                        const auto [found, entry_idx]
                                                                            = concise_hash_table_.Lookup(hash_val);
...
void JoinHashTable::MergeParallel(ThreadStateContainer *thread_state_container, const std::size_t jht_offset) {
    ...
    if (use_serial_build) {
        ...
        llvm::for_each(tl_join_tables, [this](auto *source) {
    ...
    EXECUTION_LOG_TRACE("JHT: {} merged {} JHTs. Estimated {}, actual {}. Time: {:.2f} ms ({:.2f} mtps)",
                        ...
                        tl_join_tables.size(),
                        num_elem_estimate,
                        chaining_hash_table_.GetElementCount(),
                        timer.GetElapsed(),
                        tps);
