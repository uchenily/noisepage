...
void AggregationHashTable::LookupInitial() {
    ...
    UnaryOperationExecutor::Execute<hash_t, const HashTableEntry *>(exec_settings_,
                                                                    *batch_state_->Hashes(),
                                                                    batch_state_->Entries(),
...
void AggregationHashTable::FollowNext() {
    ...
    batch_state_->KeyNotEqual()->Filter([&](uint64_t i) {
        return (raw_entries[i] = raw_entries[i]->next_) != nullptr;
    });
}
...
namespace {
    ...
    void TemplatedFixGrouping(AggregationHashTable::HashToGroupIdMap *hash_to_group_map,
        ...
        tid_list->Filter([&](const uint64_t i) {
            auto *gid = hash_to_group_map->Find(raw_hashes[i], [&](const uint16_t gid) {
                return raw_keys[gid] == raw_keys[i];
            });
            if (gid != nullptr) {
    ...
    template <typename F>
    void FixGrouping(AggregationHashTable::HashToGroupIdMap *groups,
                     const Vector                           &hashes,
                     const Vector                           &entries,
                     const Vector                           &probe_keys,
                     TupleIdList                            *tid_list,
    ...
    for (const auto key_index : key_indexes) {
        ...
        FixGrouping(batch_state_->HashToGroupMap(), // Hash-to-group mapping
                    ...
                    *key_vector,                    // Keys
                    ...
                    [this](const hash_t hash) {
                        return AllocateEntryInternal(hash);
                    });
    }
...
void AggregationHashTable::TransferMemoryAndPartitions(ThreadStateContainer *thread_states,
...
void AggregationHashTable::ExecuteParallelPartitionedScan(void                                       *query_state,
    ...
    const uint64_t tuple_count = std::accumulate(nonempty_parts.begin(),
                                                 nonempty_parts.end(),
                                                 uint64_t{0},
...
void AggregationHashTable::Repartition() {
    ...
    tbb::parallel_for_each(nonempty_tables, [&](auto table) {
        table->FlushToOverflowPartitions();
    });
...
void AggregationHashTable::MergePartitions(AggregationHashTable                  *target,
                                           void                                  *query_state,
