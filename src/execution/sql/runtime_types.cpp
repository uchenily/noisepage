...
namespace {
    ...
    constexpr int32_t K_DAYS_PER_MONTH[2][12] = {
...
void Timestamp::ExtractComponents(int32_t *year,
                                  int32_t *month,
                                  int32_t *day,
                                  int32_t *hour,
                                  int32_t *min,
...
Timestamp Timestamp::FromYMDHMSMU(int32_t year,
    ...
    // Check for major overflow.
    if ((result - time) / K_MICRO_SECONDS_PER_DAY != date) {
        throw CONVERSION_EXCEPTION(fmt::format("timestamp out of range {}-{}-{} {}:{}:{}.{}{} out of range",
                                               year,
                                               month,
                                               day,
                                               hour,
