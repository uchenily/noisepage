...
CteScanIterator::CteScanIterator(noisepage::execution::exec::ExecutionContext *exec_ctx,
                                 catalog::table_oid_t                          table_oid,
                                 uint32_t                                     *schema_cols_ids,
                                 uint32_t                                     *schema_cols_type,
                                 uint32_t                                      num_schema_cols)
    : exec_ctx_(exec_ctx)
    , cte_table_oid_(table_oid)
    , table_redo_(nullptr) {
    ...
    for (uint32_t i = 0; i < num_schema_cols; i++) {
        catalog::Schema::Column col(
            "col" + std::to_string(i),
            static_cast<execution::sql::SqlTypeId>(schema_cols_type[i]),
            false,
    ...
    exec_ctx->GetAccessor()->RegisterTempTable(
        table_oid,
        common::ManagedPointer(cte_table_),
    ...
    exec_ctx_->GetTxn()->RegisterAbortAction([=](transaction::DeferredActionManager *deferred_action_manager) {
        deferred_action_manager->RegisterDeferredAction([=]() {
            deferred_action_manager->RegisterDeferredAction([=]() {
                // Defer an action upon abort to delete the table. Delete table will need a double deferral because
                // there could be transactions not yet unlinked by the GC that depend on the table
                delete cte_table_local;
            });
        });
