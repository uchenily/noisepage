...
bool DDLExecutors::CreateTableExecutor(const common::ManagedPointer<planner::CreateTablePlanNode> node,
    ...
    if (node->HasPrimaryKey()) {
        ...
        for (const auto &parser_col : primary_key_info.primary_key_cols_) {
            ...
            if (table_col.Type() == execution::sql::SqlTypeId::Varchar
                ...
                key_cols.emplace_back(table_col.Name(),
                                      table_col.Type(),
        ...
        catalog::IndexSchema
            index_schema(key_cols, storage::index::IndexType::BPLUSTREE, true, true, false, true, options);

        // Create the index, and use its return value as overall success result
        result = result
                 && CreateIndex(accessor,
                                node->GetNamespaceOid(),
                                primary_key_info.constraint_name_,
                                table_oid,
                                index_schema);
    }
    ...
    for (const auto &unique_constraint : node->GetUniqueConstraints()) {
        ...
        catalog::IndexOptions options;
        catalog::IndexSchema
            index_schema(key_cols, storage::index::IndexType::BPLUSTREE, true, false, false, true, options);

        // Create the index, and use its return value as overall success result
        result = result
                 && CreateIndex(accessor,
                                node->GetNamespaceOid(),
                                unique_constraint.constraint_name_,
                                table_oid,
                                index_schema);
    }
