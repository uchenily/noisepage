...
void FilterManager::Clause::RunFilter(exec::ExecutionContext *exec_ctx,
                                      VectorProjection       *input_batch,
    ...
    for (const auto &term : terms_) {
        ...
        const auto exec_ns = util::TimeNanos([&]() {
            term->fn_(exec_ctx, input_batch, &temp_, opaque_context_);
        });
        ...
        EXECUTION_LOG_TRACE("Term [{}]: term-selectivity={:04.3f}, cost={:>06.3f}, rank={:.8f}",
                            term->insertion_index_,
                            term_selectivity,
                            term_cost,
                            term->rank_);
