...
void BytecodeEmitter::Bind(BytecodeLabel *label) {
    ...
    if (label->IsForwardTarget()) {
        ...
        for (const auto &jump_location : jump_locations) {
            ...
            NOISEPAGE_ASSERT((curr_offset - jump_location)
...
void BytecodeEmitter::EmitTableIterInit(Bytecode bytecode,
                                        LocalVar iter,
                                        LocalVar exec_ctx,
                                        LocalVar table_oid,
                                        LocalVar col_oids,
...
void BytecodeEmitter::EmitTempTableIterInit(Bytecode bytecode,
                                            LocalVar iter,
                                            LocalVar exec_ctx,
                                            LocalVar col_oids,
                                            uint32_t num_oids) {
...
void BytecodeEmitter::EmitParallelTableScan(LocalVar   table_oid,
                                            LocalVar   col_oids,
                                            uint32_t   num_oids,
                                            LocalVar   query_state,
                                            LocalVar   exec_ctx,
                                            LocalVar   num_threads_override,
                                            FunctionId scan_fn) {
    EmitAll(Bytecode::ParallelScanTable,
            table_oid,
            col_oids,
            num_oids,
            query_state,
...
void BytecodeEmitter::EmitAggHashTableLookup(LocalVar   dest,
                                             LocalVar   agg_ht,
                                             LocalVar   hash,
...
void BytecodeEmitter::EmitAggHashTableProcessBatch(LocalVar   agg_ht,
    ...
    EmitAll(Bytecode::AggregationHashTableProcessBatch,
            agg_ht,
            vpi,
            num_keys,
            key_cols,
...
void BytecodeEmitter::EmitAggHashTableParallelPartitionedScan(LocalVar   agg_ht,
                                                              LocalVar   context,
                                                              LocalVar   tls,
...
void BytecodeEmitter::EmitSorterInit(Bytecode   bytecode,
                                     LocalVar   sorter,
                                     LocalVar   exec_ctx,
                                     FunctionId cmp_fn,
                                     LocalVar   tuple_size) {
...
void BytecodeEmitter::EmitIndexIteratorInit(Bytecode bytecode,
                                            LocalVar iter,
                                            LocalVar exec_ctx,
                                            uint32_t num_attrs,
                                            LocalVar table_oid,
...
void BytecodeEmitter::EmitCteScanIteratorInit(Bytecode bytecode,
                                              LocalVar iter,
                                              LocalVar exec_ctx,
                                              LocalVar table_oid,
                                              LocalVar col_oids,
...
void BytecodeEmitter::EmitIndCteScanIteratorInit(Bytecode bytecode,
                                                 LocalVar iter,
                                                 LocalVar exec_ctx,
                                                 LocalVar table_oid,
                                                 LocalVar col_oids,
...
void BytecodeEmitter::EmitTestCatalogLookup(LocalVar oid_var,
                                            LocalVar exec_ctx,
                                            LocalVar table_name,
...
void BytecodeEmitter::EmitTestCatalogIndexLookup(LocalVar oid_var,
                                                 LocalVar exec_ctx,
                                                 LocalVar table_name,
                                                 uint32_t table_name_len) {
...
void BytecodeEmitter::EmitStorageInterfaceInit(Bytecode bytecode,
                                               LocalVar storage_interface,
                                               LocalVar exec_ctx,
                                               LocalVar table_oid,
                                               LocalVar col_oids,
                                               uint32_t num_oids,
...
void BytecodeEmitter::EmitStorageInterfaceGetIndexPR(Bytecode bytecode,
                                                     LocalVar pr,
                                                     LocalVar storage_interface,
                                                     LocalVar index_oid) {
