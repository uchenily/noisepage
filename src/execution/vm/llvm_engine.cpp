...
public:
    explicit TypeMap(llvm::Module *module)
        : module_(module) {
...
public:
    FunctionLocalsMap(const FunctionInfo &func_info,
                      llvm::Function     *func,
                      TypeMap            *type_map,
...
LLVMEngine::CompiledModuleBuilder::CompiledModuleBuilder(const CompilerOptions &options,
...

    {
        ...
        target_machine_.reset(target->createTargetMachine(target_triple,
                                                          llvm::sys::getHostCPUName(),
                                                          target_features.getString(),
                                                          target_options,
                                                          reloc,
                                                          {},
                                                          opt_level,
...
void LLVMEngine::CompiledModuleBuilder::DeclareStaticLocals() {
    for (const auto &local_info : tpl_module_.GetStaticLocalsInfo()) {
        ...
        auto *global_var = new llvm::GlobalVariable(*llvm_module_,
...
#ifndef NDEBUG
    if (func == nullptr) {
        auto error = fmt::format("No bytecode handler function '{}' for bytecode {}",
                                 handler_name,
    ...
    for (auto iter = tpl_module_.GetBytecodeForFunction(func_info); !iter.Done(); iter.Advance()) {
        ...
        for (uint32_t i = 0; i < Bytecodes::NumOperands(bytecode); i++) {
            ...
            case OperandType::Imm2: {
                args.push_back(
                    llvm::ConstantInt::get(type_map_->Int16Type(), iter.GetImmediateIntegerOperand(i), true));
                break;
            }
            case OperandType::Imm4: {
                args.push_back(
                    llvm::ConstantInt::get(type_map_->Int32Type(), iter.GetImmediateIntegerOperand(i), true));
                break;
            }
            case OperandType::Imm8: {
                args.push_back(
                    llvm::ConstantInt::get(type_map_->Int64Type(), iter.GetImmediateIntegerOperand(i), true));
                break;
            }
        ...
        case Bytecode::Jump: {
            ...
            std::size_t branch_target_bb_pos
                = iter.GetPosition() + Bytecodes::GetNthOperandOffset(bytecode, 0) + iter.GetJumpOffsetOperand(0);
            NOISEPAGE_ASSERT(blocks[branch_target_bb_pos] != nullptr,
