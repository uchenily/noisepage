...
void OpVPIFree(noisepage::execution::sql::VectorProjectionIterator *vpi) {
...
void OpCteScanInit(noisepage::execution::sql::CteScanIterator   *iter,
    ...
    new (iter) noisepage::execution::sql::CteScanIterator(exec_ctx,
...
void OpCteScanFree(noisepage::execution::sql::CteScanIterator *iter) {
...
void OpIndCteScanInit(noisepage::execution::sql::IndCteScanIterator *iter,
                      ...
                      uint32_t                                       num_schema_cols,
                      bool                                           is_recursive) {
    new (iter) noisepage::execution::sql::IndCteScanIterator(exec_ctx,
                                                             noisepage::catalog::table_oid_t(table_oid),
                                                             schema_cols_ids,
                                                             schema_cols_type,
                                                             num_schema_cols,
...
void OpIndCteScanFree(noisepage::execution::sql::IndCteScanIterator *iter) {
    iter->~IndCteScanIterator();
}
...
void OpFilterManagerFree(noisepage::execution::sql::FilterManager *filter_manager) {
...
void OpJoinHashTableBuild(noisepage::execution::sql::JoinHashTable *join_hash_table) {
...
void OpAggregationHashTableIteratorFree(noisepage::execution::sql::AHTIterator *iter) {
    iter->~AHTIterator();
}
...
void OpSorterInit(noisepage::execution::sql::Sorter *const                    sorter,
                  ...
                  const noisepage::execution::sql::Sorter::ComparisonFunction cmp_fn,
                  const uint32_t                                              tuple_size) {
    new (sorter) noisepage::execution::sql::Sorter(exec_ctx, cmp_fn, tuple_size);
}

void OpSorterSort(noisepage::execution::sql::Sorter *sorter) {
    sorter->Sort();
}
...
void OpSorterFree(noisepage::execution::sql::Sorter *sorter) {
    sorter->~Sorter();
}
...
void OpSorterIteratorFree(noisepage::execution::sql::SorterIterator *iter) {
    iter->~SorterIterator();
}
...
void OpStorageInterfaceInit(noisepage::execution::sql::StorageInterface  *storage_interface,
                            ...
                            uint32_t                                      num_oids,
                            bool                                          need_indexes) {
    new (storage_interface) noisepage::execution::sql::StorageInterface(exec_ctx,
                                                                        noisepage::catalog::table_oid_t(table_oid),
                                                                        col_oids,
                                                                        num_oids,
                                                                        need_indexes);
...
void OpStorageInterfaceTableUpdate(bool                                        *result,
                                   noisepage::execution::sql::StorageInterface *storage_interface,
                                   noisepage::storage::TupleSlot               *tuple_slot) {
    *result = storage_interface->TableUpdate(*tuple_slot);
}

void OpStorageInterfaceTableDelete(bool                                        *result,
                                   noisepage::execution::sql::StorageInterface *storage_interface,
                                   noisepage::storage::TupleSlot               *tuple_slot) {
...
void OpStorageInterfaceIndexInsertWithSlot(bool                                        *result,
                                           noisepage::execution::sql::StorageInterface *storage_interface,
                                           noisepage::storage::TupleSlot               *tuple_slot,
                                           bool                                         unique) {
...
void OpIndexIteratorPerformInit(noisepage::execution::sql::IndexIterator *iter) {
    iter->Init();
}

void OpIndexIteratorFree(noisepage::execution::sql::IndexIterator *iter) {
    iter->~IndexIterator();
}

void OpExecutionContextRegisterHook(noisepage::execution::exec::ExecutionContext        *exec_ctx,
                                    uint32_t                                             hook_idx,
                                    noisepage::execution::exec::ExecutionContext::HookFn hook) {
    exec_ctx->RegisterHook(hook_idx, hook);
}

void OpExecutionContextClearHooks(noisepage::execution::exec::ExecutionContext *exec_ctx) {
    exec_ctx->ClearHooks();
}
...
void OpExecOUFeatureVectorFilter(noisepage::selfdriving::ExecOUFeatureVector *const ouvec,
    ...
    ouvec->pipeline_features_->erase(
        std::remove_if(ouvec->pipeline_features_->begin(),
