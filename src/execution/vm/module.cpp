...
public:
    ...
    explicit AsyncCompileTask(Module *module)
        : module_(module) {}
...
void Module::CreateFunctionTrampoline(const FunctionInfo &func, Trampoline *trampoline) {
    ...
    llvm::sys::MemoryBlock memory
        = llvm::sys::Memory::allocateMappedMemory(default_code_size, nullptr, rw_flags, error);
    if (error) {
