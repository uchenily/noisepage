...
#define DEBUG_TRACE_INSTRUCTIONS(op)                                                                                   \
    do {                                                                                                               \
        ...
        bytecode_counts_[op]++;                                                                                        \
        EXECUTION_LOG_DEBUG("{0:p}: {1:s}",                                                                            \
                            ip - sizeof(std::underlying_type_t<Bytecode>),                                             \
...
#undef GEN_ASSIGN
    ...
    OP(ExecOUFeatureVectorRecordFeature)
        ...
        auto  feature_attribute = static_cast<selfdriving::ExecutionOperatingUnitFeatureAttribute>(
            frame->LocalAt<uint32_t>(READ_LOCAL_ID()));
        auto mode = static_cast<selfdriving::ExecutionOperatingUnitFeatureUpdateMode>(
            frame->LocalAt<uint32_t>(READ_LOCAL_ID()));
...
#undef GEN_MATH_OPS
    ...
    OP(AggregationHashTableLookup)
        ...
        auto key_eq_fn
            = reinterpret_cast<sql::AggregationHashTable::KeyEqFn>(module_->GetRawFunctionImpl(key_eq_fn_id));
    ...
    OP(AggregationHashTableProcessBatch)
        ...
        OpAggregationHashTableProcessBatch(agg_hash_table,
                                           vpi,
                                           num_keys,
                                           key_cols,
                                           init_agg_fn,
    ...
    OP(AggregationHashTableParallelPartitionedScan)
        ...
        OpAggregationHashTableParallelPartitionedScan(agg_hash_table,
                                                      query_state,
                                                      thread_state_container,
                                                      scan_partition_fn);
        DISPATCH_NEXT();
...
#undef ATOMIC_CMPXCHG_OP
    ...
    OP(TestCatalogLookup)
        ...
        auto *exec_ctx = frame->LocalAt<exec::ExecutionContext *>(READ_LOCAL_ID());
        auto *table_name
            = module_->GetBytecodeModule()->AccessStaticLocalDataRaw(LocalVar::Decode(READ_STATIC_LOCAL_ID()));
