    ...
    if (policy == transaction::ReplicationPolicy::ASYNC) {
        ...
        // that the replicas have applied their corresponding transactions.
        {
            ...
            if (was_empty) {
                ...
                //            this would be done by the Messenger's dedicated thread as part of the server-loop
                //            callback.
                ProcessTxnCallbacks();
            }
        }
...
void PrimaryReplicationManager::Handle(const messenger::ZmqMessage &zmq_msg, const TxnAppliedMsg &msg) {
    REPLICATION_LOG_TRACE(fmt::format("[RECV] TxnAppliedMsg from {}: ID {} TXN {}",
                                      zmq_msg.GetRoutingId(),
                                      msg.GetMessageId(),
                                      msg.GetAppliedTxnId()));
    ...
    while (!txn_callbacks_.empty()) {
        ...
        for (auto vec_it = callbacks.begin(); vec_it != callbacks.end();) {
            ...
            if (!callback.is_from_read_only_) {
                // Otherwise, the commit record was replicated, so check if all the replicas have applied the
                // transaction.
