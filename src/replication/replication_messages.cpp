...
MessageWrapper::MessageWrapper()
    : underlying_message_(std::make_unique<MessageFormat>()) {}
...
void MessageWrapper::FromJson(const common::json &j) {
    *(this->underlying_message_) = j;
}
...
ReplicationMessageMetadata::ReplicationMessageMetadata(msg_id_t msg_id)
    : msg_id_(msg_id) {}
...
NotifyOATMsg::NotifyOATMsg(ReplicationMessageMetadata metadata,
...
RecordsBatchMsg::RecordsBatchMsg(ReplicationMessageMetadata  metadata,
                                 record_batch_id_t           batch_id,
...
TxnAppliedMsg::TxnAppliedMsg(const MessageWrapper &message)
    : BaseReplicationMessage(message)
    , applied_txn_id_(message.Get<transaction::timestamp_t>(key_applied_txn_id)) {}
