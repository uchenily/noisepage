...
void ReplicaReplicationManager::Handle(const messenger::ZmqMessage &zmq_msg, const NotifyOATMsg &msg) {
    REPLICATION_LOG_TRACE(fmt::format("[RECV] NotifyOATMsg from {}: OAT {} BATCH {}",
                                      zmq_msg.GetRoutingId(),
                                      msg.GetOldestActiveTxn(),
                                      msg.GetBatchId()));
...
void ReplicaReplicationManager::Handle(const messenger::ZmqMessage &zmq_msg, const RecordsBatchMsg &msg) {
    REPLICATION_LOG_TRACE(fmt::format("[RECV] RecordsBatchMsg from {}: ID {} BATCH {}",
                                      zmq_msg.GetRoutingId(),
                                      msg.GetMessageId(),
                                      msg.GetBatchId()));
