...
void Forecaster::PerformTraining() {
        ...
        = metrics_output == metrics::MetricsOutput::DB || metrics_output == metrics::MetricsOutput::CSV_AND_DB;
    {
        ...
        if (segment_information.empty() || !success) {
            ...
            model_server_manager_->TrainForecastModel(models,
                                                      input_path,
...
std::pair<WorkloadMetadata, bool> Forecaster::RetrieveWorkloadMetadata(
    ...
    bool result = true;
    {
        ...
        task_manager_->AddTask(std::make_unique<task::TaskDML>(catalog::INVALID_DATABASE_OID,
                                                               "SELECT * FROM noisepage_forecast_texts",
...

    {
        ...
        auto to_row_fn = [&metadata, cves_conv](const std::vector<execution::sql::Val *> &values) {
            ...
            auto *param_val = static_cast<execution::sql::StringVal *>(values[2]);
            {
                ...
                metadata.query_id_to_params_[qid].emplace_back(
                    cves_conv(metadata, qid, std::string(param_val->StringView())));
            }
        };

        auto query = fmt::format("SELECT * FROM noisepage_forecast_parameters WHERE ts >= {} AND ts <= {}",
                                 bounds.first,
                                 bounds.second);
        task_manager_->AddTask(std::make_unique<task::TaskDML>(catalog::INVALID_DATABASE_OID,
                                                               query,
...
std::unordered_map<int64_t, std::vector<double>> Forecaster::GetSegmentInformation(std::pair<uint64_t, uint64_t> bounds,
    ...
    auto     to_row_fn
        = [&segments, &segment_number, &low_timestamp, interval](const std::vector<execution::sql::Val *> &values) {
...
std::unique_ptr<selfdriving::WorkloadForecast> Forecaster::LoadWorkloadForecast(WorkloadForecastInitMode mode) {
    ...
    if (infer_from_internal) {
        ...
        result = model_server_manager_->InferForecastModel(&segment_information,
                                                           forecast_model_save_path_,
                                                           models,
                                                           nullptr,
                                                           workload_forecast_interval_,
    ...
    if (infer_from_disk) {
        ...
        result = model_server_manager_->InferForecastModel(input_path,
                                                           forecast_model_save_path_,
                                                           models,
                                                           nullptr,
                                                           workload_forecast_interval_,
