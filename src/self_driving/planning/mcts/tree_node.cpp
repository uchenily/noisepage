...
TreeNode::TreeNode(common::ManagedPointer<TreeNode> parent,
...
common::ManagedPointer<TreeNode> TreeNode::SampleChild() {
    ...
    for (auto &child : children_) {
        ...
        // https://en.wikipedia.org/wiki/Monte_Carlo_tree_search#Exploration_and_exploitation The first additive term
        // was changed to be the inverse of a normalized value, since smaller cost is preferred
...
TreeNode::Selection(common::ManagedPointer<TreeNode>                              root,
    ...
    for (auto action : actions_on_path) {
        ...
        PilotUtil::ApplyAction(planning_context,
                               action_map.at(action)->GetSQLCommand(),
...
void TreeNode::ChildrenRollout(
    ...
    for (const auto &action_id : candidate_actions) {
        ...
        // We may apply actions to reduce memory consumption in future, so we only need to evaluate the memory
        // constraint up to action_plan_end_index_
        for (auto segment_index = action_start_segment_index_; segment_index <= action_plan_end_index_;
             segment_index++) {
            size_t memory = PilotUtil::CalculateMemoryConsumption(planning_context->GetMemoryInfo(),
                                                                  new_action_state,
                                                                  segment_index,
                                                                  action_map);
        ...
        size_t action_plan_end_memory_consumption
            = PilotUtil::CalculateMemoryConsumption(planning_context->GetMemoryInfo(),
                                                    new_action_state,
                                                    action_plan_end_index_,
                                                    action_map);
        ...
        if (satisfy_memory_constraint) {
            PilotUtil::ApplyAction(*planning_context,
                                   action_ptr->GetSQLCommand(),
                                   action_ptr->GetDatabaseOid(),
                                   Pilot::WHAT_IF);
            ...
            if (action_ptr->GetActionType() != ActionType::CREATE_INDEX) {
                ...
                if (action_state_cost_map->find(new_action_state) != action_state_cost_map->end()) {
                    ...
                    SELFDRIVING_LOG_TRACE(
                        "Get child cost from map with action {} start interval {} end interval {}: {}",
                        action_ptr->GetActionID().UnderlyingValue(),
                        action_start_segment_index_,
                        action_plan_end_index_,
                        child_segment_cost);
                } else {
                    // Compute cost and add to the cache
                    child_segment_cost = PilotUtil::ComputeCost(planning_context,
                                                                forecast,
                                                                action_start_segment_index_,
                                                                action_plan_end_index_,
                                                                std::nullopt,
                ...
                // Action needs time to finish. Need to use the cost cache with both the action state and the action as
                // key
                ...
                if (action_apply_cost_map->find(key_pair) != action_apply_cost_map->end()) {
                    ...
                    PilotUtil::ApplyAction(*planning_context,
                                           action_map.at(reverse_action)->GetSQLCommand(),
                                           action_map.at(reverse_action)->GetDatabaseOid(),
                                           Pilot::WHAT_IF);
                    ...
                    action_plan_end_memory_consumption
                        = PilotUtil::CalculateMemoryConsumption(planning_context->GetMemoryInfo(),
                                                                new_action_state,
                                                                action_plan_end_index_,
                                                                action_map);
            ...
            if (satisfy_memory_constraint) {
                ...
                if (action_state_cost_map->find(new_action_state) != action_state_cost_map->end()) {
                    ...
                    SELFDRIVING_LOG_TRACE(
                        "Get later cost from map with action {} start interval {} end interval {}: {}",
                        action_ptr->GetActionID().UnderlyingValue(),
        ...
        new_action_state.SetIntervals(action_plan_end_index_ + 1, tree_end_segment_index);
        children_.push_back(std::make_unique<TreeNode>(common::ManagedPointer(this),
                                                       action_id,
                                                       action_plan_end_index_ + 1,
                                                       child_segment_cost,
                                                       later_segments_cost,
...
void TreeNode::BackPropogate(const PlanningContext                                        &planning_context,
    ...
    while (curr != nullptr && curr->parent_ != nullptr) {
        ...
        PilotUtil::ApplyAction(planning_context,
                               action_map.at(rev_action)->GetSQLCommand(),
                               action_map.at(rev_action)->GetDatabaseOid(),
                               Pilot::WHAT_IF);
        if (use_min_cost) {
            curr->cost_ = std::min(curr->cost_, expanded_cost);
        } else {
            // All ancestors of the expanded leaf need to updated with a weight increase of num_expansion, and a new
            // weight
