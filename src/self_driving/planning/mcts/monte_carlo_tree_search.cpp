...
MonteCarloTreeSearch::MonteCarloTreeSearch(common::ManagedPointer<PlanningContext>               planning_context,
    ...
    IndexActionGenerator().GenerateActions(plans,
                                           ...
                                           &action_map_,
                                           &candidate_actions_);
    ChangeKnobActionGenerator().GenerateActions(plans,
                                                planning_context_->GetSettingsManager(),
                                                &action_map_,
                                                &candidate_actions_);
    ...
    for (auto &[action_id, action] : action_map_) {
        if (action->GetActionType() == ActionType::CREATE_INDEX) {
            ...
            auto drop_action
                = reinterpret_cast<DropIndexAction *>(action_map_.at(action->GetReverseActions().at(0)).get());
    ...
    root_ = std::make_unique<TreeNode>(nullptr,
                                       static_cast<action_id_t>(NULL_ACTION),
                                       0,
                                       0,
                                       later_cost,
                                       planning_context_->GetMemoryInfo().initial_memory_bytes_,
                                       action_state);
...
void MonteCarloTreeSearch::RunSimulation(uint64_t simulation_number, uint64_t memory_constraint) {
    for (uint64_t i = 0; i < simulation_number; i++) {
        ...
        vertex->ChildrenRollout(planning_context_.Get(),
                                forecast_,
                                levels_to_plan_.at(vertex->GetDepth()),
                                end_segment_index_,
                                action_map_,
                                candidate_actions,
                                &action_state_cost_map_,
...
void MonteCarloTreeSearch::BestAction(std::vector<std::vector<pilot::ActionTreeNode>> *best_action_seq, size_t topk) {
    ...
    while (!curr_node->IsLeaf()) {
        ...
        for (size_t i = 0; i < order.size() && i < topk; i++) {
            ...
            top.emplace_back(curr_node->GetTreeNodeId(),
                             child->GetTreeNodeId(),
                             action,
                             child->GetCost(),
                             action_info->GetDatabaseOid(),
                             action_info->GetSQLCommand(),
                             child->GetActionStartSegmentIndex(),
