...
GraphSolver::GraphSolver(PlanningContext                                              *planning_context,
    ...
    for (uint64_t segment_index = 0; segment_index <= end_segment_index; segment_index++) {
        ...
        curr_level.push_back(
            std::make_unique<SeqNode>(std::set<action_id_t>(), default_segment_cost.at(segment_index)));
        ...
        SELFDRIVING_LOG_DEBUG("[InitGraph] level {}, config_set {{}}, node_cost {}, best_dist {}",
                              segment_index,
        ...
        for (auto const &config_set : candidate_configurations_by_segment.at(segment_index)) {
            ...
            SELFDRIVING_LOG_DEBUG("[InitGraph] level {}, config_set {}, node_cost {}, best_dist {}",
                                  segment_index,
                                  PilotUtil::ConfigToString(config_set),
                                  node_cost,
                                  nodes_by_segment_index_.at(segment_index).back()->GetBestDist());
...
double GraphSolver::ComputeConfigCost(PlanningContext                                              *planning_context,
    ...
    for (auto const action : config_set) {
        PilotUtil::ApplyAction(*planning_context,
                               ...
                               structure_map.at(action)->GetDatabaseOid(),
                               Pilot::WHAT_IF);
    }
    ...
    for (auto const action : config_set) {
        ...
        auto rev_action = structure_map.at(action)->GetReverseActions().at(0);
        PilotUtil::ApplyAction(*planning_context,
                               structure_map.at(rev_action)->GetSQLCommand(),
                               structure_map.at(rev_action)->GetDatabaseOid(),
                               Pilot::WHAT_IF);
    }
...
double GraphSolver::RecoverShortestPath(PathSolution *shortest_path) {
    ...
    while (curr_node != nullptr) {
        ...
        SELFDRIVING_LOG_DEBUG(" Reversed LEVEL {} : config {}, node_cost {}, best_dist {}",
                              ...
                              PilotUtil::ConfigToString(best_config_path->back()),
                              curr_node->GetNodeCost(),
