...
SequenceTuning::SequenceTuning(common::ManagedPointer<PlanningContext>               planning_context,
    ...
    IndexActionGenerator().GenerateActions(plans,
                                           planning_context_->GetSettingsManager(),
                                           &structure_map_,
                                           &candidate_actions);
    ...
    for (auto &action : candidate_actions) {
        if (structure_map_.at(action)->GetActionType() == ActionType::CREATE_INDEX) {
            ...
            SELFDRIVING_LOG_DEBUG("Candidate structure: ID {} Command {}",
                                  action,
    ...
    for (uint64_t segment_index = 0; segment_index <= end_segment_index_; segment_index++) {
        default_segment_cost.emplace_back(PilotUtil::ComputeCost(planning_context_.Get(),
                                                                 forecast_,
                                                                 segment_index,
                                                                 segment_index,
                                                                 std::nullopt,
...
void SequenceTuning::BestAction(
    ...
    for (auto structure_id : candidate_structures_) {
        ...
        double best_path_cost [[maybe_unused]] = GraphSolver(planning_context_.Get(),
                                                             ...
                                                             end_segment_index_,
                                                             structure_map_,
                                                             default_segment_cost_,
                                                             singleton_action_repeated,
                                                             memory_constraint)
        ...
        SELFDRIVING_LOG_INFO("[cost-based pruning] for structure \"{}\" finds best path distance {} with {} configs",
                             structure_map_.at(structure_id)->GetSQLCommand(),
                             best_path_cost,
...
double SequenceTuning::UnionPair(const PathSolution &path_one,
    ...
    double best_unioned_dist = GraphSolver(planning_context_.Get(),
                                           forecast_,
                                           end_segment_index_,
                                           structure_map_,
                                           default_segment_cost_,
                                           candidate_structures_by_segment,
                                           memory_constraint)
...
void SequenceTuning::ExtractActionsFromConfigPath(
    ...
    for (uint64_t config_idx = 1; config_idx < best_final_config_path.size(); config_idx++) {
        ...
        auto action_id_set = ExtractActionsFromConfigTransition(structure_map_,
...
void SequenceTuning::MergeConfigs(std::multiset<PathSolution>     *global_path_set,
    ...
    while (!global_path_set->empty()) {
        ...
        // if there's a pair whose unioned solution is better than the least-cost solution alone, remove the other path
        // and add the combined solution to P
        if (best_union_cost < least_cost_path.path_length_) {
            ...
            SELFDRIVING_LOG_DEBUG("[GreedySeq] new path added to global_path_set with distance {} number of config {}",
                                  best_union_cost,
                                  best_merged_solution.unique_config_on_path_.size());
