...
Pilot::Pilot(std::string                                             ou_model_save_path,
             std::string                                             interference_model_save_path,
             std::string                                             forecast_model_save_path,
    ...
    : planning_context_(std::move(ou_model_save_path),
                        std::move(interference_model_save_path),
                        catalog,
                        metrics_thread,
                        ...
                        stats_storage,
                        txn_manager,
                        std::move(query_exec_util),
                        task_manager)
    , forecaster_(std::move(forecast_model_save_path),
                  metrics_thread,
                  model_server_manager,
                  settings_manager,
                  task_manager,
...
void Pilot::ActionSearch(std::vector<ActionTreeNode> *best_action_seq) {
    ...
    util::SelfDrivingRecordingUtil::RecordAppliedAction(timestamp,
                                                        best_action.GetActionId(),
                                                        best_action.GetCost(),
                                                        best_action.GetDbOid(),
                                                        best_action.GetActionText(),
        ...
        for (auto const &action UNUSED_ATTRIBUTE : action_set) {
            SELFDRIVING_LOG_INFO(
                fmt::format("Action Selected: Time Interval: {}; Action Command: {} Applied to Database {}",
                            action_set_idx,
                            action.first,
                            static_cast<uint32_t>(action.second)));
        }
