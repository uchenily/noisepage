...
void PilotUtil::ApplyAction(const pilot::PlanningContext &planning_context,
    ...
    if (is_query_ddl) {
        ...
        if (query_exec_util->CompileQuery(sql_query,
                                          nullptr,
                                          nullptr,
...
void PilotUtil::GetQueryPlans(const PlanningContext                                   &planning_context,
    ...
    for (auto qid : qids) {
        ...
        auto result = query_exec_util->PlanStatement(query_text,
                                                     params,
                                                     param_types,
                                                     std::make_unique<optimizer::TrivialCostModel>());
...
void PilotUtil::SumFeatureInPlace(std::vector<double>       *feature,
                                  const std::vector<double> &delta_feature,
...
auto PilotUtil::ComputeCost(PlanningContext                         *planning_context,
    ...
    PilotUtil::ExecuteForecast(planning_context,
                               forecast,
                               start_segment_index,
                               end_segment_index,
                               action,
...
auto PilotUtil::CollectPipelineFeatures(const PlanningContext                                &planning_context,
    ...
    for (const auto &qid : qids) {
        ...
        if (execute_query) {
            ...
            //  using the stats to get the features instead of executing queries. We might eventually delete this code
            //  path as well.
            planning_context.GetTaskManager()->AddTask(
                std::make_unique<task::TaskDML>(db_oid,
                                                query_text,
                                                std::make_unique<optimizer::TrivialCostModel>(),
                                                ...
                                                true,
                                                true,
                                                std::make_optional<execution::query_id_t>(qid),
            ...
            for (auto &param : *params) {
                query_util->CompileQuery(query_text,
                                         common::ManagedPointer(&param),
                ...
                for (auto &iter : ous->GetPipelineFeatureMap()) {
                    ...
                    aggregated_data->RecordPipelineData(qid,
                                                        iter.first,
                                                        0,
                                                        std::vector<ExecutionOperatingUnitFeature>(iter.second),
                                                        resource_metrics);
                }
    ...
    for (auto it = aggregated_data->pipeline_data_.begin(); it != aggregated_data->pipeline_data_.end(); it++) {
        SELFDRIVING_LOG_DEBUG(fmt::format("qid: {}; pipeline_id: {}",
                                          static_cast<uint>(it->query_id_),
...
void PilotUtil::OUModelInference(pilot::PlanningContext                                        *planning_context,
    ...
    std::list<std::tuple<execution::query_id_t,
                         execution::pipeline_id_t,
    ...
    for (auto &ou_map_it : ou_to_features) {
        ...
        // TODO(lin): There's a bit duplicated logic between this and InterferenceModelInference(), but I haven't found
        // a
        ...
        if (!inference_features.empty()) {
            ...
            auto res = planning_context->GetModelServerManager()->InferOUModel(
                OperatingUnitUtil::ExecutionOperatingUnitTypeToString(ou_type),
                planning_context->GetOuModelSavePath(),
                inference_features);
            if (!res.second) {
                throw PILOT_EXCEPTION("Inference through model server manager has error",
                                      common::ErrorCode::ERRCODE_WARNING);
            }
...
void PilotUtil::InterferenceModelInference(PlanningContext                                      *planning_context,
    ...
    for (auto i = start_segment_index; i <= end_segment_index; i++) {
        ...
        for (auto const &id_to_query_sum : query_feat_sum) {
            if (id_to_num_exec.find(id_to_query_sum.first) != id_to_num_exec.end()) {
                ...
                SumFeatureInPlace(&normalized_feat_sum,
                                  id_to_query_sum.second,
        ...
        // curr_feat_sum now holds the sum of ous for queries contained in this segment (averaged over diff set of
        // param) multiplied by number of execution of the query containing it and normalized by its interval
        for (auto const &pipeline_to_pred : pipeline_to_prediction) {
    ...
    if (!inference_features.empty()) {
        ...
        auto inference_result = planning_context->GetModelServerManager()->InferInterferenceModel(
            planning_context->GetInterferenceModelSavePath(),
            inference_features);
...
void PilotUtil::GroupFeaturesByOU(
    ...
    for (const auto &data_it : pipeline_data) {
        ...
        // for each operating unit of the pipeline, we push it to the corresponding collection in ou_to_features for
        // block inference
        ...
        pipeline_to_ou_position->emplace_back(pipeline_qids.at(pipeline_idx),
                                              data_it.pipeline_id_,
...
void PilotUtil::ComputeTableSizeRatios(const PlanningContext  &planning_context,
    ...
    auto query = fmt::format("select starelid, max(stanumrows) as num_rows from pg_statistic group by starelid "
    ...
    for (auto db_oid : planning_context.GetDBOids()) {
        ...
        common::Future<task::DummyResult> sync;
        task_manager->AddTask(std::make_unique<task::TaskDML>(db_oid,
                                                              query,
                                                              std::make_unique<optimizer::TrivialCostModel>(),
                                                              false,
                                                              to_row_fn,
    ...
    for (const auto &[query_id, query_text] : metadata.query_id_to_text_) {
        ...
        if (statement->GetType() == parser::StatementType::DELETE) {
            ...
            if (delete_statement->GetDeleteCondition() != nullptr) {
                // TODO(lin): right now assuming only deleting one row when there's a condition. Need cardinality
                // estimation
...
void PilotUtil::EstimateCreateIndexAction(PlanningContext   *planning_context,
                                          CreateIndexAction *create_action,
    ...
    std::unique_ptr<metrics::PipelineMetricRawData> aggregated_data
        = std::make_unique<metrics::PipelineMetricRawData>();
    for (auto &iter : ous->GetPipelineFeatureMap()) {
    ...
    PilotUtil::OUModelInference(planning_context,
                                pipeline_qids,
                                aggregated_data->pipeline_data_,
                                &pipeline_to_prediction);
...
void PilotUtil::ExecuteForecast(PlanningContext                                      *planning_context,
    ...
    auto pipeline_data = PilotUtil::CollectPipelineFeatures(*planning_context,
                                                            common::ManagedPointer(forecast),
                                                            start_segment_index,
                                                            end_segment_index,
                                                            &pipeline_qids,
                                                            !Pilot::WHAT_IF);
    ...
    PilotUtil::InterferenceModelInference(planning_context,
                                          common::ManagedPointer(forecast),
                                          start_segment_index,
                                          end_segment_index,
                                          action,
