...
auto Catalog::CreateDatabaseEntry(const common::ManagedPointer<transaction::TransactionContext> txn,
    ...
    *(reinterpret_cast<db_oid_t *>(
        redo->Delta()->AccessForceNotNull(pg_database_all_cols_prm_[postgres::PgDatabase::DATOID.oid_])))
        = db;
    *(reinterpret_cast<storage::VarlenEntry *>(
        redo->Delta()->AccessForceNotNull(pg_database_all_cols_prm_[postgres::PgDatabase::DATNAME.oid_])))
        = name_varlen;
    *(reinterpret_cast<DatabaseCatalog **>(
        redo->Delta()->AccessForceNotNull(pg_database_all_cols_prm_[postgres::PgDatabase::DAT_CATALOG.oid_])))
        = dbc;
...
auto Catalog::DeleteDatabaseEntry(const common::ManagedPointer<transaction::TransactionContext> txn, db_oid_t db)
    ...
    *(reinterpret_cast<db_oid_t *>(pr->AccessForceNotNull(0))) = db;
    ...
    NOISEPAGE_ASSERT(
        ...
        "function was called with an oid that doesn't exist in the Catalog, but binding somehow succeeded. That "
        "doesn't "
