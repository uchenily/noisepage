...
auto DatabaseCatalog::SetIndexPointer(const common::ManagedPointer<transaction::TransactionContext> txn,
    ...
    // This needs to be deferred because if any items were subsequently inserted into this index, they will have
    // deferred abort actions that will be above this action on the abort stack.  The defer ensures we execute after
    // them.
...
auto DatabaseCatalog::TryLock(const common::ManagedPointer<transaction::TransactionContext> txn) -> bool {
    ...
    if (write_lock_.compare_exchange_strong(current_val, txn_id)) {
        ...
        txn->RegisterCommitAction([=]() -> void {
            write_lock->store(txn->FinishTime());
        });
        txn->RegisterAbortAction([=]() -> void {
            write_lock->store(current_val);
        });
        return true;
    }
    txn->SetMustAbort(); // though no changes were written to the storage layer, we'll treat this as a DDL change
