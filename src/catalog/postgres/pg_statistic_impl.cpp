...
PgStatisticImpl::PgStatisticImpl(db_oid_t db_oid)
    : db_oid_(db_oid) {}
...
void PgStatisticImpl::Bootstrap(common::ManagedPointer<transaction::TransactionContext> txn,
    ...
    dbc->BootstrapTable(txn,
                        PgStatistic::STATISTIC_TABLE_OID,
                        PgNamespace::NAMESPACE_CATALOG_NAMESPACE_OID,
                        PG_STATISTIC_TABLE_NAME,
                        Builder::GetStatisticTableSchema(),
                        statistics_);
    dbc->BootstrapIndex(txn,
                        PgNamespace::NAMESPACE_CATALOG_NAMESPACE_OID,
                        PgStatistic::STATISTIC_TABLE_OID,
                        PgStatistic::STATISTIC_OID_INDEX_OID,
                        PG_STATISTIC_INDEX_NAME,
                        Builder::GetStatisticOidIndexSchema(db_oid_),
                        statistic_oid_index_);
}
...
bool PgStatisticImpl::DeleteColumnStatistics(const common::ManagedPointer<transaction::TransactionContext> txn,
    ...
    std::vector<storage::TupleSlot> index_results;
    {
        ...
        pr_lo->Set<col_oid_t, false>(oid_prm.at(indexkeycol_oid_t(2)),
                                     col_oid_t(std::numeric_limits<uint32_t>::min()),
                                     false);

        // High key (table_oid_t + 1, min col_oid_t)
        pr_hi->Set<table_oid_t, false>(oid_prm.at(indexkeycol_oid_t(1)), table_oid + 1, false);
        pr_hi->Set<col_oid_t, false>(oid_prm.at(indexkeycol_oid_t(2)),
                                     col_oid_t(std::numeric_limits<uint32_t>::min()),
                                     false);
...
optimizer::TableStats PgStatisticImpl::GetTableStatistics(common::ManagedPointer<transaction::TransactionContext> txn,
    ...
    std::vector<storage::TupleSlot> index_results;
    {
        ...
        // Low key (table_oid_t, min col_oid_t)
        pr->Set<table_oid_t, false>(oid_prm.at(indexkeycol_oid_t(1)), table_oid, false);
        pr->Set<col_oid_t, false>(oid_prm.at(indexkeycol_oid_t(2)),
                                  col_oid_t(std::numeric_limits<uint32_t>::min()),
                                  false);
...
PgStatisticImpl::CreateColumnStats(common::ManagedPointer<storage::ProjectedRow> all_cols_pr,
    ...
    case execution::sql::SqlTypeId::Boolean:
        return CreateColumnStats<execution::sql::BoolVal>(table_oid,
                                                          col_oid,
                                                          num_rows,
                                                          non_null_rows,
                                                          distinct_values,
    ...
    case execution::sql::SqlTypeId::BigInt:
        return CreateColumnStats<execution::sql::Integer>(table_oid,
                                                          col_oid,
                                                          num_rows,
                                                          non_null_rows,
                                                          distinct_values,
    ...
    case execution::sql::SqlTypeId::Double:
        return CreateColumnStats<execution::sql::Real>(table_oid,
                                                       col_oid,
                                                       num_rows,
                                                       non_null_rows,
                                                       distinct_values,
    ...
    case execution::sql::SqlTypeId::Decimal:
        return CreateColumnStats<execution::sql::DecimalVal>(table_oid,
                                                             col_oid,
                                                             num_rows,
                                                             non_null_rows,
                                                             distinct_values,
    ...
    case execution::sql::SqlTypeId::Timestamp:
        return CreateColumnStats<execution::sql::TimestampVal>(table_oid,
                                                               col_oid,
                                                               num_rows,
                                                               non_null_rows,
                                                               distinct_values,
    ...
    case execution::sql::SqlTypeId::Date:
        return CreateColumnStats<execution::sql::DateVal>(table_oid,
                                                          col_oid,
                                                          num_rows,
                                                          non_null_rows,
                                                          distinct_values,
    ...
    case execution::sql::SqlTypeId::Varbinary:
        return CreateColumnStats<execution::sql::StringVal>(table_oid,
                                                            col_oid,
                                                            num_rows,
                                                            non_null_rows,
                                                            distinct_values,
...
template <typename T>
std::unique_ptr<optimizer::ColumnStatsBase>
PgStatisticImpl::CreateColumnStats(table_oid_t                 table_oid,
                                   col_oid_t                   col_oid,
                                   size_t                      num_rows,
    ...
    return std::make_unique<optimizer::ColumnStats<T>>(
        ...
        std::make_unique<optimizer::Histogram<CppType>>(std::move(histogram)),
        type);
}
