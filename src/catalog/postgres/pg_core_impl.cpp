...
PgCoreImpl::PgCoreImpl(const db_oid_t db_oid)
    : db_oid_(db_oid) {}
...
void PgCoreImpl::BootstrapPRIsPgAttribute() {
    ...
    const std::vector<col_oid_t> get_columns_oids{PgAttribute::ATTNUM.oid_,
                                                  PgAttribute::ATTNAME.oid_,
                                                  PgAttribute::ATTTYPID.oid_,
                                                  PgAttribute::ATTTYPMOD.oid_,
                                                  PgAttribute::ATTNOTNULL.oid_,
                                                  PgAttribute::ADSRC.oid_};
    // TODO(Matt): Maybe length should still be read back out from the table rather than relying ton Column's
    // constructor?
...
void PgCoreImpl::BootstrapPgNamespace(common::ManagedPointer<transaction::TransactionContext> txn,
    ...
    dbc->BootstrapTable(txn,
                        PgNamespace::NAMESPACE_TABLE_OID,
                        PgNamespace::NAMESPACE_CATALOG_NAMESPACE_OID,
                        "pg_namespace",
                        Builder::GetNamespaceTableSchema(),
                        namespaces_);
    dbc->BootstrapIndex(txn,
                        PgNamespace::NAMESPACE_CATALOG_NAMESPACE_OID,
                        PgNamespace::NAMESPACE_TABLE_OID,
                        PgNamespace::NAMESPACE_OID_INDEX_OID,
                        "pg_namespace_oid_index",
                        Builder::GetNamespaceOidIndexSchema(db_oid_),
                        namespaces_oid_index_);
    dbc->BootstrapIndex(txn,
                        PgNamespace::NAMESPACE_CATALOG_NAMESPACE_OID,
                        PgNamespace::NAMESPACE_TABLE_OID,
                        PgNamespace::NAMESPACE_NAME_INDEX_OID,
                        "pg_namespace_name_index",
                        Builder::GetNamespaceNameIndexSchema(db_oid_),
                        namespaces_name_index_);
}

void PgCoreImpl::BootstrapPgClass(common::ManagedPointer<transaction::TransactionContext> txn,
                                  common::ManagedPointer<DatabaseCatalog>                 dbc) {
    dbc->BootstrapTable(txn,
                        PgClass::CLASS_TABLE_OID,
                        PgNamespace::NAMESPACE_CATALOG_NAMESPACE_OID,
                        "pg_class",
...
void PgCoreImpl::BootstrapPgAttribute(common::ManagedPointer<transaction::TransactionContext> txn,
    ...
    dbc->BootstrapIndex(txn,
                        PgNamespace::NAMESPACE_CATALOG_NAMESPACE_OID,
                        PgAttribute::COLUMN_TABLE_OID,
                        PgAttribute::COLUMN_NAME_INDEX_OID,
...
std::function<void(void)> PgCoreImpl::GetTearDownFn(common::ManagedPointer<transaction::TransactionContext> txn,
    ...
    delete[] buffer;
    return [garbage_collector{dbc->garbage_collector_},
...
bool PgCoreImpl::DeleteNamespace(const common::ManagedPointer<transaction::TransactionContext> txn,
    ...
    // Get the objects in this namespace
    {
        ...
        for (const auto &object : ns_objects) {
            ...
            if (object.second == PgClass::RelKind::REGULAR_TABLE) {
                if (!dbc->DeleteTable(txn,
                                      static_cast<table_oid_t>(object.first))) { // Write-lock failed. Ask to abort.
                    delete[] buffer;
                    return false;
                }
            }
    ...
    // so we can be slightly less efficient than optimal.
    {
        auto ns_objects = GetNamespaceClassOids(txn, ns_oid);
        for (const auto &object : ns_objects) {
            // Delete all of the straggler indexes that may have been built on tables in other namespaces. We shouldn't
            // get any double-deletions because indexes on tables will already be invisible to us (logically deleted
            // already).
            if (object.second == PgClass::RelKind::INDEX) {
                if (!dbc->DeleteIndex(txn,
                                      static_cast<index_oid_t>(object.first))) { // Write-lock failed. Ask to abort.
                    delete[] buffer;
                    return false;
                }
            }
...
bool PgCoreImpl::SetClassPointer(const common::ManagedPointer<transaction::TransactionContext> txn,
    ...
    NOISEPAGE_ASSERT(pr_init.ProjectedRowSize() >= oid_pri.ProjectedRowSize(),
    ...
    std::vector<storage::TupleSlot> index_results;
    {
        ...
        NOISEPAGE_ASSERT(index_results.size() == 1,
                         "Incorrect number of results from index scan. Expect 1 because it's a unique index. "
                         "0 implies that function was called with an oid that doesn't exist in the Catalog, which "
                         "implies a programmer"
...
bool PgCoreImpl::CreateIndexEntry(const common::ManagedPointer<transaction::TransactionContext> txn,
    ...
    // Insert into indexes on pg_class. Insert into pg_index.
    {
        ...
        {
            auto *const indexes_insert_redo
                = txn->StageWrite(db_oid_, PgIndex::INDEX_TABLE_OID, pg_index_all_cols_pri_);
            ...
            NOISEPAGE_ASSERT(
                (class_name_index_init.ProjectedRowSize() >= indexes_oid_index_init.ProjectedRowSize())
            ...
            // Insert into pg_index_oid_index.
            {
                ...
                index_pr->Set<index_oid_t, false>(0, index_oid, false);
                if (!indexes_oid_index_->InsertUnique(txn,
                                                      *index_pr,
                                                      indexes_tuple_slot)) { // OID conflict. Ask to abort.
                    delete[] index_buffer;
                    return false;
                }
            }
            ...
            // Insert into pg_index_table_index.
            {
                ...
                index_pr->Set<table_oid_t, false>(0, table_oid, false);
                if (!indexes_table_index_->Insert(txn,
                                                  *index_pr,
                                                  indexes_tuple_slot)) { // Duplicate value. Ask to abort.
                    delete[] index_buffer;
                    return false;
                }
            }
    ...
    // Update pg_class with the new index schema.
    {
        ...
        auto *new_schema = new IndexSchema(cols,
                                           schema.Type(),
                                           schema.Unique(),
                                           schema.Primary(),
                                           schema.Exclusion(),
                                           schema.Immediate(),
                                           schema.GetIndexOptions());
        txn->RegisterAbortAction([=]() {
            delete new_schema;
        });
...
bool PgCoreImpl::DeleteIndex(const common::ManagedPointer<transaction::TransactionContext> txn,
    ...
    storage::index::Index *index_ptr;
    {
        ...
        std::vector<storage::TupleSlot> index_results;
        {
            ...
            NOISEPAGE_ASSERT(
                ...
                "Incorrect number of results from index scan. Expect 1 because it's a unique index."
                "0 implies that function was called with an oid that doesn't exist in the Catalog, but binding somehow "
                "succeeded. That doesn't make sense. Was a DROP plan node reused twice? IF EXISTS should be handled in "
                "the "
                "Binder, rather than pushing logic here.");
        }
    ...
    // Delete from pg_index and associated indexes.
    {
        ...
        auto table_pr = common::ManagedPointer(delete_index_pri_.InitializeRow(buffer));
        {
            ...
            NOISEPAGE_ASSERT(
                index
                    == *(reinterpret_cast<const index_oid_t *const>(
        ...
        // Everything succeeded from an MVCC standpoint, so register a deferred action for the GC to delete the index
        // with txn manager. See base function comment.
        txn->RegisterCommitAction([=, garbage_collector{dbc->garbage_collector_}](
    ...
    std::vector<std::pair<common::ManagedPointer<storage::index::Index>, const IndexSchema &>> index_objects;
    {
        ...
        for (const auto &slot : class_tuple_slots) {
            ...
            NOISEPAGE_ASSERT(
                ...
                "SetIndexPointer?");
            auto *schema = reinterpret_cast<IndexSchema *>(
                *PgClass::REL_SCHEMA.Get(class_select_pr, get_class_object_and_schema_prm_));
...
std::vector<Column> PgCoreImpl::GetColumns(const common::ManagedPointer<transaction::TransactionContext> txn,
    ...
    // TODO(Matt): do we have any way to assert that we got the number of attributes we expect? From another attribute
    // in another catalog table maybe?
...
Column PgCoreImpl::MakeColumn(storage::ProjectedRow *const pr, const storage::ProjectionMap &pr_map) {
    ...
    // I believe it is OK that the unique ptr goes out of scope because right now both Column constructors copy the
    // expr.
