...
PgProcImpl::PgProcImpl(db_oid_t db_oid)
    : db_oid_(db_oid) {}
...
void PgProcImpl::Bootstrap(common::ManagedPointer<transaction::TransactionContext> txn,
    ...
    dbc->BootstrapIndex(txn,
                        PgNamespace::NAMESPACE_CATALOG_NAMESPACE_OID,
                        PgProc::PRO_TABLE_OID,
                        PgProc::PRO_OID_INDEX_OID,
                        "pg_proc_oid_index",
                        Builder::GetProcOidIndexSchema(db_oid_),
                        procs_oid_index_);
    dbc->BootstrapIndex(txn,
                        PgNamespace::NAMESPACE_CATALOG_NAMESPACE_OID,
                        PgProc::PRO_TABLE_OID,
                        PgProc::PRO_NAME_INDEX_OID,
                        "pg_proc_name_index",
                        Builder::GetProcNameIndexSchema(db_oid_),
                        procs_name_index_);
        ...
        for (const auto &candidate_proc_slot : results) {
            ...
            if (candidate_proc_nargs <= input_arg_types.size()) {
                ...
                if (candidate_proc_nargs == 0) {
                    ...
                    // Right now we know that both arg lists (function's and input) are non-empty, and that the input
                    // args is at least as long as the function's args. Start trying to match types
                    ...
                    const auto candidate_proc_args_vector = candidate_proc_args->DeserializeArray<
                        type_oid_t>(); // TODO(Matt): could maybe elide this copy with
                                       // VarlenEntry.Content() shenanigans, but then need a bespoke way to compare
                                       // against input args in their vector. Previous version of this function
                                       // constructed a VarlenEntry of input args to compare, which seems worse.
                    if (candidate_proc_args_vector == input_arg_types) {
                        ...
                        if (candidate_proc_variadic != INVALID_TYPE_OID) {
                            ...
                            if (prefix_match) {
                                ...
                                for (uint16_t i = candidate_proc_nargs;
                                     i < input_arg_types.size() && variadic_args_match;
                                     i++) {
                                ...
                                if (variadic_args_match) {
                                    // we satisfied both conditions described above, this function can match the input
                                    // arguments
                                    match = true;
                                }
                            }
...
void PgProcImpl::BootstrapProcs(const common::ManagedPointer<transaction::TransactionContext> txn,
    ...
    auto create_fn = [&](const std::string              &procname,
                         const type_oid_t                variadic_type,
                         const std::vector<std::string> &args,
                         const std::vector<type_oid_t>  &arg_types,
                         type_oid_t                      rettype) {
        ...
        CreateProcedure(txn,
                        proc_oid_t{dbc->next_oid_++},
                        procname,
                        PgLanguage::INTERNAL_LANGUAGE_OID,
...
void PgProcImpl::BootstrapProcContext(const common::ManagedPointer<transaction::TransactionContext> txn,
                                      ...
                                      const execution::ast::Builtin                                 builtin,
    ...
    const auto *const func_context = new execution::functions::FunctionContext(std::move(func_name),
                                                                               func_ret_type,
                                                                               std::move(arg_types),
                                                                               builtin,
...
void PgProcImpl::BootstrapProcContexts(const common::ManagedPointer<transaction::TransactionContext> txn,
    ...
    auto create_fn = [&](std::string                            &&func_name,
                         execution::sql::SqlTypeId                func_ret_type,
                         std::vector<execution::sql::SqlTypeId> &&arg_types,
                         execution::ast::Builtin                  builtin,
                         bool                                     is_exec_ctx_required) {
        BootstrapProcContext(txn,
                             dbc,
                             std::move(func_name),
                             func_ret_type,
                             std::move(arg_types),
