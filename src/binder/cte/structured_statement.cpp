...
void StructuredStatement::BuildFromVisit(common::ManagedPointer<parser::SelectStatement> select,
                                         LexicalScope                                   *scope,
    ...
    for (const auto &table_ref : select->GetSelectWith()) {
        ...
        // Add the scope and the corresponding reference to the enclosing scope
        scope->AddReference(
            std::make_unique<ContextSensitiveTableRef>(table_ref, RefType::WRITE, scope, new_scope.get()));
...
void StructuredStatement::BuildFromVisit(common::ManagedPointer<parser::TableRef> table_ref,
                                         LexicalScope                            *scope,
                                         StructuredStatement::BuildContext       *context) {
...
auto StructuredStatement::InvariantsSatisfied(const LexicalScope &scope) -> bool {
    ...
    return satisfied
           && std::all_of(scope.EnclosedScopes().cbegin(),
                          scope.EnclosedScopes().cend(),
                          [](const std::unique_ptr<LexicalScope> &scope) {
                              return InvariantsSatisfied(*scope);
                          });
}
...
auto StructuredStatement::MutableReferences() -> std::vector<ContextSensitiveTableRef *> {
    ...
    for (auto *scope : flat_scopes_) {
        std::transform(scope->References().begin(),
                       scope->References().end(),
                       std::back_inserter(references),
                       [](std::unique_ptr<ContextSensitiveTableRef> &r) {
                           return r.get();
                       });
    }
    return references;
