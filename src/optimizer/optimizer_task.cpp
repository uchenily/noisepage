...
// Base class
//===--------------------------------------------------------------------===//
void OptimizerTask::ConstructValidRules(GroupExpression              *group_expr,
...
void OptimizeExpression::Execute() {
    ...
    OPTIMIZER_LOG_DEBUG("OptimizeExpression::execute() op {0}, valid rules : {1}",
                        static_cast<int>(group_expr_->Contents()->GetOpType()),
                        valid_rules.size());
    // Apply rule
...
void ApplyRule::Execute() {
    ...
    while (iterator.HasNext()) {
        ...
        for (const auto &new_expr : after) {
            ...
            if (context_->GetOptimizerContext()->RecordOptimizerNodeIntoGroup(common::ManagedPointer(new_expr),
                                                                              &new_gexpr,
                                                                              g_id)) {
        ...
        if (cur_child_idx_ == static_cast<int>(group_expr_->GetChildrenGroupsSize())) {
            ...
            if (meet_requirement && cur_total_cost_ <= context_->GetCostUpperBound()) {
                ...
                if (memo_enforced_expr != nullptr) { // Enforcement takes place
                    cur_group->SetExpressionCost(memo_enforced_expr,
                                                 cur_total_cost_,
                                                 context_->GetRequiredProperties()->Copy());
                    ...
                    cur_group->SetExpressionCost(group_expr_,
                                                 cur_total_cost_,
                                                 context_->GetRequiredProperties()->Copy());
                }
            }
