...
void PlanGenerator::CorrectOutputPlanWithProjection() {
    ...
    for (auto &col : required_cols_) {
        ...
        if (child_expr_map.find(col) != child_expr_map.end()) {
            ...
            auto dve
                = std::make_unique<parser::DerivedValueExpression>(col->GetReturnValueType(), 0, child_expr_map[col]);
...
void PlanGenerator::Visit(const InnerHashJoin *op) {
    ...
    auto join_predicate
        = parser::ExpressionUtil::ConvertExprCVNodes(common::ManagedPointer(eval_pred.get()), children_expr_map_)
              .release();
    RegisterPointerCleanup<parser::AbstractExpression>(join_predicate, true, true);
...
void PlanGenerator::Visit(const HashGroupBy *op) {
    ...
    BuildAggregatePlan(planner::AggregateStrategyType::HASH,
                       &op->GetColumns(),
...
void PlanGenerator::Visit(const SortGroupBy *op) {
    ...
    auto having_predicates = parser::ExpressionUtil::JoinAnnotatedExprs(op->GetHaving());
    BuildAggregatePlan(planner::AggregateStrategyType::SORTED,
                       &op->GetColumns(),
                       common::ManagedPointer(having_predicates.get()));
}
...
void PlanGenerator::Visit(const Update *op) {
    ...
    for (auto index : indexes) {
        for (auto &column : index.second.GetColumns()) {
            ...
            parser::ExpressionUtil::GetTupleValueExprs(
                &cves,
                common::ManagedPointer(const_cast<parser::AbstractExpression *>(column.StoredExpression().Get())));
        }
    }
    ...
    for (auto &update : updates) {
        ...
        auto upd_value
            = parser::ExpressionUtil::EvaluateExpression(children_expr_map_, update->GetUpdateValue()).release();
...
void PlanGenerator::Visit(const CreateDatabase *create_database) {
    output_plan_
        = planner::CreateDatabasePlanNode::Builder().SetDatabaseName(create_database->GetDatabaseName()).Build();
}
...
void PlanGenerator::Visit(const CreateIndex *create_index) {
    ...
    auto idx_schema = std::make_unique<catalog::IndexSchema>(std::move(cols),
                                                             schema->Type(),
                                                             schema->Unique(),
                                                             schema->Primary(),
                                                             schema->Exclusion(),
