...
void InputColumnDeriver::Visit(const LeftSemiHashJoin *op) {
    JoinHelper(op);
}

void InputColumnDeriver::Visit(const LeftHashJoin *op) {
    JoinHelper(op);
}
...
void InputColumnDeriver::InputBaseTableColumns(const parser::AliasType &alias,
                                               catalog::db_oid_t        db,
...
void InputColumnDeriver::Visit(const Analyze *op) {
    ...
    for (const auto &col_oid : op->GetColumns()) {
        ...
        for (const auto &col_info : catalog::postgres::PgStatisticImpl::ANALYZE_AGGREGATES) {
            auto *agg_expr = OptimizerUtil::GenerateAggregateExpr(col,
                                                                  col_info.aggregate_type_,
                                                                  col_info.distinct_,
                                                                  parser::AliasType(),
                                                                  op->GetDatabaseOid(),
                                                                  op->GetTableOid());
