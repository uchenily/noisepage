...
void QueryToOperatorTransformer::Visit(common::ManagedPointer<parser::SelectStatement> op) {
    ...
    if (!op->GetSelectWith().empty()) {
        for (auto with : op->GetSelectWith()) {
            ...
            for (auto &alias : with->GetCteColumnAliases()) {
                columns1.emplace_back(alias.GetName(),
                                      col_types[i],
                                      false,
                                      parser::ConstantValueExpression(col_types[i]),
            ...
            for (auto &elem : with->GetCteColumnAliases()) {
                ...
                txn_context->RegisterAbortAction([=] {
                    delete cve;
                });
            ...
            size_t                               ind = 0;
            for (auto &alias : with->GetCteColumnAliases()) {
                columns.emplace_back(alias.GetName(),
                                     col_types[ind],
                                     false,
                                     parser::ConstantValueExpression(col_types[ind]),
                                     catalog::MakeTempOid<catalog::col_oid_t>(alias.GetSerialNo().UnderlyingValue()));
                ind++;
            }

            auto cte_scan_expr
                = std::make_unique<OperatorNode>(LogicalCteScan::Make(with->GetAlias().GetName(),
                                                                      with->GetTableName(),
                                                                      oid,
                                                                      catalog::Schema(std::move(columns)),
                                                                      {},
                                                                      with->GetCteType(),
                                                                      {})
    ...
    if (!predicates_.empty()) {
        auto filter_expr = std::make_unique<OperatorNode>(
            LogicalFilter::Make(std::move(predicates_)).RegisterWithTxnContext(txn_context),
            std::vector<std::unique_ptr<AbstractOptimizerNode>>{},
            txn_context);
    ...
    if (QueryToOperatorTransformer::RequireAggregation(common::ManagedPointer(op))) {
        ...
        if (op->GetSelectGroupBy() == nullptr) {
            ...
            agg_expr
                = std::make_unique<OperatorNode>(LogicalAggregateAndGroupBy::Make().RegisterWithTxnContext(txn_context),
                                                 std::vector<std::unique_ptr<AbstractOptimizerNode>>{},
                                                 txn_context);
            ...
            if (!having.empty()) {
                auto filter_expr = std::make_unique<OperatorNode>(
                    LogicalFilter::Make(std::move(having)).RegisterWithTxnContext(txn_context),
                    std::vector<std::unique_ptr<AbstractOptimizerNode>>{},
                    txn_context);
                filter_expr->PushChild(std::move(output_expr_));
                output_expr_ = std::move(filter_expr);
...
void QueryToOperatorTransformer::Visit(common::ManagedPointer<parser::TableRef> node) {
    ...
    if (node->GetSelect() != nullptr) {
        ...
        if (it != cte_table_name_.end()) {
            ...
            std::vector<execution::sql::SqlTypeId> col_types{};
            auto cte_scan_expr = std::make_unique<OperatorNode>(LogicalCteScan::Make(node->GetAlias().GetName(),
                                                                                     node->GetTableName(),
                                                                                     cte_oids_[index],
                                                                                     cte_schemas_[index],
                                                                                     cte_expressions_[index],
                                                                                     cte_type_[index],
                                                                                     {})
                                                                    .RegisterWithTxnContext(txn_context),
                                                                std::vector<std::unique_ptr<AbstractOptimizerNode>>{},
...
void QueryToOperatorTransformer::Visit(common::ManagedPointer<parser::CreateFunctionStatement> op) {
    ...
    auto create_expr
        = std::make_unique<OperatorNode>(LogicalCreateFunction::Make(catalog::INVALID_DATABASE_OID,
                                                                     accessor_->GetDefaultNamespace(),
                                                                     op->GetFuncName(),
                                                                     op->GetPLType(),
                                                                     op->GetFuncBody(),
                                                                     std::move(function_param_names),
                                                                     std::move(function_param_types),
                                                                     op->GetFuncReturnType()->GetDataType(),
                                                                     op->GetFuncParameters().size(),
                                                                     op->ShouldReplace())
                                             .RegisterWithTxnContext(txn_context),
                                         std::vector<std::unique_ptr<AbstractOptimizerNode>>{},
                                         txn_context);
...
void QueryToOperatorTransformer::Visit(common::ManagedPointer<parser::CreateStatement> op) {
    ...
    case parser::CreateStatement::CreateType::kTable:
        create_expr = std::make_unique<OperatorNode>(
            LogicalCreateTable::Make(accessor_->GetNamespaceOid(op->GetNamespaceName()),
                                     op->GetTableName(),
                                     op->GetColumns(),
                                     op->GetForeignKeys())
                .RegisterWithTxnContext(txn_context),
    ...
    case parser::CreateStatement::CreateType::kIndex: {
        ...
        for (auto &attr : op->GetIndexAttributes()) {
            if (attr.HasExpr()) {
                ...
                auto        unique_col_expr
                    = std::make_unique<parser::ColumnValueExpression>(parser::AliasType(op->GetTableName()),
                                                                      attr.GetName(),
                                                                      db_oid_,
                                                                      tb_oid,
                                                                      table_col.Oid(),
        ...
        create_expr
            = std::make_unique<OperatorNode>(LogicalCreateIndex::Make(db_oid_,
                                                                      accessor_->GetDefaultNamespace(),
                                                                      accessor_->GetTableOid(op->GetTableName()),
                                                                      op->GetIndexType(),
                                                                      op->IsUniqueIndex(),
                                                                      op->GetIndexName(),
                                                                      std::move(entries),
...
void QueryToOperatorTransformer::Visit(common::ManagedPointer<parser::InsertStatement> op) {
    ...
    if (is_select_insert) {
        auto insert_expr = std::make_unique<OperatorNode>(
            LogicalInsertSelect::Make(target_db_id, target_table_id, std::move(col_ids))
                .RegisterWithTxnContext(txn_context),
            std::vector<std::unique_ptr<AbstractOptimizerNode>>{},
            txn_context);
...
void QueryToOperatorTransformer::Visit(common::ManagedPointer<parser::DeleteStatement> op) {
    ...
    auto                                                delete_expr = std::make_unique<OperatorNode>(
        LogicalDelete::Make(target_db_id, target_table_alias.GetName(), target_table_id)
            .RegisterWithTxnContext(txn_context),
        std::move(c),
        txn_context);
    ...
    if (op->GetDeleteCondition() != nullptr) {
        ...
        table_scan = std::make_unique<OperatorNode>(
            LogicalGet::Make(target_db_id, target_table_id, {}, target_table_alias, true)
                .RegisterWithTxnContext(txn_context),
            std::vector<std::unique_ptr<AbstractOptimizerNode>>{},
            txn_context);
    }
...
void QueryToOperatorTransformer::Visit(common::ManagedPointer<parser::DropStatement> op) {
    ...
    case parser::DropStatement::DropType::kDatabase:
        drop_expr
            = std::make_unique<OperatorNode>(LogicalDropDatabase::Make(db_oid_).RegisterWithTxnContext(txn_context),
                                             std::vector<std::unique_ptr<AbstractOptimizerNode>>{},
                                             txn_context);
        break;
...
void QueryToOperatorTransformer::Visit(common::ManagedPointer<parser::CopyStatement> op) {
    ...
    if (op->IsFrom()) {
        ...
        // as the root.
        auto get_op = std::make_unique<OperatorNode>(LogicalExternalFileGet::Make(op->GetExternalFileFormat(),
                                                                                  op->GetFilePath(),
                                                                                  op->GetDelimiter(),
                                                                                  op->GetQuoteChar(),
                                                                                  op->GetEscapeChar())
...
void QueryToOperatorTransformer::CollectPredicates(common::ManagedPointer<parser::AbstractExpression> expr,
    ...
    for (const auto &pred : predicate_ptrs) {
        if (!QueryToOperatorTransformer::IsSupportedConjunctivePredicate(pred)) {
            throw NOT_IMPLEMENTED_EXCEPTION(("Expression type "
                                             + std::to_string(static_cast<int>(pred.Get()->GetExpressionType()))
                                             + " is not supported")
                                                .c_str());
        }
    }
        ...
        if (values.size() < column_objects.size()) {
            for (auto i = values.size(); i != column_objects.size(); ++i) {
                ...
                if (!column_objects[i].Nullable() && column_objects[i].StoredExpression() == nullptr) {
                    throw CATALOG_EXCEPTION(
                        ("Null value in column \"" + column_objects[i].Name() + "\" violates not-null constraint")
                            .c_str());
                }
            }
        }
