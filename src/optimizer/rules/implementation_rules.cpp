        ...
        if (IndexUtil::CheckSortProperty(sort_prop)) {
            ...
            for (auto index : indexes) {
                if (IndexUtil::SatisfiesSortWithIndex(accessor, sort_prop, get->GetTableOid(), index)) {
                    ...
                    auto predicate_satisfaction = IndexUtil::SatisfiesPredicateWithIndex(accessor,
                                                                                         get->GetTableOid(),
                                                                                         get->GetTableAlias(),
                                                                                         index,
                                                                                         preds,
...
void LogicalAggregateToPhysicalAggregate::Transform(common::ManagedPointer<AbstractOptimizerNode>        input,
    ...
    auto result = std::make_unique<OperatorNode>(
        Aggregate::Make().RegisterWithTxnContext(context->GetOptimizerContext()->GetTxn()),
        std::move(c),
        context->GetOptimizerContext()->GetTxn());
    transformed->emplace_back(std::move(result));
}
...
void LogicalInnerJoinToPhysicalInnerIndexJoin::Transform(
    ...
    auto                                                new_child
        = std::make_unique<OperatorNode>(LogicalGet::Make(r_child->GetDatabaseOid(),
                                                          r_child->GetTableOid(),
                                                          join_preds,
                                                          r_child->GetTableAlias(),
                                                          r_child->GetIsForUpdate())
                                             .RegisterWithTxnContext(context->GetOptimizerContext()->GetTxn()),
                                         std::move(empty),
                                         context->GetOptimizerContext()->GetTxn());
    ...
    for (const auto &idx_op : transform) {
        ...
        if (!idx_scan->GetBounds().empty()) {
            ...
            auto result
                = std::make_unique<OperatorNode>(InnerIndexJoin::Make(idx_scan->GetTableOID(),
                                                                      idx_scan->GetIndexOID(),
                                                                      idx_scan->GetIndexScanType(),
                                                                      idx_scan->GetBounds(),
                                                                      join_preds)
                                                     .RegisterWithTxnContext(context->GetOptimizerContext()->GetTxn()),
                                                 std::move(child),
...
void LogicalExportToPhysicalExport::Transform(common::ManagedPointer<AbstractOptimizerNode>        input,
    ...
    auto result_plan
        = std::make_unique<OperatorNode>(ExportExternalFile::Make(export_op->GetFormat(),
                                                                  std::move(file),
...
void LogicalCreateFunctionToPhysicalCreateFunction::Transform(
    ...
    auto op = std::make_unique<OperatorNode>(CreateFunction::Make(cf_op->GetDatabaseOid(),
                                                                  cf_op->GetNamespaceOid(),
                                                                  cf_op->GetFunctionName(),
                                                                  cf_op->GetUDFLanguage(),
                                                                  cf_op->GetFunctionBody(),
                                                                  cf_op->GetFunctionParameterNames(),
                                                                  cf_op->GetFunctionParameterTypes(),
                                                                  cf_op->GetReturnType(),
                                                                  cf_op->GetParamCount(),
                                                                  cf_op->IsReplace())
                                                 .RegisterWithTxnContext(context->GetOptimizerContext()->GetTxn()),
...
void LogicalCreateIndexToPhysicalCreateIndex::Transform(
    ...
    auto schema = std::make_unique<catalog::IndexSchema>(std::move(cols),
                                                         idx_type,
                                                         ci_op->IsUnique(),
                                                         false, // is_primary
                                                         false, // is_exclusion
...
void LogicalCreateTriggerToPhysicalCreateTrigger::Transform(
    ...
    auto op = std::make_unique<OperatorNode>(CreateTrigger::Make(ct_op->GetDatabaseOid(),
                                                                 ct_op->GetNamespaceOid(),
                                                                 ct_op->GetTableOid(),
                                                                 ct_op->GetTriggerName(),
                                                                 ct_op->GetTriggerFuncName(),
                                                                 ct_op->GetTriggerArgs(),
                                                                 ct_op->GetTriggerColumns(),
                                                                 ct_op->GetTriggerWhen(),
                                                                 ct_op->GetTriggerType())
                                                 .RegisterWithTxnContext(context->GetOptimizerContext()->GetTxn()),
                                             std::vector<std::unique_ptr<AbstractOptimizerNode>>(),
                                             context->GetOptimizerContext()->GetTxn());
...
void LogicalCteScanToPhysicalCteScanIterative::Transform(
    ...
    auto result_plan
        = std::make_unique<OperatorNode>(CteScan::Make(logical_op->GetExpressions(),
                                                       std::string(logical_op->GetTableAlias()),
                                                       logical_op->GetTableOid(),
                                                       logical_op->GetCTEType(),
                                                       logical_op->GetScanPredicate(),
...
void LogicalCteScanToPhysicalCteScan::Transform(common::ManagedPointer<AbstractOptimizerNode>        input,
    ...
    auto result_plan
        = std::make_unique<OperatorNode>(CteScan::Make(logical_op->GetExpressions(),
                                                       std::string(logical_op->GetTableName()),
                                                       logical_op->GetTableOid(),
                                                       logical_op->GetCTEType(),
                                                       logical_op->GetScanPredicate(),
                                                       catalog::Schema(logical_op->GetTableSchema()))
