...
void RewritePushImplicitFilterThroughJoin::Transform(common::ManagedPointer<AbstractOptimizerNode>        input,
    ...
    if (!left_predicates.empty()) {
        ...
        left_branch
            = std::make_unique<OperatorNode>(LogicalFilter::Make(std::move(left_predicates))
    ...
    if (!right_predicates.empty()) {
        ...
        right_branch
            = std::make_unique<OperatorNode>(LogicalFilter::Make(std::move(right_predicates))
                                                 .RegisterWithTxnContext(context->GetOptimizerContext()->GetTxn()),
                                             std::move(c),
                                             context->GetOptimizerContext()->GetTxn());
    } else {
...
void RewritePushExplicitFilterThroughJoin::Transform(common::ManagedPointer<AbstractOptimizerNode>        input,
    ...
    for (auto &join_predicate : join_predicates) {
        ...
        if (join_predicate.GetExpr()->GetExpressionType() == parser::ExpressionType::COMPARE_IN) {
            ...
            auto semi_join_predicate
                = AnnotatedExpression(common::ManagedPointer<parser::AbstractExpression>(new_join_expr),
...
void RewriteEmbedFilterIntoChildlessCteScan::Transform(common::ManagedPointer<AbstractOptimizerNode>        input,
    ...
    std::vector<AnnotatedExpression> predicates = input->Contents()->GetContentsAs<LogicalFilter>()->GetPredicates();
    std::vector<std::unique_ptr<AbstractOptimizerNode>> c;
    auto output = std::make_unique<OperatorNode>(LogicalCteScan::Make(get->GetTableAlias(),
                                                                      get->GetTableName(),
                                                                      get->GetTableOid(),
                                                                      get->GetTableSchema(),
                                                                      {},
...
void RewritePullFilterThroughMarkJoin::Transform(common::ManagedPointer<AbstractOptimizerNode>        input,
    ...
    auto join
        = std::make_unique<OperatorNode>(input->Contents(), std::move(c), context->GetOptimizerContext()->GetTxn());

    std::vector<std::unique_ptr<AbstractOptimizerNode>> cc;
    cc.emplace_back(std::move(join));
    auto output = std::make_unique<OperatorNode>(join_children[1]->Contents(),
                                                 std::move(cc),
                                                 context->GetOptimizerContext()->GetTxn());
    transformed->emplace_back(std::move(output));
}
...
void RewriteUnionWithRecursiveCTE::Transform(common::ManagedPointer<AbstractOptimizerNode>        input,
    ...
    auto new_root
        = std::make_unique<OperatorNode>(LogicalCteScan::Make(cte_scan->GetTableAlias(),
                                                              cte_scan->GetTableName(),
                                                              cte_scan->GetTableOid(),
...
void RewritePullFilterThroughAggregation::Transform(common::ManagedPointer<AbstractOptimizerNode>        input,
    ...
    ExtractCorrelatedPredicatesWithAggregate(predicates,
                                             child_group_aliases_set,
