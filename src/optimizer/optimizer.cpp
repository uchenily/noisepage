...
auto Optimizer::ChooseBestPlan(transaction::TransactionContext                                       *txn,
    ...
    auto               output_input_cols_pair
        = deriver.DeriveInputColumns(gexpr, required_props, required_cols, &context_->GetMemo());
    ...
    for (size_t i = 0; i < child_groups.size(); ++i) {
        ...
        auto child_plan
            = ChooseBestPlan(txn, accessor, child_groups[i], required_input_props[i], input_cols[i], generator);
    ...
    planner::PlanMetaData::PlanNodeMetaData plan_node_meta_data(group->GetNumRows(),
                                                                group->GetTableNumRows(),
                                                                group->GetFilterColumnSelectivities());
    auto                                    plan = generator->ConvertOpNode(txn,
                                         accessor,
                                         op,
                                         required_props,
                                         required_cols,
    ...
    if (op->Contents()->GetOpType() == OpType::CTESCAN && !child_groups.empty()) {
        ...
        if (plan->GetPlanNodeType() == planner::PlanNodeType::CTESCAN) {
            ...
            auto cte_scan_plan_node
                = reinterpret_cast<planner::CteScanPlanNode *>(&(*plan->GetChildren().begin()->Get()));
            context_->SetCTESchema(cte_scan_plan_node->GetTableOid(), *(cte_scan_plan_node)->GetTableSchema());
        }
    }
...
void Optimizer::ExecuteTaskStack(OptimizerTaskStack  *task_stack,
                                 group_id_t           root_group_id,
