...
std::pair<bool, bool> IndexUtil::CheckPredicates(
    ...
    const std::unordered_set<catalog::col_oid_t>                                          &mapped_cols,
    const std::vector<AnnotatedExpression>                                                &predicates,
    bool                                                                                   allow_cves,
    ...
    for (auto &col : schema.GetColumns()) {
        ...
        if (open_highs.find(oid) != open_highs.end() && open_lows.find(oid) != open_lows.end()) {
            ...
            bounds->insert(std::make_pair(
                oid,
                std::vector<planner::IndexExpression>{open_highs[oid], planner::IndexExpression(nullptr)}));
            ...
            bounds->insert(std::make_pair(
                oid,
                std::vector<planner::IndexExpression>{planner::IndexExpression(nullptr), open_lows[oid]}));
        }
    }
