...
auto GroupBindingIterator::Next() -> std::unique_ptr<AbstractOptimizerNode> {
    if (pattern_->Type() == OpType::LEAF) {
        ...
        return std::make_unique<OperatorNode>(LeafOperator::Make(group_id_).RegisterWithTxnContext(txn_),
                                              std::move(c),
                                              txn_);
    }
...
GroupExprBindingIterator::GroupExprBindingIterator(const Memo                      &memo,
...
auto GroupExprBindingIterator::HasNext() -> bool {
    ...
    if (has_next_) {
        ...
        for (; first_modified_idx >= 0; --first_modified_idx) {
            const std::vector<std::unique_ptr<AbstractOptimizerNode>> &child_binding
