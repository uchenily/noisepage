...
auto StatsStorage::GetTableStats(const catalog::db_oid_t    database_id,
    ...
    return LatchedTableStatsReference(table_stats_value.table_stats_,
                                      std::move(shared_stats_storage_latch),
                                      std::move(shared_table_stats_latch));
}

void StatsStorage::MarkStatsStale(catalog::db_oid_t                      database_id,
    ...
    if (table_stats_value_it != table_stats_storage_.end()) {
        ...
        /*
         * We don't need an exclusive latch because it's ok to mark something stale while someone is reading it. The
         * worst that happens is they end up using slightly stale statistics without realizing it.
         */
...
void StatsStorage::InsertTableStats(catalog::db_oid_t         database_id,
                                    catalog::table_oid_t      table_id,
...
void StatsStorage::UpdateStaleColumns(catalog::table_oid_t      table_id,
