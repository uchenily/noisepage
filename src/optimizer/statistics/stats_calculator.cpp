...
void StatsCalculator::Visit(const LogicalGet *op) {
    ...
    if (root_group->GetNumRows() == Group::UNINITIALIZED_NUM_ROWS) {
        ...
        auto est = EstimateCardinalityForFilter(root_group,
                                                table_num_rows,
                                                latched_table_stats_reference.table_stats_,
                                                op->GetPredicates());
    ...
    if (root_group->GetNumRows() == Group::UNINITIALIZED_NUM_ROWS) {
        ...
        for (const auto &annotated_expr : op->GetJoinPredicates()) {
            ...
            if (annotated_expr.GetExpr()->GetExpressionType() == parser::ExpressionType::COMPARE_EQUAL
                ...
                /*
                 * TODO(Joseph Koshakow) This isn't really that accurate, it doesn't take into account overlap of
                 * predicates i.e. if predicate 1 matches two rows and predicate 2 matches the same two rows, then
                 * predicate 2 will have no affect on the total row count but we will unnecessary lower the total row
                 * count.
                 */
...
auto StatsCalculator::CalculateSelectivityForPredicate(Group            *group,
    ...
    if (expr->GetExpressionType() == parser::ExpressionType::OPERATOR_NOT) {
        ...
        if (expr->GetChild(right_index)->GetExpressionType() == parser::ExpressionType::VALUE_CONSTANT) {
            ...
            value
                = std::unique_ptr<parser::ConstantValueExpression>{reinterpret_cast<parser::ConstantValueExpression *>(
