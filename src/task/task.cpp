...
void TaskDML::Execute(common::ManagedPointer<util::QueryExecUtil> query_exec_util,
    ...
    if (params_.empty()) {
        result = query_exec_util->ExecuteDML(query_text_,
                                             nullptr,
                                             nullptr,
                                             tuple_fn_,
                                             metrics_manager_,
        ...
        result = query_exec_util->CompileQuery(query_text_,
                                               common::ManagedPointer(&params_0),
                                               common::ManagedPointer(&param_types_),
                                               std::move(cost_model_),
                                               override_qid_,
                                               settings_);
        ...
        if (result) {
            for (auto &param_vec : params_) {
                ...
                result &= query_exec_util->ExecuteQuery(query_text_,
                                                        tuple_fn_,
                                                        common::ManagedPointer(&param_vec),
                                                        metrics_manager_,
                                                        settings_);
            }
        }
