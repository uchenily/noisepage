...
TaskManager::TaskManager(common::ManagedPointer<common::DedicatedThreadRegistry> thread_registry,
    ...
    , master_util_(std::move(util)) {
    for (uint32_t i = 0; i < num_workers; i++) {
        task_runners_.push_back(thread_registry_->RegisterDedicatedThread<TaskRunner>(
            this,
            util::QueryExecUtil::ConstructThreadLocal(common::ManagedPointer(master_util_)),
...
void TaskManager::WaitForFlush() {
    ...
    notify_cv_.wait(lock, [&] {
        return busy_workers_ == 0 && queue_.empty();
    });
}
...
auto TaskManager::GetTaskWithKillFlag(bool const *kill) -> std::unique_ptr<Task> {
    std::unique_ptr<Task> task = nullptr;
    {
        std::unique_lock<std::mutex> lock(queue_mutex_);
        queue_cv_.wait(lock, [&] {
            return (*kill) || !queue_.empty();
        });
