...
void PostgresPacketWriter::WriteNoData() {
    BeginPacket(NetworkMessageType::PG_NO_DATA_RESPONSE).EndPacket();
}
...
void PostgresPacketWriter::WriteRowDescription(const std::vector<planner::OutputSchema::Column> &columns,
    ...
    for (uint32_t i = 0; i < columns.size(); i++) {
        ...
        NOISEPAGE_ASSERT(
            field_formats.size() == columns.size() || field_formats.size() == 1,
        ...
        const auto &name = columns[i].GetExpr()->GetAlias().GetName().empty()
                               ? columns[i].GetName()
...
void PostgresPacketWriter::WriteParseCommand(const std::string          &destinationStmt,
...
void PostgresPacketWriter::WriteBindCommand(const std::string                         &destinationPortal,
                                            const std::string                         &sourcePreparedStmt,
...
void PostgresPacketWriter::WriteSyncCommand() {
    BeginPacket(NetworkMessageType::PG_SYNC_COMMAND).EndPacket();
}
...
void PostgresPacketWriter::WriteParseComplete() {
    BeginPacket(NetworkMessageType::PG_PARSE_COMPLETE).EndPacket();
}

void PostgresPacketWriter::WriteCloseComplete() {
    BeginPacket(NetworkMessageType::PG_CLOSE_COMPLETE).EndPacket();
}

void PostgresPacketWriter::WriteBindComplete() {
    BeginPacket(NetworkMessageType::PG_BIND_COMPLETE).EndPacket();
}
...
auto PostgresPacketWriter::WriteBinaryAttribute(const execution::sql::Val *const val,
    ...
    if (val->is_null_) {
        ...
        default:
            UNREACHABLE("Unsupported type for binary serialization. This is either a new type, or an oversight when "
                        "reading JDBC "
                        "source code.");
        }
    }
...
auto PostgresPacketWriter::WriteTextAttribute(const execution::sql::Val *const val,
    ...
    if (val->is_null_) {
        ...
        case execution::sql::SqlTypeId::Boolean: {
            // Don't allocate an actual string for a BOOLEAN, just wrap a std::string_view, write the value directly,
            // and continue
        ...
        case execution::sql::SqlTypeId::Varbinary: {
            // Don't allocate an actual string for a VARCHAR, just wrap a std::string_view, write the value directly,
            // and continue
