...
void TerrierServer::RegisterSocket() {
        ...
        if constexpr (is_networked_socket) { // NOLINT
            ...
            if (socket_path.length() > sizeof(sun.sun_path)) {
                throw NETWORK_PROCESS_EXCEPTION(
                    fmt::format("Failed to name {} socket (must have length <= {} characters).",
                                socket_description,
                                sizeof(sun.sun_path)));
            }
    ...
    if (socket_fd < 0) {
        throw NETWORK_PROCESS_EXCEPTION(
            fmt::format("Failed to open {} socket: {}", socket_description, strerror(errno)));
    }
    ...
    if (status < 0) {
        ...
        if constexpr (!is_networked_socket) { // NOLINT
            ...
            if (errno == EADDRINUSE) {
                // I find this disgusting, but it's the approach favored by a bunch of software that uses Unix domain
                // sockets. BSD syslogd, for example, does this in *every* case--error handling or not--and I'm not one
                // to question it.
                recovered
            ...
            if (recovered) {
                NETWORK_LOG_INFO("Recovered! Managed to bind {} socket by purging a pre-existing bind.",
                                 socket_description);
            } else {
                throw NETWORK_PROCESS_EXCEPTION(
                    fmt::format("Failed to bind and recover {} socket.", socket_description));
            }
        } else { // NOLINT
