...
void QueryTraceMetadata::RecordQueryParamSample(uint64_t              timestamp,
...
void QueryTraceMetricRawData::WriteToDB(
    ...
    if (write_parameters) {
        ...
        util::SelfDrivingRecordingUtil::RecordQueryParameters(write_timestamp,
                                                              &metadata_.qid_param_samples_,
                                                              task_manager,
                                                              out_params);
    ...
    if (high_timestamp_ >= low_timestamp_ + QueryTraceMetricRawData::query_segment_interval && !freqs.empty()) {
        NOISEPAGE_ASSERT(metadata_.iterator_ == metadata_.timeseries_.end(),
                         "Expect the timseries data to be exhausted");
...
void QueryTraceMetric::RecordQueryText(catalog::db_oid_t           db_oid,
