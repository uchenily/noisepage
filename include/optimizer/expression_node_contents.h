...
public:
     ...
     */
    ExpressionNodeContents(const ExpressionNodeContents &other)
        : AbstractOptimizerNodeContents(other) {
        expr_ = other.expr_;
    }
     ...
     */
    explicit ExpressionNodeContents(common::ManagedPointer<parser::AbstractExpression> expr) {
        expr_ = expr;
    }
    ...
    CopyWithChildren(std::vector<std::unique_ptr<parser::AbstractExpression>> children);
