...
public:
    ...
    std::unique_ptr<AbstractOptimizerNode> Copy() override {
        ...
        for (auto &op : children_) {
            ...
            NOISEPAGE_ASSERT(op->Contents()->GetOpType() != OpType::UNDEFINED,
    ...
    bool operator==(const OperatorNode &other) const {
        if (contents_->GetOpType() != other.contents_->GetOpType())
            return false;
        if (contents_->GetExpType() != other.contents_->GetExpType())
            return false;
     ...
     */
    OperatorNode(OperatorNode &&op) noexcept
        : contents_(op.contents_)
        , children_(std::move(op.children_)) {}
     ...
     */
    void PushChild(std::unique_ptr<AbstractOptimizerNode> child) override {
