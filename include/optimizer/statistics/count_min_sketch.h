...
public:
     ...
     */
    explicit CountMinSketch(uint64_t width)
        : total_count_{0} {
...
private:
     ...
     * that decltype(execution::sql::StringVal::val_) is storage::VarlenEntry and storage::VarlenEntry has a pointer to
     * a byte array to store the string contents. So madoka::Sketch would end up using the address of this byte array as
     * part of the key instead of the actual contents. So two storage::VarlenEntry with the same underlying strings
     * would be treated as different because the addresses for their string contents are different. Converting all keys
     * to doubles gets around this issue.
     */
    ...
    static NormalizedKeyType NormalizeKey(const decltype(execution::sql::StringVal::val_) &key) {
        return key.Hash();
    }
    ...
    static NormalizedKeyType NormalizeKey(const int &key) {
        return static_cast<NormalizedKeyType>(key);
    }
    ...
    static NormalizedKeyType NormalizeKey(const char *key) {
        return NormalizeKey(std::string(key));
    }
     ...
     * each call is on a separate instance. This is because we use the instance's address in memory to create a file
     * name. Currently there is no need to make this thread safe, but if you need to make it thread safe in the future
     ...
     * opened an issue on the library to see if they will add this feature. If they do then we should switch to using
     * the new feature. https://github.com/s-yata/madoka/issues/2
     *
     * @param[out] size length of byte array
    ...
    std::string CreateAndOpenTempFile(int *fd) const {
        try {
            ...
            // We use this CountMinSketch's address in memory in the file name to garauntee that the file name is unique
            // in case multiple instances of CountMinSketch are being serialized at the same time
    ...
    void CloseAndDeleteTempFile(int fd, const std::string &file_name) const {
        ...
        if (std::remove(file_name.c_str()) == -1) {
            throw OPTIMIZER_EXCEPTION(
                fmt::format("Failed to delete temporary file {} with errno {}", file_name, errno));
        }
    }
};
