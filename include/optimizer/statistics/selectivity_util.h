...
public:
    ...
    static double LessThan(common::ManagedPointer<ColumnStats<T>> column_stats, const ValueCondition &condition) {
        ...
        NOISEPAGE_ASSERT(res + std::numeric_limits<double>::epsilon() >= 0
                             && res <= 1 + std::numeric_limits<double>::epsilon(),
     ...
     */
    template <typename T>
    static double LessThanOrEqualTo(common::ManagedPointer<ColumnStats<T>> column_stats,
                                    const ValueCondition                  &condition);
