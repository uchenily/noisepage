...
public:
     ...
     * @param out_buffer output buffer. The object should already contain projection list information. @see
     * ProjectedRow.
    ...
    auto Delete(const common::ManagedPointer<transaction::TransactionContext> txn, const TupleSlot slot) const -> bool {
        ...
        NOISEPAGE_ASSERT(reinterpret_cast<LogRecord *>(txn->redo_buffer_.LastRecord())
                                 ...
                                 ->GetTupleSlot()
                             == slot,
                         "This Delete is not the most recent entry in the txn's RedoBuffer. Was StageDelete called "
                         "immediately before?");
     ...
     */
    void Scan(const common::ManagedPointer<transaction::TransactionContext> txn,
    ...
    /**
     * Generates an ProjectedColumnsInitializer for the execution layer to use. This performs the translation from
     * col_oid to col_id for the Initializer's constructor so that the execution layer doesn't need to know anything
     * about col_id.
    ...
    /**
     * Generates an ProjectedRowInitializer for the execution layer to use. This performs the translation from col_oid
     * to col_id for the Initializer's constructor so that the execution layer doesn't need to know anything about
     * col_id.
     * @param col_oids set of col_oids to be projected
