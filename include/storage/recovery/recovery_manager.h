...
class RecoveryManager : public common::DedicatedThreadOwner {
    ...
    public:
        ...
        void Terminate() override {
            recovery_manager_->recovery_task_loop_again_ = false;
        }
...
public:
    ...
    bool IsRecoveryTaskRunning() const {
        return recovery_task_ != nullptr;
    }

    /** @return The ID of the last transaction that was applied. */
    transaction::timestamp_t GetLastAppliedTransactionId() const {
        return last_applied_txn_id_;
    }
...
private:
    ...
    // TODO(Gus): This map may get huge, benchmark whether this becomes a problem and if we need a more sophisticated
    // data structure
    ...
    // though snapshot isolation would handle write-write conflicts, DDL changes such as DROP TABLE combined with GC
    // could lead to issues if we don't execute transactions in complete serial order.
    std::set<transaction::timestamp_t> deferred_txns_;

    // Used during recovery from log. Maps a the txn id from the persisted txn to its changes we have buffered. We
    // buffer changes until commit time. This ensures serializability, and allows us to skip changes from aborted txns.
     ...
     */
    void Recover() {
        if (log_provider_ != nullptr)
            RecoverFromLogs(log_provider_);
    }
    ...
    /**
     * Replay any transaction who's txn start time is less than upper_bound. If upper_bound ==
     * transaction::NO_ACTIVE_TXN, it will replay all deferred transactions
    ...
    GetSqlTable(transaction::TransactionContext *txn, catalog::db_oid_t db_oid, catalog::table_oid_t table_oid);
     ...
     */
    void UpdateIndexesOnTable(transaction::TransactionContext          *txn,
                              catalog::db_oid_t                         db_oid,
     ...
     */
    uint32_t
    ProcessSpecialCasePGDatabaseRecord(transaction::TransactionContext                          *txn,
                                       std::vector<std::pair<LogRecord *, std::vector<byte *>>> *buffered_changes,
                                       uint32_t                                                  start_idx);
     ...
     * @return list of oids
     */
    // TODO(John): Currently this is being used to extract values from redo records for catalog tables. You should look
    // at adding constants for col_id_t for catalog tables.
    ...
    GetCatalogIndex(catalog::index_oid_t oid, const common::ManagedPointer<catalog::DatabaseCatalog> &db_catalog);
