...
private:
    ...
    struct Block {
        ...
        // TODO(Tianyu): Is header access going to be too slow? If so, consider storing offsets to jump to within
        // headers as well.
...
public:
    ...
    byte *AccessForceNotNull(const TupleSlot slot, const col_id_t col_id) const {
        ...
        if (!bitmap->Test(slot.GetOffset()))
            bitmap->Flip(slot.GetOffset(), false);
        return ColumnStart(slot.GetBlock(), col_id) + layout_.AttrSize(col_id) * slot.GetOffset();
    }
     ...
     */
    static uint32_t ClearBit(uint32_t val) {
     ...
     */
    static uint32_t SetBit(uint32_t val) {
