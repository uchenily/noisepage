...
public:
    ...
    private:
        ...
        static auto InvalidTupleSlot() -> TupleSlot {
            return {nullptr, 0};
        }
     ...
     */
    bool
    Select(common::ManagedPointer<transaction::TransactionContext> txn, TupleSlot slot, ProjectedRow *out_buffer) const;
     ...
     */
    void Scan(common::ManagedPointer<transaction::TransactionContext> txn,
     ...
     * concurrent accesses are happening, as inserts maybe in flight. However, the number given is always
     ...
     * equivalent to a noop and false is returned. If return is false, undo's table pointer is nullptr (used in Abort
     * and GC)
     *
     * @param txn the calling transaction
    ...
    /**
     * Deletes the given TupleSlot, this will call StageDelete on the provided txn to generate the RedoRecord for
     * delete. The rest of the behavior follows Update's behavior.
     * @param txn the calling transaction
    ...
    const BlockLayout &GetBlockLayout() const {
        return accessor_.GetBlockLayout();
    }
    ...
    uint64_t GetNumTuple() const {
        return GetBlockLayout().NumSlots() * blocks_size_;
    }
...
private:
    ...
    // Performs a visibility check on the designated TupleSlot. Note that this does not traverse a version chain, so
    // this information alone is not enough to determine visibility of a tuple to a transaction. This should be used
    // along with a version chain traversal to determine if a tuple's versions are actually visible to a txn. The
    // criteria for visibility of a slot are presence (slot is occupied) and not deleted (logical delete bitmap is
    // non-NULL).
    ...
    /**
     * Determine if a Tuple is visible (present and not deleted) to the given transaction. It's effectively Select's
     * logic (follow a version chain if present) without the materialization. If the logic of Select changes, this
