...
protected:
     ...
     */
    explicit Index(IndexMetadata metadata)
...
public:
     ...
     */
    virtual bool
    Insert(common::ManagedPointer<transaction::TransactionContext> txn, const ProjectedRow &tuple, TupleSlot location)
        = 0;
     ...
     */
    virtual bool InsertUnique(common::ManagedPointer<transaction::TransactionContext> txn,
     ...
     */
    virtual void
    Delete(common::ManagedPointer<transaction::TransactionContext> txn, const ProjectedRow &tuple, TupleSlot location)
        = 0;
     ...
     */
    virtual void
    ScanKey(const transaction::TransactionContext &txn, const ProjectedRow &key, std::vector<TupleSlot> *value_list)
        = 0;
     ...
     */
    virtual void ScanAscending(const transaction::TransactionContext &txn,
                               ScanType                               scan_type,
                               uint32_t                               num_attrs,
                               ProjectedRow                          *low_key,
                               ProjectedRow                          *high_key,
                               uint32_t                               limit,
     ...
     * @param[out] value_list the values associated with the keys
     */
    virtual void ScanDescending(const transaction::TransactionContext &txn,
                                const ProjectedRow                    &low_key,
                                const ProjectedRow                    &high_key,
                                std::vector<TupleSlot>                *value_list) {
        NOISEPAGE_ASSERT(false, "You called a method on an index type that hasn't implemented it.");
    }
     ...
     */
    virtual void ScanLimitDescending(const transaction::TransactionContext &txn,
                                     const ProjectedRow                    &low_key,
                                     const ProjectedRow                    &high_key,
                                     std::vector<TupleSlot>                *value_list,
                                     uint32_t                               limit) {
    ...
    const ProjectedRowInitializer &GetProjectedRowInitializer() const {
        return metadata_.GetProjectedRowInitializer();
    }

    /**
     * @return IndexKeyKind selected by the IndexBuilder at index construction
     */
    IndexKeyKind KeyKind() const {
        return metadata_.KeyKind();
    }
};
