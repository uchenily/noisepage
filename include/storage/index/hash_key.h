...
public:
     ...
     * @param metadata index information, primarily attribute sizes and the precomputed offsets to translate PR layout
     * to HashKey
     * @param num_attrs Number of attributes
     */
    void SetFromProjectedRow(const storage::ProjectedRow &from,
                             const IndexMetadata         &metadata,
                             [[maybe_unused]] size_t      num_attrs) {
        // we hash and compare KeySize bytes in all of our operations. Since there might be over-provisioned bytes, we
        // want to make sure the entire key is memset to 0
...
struct equal_to<noisepage::storage::index::HashKey<KeySize>> {
    ...
    bool operator()(const noisepage::storage::index::HashKey<KeySize> &lhs,
        ...
        // you're technically comparing more bytes than you need to, but hopefully key size isn't wildly
        // over-provisioned
