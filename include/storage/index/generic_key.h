...
public:
    ...
    void SetFromProjectedRow(const storage::ProjectedRow &from, const IndexMetadata &metadata, size_t num_attrs) {
        ...
        if (metadata.MustInlineVarlen()) {
            ...
            for (uint16_t i = 0; i < num_attrs; i++) {
                ...
                NOISEPAGE_ASSERT(offset == pr->ColumnIds()[i].UnderlyingValue(),
            ...
            NOISEPAGE_ASSERT(reinterpret_cast<uintptr_t>(GetProjectedRow()) + from.Size()
                                 <= reinterpret_cast<uintptr_t>(this) + KeySize,
                             "ProjectedRow will access out of bounds.");
     ...
     * These comparisons are merely for ordering semantics. If it makes sense for this to be moved somewhere more
     * general purpose in the future, feel free to do so, but for now the only use is in GenericKey and I don't want to
     * encourage their misuse elsewhere in the system.
     */
...
#define COMPARE_FUNC(OP)                                                                                               \
    ...
    default:                                                                                                           \
         ...
         */
        static bool CompareLessThan(const execution::sql::SqlTypeId type_id,
         ...
         */
        static bool CompareGreaterThan(const execution::sql::SqlTypeId type_id,
                                       const byte *const               lhs_attr,
                                       const byte *const               rhs_attr) {
            COMPARE_FUNC(>) // NOLINT
        }
         ...
         */
        static bool
        CompareEquals(const execution::sql::SqlTypeId type_id, const byte *const lhs_attr, const byte *const rhs_attr) {
            COMPARE_FUNC(==)
        }
    };
    ...
    bool PartialLessThan(const GenericKey<KeySize>            &rhs,
        ...
        for (uint16_t i = 0; i < num_attrs; i++) {
            ...
            if (noisepage::storage::index::GenericKey<KeySize>::TypeComparators::CompareLessThan(type_id,
                                                                                                 lhs_attr,
                                                                                                 rhs_attr))
                return true;
            if (noisepage::storage::index::GenericKey<KeySize>::TypeComparators::CompareGreaterThan(type_id,
                                                                                                    lhs_attr,
                                                                                                    rhs_attr))
                return false;
...
public:
    ...
    size_t operator()(noisepage::storage::index::GenericKey<KeySize> const &key) const {
        ...
        for (uint16_t i = 0; i < key_cols.size(); i++) {
            ...
            running_hash
                = XXH3_64bits_withSeed(reinterpret_cast<const void *>(attr), inlined_attr_sizes[i], running_hash);
        }
...
struct equal_to<noisepage::storage::index::GenericKey<KeySize>> {
    ...
    bool operator()(const noisepage::storage::index::GenericKey<KeySize> &lhs,
        ...
        for (uint16_t i = 0; i < key_cols.size(); i++) {
            ...
            if (!noisepage::storage::index::GenericKey<KeySize>::TypeComparators::CompareEquals(type_id,
                                                                                                lhs_attr,
                                                                                                rhs_attr)) {
