...
public:
     ...
     */
    explicit DiskLogConsumerTask(const std::chrono::microseconds                       persist_interval,
...
private:
    ...
    // Condition variable to signal disk log consumer task thread to wake up and flush buffers to disk or if shutdown
    // has initiated, then quit
