...
public:
     ...
     * moved at runtime -- this exists solely so that std::vector's emplace_back requirement of being both
     * MoveInsertable and EmplaceConstructible will be satisfied.
     */
     ...
     * only until it is full. If buffer gets full, then call FlushBuffer() and call BufferWrite(..) again with the
     * correct offset of the data, depending on the number of bytes that were already written.
    ...
    uint32_t BufferWrite(const void *const data, uint32_t size) {
        // If we still do not have buffer space after flush, the write is too large to be buffered. We partially write
    ...
    void Persist() {
#if __APPLE__
        // macOS provides fcntl(out_, F_FULLFSYNC) to guarantee that on-disk buffers are flushed. AFAIK there is no
        // portable way to do this on Linux so we'll just keep fsync for now.
        if (fsync(out_) == -1)
            throw std::runtime_error("fsync failed with errno " + std::to_string(errno));
#else
        if (fdatasync(out_) == -1)
            throw std::runtime_error("fdatasync failed with errno " + std::to_string(errno));
#endif
    }
    ...
    bool IsBufferFull() const {
        return buffer_size_ == common::Constants::LOG_BUFFER_SIZE;
    }
...
private:
    ...
    bool CanBuffer(uint32_t size) {
        return common::Constants::LOG_BUFFER_SIZE - buffer_size_ >= size;
    }
...
public:
    ...
    /**
     * Closes log file if it has not been closed already. While Read will close the file if it reaches the end, this
     * will handle cases where we destroy the reader before reading the whole file.
     */
    ~BufferedLogReader() {
        if (in_ != -1)
     ...
     */
    bool HasMore() {
        return filled_size_ > read_head_ || in_ != -1;
    }
