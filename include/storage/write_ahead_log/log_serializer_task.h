...
public:
    ...
    explicit LogSerializerTask(
        const std::chrono::microseconds                                              serialization_interval,
...
private:
    ...
    // TODO(Gus): If we guarantee there is only one TSManager in the system, this can just be a vector. We could also
    // pass TS into the serializer instead of having a pointer for it in every commit/abort record
    ...
    /**
     * Process all the accumulated log records and serialize them to log consumer tasks. It's important that we
     * serialize the logs in order to ensure that a single transaction's logs are ordered. Only a single thread can
     * serialize the logs (without more sophisticated ordering checks).
