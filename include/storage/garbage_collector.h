...
public:
    /**
     * Constructor for the Garbage Collector that requires a pointer to the TransactionManager. This is necessary for
     * the GC to invoke the TM's function for handing off the completed transactions queue.
     ...
     */
    void SetGCInterval(uint64_t gc_interval) {
...
private:
    ...
    // timestamp of the last time GC unlinked anything. We need this to know when unlinked versions are safe to
    // deallocate
