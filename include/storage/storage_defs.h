...
public:
     ...
     * Since the block size is less then (1<<20) the uppper 12 bits of insert_head_ is free. We use the first bit
     * (1<<31) to indicate if the block is insertable. If the first bit is 0, the block is insertable, otherwise one txn
     * is inserting to this block
     */
     ...
     */
    uint32_t GetInsertHead() {
        return INT32_MAX & insert_head_.load();
    }
};
...
public:
     ...
     */
    void Delete(RawBlock *const ptr) {
        delete ptr;
    }
};
