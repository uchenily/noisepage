...
public:
    ...
    /**
     * @return The maximum size of the varlen field, in bytes, that can be inlined within the object. Any objects that
     * are larger need to be stored as a pointer to a separate buffer.
     */
     ...
     * inlined.
     */
    static constexpr uint32_t PrefixSize() {
    ...
    const byte *Content() const {
        return IsInlined() ? prefix_ : content_;
    }
    ...
    static bool CompareEqualOrNot(const VarlenEntry &left, const VarlenEntry &right) {
        ...
        //  We're going to mask off the reclaim bit. Because the reclaim bit is the sign bit of the size, it ends up
        //  near the middle of this 8 byte sequence due to endianness with that int32_t. The mask defined below passes
        //  through all of the bits except the MSB of size. Its position is due to the original endianness of the
        //  int32_t being embedded within a uint64_t (which is also subject to endianness shenanigans).
        ...
        if (size_and_prefix_same) {
            ...
            if (left.IsInlined()) {
                ...
                // inspect the remaining non-inlined bytes, skipping prefix-size bytes since those are duplicated at the
                // start of content
                NOISEPAGE_ASSERT(
                    std::memcmp(left.content_, &left.prefix_, PrefixSize()) == 0,
                ...
                NOISEPAGE_ASSERT(
                    std::memcmp(right.content_, &right.prefix_, PrefixSize()) == 0,
                    "The prefix should be at the beginning of the non-inlined content again. We assert this since "
                    "we're about to skip it on the real comparison.");
                if (std::memcmp(left.content_ + PrefixSize(), right.content_ + PrefixSize(), left.Size() - PrefixSize())
                    == 0) {
                    return EqualityCheck;
                }
            }
     ...
     */
    bool operator==(const VarlenEntry &that) const {
        return CompareEqualOrNot<true>(*this, that);
    }
     ...
     */
    bool operator!=(const VarlenEntry &that) const {
        return CompareEqualOrNot<false>(*this, that);
    }
...
struct VarlenContentCompare {
     ...
     */
    bool operator()(const VarlenEntry &lhs, const VarlenEntry &rhs) const {
        return VarlenEntry::Compare(lhs, rhs) < 0;
    }
};
