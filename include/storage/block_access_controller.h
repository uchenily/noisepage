...
public:
    ...
    /**
     * Checks whether the block is safe for in-place reads. If the result returns true, the lock is successfully
     * acquired and will need to be explicitly dropped via invocation of ReleaseRead. Otherwise, the block cannot be
     * accessed in-place and the reader should try the transactional path and make a snapshot of the block.
     * @return whether reading in-place is allowed for this block.
     */
    bool TryAcquireInPlaceRead() {
        // TODO(Tianyu): This probably does not scale well. But our assumed workload is that readers will not be
        // contending for access in a tight loop, so maybe it's fine.
        while (true) {
            ...
            if (curr_state.first != BlockState::FROZEN)
                return false;
            // Increment reader count while holding the rest constant
            if (UpdateAtomically(curr_state, {curr_state.first, curr_state.second + 1}))
                return true; // NOLINT
        }
    }
...
private:
    ...
    bool UpdateAtomically(const std::pair<BlockState, uint32_t> &expected, std::pair<BlockState, uint32_t> desired) {
        ...
        return reinterpret_cast<std::atomic<uint64_t> *>(bytes_)->compare_exchange_strong(expected_bytes,
                                                                                          *desired_bytes);
    }
};
} // namespace noisepage::storage
