...
public:
    ...
    void SetFormat(const ExplainStatementFormat format) {
        format_ = format;
    }

    /** @return the SQL statement to be explained */
    common::ManagedPointer<SQLStatement> GetSQLStatement() const {
        return common::ManagedPointer(real_sql_stmt_);
    }

    /** @return format of the EXPLAIN */
    ExplainStatementFormat GetFormat() const {
        return format_;
    }
