...
public:
    ...
    common::ManagedPointer<TableRef> GetDeletionTable() const {
        return common::ManagedPointer(table_ref_);
    }

    /** @return expression that represents deletion condition */
    common::ManagedPointer<AbstractExpression> GetDeleteCondition() {
        return expr_;
    }
    ...
    std::vector<common::ManagedPointer<TableRef>> GetDeleteWith() const {
        ...
        std::transform(with_tables_.cbegin(), with_tables_.cend(), std::back_inserter(table_refs), [](const auto &ref) {
