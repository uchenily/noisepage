...
public:
    ...
    static void GetAggregateExprs(std::vector<common::ManagedPointer<AggregateExpression>> *aggr_exprs,
        ...
        if (ExpressionUtil::IsAggregateExpression(expr->GetExpressionType())) {
            ...
            for (const auto &child : expr->GetChildren())
                GetAggregateExprs(aggr_exprs, child);
        }
    }
     ...
     */
    static bool IsColumnRefExpression(ExpressionType type) {
     ...
     */
    static bool IsConstantExpression(ExpressionType type) {
     ...
     */
    static bool IsParamExpression(ExpressionType type) {
        return type == ExpressionType::VALUE_PARAMETER;
    }
...
public:
    ...
    ConvertExprCVNodes(common::ManagedPointer<AbstractExpression> expr,
        ...
        for (size_t i = 0; i < expr->GetChildrenSize(); i++) {
            ...
            for (auto &child_expr_map : child_expr_maps) {
                if (child_expr->GetExpressionType() != ExpressionType::COLUMN_VALUE
    ...
    static bool EqualExpressions(const std::vector<common::ManagedPointer<AbstractExpression>> &l,
                                 const std::vector<common::ManagedPointer<AbstractExpression>> &r,
                                 bool                                                           ordered = false) {
        if (l.size() != r.size())
            return false;
     ...
     */
    static std::unique_ptr<AbstractExpression>
    JoinAnnotatedExprs(const std::vector<optimizer::AnnotatedExpression> &exprs) {
        if (exprs.empty()) {
            return nullptr;
        }
