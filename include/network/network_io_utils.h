...
public:
     ...
     * @param begin
     */
    ReadBufferView(size_t size, ByteBuf::const_iterator begin)
        : size_(size)
        , begin_(begin) {}
    /**
...
public:
     ...
     * Instantiates a new buffer and reserve capacity many bytes.
     */
    explicit ReadBuffer(size_t capacity = SOCKET_BUFFER_CAPACITY)
        : Buffer(capacity) {}
...
public:
    ...
    common::ManagedPointer<WriteBuffer> FlushHead() {
        if (buffers_.size() > offset_)
            return common::ManagedPointer(buffers_[offset_]);
        return nullptr;
    }
