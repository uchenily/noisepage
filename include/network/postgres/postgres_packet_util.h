...
public:
    ...
    /**
     * Given a read buffer that starts at a binary value, consumes it and returns a ConstantValueExpression for that
     * type
     * @param read_buffer incoming postgres packet with next field as a value
     * @param size size of the value
     * @param type internal type of the value
     * @return ConstantValueExpression containing the value from the packet
     */
    static parser::ConstantValueExpression
    BinaryValueToInternalValue(common::ManagedPointer<ReadBufferView> read_buffer,
                               int32_t                                size,
                               execution::sql::SqlTypeId              type);
     ...
     * @return vector of internal types for the parameters
     */
    static std::vector<parser::ConstantValueExpression>
    ReadParameters(common::ManagedPointer<ReadBufferView>        read_buffer,
