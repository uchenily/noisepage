...
public:
    ...
    /**
     * @warning In the src/ folder this should only be called by the Garbage Collector to adhere to MVCC semantics.
     * Tests are allowed to deterministically delete them in ways that violate the current MVCC semantics, but you
     * should really know what you're doing when you delete a TransactionContext since its UndoRecords may still be
     ...
     * MVCC semantics define it as StartTime + INT64_MIN. TransactionContexts generated outside of the
     ...
     * @warning If you call StageDelete, the operation WILL be logged to disk. If you StageDelete anything that you
     * didn't succeed in writing into the table or decide you don't want to use, the transaction MUST abort.
     */
    void
    StageDelete(const catalog::db_oid_t db_oid, const catalog::table_oid_t table_oid, const storage::TupleSlot slot) {
        const uint32_t size = storage::DeleteRecord::Size();
        storage::DeleteRecord::Initialize(redo_buffer_.NewEntry(size, GetTransactionPolicy()),
                                          start_time_,
                                          db_oid,
                                          table_oid,
                                          slot);
    ...
    void RegisterAbortAction(const std::function<void()> &a) {
        RegisterAbortAction([=](transaction::DeferredActionManager * /*unused*/) {
            a();
        });
    }
    ...
    void RegisterCommitAction(const std::function<void()> &a) {
        RegisterCommitAction([=](transaction::DeferredActionManager * /*unused*/) {
            a();
        });
    }
    ...
    void SetDurabilityPolicy(DurabilityPolicy durability_policy) {
        durability_policy_ = durability_policy;
    }
    ...
    void SetReplicationPolicy(ReplicationPolicy replication_policy) {
        NOISEPAGE_ASSERT(replication_policy == ReplicationPolicy::DISABLE
                             || durability_policy_ != DurabilityPolicy::DISABLE,
...
private:
    ...
    // cannot be allowed to commit. Currently, it is flipped by indexes (on unique-key conflicts) or SqlTable
    // (write-write conflicts) and checked in Commit().
