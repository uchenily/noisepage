...
public:
     ...
     */
    common::ManagedPointer<std::vector<parser::ConstantValueExpression>> GetParameters() const {
        return parameters_;
    }
    ...
    /**
     * Convenience function. Common case of wanting the left and right children to have compatible types, where one
     * child currently has the correct type and the other child's type must be derived from the correct child.
     *
    ...
    /**
     * Convenience function. Check that the desired type of the child expression matches previously specified
     * constraints. Throws a BINDER_EXCEPTION if a type constraint fails.
     *
     ...
     */
    void AddCTETableName(const std::string &cte_table_name) {
     ...
     */
    bool HasCTETableName(const std::string &cte_table_name) const {
        return cte_table_names_.count(cte_table_name) > 0;
    }
