...
public:
     ...
     */
    static std::unordered_set<const ContextSensitiveTableRef *>
    ResolveDependenciesFor(const ContextSensitiveTableRef &table_ref);
...
private:
    ...
    FindRef(std::string_view alias, RefType type, std::size_t depth, std::size_t position) const;
