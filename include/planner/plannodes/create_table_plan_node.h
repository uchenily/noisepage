...
struct PrimaryKeyInfo {
    ...
    bool operator==(const PrimaryKeyInfo &rhs) const {
        if (constraint_name_ != rhs.constraint_name_)
            return false;

        if (primary_key_cols_.size() != rhs.primary_key_cols_.size())
            return false;
...
struct ForeignKeyInfo {
    ...
    bool operator==(const ForeignKeyInfo &rhs) const {
        ...
        // Sink Table Name
        if (sink_table_name_ != rhs.sink_table_name_)
            return false;

        // Update TransactionAction
        if (upd_action_ != rhs.upd_action_)
            return false;

        // Delete TransactionAction
        if (del_action_ != rhs.del_action_)
            return false;

        // Foreign Key Sources
        if (foreign_key_sources_ != rhs.foreign_key_sources_)
            return false;

        // Foreign Key Sinks
        if (foreign_key_sinks_ != rhs.foreign_key_sinks_)
            return false;
...
struct UniqueInfo {
    ...
    bool operator==(const UniqueInfo &rhs) const {
        ...
        // Unique Columns
        if (unique_cols_ != rhs.unique_cols_)
            return false;
...
struct CheckInfo {
     ...
     */
    CheckInfo(std::vector<std::string>        check_cols,
    ...
    bool operator==(const CheckInfo &rhs) const {
        ...
        // Expression Type
        if (expr_type_ != rhs.expr_type_)
            return false;

        // Expression Value
        if (expr_value_ != rhs.expr_value_)
            return false;

        // Check Columns
        if (check_cols_ != rhs.check_cols_)
            return false;
    ...
    public:
        ...
        Builder &ProcessForeignKeyConstraint(const std::string                                     &table_name,
            ...
            fkey_info.constraint_name_
                = catalog::postgres::NameBuilder::MakeName(table_name,
                                                           fkey_info.sink_table_name_,
                                                           catalog::postgres::NameBuilder::FOREIGN_KEY);
        ...
        Builder &ProcessUniqueConstraint(const common::ManagedPointer<parser::ColumnDefinition> col) {
            ...
            unique_info.constraint_name_
                = catalog::postgres::NameBuilder::MakeName(table_name_,
                                                           col->GetColumnName(),
                                                           catalog::postgres::NameBuilder::UNIQUE_KEY);
        ...
        Builder &ProcessCheckConstraint(const common::ManagedPointer<parser::ColumnDefinition> col) {
            ...
            if (col->GetCheckExpression()->GetReturnValueType() == execution::sql::SqlTypeId::Boolean) {
                ...
                CheckInfo check_info(check_cols,
                                     "con_check",
                                     col->GetCheckExpression()->GetExpressionType(),
                                     std::move(tmp_value));
...
public:
     ...
     */
    common::ManagedPointer<catalog::Schema> GetSchema() const {
     ...
     */
    const PrimaryKeyInfo &GetPrimaryKey() const {
        return primary_key_;
    }
     ...
     */
    const std::vector<ForeignKeyInfo> &GetForeignKeys() const {
     ...
     */
    const std::vector<UniqueInfo> &GetUniqueConstraints() const {
        return con_uniques_;
    }
     ...
     */
    const std::vector<CheckInfo> &GetCheckConstraints() const {
        return con_checks_;
    }
