...
public:
     ...
     */
    const std::unordered_map<execution::query_id_t, double> &GetIdToNumexec() const {
        return id_to_num_exec_;
    }

private:
    friend class WorkloadForecast;

    void AddQueryInfo(execution::query_id_t qid, double freq) {
        id_to_num_exec_[qid] += freq;
    }
