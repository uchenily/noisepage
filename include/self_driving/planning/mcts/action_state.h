...
public:
     ...
     */
    void UpdateKnobValue(const std::string &knob, int64_t value) {
        knob_values_[knob] = value;
    }
    ...
    bool operator==(const ActionState &a) const {
        ...
        if (start_interval_ != a.start_interval_)
            return false;
        if (end_interval_ != a.end_interval_)
            return false;
        if (knob_values_ != a.knob_values_)
            return false;
        if (created_indexes_ != a.created_indexes_)
            return false;
        if (dropped_indexes_ != a.dropped_indexes_)
            return false;
        return true;
    }
    ...
    const std::set<std::string> &GetCreatedIndexes() const {
        return created_indexes_;
    }
    /** @return The set of dropped indexes */
    const std::set<std::string> &GetDroppedIndexes() const {
        return dropped_indexes_;
    }
    /** @return Map from an index name to the action id that created/dropped that index */
    const std::unordered_map<std::string, action_id_t> &GetIndexActionMap() const {
        return index_action_map_;
    }
...
public:
    /** @brief Hash operator */
    size_t operator()(const ActionState &a) const {
        return a.Hash();
    }
};
