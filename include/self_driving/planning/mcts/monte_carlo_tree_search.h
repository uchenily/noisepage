...
namespace pilot {
    ...
    public:
         ...
         */
        ActionTreeNode(tree_node_id_t    parent_node_id,
                       tree_node_id_t    tree_node_id,
        ...
        tree_node_id_t GetParentNodeId() const {
            return parent_node_id_;
        }

        /** @return tree node id */
        tree_node_id_t GetTreeNodeId() const {
        ...
        catalog::db_oid_t GetDbOid() const {
            return db_oid_;
        }
        ...
        uint64_t GetActionStartSegmentIndex() const {
            return action_start_segment_index_;
        }

        /** @return action plan end index */
        uint64_t GetActionPlanEndIndex() const {
    ...
    private:
        ...
        bool
            use_min_cost_; // Use the minimum cost of all leaves (instead of the average) as the cost for internal nodes
        ...
        // The first element (ActionState) represents the state of the applied actions and the workload intervals to
        // compute the cost; the second element (action) represents an action that is applied at the start_interval_ of
        // the ActionState (currently only used for CreateIndexActions)
        std::
            unordered_map<std::pair<ActionState, action_id_t>, std::pair<double, uint64_t>, ActionStateActionPairHasher>
