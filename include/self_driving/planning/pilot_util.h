...
namespace pilot {
    ...
    public:
         ...
         * @param pipeline_qids vector of real pipeline qids to be populated (necessary to restore the qids to the
         * original forecasted qids due to the auto-incremental nature of the qids in pipeline metrics)
         ...
         */
        static std::unique_ptr<metrics::PipelineMetricRawData>
        CollectPipelineFeatures(const pilot::PlanningContext            &planning_context,
         ...
         * @param planning_context pilot planning context
         * @param pipeline_qids vector of real qids (those from forecast) for pipelines in pipeline data; necessary
         * since the auto-incremental nature of qid in pipeline metrics
         ...
         * populating it)
         */
        static void ApplyAction(const pilot::PlanningContext &planning_context,
         ...
         * @param plan_vecs the vector that would store the generated abstract plans of forecasted queries before the
         * end index
         */
         ...
         * @param end_segment_index end index (inclusive)
         * @param action an optional action to apply concurrently
         * @param action_segments the number of segments that the action span. This may be modified by ComputeCost()
         * with the interference model inference
         * @return total latency of queries calculated based on their num of exec
         */
        static double ComputeCost(pilot::PlanningContext                  *planning_context,
                                  common::ManagedPointer<WorkloadForecast> forecast,
                                  uint64_t                                 start_segment_index,
                                  uint64_t                                 end_segment_index,
                                  std::optional<AbstractAction *>          action,
        ...
        static size_t CalculateMemoryConsumption(
            const pilot::MemoryInfo                                                    &memory_info,
            const pilot::ActionState                                                   &action_state,
    ...
    private:
         ...
         */
        static void ComputeTableSizeRatios(const pilot::PlanningContext &planning_context,
                                           const WorkloadForecast       *forecast,
                                           pilot::MemoryInfo            *memory_info);

        /**
         * Get the current table and index heap memory usage
         * TODO(lin): we should get this information from the stats if the pilot is not running on the primary. But
         * since we don't have this in stats yet we're directly getting the information from c++ objects.
         * @param planning_context pilot planning context
         * @param memory_info Object that stores the returned memory info
         */
        static void ComputeTableIndexSizes(const pilot::PlanningContext &planning_context,
                                           pilot::MemoryInfo            *memory_info);

        /**
         * Execute, collect pipeline metrics, and get ou prediction for each pipeline under different query parameters
         * for queries between start and end segment indices (both inclusive) in workload forecast.
         * @param planning_context pilot planning context
         * @param forecast workload forecast information
         ...
         */
        static void
        SumFeatureInPlace(std::vector<double> *feature, const std::vector<double> &delta_feature, double normalization);
         ...
         * To recover the result for each pipeline, also maintain a multimap pipeline_to_ou_position
         * @param pipeline_to_ou_position list of tuples describing the ous associated with each pipeline, there could
         * be multiple entry with the same query id and pipeline id, since we consider pipeline under different query
         * parameters
