...
namespace pilot {
    ...
    public:
        /**
         * Constructing a sequence node representing a set of statements (segment) with a configuration (set of
         * structures).
         ...
         */
        common::ManagedPointer<SeqNode> GetBestParent() {
            return best_parent_;
        }
         ...
         */
        const std::set<action_id_t> &GetConfig() {
        ...
        void RelaxNode(const std::map<pilot::action_id_t, std::unique_ptr<pilot::AbstractAction>> &structure_map,
            ...
            for (auto &parent : parents) {
                double transit_cost
                    = SequenceTuning::ConfigTransitionCost(structure_map, parent->configuration_, configuration_);
                if (best_parent_ == nullptr
                    || parent->best_source_dist_ + transit_cost + node_cost_ < best_source_dist_) {
