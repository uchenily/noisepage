...
public:
    ...
    const std::map<Knob, std::unique_ptr<parser::AbstractExpression>> &GetOptions() const {
        return options_;
    }
...
public:
    ...
    public:
        ...
        Column(std::string                       name,
               const execution::sql::SqlTypeId   type,
               const int32_t                     type_modifier,
               const bool                        nullable,
               const parser::AbstractExpression &definition)
            : name_(std::move(name))
            , type_(type)
            , attr_length_(execution::sql::GetSqlTypeIdSize(type_))
            , type_modifier_(type_modifier == -1 ? 0 : type_modifier)
            , // TODO(Matt): this is a hack around unlimited
         ...
         */
        uint16_t AttributeLength() const {
            return attr_length_;
        }
    ...
    private:
        ...
        void Validate() const {
            ...
            if (type_ == execution::sql::SqlTypeId::Varchar || type_ == execution::sql::SqlTypeId::Varbinary) {
                ...
                // TODO(Matt): uncomment this assertion once we decide what a reasonable default is for unlimited
                // varlens
                ...
                NOISEPAGE_ASSERT(type_modifier_ == -1,
                                 "Invalid attribute modifier. Should be -1 for types that don't use it.");
            }
        }
        ...
        void SetOid(indexkeycol_oid_t oid) {
            oid_ = oid;
        }

        void SetTypeModifier(const int32_t type_modifier) {
            type_modifier_ = type_modifier;
        }

        void SetTypeId(const execution::sql::SqlTypeId type) {
            type_ = type;
        }
     ...
     */
    IndexSchema(std::vector<Column>             columns,
                const storage::index::IndexType type,
                const bool                      is_unique,
                const bool                      is_primary,
                const bool                      is_exclusion,
        ...
        NOISEPAGE_ASSERT((is_primary && is_unique) || (!is_primary),
                         "is_primary requires is_unique to be true as well.");
     ...
     */
    storage::index::IndexType Type() const {
        return type_;
    }
     ...
     */
    void SetType(storage::index::IndexType index_type) {
        type_ = index_type;
    }
     ...
     */
    const IndexOptions &GetIndexOptions() const {
