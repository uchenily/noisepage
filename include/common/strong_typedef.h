...
public:
     ...
     */
    constexpr explicit StrongTypeAlias(const IntType &val)
        : val_(val) {}
    /**
     * Move constructs a new StrongTypeAlias.
     * @param val const reference to the underlying type.
     */
    constexpr explicit StrongTypeAlias(IntType &&val)
        : val_(std::move(val)) {}

    /**
     * @return the underlying value
     */
    constexpr IntType &UnderlyingValue() {
        return val_;
    }
     ...
     * @return the underlying value
     */
    constexpr const IntType &UnderlyingValue() const {
        return val_;
    }
...
struct atomic<noisepage::common::StrongTypeAlias<Tag, IntType>> {
    ...
    explicit atomic(t val)
        : underlying_{val.UnderlyingValue()} {}
