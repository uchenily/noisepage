...
public:
    ...
    std::optional<std::pair<Result, bool>> WaitFor(std::chrono::milliseconds wait_millis) {
        bool timed_out;
        {
            ...
            timed_out = !cvar_.wait_for(lock, wait_millis, [&] {
                return done_.load();
            });
        }
     ...
     * This wait is dangerous to use, do not use it unless you guarantee that the future will be signaled no matter
     * what. An example that you should handle is simply shutting down the DBMS -- can you guarantee that the future
     * will not be stuck waiting? This would cause the DBMS to be stuck until forcefully killed, which results in the
     * DBMS having a bad exit code, and thus it will fail CI, so on and so forth.
     *
     * @warning This wait is dangerous to use since the DBMS will not exit cleanly if still waiting. You have been
     * warned.
     *
