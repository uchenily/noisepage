...
public:
    ...
    void Merge(ChunkedArray<Type, Size> *merge) {
        chunks_.insert(chunks_.end(),
                       std::make_move_iterator(merge->chunks_.begin()),
    ...
    public:
         ...
         */
        explicit Iterator(typename std::vector<ChunkSlot<TypeT, SizeT>> *chunks) noexcept
            : chunks_(chunks) {}
        ...
        Iterator(typename std::vector<ChunkSlot<TypeT, SizeT>> *chunks, std::size_t chunk_pos) noexcept
            : chunks_(chunks)
            , chunks_pos_(chunk_pos) {}

        /** @return The current element. */
        Type &operator*() const noexcept {
            return (*chunks_)[chunks_pos_].slots_[cur_pos_];
        }
