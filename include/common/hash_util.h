...
public:
     ...
     */
    template <typename T>
    static auto Hash(const T &obj) -> std::enable_if_t<!std::is_arithmetic_v<T> && !std::is_same<T, std::string>::value
    ...
    /**
     * Compute the hash value of an arithmetic input. The input is allowed to be either an integral number (8- to
     * 64-bits) or floating point number.
