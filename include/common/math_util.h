...
public:
    ...
    static constexpr bool IsPowerOf2(uint64_t val) {
        return llvm::isPowerOf2_64(val);
    }

    /**
     * @return The next power of two strictly greater than the input value.
     */
    static uint64_t NextPowerOf2(uint64_t val) {
        return llvm::NextPowerOf2(val);
    }

    /**
     * @return The next power of two greater than or equal to the input value.
     */
    static uint64_t PowerOf2Ceil(uint64_t val) {
        return llvm::PowerOf2Ceil(val);
    }
