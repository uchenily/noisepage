...
public:
     ...
     */
    OperatorTranslator(const planner::AbstractPlanNode        &plan,
    ...
    /** @return Feature type. */
    selfdriving::ExecutionOperatingUnitType GetFeatureType() const {
        return feature_type_;
    }

    /** @return The plan node as a generic node. */
    const planner::AbstractPlanNode *Op() const {
        return &plan_;
    }

    /** @return The address of the current tuple slot, if any. */
    virtual ast::Expr *GetSlotAddress() const {
        UNREACHABLE("This translator does not deal with tupleslots.");
    }
...
protected:
     ...
     */
    void GetAllChildOutputFields(uint32_t                              child_index,
    ...
    virtual bool IsCountersPassThrough() const {
        return false;
    }
    ...
    void FeatureRecord(FunctionBuilder                                    *function,
                       selfdriving::ExecutionOperatingUnitType             feature_type,
    ...
    void FeatureArithmeticRecordSet(FunctionBuilder           *function,
                                    const Pipeline            &pipeline,
                                    execution::translator_id_t translator_id,
                                    ast::Expr                 *val) const;
    /** Record arithmetic feature values by multiplying existing feature values by val. */
    void FeatureArithmeticRecordMul(FunctionBuilder           *function,
                                    const Pipeline            &pipeline,
                                    execution::translator_id_t translator_id,
                                    ast::Expr                 *val) const;
    ...
    GetHookParams(const Pipeline &pipeline, ast::Identifier *arg, ast::Expr *arg_type) const;

private:
    // For mini-runner stuff.
    friend class Pipeline;
    /** Set the child translator. */
    void SetChildTranslator(common::ManagedPointer<OperatorTranslator> translator) {
        child_translator_ = translator;
    }
    /** Set the parent translator.. */
    void SetParentTranslator(common::ManagedPointer<OperatorTranslator> translator) {
        parent_translator_ = translator;
    }
    friend class selfdriving::OperatingUnitRecorder;
    /** @return The child translator. */
    common::ManagedPointer<OperatorTranslator> GetChildTranslator() const {
        return child_translator_;
    }
    /** @return The parent translator. */
    common::ManagedPointer<OperatorTranslator> GetParentTranslator() const {
        return parent_translator_;
    }
