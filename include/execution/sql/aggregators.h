...
public:
     ...
     */
    void Advance(const Val &val) {
        count_ += static_cast<uint64_t>(!val.is_null_);
    }

    /**
     * Merge this count with the @em that count.
     */
    void Merge(const CountAggregate &that) {
        count_ += that.count_;
    }
...
public:
     ...
     * Merge this count with the @em that count.
     */
    void Merge(const CountStarAggregate &that) {
        count_ += that.count_;
    }
...
public:
     ...
     */
    SumAggregate()
        : sum_(static_cast<decltype(T::val_)>(0)) {
        sum_.is_null_ = true;
    }
...
public:
     ...
     */
    MaxAggregate()
        : max_(std::numeric_limits<decltype(T::val_)>::min()) {
        max_.is_null_ = true;
    }
...
public:
     ...
     */
    MinAggregate()
        : min_(std::numeric_limits<decltype(T::val_)>::max()) {
        min_.is_null_ = true;
    }
...
public:
    ...
    */
     ...
     * @return the TopK object.
     */
    const optimizer::TopKElements<CppType> &GetTopK() const {
        return top_k_;
    }
...
public:
     ...
     * @return the Histogram object
     */
    const optimizer::Histogram<CppType> &GetHistogram() const {
        return histogram_;
    }
