...
public:
    ...
    public:
        ...
        uint16_t NumColumns() const {
            return underlying_->GetColumnCount();
        }

        /**
         * @return pointer to the start of the array of column ids
         */
        const std::vector<storage::col_id_t> &ColumnIds() const {
        ...
        void SetTupleSlot(const storage::TupleSlot &slot) {
            underlying_->SetTupleSlot(slot, row_offset_);
        }

    private:
        friend class VectorProjection;
        RowView(VectorProjection *underlying, uint32_t row_offset)
     ...
     */
    bool IsEmpty() const {
        return GetSelectedTupleCount() == 0;
    }
     ...
     */
    const TupleIdList *GetFilteredTupleIdList() {
        return filter_;
    }
     ...
     */
    uint64_t GetSelectedTupleCount() const {
        return columns_.empty() ? 0 : columns_[0]->GetCount();
    }
    ...
    uint64_t GetTotalTupleCount() const {
        return columns_.empty() ? 0 : columns_[0]->GetSize();
    }
     ...
     */
    uint64_t GetTupleCapacity() const {
        return columns_.empty() ? 0 : columns_[0]->GetCapacity();
    }
    ...
    double ComputeSelectivity() const {
        return IsEmpty() ? 0 : owned_tid_list_.ComputeSelectivity();
    }
     ...
     */
    void SetTupleSlot(const storage::TupleSlot &slot, uint32_t row_offset) {
        tuple_slots_[row_offset] = slot;
    }

    /** @return The tuple slot at the specified row offset. */
    storage::TupleSlot GetTupleSlot(uint32_t row_offset) {
        return tuple_slots_[row_offset];
    }
...
private:
     ...
     */
    RowView InterpretAsRow(uint32_t row_offset) {
