...
struct Val {
     ...
     */
    explicit Val(bool is_null) noexcept
        : is_null_(is_null) {}
};
...
struct BoolVal : public Val {
     ...
     */
    explicit BoolVal(bool val) noexcept
        : Val(false)
        , val_(val) {}
     ...
     */
    bool ForceTruth() const noexcept {
        return !is_null_ && val_;
    }
...
struct Integer : public Val {
     ...
     */
    explicit Integer(int64_t val) noexcept
        : Val(false)
        , val_(val) {}
...
struct Real : public Val { // TODO(Matt): should this be refactored to Double for consistency?
     ...
     */
    explicit Real(float val) noexcept
        : Val(false)
        , val_(val) {}
     ...
     * @param val The initial value.
     */
    explicit Real(double val) noexcept
        : Val(false)
        , val_(val) {}
...
struct DecimalVal : public Val {
     ...
     */
    explicit DecimalVal(Decimal64 val) noexcept
        : Val(false)
        , val_(val) {}
...
struct StringVal : public Val {
     ...
     */
    explicit StringVal(storage::VarlenEntry val) noexcept
        : Val(false)
        , val_(val) {}
    ...
    static uint32_t InlineThreshold() {
        return storage::VarlenEntry::InlineThreshold();
    }
    ...
    static storage::VarlenEntry CreateVarlen(const StringVal &str, bool own) {
        if (str.is_null_) {
            // TODO(WAN): matt points out that this is rather strange, but it currently exists in upstream/master. Fix
            // later.
        ...
        return noisepage::storage::VarlenEntry::CreateInline(
            reinterpret_cast<const noisepage::byte *>(str.GetContent()),
            str.GetLength());
    }
    ...
    const char *GetContent() const noexcept {
        return reinterpret_cast<const char *>(val_.Content());
    }
...
struct DateVal : public Val {
     ...
     */
    explicit DateVal(Date val) noexcept
        : Val(false)
        , val_(val) {}
     ...
     */
    explicit DateVal(Date::NativeType val) noexcept
        : DateVal(Date{val}) {}
...
struct TimestampVal : public Val {
     ...
     */
    explicit TimestampVal(Timestamp val) noexcept
        : Val(false)
        , val_(val) {}
     ...
     */
    explicit TimestampVal(Timestamp::NativeType val) noexcept
        : TimestampVal(Timestamp{val}) {}
