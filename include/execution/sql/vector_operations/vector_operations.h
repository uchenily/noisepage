...
public:
     ...
     */
    static void Cast(const exec::ExecutionSettings &exec_settings,
                     const Vector                  &source,
                     Vector                        *target,
                     SqlTypeId                      source_type,
     ...
     */
    static void
    Add(const exec::ExecutionSettings &exec_settings, const Vector &left, const Vector &right, Vector *result);
     ...
     */
    static void
    Subtract(const exec::ExecutionSettings &exec_settings, const Vector &right, Vector *result, const Vector &left);
     ...
     */
    static void
    Multiply(const exec::ExecutionSettings &exec_settings, const Vector &left, const Vector &right, Vector *result);
     ...
     */
    static void
    Divide(const exec::ExecutionSettings &exec_settings, const Vector &left, const Vector &right, Vector *result);
     ...
     */
    static void SelectEqual(const exec::ExecutionSettings &exec_settings,
                            const Vector                  &left,
                            const Vector                  &right,
     ...
     * @param[in,out] tid_list The list of TIDs to read and update.
     */
    static void SelectGreaterThan(const exec::ExecutionSettings &exec_settings,
                                  const Vector                  &left,
                                  const Vector                  &right,
                                  TupleIdList                   *tid_list);
     ...
     * @param[in,out] tid_list The list of TIDs to read and update.
     */
    static void SelectGreaterThanEqual(const exec::ExecutionSettings &exec_settings,
                                       const Vector                  &left,
                                       const Vector                  &right,
                                       TupleIdList                   *tid_list);
     ...
     * @param[in,out] tid_list The list of TIDs to read and update.
     */
    static void SelectLessThan(const exec::ExecutionSettings &exec_settings,
                               const Vector                  &left,
                               const Vector                  &right,
                               TupleIdList                   *tid_list);
     ...
     * @param[in,out] tid_list The list of TIDs to read and update.
     */
    static void SelectLessThanEqual(const exec::ExecutionSettings &exec_settings,
                                    const Vector                  &left,
                                    const Vector                  &right,
                                    TupleIdList                   *tid_list);
     ...
     * @param[in,out] tid_list The list of TIDs to read and update.
     */
    static void SelectNotEqual(const exec::ExecutionSettings &exec_settings,
                               const Vector                  &left,
                               const Vector                  &right,
                               TupleIdList                   *tid_list);
     ...
     */
    static void
    SelectLike(const exec::ExecutionSettings &exec_settings, const Vector &a, const Vector &b, TupleIdList *tid_list);
     ...
     * @param[in,out] tid_list The list of TIDs to check, and the output of the check.
     */
    static void SelectNotLike(const exec::ExecutionSettings &exec_settings,
                              const Vector                  &a,
                              const Vector                  &b,
                              TupleIdList                   *tid_list);
     ...
     */
    static void
    GatherAndSelectEqual(const Vector &input, const Vector &pointers, std::size_t offset, TupleIdList *tid_list);
     ...
     */
    static void
    GatherAndSelectGreaterThan(const Vector &input, const Vector &pointers, std::size_t offset, TupleIdList *tid_list);
     ...
     * @param[in,out] tid_list The list of TIDs to read and update.
     */
    static void GatherAndSelectGreaterThanEqual(const Vector &input,
     ...
     */
    static void
    GatherAndSelectLessThan(const Vector &input, const Vector &pointers, std::size_t offset, TupleIdList *tid_list);
     ...
     * @param[in,out] tid_list The list of TIDs to read and update.
     */
    static void GatherAndSelectLessThanEqual(const Vector &input,
                                             const Vector &pointers,
                                             std::size_t   offset,
                                             TupleIdList  *tid_list);
     ...
     */
    static void
    GatherAndSelectNotEqual(const Vector &input, const Vector &pointers, std::size_t offset, TupleIdList *tid_list);
    ...
    static void Exec(const Vector &vector, F &&f, uint64_t offset = 0, uint64_t count = 0) {
        ...
        if (tid_list != nullptr) {
            ...
            if (offset == 0 && count == vector.GetCount()) {
                uint64_t k = 0;
                tid_list->ForEach([&](const uint64_t i) {
                    f(i, k++);
                });
            } else {
    ...
    static void ExecTyped(const Vector &vector, F &&f) {
        ...
        Exec(vector, [&](const uint64_t i, const uint64_t k) {
            f(data[i], i, k);
        });
    }
};
