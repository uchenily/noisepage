...
class VectorProjectionIterator {
    ...
    static constexpr SelectionVector FULL_INCREMENTAL_SELECTION_VECTOR = []() noexcept {
        SelectionVector ret{};
        for (sel_t i = 0; i < common::Constants::K_DEFAULT_VECTOR_SIZE; i++)
            ret[i] = i;
        return ret;
    }();
...
public:
     ...
     */
    VectorProjection *GetVectorProjection() const {
        return vector_projection_;
    }
     ...
     */
    uint32_t GetSelectedTupleCount() const {
        return vector_projection_->GetSelectedTupleCount();
    }

    /**
     * @return The total number of tuples in the projection, including filtered out tuples.
     */
    uint32_t GetTotalTupleCount() const {
        return vector_projection_->GetTotalTupleCount();
    }
...
inline sel_t VectorProjectionIterator::GetPosition() const {
...
inline void VectorProjectionIterator::Advance() {
    sel_vector_read_idx_++;
}
...
inline void VectorProjectionIterator::SynchronizedForEach(std::initializer_list<VectorProjectionIterator *> iters,
    ...
    NOISEPAGE_ASSERT(std::all_of(iters.begin(),
                                 iters.end(),
                                 [](auto vpi) {
                                     return vpi->IsFiltered();
                                 })
                         || std::none_of(iters.begin(),
                                         iters.end(),
                                         [](auto vpi) {
                                             return vpi->IsFiltered();
                                         }),
    ...
    if ((*iters.begin())->IsFiltered()) {
        for (; std::all_of(iters.begin(),
                           iters.end(),
                           [](auto vpi) {
                               return vpi->HasNextFiltered();
                           });
             std::for_each(iters.begin(), iters.end(), [](auto vpi) {
                 vpi->AdvanceFiltered();
             })) {
            f();
        }
