...
public:
    ...
    uint64_t GetTotalMemoryUsage() const {
        return sizeof(HashTableEntry *) * GetCapacity();
    }
...
protected:
    ...
    uint64_t BucketPosition(const hash_t hash) const {
        return hash & mask_;
    }
...
private:
    ...
    void AddElementCount(uint64_t v) {
        num_elements_.fetch_add(v, std::memory_order_relaxed);
    }
