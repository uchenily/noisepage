...
public:
     ...
     */
    bool operator==(const GenericValue &that) const {
        return Equals(that);
    }
     ...
     */
    static GenericValue CreateFloat(float value) {
     ...
     */
    static GenericValue
    CreateTimestamp(int32_t year, int32_t month, int32_t day, int32_t hour, int32_t min, int32_t sec);
...
private:
    ...
    explicit GenericValue(TypeId type_id)
        : type_id_(type_id)
        , is_null_(true)
        , value_() {}
