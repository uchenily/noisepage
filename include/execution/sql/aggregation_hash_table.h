...
public:
     ...
     */
    AggregationHashTable(const exec::ExecutionSettings &exec_settings,
     ...
     */
    void ProcessBatch(VectorProjectionIterator    *input_batch,
                      const std::vector<uint32_t> &key_indexes,
                      VectorInitAggFn              init_agg_fn,
                      VectorAdvanceAggFn           advance_agg_fn,
     ...
     */
    void
    ExecuteParallelPartitionedScan(void *query_state, ThreadStateContainer *thread_states, ScanPartitionFn scan_fn);
    ...
    uint64_t GetTupleCount() const {
        return hash_table_.GetElementCount();
    }
...
private:
    ...
    bool NeedsToGrow() const noexcept {
        return hash_table_.GetElementCount() >= max_fill_;
    }
...
private:
    ...
    public:
        ...
        libcount::HLL *HLL() {
            return hll_estimator_.get();
        }
        VectorProjection *Projection() {
            return &hash_and_entries_;
        }
        Vector *Hashes() {
            return hash_and_entries_.GetColumn(0);
        }
        Vector *Entries() {
            return hash_and_entries_.GetColumn(1);
        }
        TupleIdList *GroupsFound() {
            return &groups_found_;
        }
        TupleIdList *GroupsNotFound() {
            return &groups_not_found_;
        }
        TupleIdList *KeyNotEqual() {
            return &key_not_equal_;
        }
        TupleIdList *KeyEqual() {
            return &key_equal_;
        }
        HashToGroupIdMap *HashToGroupMap() {
            return hash_to_group_map_.get();
        }
...
inline HashTableEntry *AggregationHashTable::LookupEntryInternal(hash_t                        hash,
...
inline byte *
AggregationHashTable::Lookup(hash_t hash, AggregationHashTable::KeyEqFn key_eq_fn, const void *probe_tuple) {
...
public:
     ...
     */
    AHTVectorIterator(const AggregationHashTable    &agg_hash_table,
                      const catalog::Schema::Column *column_info,
                      uint32_t                       num_cols,
                      TransposeFn                    transpose_fn);
...
public:
     ...
     */
    HashTableEntry *GetEntryForRow() const {
        return curr_;
    }
