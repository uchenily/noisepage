...
public:
    ...
    explicit MemPoolPtr(std::nullptr_t)
        : obj_(nullptr) {}

    /**
     * Copy.
     */
    MemPoolPtr(const MemPoolPtr<T> &ptr)
        : obj_(ptr.obj_) {}

    /**
     * Move.
     */
    MemPoolPtr(MemPoolPtr<T> &&ptr) noexcept
        : obj_(ptr.obj_) {
        ptr.obj_ = nullptr;
    }
...
public:
     ...
     * Get the tracker
     */
    common::ManagedPointer<MemoryTracker> GetTracker() {
        return tracker_;
    }
...
public:
     ...
     */
    MemoryPoolAllocator(MemoryPool *memory)
        : memory_(memory) {} // NOLINT
    ...
    T *allocate(std::size_t n) {
        return memory_->AllocateArray<T>(n, false);
    } // NOLINT
