...
public:
    ...
    uint64_t GetTotalMemoryUsage() const {
        return sizeof(SlotGroup) * num_groups_;
    }
     ...
     */
    uint64_t GetOverflowEntryCount() const {
        return num_overflow_;
    }
...
private:
    ...
    uint64_t SlotIndex(hash_t hash) const noexcept {
        return hash & slot_mask_;
    }

    // Given a slot index, return the index of the group it falls to
    uint64_t GroupIndex(uint64_t slot_idx) const noexcept {
        return slot_idx >> LOG_SLOTS_PER_GROUP;
    }

    // Given a slot index, return the index of the bit in the group's slot chunk
    uint64_t GroupSlotIndex(uint64_t slot_index) const noexcept {
        return slot_index & GROUP_BIT_MASK;
    }
