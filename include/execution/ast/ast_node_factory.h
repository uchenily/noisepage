...
public:
     ...
     */
    explicit AstNodeFactory(util::Region *region)
        : region_(region) {}
     ...
     */
    DeclStmt *NewDeclStmt(Decl *decl) {
        return new (region_) DeclStmt(decl);
    }
     ...
     */
    LitExpr *NewIntLiteral(const SourcePosition &pos, int64_t num) {
     ...
     */
    LitExpr *NewFloatLiteral(const SourcePosition &pos, double num) {
        return new (region_) LitExpr(pos, num);
    }
     ...
     */
    ImplicitCastExpr *
    NewImplicitCastExpr(const SourcePosition &pos, CastKind cast_kind, Type *target_type, Expr *input) {
