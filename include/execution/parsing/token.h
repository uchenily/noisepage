...
#undef T
    ...
    static const char *GetName(Type type) {
        return token_names[static_cast<uint32_t>(type)];
    }
    ...
    static const char *GetString(Type type) {
        return token_strings[static_cast<uint32_t>(type)];
    }

    /**
     * @return The precedence of a given token.
     */
    static uint32_t GetPrecedence(Type type) {
        return TOKEN_PRECEDENCES[static_cast<uint32_t>(type)];
    }

    /**
     * @return The lowest precedence of all tokens.
     */
    static uint32_t LowestPrecedence() {
        return 0;
    }
