...
#undef DECLARE_AST_VISIT_METHOD
    ...
    private:
        ...
        bool
        CheckArgCountBetween(ast::CallExpr *call, uint32_t min_expected_arg_count, uint32_t max_expected_arg_count);

        // Check boolean logic operands: and, or
        CheckResult
        CheckLogicalOperands(parsing::Token::Type op, const SourcePosition &pos, ast::Expr *left, ast::Expr *right);

        // Check operands to an arithmetic operation: +, -, *, etc.
        CheckResult
        CheckArithmeticOperands(parsing::Token::Type op, const SourcePosition &pos, ast::Expr *left, ast::Expr *right);

        CheckResult
        CheckComparisonOperands(parsing::Token::Type op, const SourcePosition &pos, ast::Expr *left, ast::Expr *right);

        // Check operands to a comparison where at least one of the inputs is a SQL value.
        CheckResult CheckSqlComparisonOperands(parsing::Token::Type  op,
        ...
        public:
            FunctionSemaScope(Sema *check, ast::FunctionLitExpr *func)
                : prev_func_(check->GetCurrentFunction())
        ...
        ast::FunctionLitExpr *GetCurrentFunction() const {
            return curr_func_;
        }
