...
public:
     ...
     */
    [[nodiscard]]
    static void *MallocAligned(const std::size_t size, const std::size_t alignment) {
     ...
     */
    [[nodiscard]]
    static void *MallocHuge(const std::size_t size, const bool populate) {
    ...
    template <typename T>
    [[nodiscard]]
    static T *TrackMallocHugeArray(common::ManagedPointer<sql::MemoryTracker> tracker,
    ...
    template <typename T>
    static void
    TrackFreeHugeArray(common::ManagedPointer<sql::MemoryTracker> tracker, T arr[], const std::size_t num_elems) {
        NOISEPAGE_ASSERT(arr != nullptr, "Cannot free NULL pointer");
        NOISEPAGE_ASSERT(num_elems != 0, "Cannot free zero-sized memory");
        std::size_t size = sizeof(T) * num_elems;

        if (tracker != nullptr)
            tracker->Decrement(size);
