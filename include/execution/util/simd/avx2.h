...
public:
     ...
     */
    explicit Vec256b(const __m256i &reg)
        : reg_(reg) {}

    /**
     * Type-cast operator so that Vec*'s can be used directly with intrinsics.
     */
    ALWAYS_INLINE operator __m256i() const {
        return reg_;
    } // NOLINT
...
public:
     ...
     */
    explicit Vec4(int64_t val) {
        reg_ = _mm256_set1_epi64x(val);
    }
    /**
     * Create a vector whose contents are the 256-bit register reg.
     * @param reg initial contents of the vector
     */
    explicit Vec4(const __m256i &reg)
        : Vec256b(reg) {}
    /**
...
ALWAYS_INLINE inline void Vec4::Store(int64_t *arr) const {
...
public:
     ...
     * @param val initial value for entire vector
     */
    explicit Vec8(int32_t val) {
        reg_ = _mm256_set1_epi32(val);
    }
    /**
     * Create a vector whose contents are the 256-bit register reg.
     * @param reg initial contents of the vector
     */
    explicit Vec8(const __m256i &reg)
        : Vec256b(reg) {}
    /**
     ...
     * @return the element at the given index
     */
    ALWAYS_INLINE int32_t operator[](uint32_t index) const {
        return Extract(index);
    }
};
...
public:
     ...
     */
    explicit Vec8Mask(const __m256i &reg)
        : Vec8(reg) {}

    /**
     * Extract the value of the bit at index idx in this mask.
     */
    bool Extract(uint32_t idx) const {
        return Vec8::Extract(idx) != 0;
    }
...
public:
     ...
     */
    explicit Vec4Mask(const __m256i &mask)
        : Vec4(mask) {}
    ...
    int32_t Extract(uint32_t index) const {
        return static_cast<int32_t>(Vec4::Extract(index) != 0);
    }
...
ALWAYS_INLINE inline Vec4 operator>>(const Vec4 &a, const uint32_t shift) {
