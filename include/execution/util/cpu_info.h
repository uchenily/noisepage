...
public:
    ...
    enum Feature : uint8_t {
        ...
        // Don't add any features below this comment. If you add more features, remember to modify the value of MAX
        // below.
        MAX,
    };
     ...
     */
    uint32_t GetNumProcessors() const noexcept {
     ...
     */
    uint32_t GetNumPhysicalCores() const noexcept {
        return num_physical_cores_;
    }

    /**
     * @return The total number of logical cores in the system.
     */
    uint32_t GetNumLogicalCores() const noexcept {
        return num_logical_cores_;
    }

    /**
     * @return The size of the cache at level @em level in bytes.
     */
    uint32_t GetCacheSize(const CacheLevel level) const noexcept {
  ...
  y   * @return The size of a cache line at level @em level.
     ...
     */
    uint64_t GetRefCyclesUs() const {
        return ref_cycles_us_;
    }
