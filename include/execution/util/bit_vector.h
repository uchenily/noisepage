...
public:
    ...
    private:
        ...
        BitReference(WordType *word, uint32_t bit_pos)
            : word_(word)
            , mask_(WordType(1) << bit_pos) {}
     ...
     */
    explicit BitVector(Allocator allocator = Allocator())
        : words_(allocator) {}
    ...
    explicit BitVector(uint32_t num_bits, Allocator allocator = Allocator())
        : num_bits_(num_bits)
        , words_(NumNeededWords(num_bits), WordType(0), allocator) {
    ...
    BitVector &SetWord(const uint32_t word_position, const WordType word_val) {
        ...
        if (word_position == GetNumWords() - 1)
            ZeroUnusedBits();
        return *this;
    }
     ...
     */
    void Reserve(const uint32_t num_bits) {
        words_.reserve(NumNeededWords(num_bits));
    }
     ...
     */
    uint32_t FindFirst() const noexcept {
        return FindFrom(0);
    }
     ...
     */
    uint32_t GetNumBits() const noexcept {
        return num_bits_;
    }
     ...
     */
    uint32_t GetNumWords() const noexcept {
        return words_.size();
    }
     ...
     */
    const WordType *GetWords() const noexcept {
        return words_.data();
    }

private:
    // The number of bits in the last word
    uint32_t GetNumExtraBits() const {
        return num_bits_ % WORD_SIZE_BITS;
    }
