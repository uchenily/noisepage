...
public:
    /**
     * Construct a region with the given name @em name. No allocations are performed upon construction, only at the
     * first call to @em Allocate().
     */
     ...
     */
    uint64_t AlignmentWaste() const {
        return alignment_waste_;
    }
...
private:
    ...
    struct Chunk {
        ...
        uintptr_t End() const {
            return reinterpret_cast<uintptr_t>(this) + size_;
        }
    };
