...
VM_OP_HOT void OpNot(bool *const result, const bool input) {
    *result = !input;
}
...
#define ARITHMETIC(type, ...)                                                                                          \
    ...
    VM_OP_HOT void OpMul##_##type(type *result, type lhs, type rhs) {                                                  \
        *result = lhs * rhs;                                                                                           \
                                                                                                                       ...
                                                                                                                       \
    /* Primitive negation */                                                                                           \
    VM_OP_HOT void OpNeg##_##type(type *result, type input) {                                                          \
        *result = -input;                                                                                              \
    }                                                                                                                  \
                                                                                                                       \
...
VM_OP_HOT void OpDeref1(int8_t *dest, const int8_t *const src) {
    *dest = *src;
}

VM_OP_HOT void OpDeref2(int16_t *dest, const int16_t *const src) {
    *dest = *src;
}

VM_OP_HOT void OpDeref4(int32_t *dest, const int32_t *const src) {
    *dest = *src;
}

VM_OP_HOT void OpDeref8(int64_t *dest, const int64_t *const src) {
    *dest = *src;
}
...
VM_OP_HOT void OpAssign1(int8_t *dest, int8_t src) {
    *dest = src;
}

VM_OP_HOT void OpAssign2(int16_t *dest, int16_t src) {
    *dest = src;
}

VM_OP_HOT void OpAssign4(int32_t *dest, int32_t src) {
    *dest = src;
}
...
VM_OP_HOT void OpAssignImm1(int8_t *dest, int8_t src) {
    *dest = src;
}

VM_OP_HOT void OpAssignImm2(int16_t *dest, int16_t src) {
    *dest = src;
}

VM_OP_HOT void OpAssignImm4(int32_t *dest, int32_t src) {
    *dest = src;
}

VM_OP_HOT void OpAssignImm8(int64_t *dest, int64_t src) {
    *dest = src;
}

VM_OP_HOT void OpAssignImm4F(float *dest, float src) {
    *dest = src;
}

VM_OP_HOT void OpAssignImm8F(double *dest, double src) {
    *dest = src;
}

VM_OP_HOT void OpLea(noisepage::byte **dest, noisepage::byte *base, uint32_t offset) {
    *dest = base + offset;
}

VM_OP_HOT void
OpLeaScaled(noisepage::byte **dest, noisepage::byte *base, uint32_t index, uint32_t scale, uint32_t offset) {
    *dest = base + (scale * index) + offset;
}

VM_OP_HOT bool OpJump() {
    return true;
}

VM_OP_HOT bool OpJumpIfTrue(bool cond) {
    return cond;
}

VM_OP_HOT bool OpJumpIfFalse(bool cond) {
    return !cond;
}
...
VM_OP_COLD void
OpExecOUFeatureVectorRecordFeature(noisepage::selfdriving::ExecOUFeatureVector                    *ouvec,
                                   noisepage::execution::pipeline_id_t                             pipeline_id,
...
VM_OP_HOT void OpParallelScanTable(uint32_t                                                     table_oid,
                                   uint32_t                                                    *col_oids,
                                   uint32_t                                                     num_oids,
    ...
    noisepage::execution::sql::TableVectorIterator::ParallelScan(table_oid,
                                                                 col_oids,
                                                                 num_oids,
                                                                 query_state,
                                                                 exec_ctx,
...
VM_OP_HOT void OpVPIAdvance(noisepage::execution::sql::VectorProjectionIterator *vpi) {
    vpi->Advance();
}
...
VM_OP_HOT void OpVPIReset(noisepage::execution::sql::VectorProjectionIterator *vpi) {
    vpi->Reset();
}

VM_OP_HOT void OpVPIResetFiltered(noisepage::execution::sql::VectorProjectionIterator *vpi) {
    vpi->ResetFiltered();
}
...
#define GEN_VPI_SET(Name, SqlValueType, CppType)                                                                       \
    VM_OP_HOT void OpVPISet##Name(noisepage::execution::sql::VectorProjectionIterator *const vpi,                      \
                                  noisepage::execution::sql::SqlValueType                   *input,                    \
                                  const uint32_t                                             col_idx) {                                                            \
...
VM_OP_HOT void OpVPIGetPointer(noisepage::byte                                          **out,
                               noisepage::execution::sql::VectorProjectionIterator *const vpi,
                               const uint32_t                                             col_idx) {
...
VM_OP_HOT void OpHashInt(noisepage::hash_t *const                        hash_val,
                         const noisepage::execution::sql::Integer *const input,
                         const noisepage::hash_t                         seed) {
    *hash_val = input->is_null_ ? 0 : noisepage::common::HashUtil::HashCrc(input->val_, seed);
}

VM_OP_HOT void OpHashBool(noisepage::hash_t *const                        hash_val,
                          const noisepage::execution::sql::BoolVal *const input,
                          const noisepage::hash_t                         seed) {
    *hash_val = input->is_null_ ? 0 : noisepage::common::HashUtil::HashCrc(input->val_, seed);
}

VM_OP_HOT void OpHashReal(noisepage::hash_t *const                     hash_val,
                          const noisepage::execution::sql::Real *const input,
                          const noisepage::hash_t                      seed) {
    *hash_val = input->is_null_ ? 0 : noisepage::common::HashUtil::HashCrc(input->val_, seed);
}

VM_OP_HOT void OpHashString(noisepage::hash_t *const                          hash_val,
                            const noisepage::execution::sql::StringVal *const input,
                            const noisepage::hash_t                           seed) {
    *hash_val = input->is_null_ ? 0 : input->val_.Hash(seed);
}

VM_OP_HOT void OpHashDate(noisepage::hash_t *const                        hash_val,
                          const noisepage::execution::sql::DateVal *const input,
                          const noisepage::hash_t                         seed) {
    *hash_val = input->is_null_ ? 0 : input->val_.Hash(seed);
}
...
#define GEN_VECTOR_FILTER(Name)                                                                                        \
    VM_OP_HOT void OpVectorFilter##Name(const noisepage::execution::exec::ExecutionSettings &exec_settings,            \
                                        ...
                                        noisepage::execution::sql::TupleIdList              *tid_list) {                            \
        noisepage::execution::sql::VectorFilterExecutor::Select##Name(exec_settings,                                   \
                                                                      vector_projection,                               \
                                                                      left_col_idx,                                    \
                                                                      right_col_idx,                                   \
                                                                      tid_list);                                       \
    ...
    VM_OP_HOT void OpVectorFilter##Name##Val(const noisepage::execution::exec::ExecutionSettings &exec_settings,       \
                                             ...
                                             noisepage::execution::sql::TupleIdList              *tid_list) {                       \
        noisepage::execution::sql::VectorFilterExecutor::Select##Name##Val(exec_settings,                              \
                                                                           vector_projection,                          \
                                                                           left_col_idx,                               \
                                                                           *val,                                       \
                                                                           tid_list);                                  \
...
VM_OP_HOT void OpInitSqlNull(noisepage::execution::sql::Val *result) {
    *result = noisepage::execution::sql::Val(true);
}
...
VM_OP_HOT void OpInitTimestampYMDHMSMU(noisepage::execution::sql::TimestampVal *result,
                                       int32_t                                  year,
                                       int32_t                                  month,
                                       int32_t                                  day,
                                       int32_t                                  hour,
...
VM_OP_HOT void
OpAggregationHashTableAllocTuplePartitioned(noisepage::byte                                **result,
                                            noisepage::execution::sql::AggregationHashTable *agg_hash_table,
                                            const noisepage::hash_t                          hash_val) {
...
VM_OP_HOT void
OpAggregationHashTableProcessBatch(noisepage::execution::sql::AggregationHashTable *const agg_hash_table,
                                   noisepage::execution::sql::VectorProjectionIterator   *vpi,
                                   const uint32_t                                         num_keys,
                                   const uint32_t                                         key_cols[],
                                   const noisepage::execution::sql::AggregationHashTable::VectorInitAggFn    init_fn,
                                   const noisepage::execution::sql::AggregationHashTable::VectorAdvanceAggFn advance_fn,
...
VM_OP void
OpAggregationHashTableBuildAllHashTablePartitions(noisepage::execution::sql::AggregationHashTable *agg_hash_table,
                                                  void                                            *query_state);
...
VM_OP void OpAggregationHashTableMergePartitions(
    noisepage::execution::sql::AggregationHashTable                  *agg_hash_table,
    noisepage::execution::sql::AggregationHashTable                  *target_agg_hash_table,
    void                                                             *query_state,
...
VM_OP_HOT void OpAggregationHashTableIteratorNext(noisepage::execution::sql::AHTIterator *iter) {
    iter->Next();
}
...
VM_OP_HOT void
OpAggregationOverflowPartitionIteratorHasNext(bool                                                    *has_more,
                                              noisepage::execution::sql::AHTOverflowPartitionIterator *iter) {
    *has_more = iter->HasNext();
}

VM_OP_HOT void
OpAggregationOverflowPartitionIteratorNext(noisepage::execution::sql::AHTOverflowPartitionIterator *iter) {
    iter->Next();
}

VM_OP_HOT void
OpAggregationOverflowPartitionIteratorGetHash(noisepage::hash_t                                       *hash_val,
                                              noisepage::execution::sql::AHTOverflowPartitionIterator *iter) {
    *hash_val = iter->GetRowHash();
}

VM_OP_HOT void
OpAggregationOverflowPartitionIteratorGetRow(const noisepage::byte                                  **row,
                                             noisepage::execution::sql::AHTOverflowPartitionIterator *iter) {
    *row = iter->GetRow();
}

VM_OP_HOT void
OpAggregationOverflowPartitionIteratorGetRowEntry(noisepage::execution::sql::HashTableEntry              **entry,
                                                  noisepage::execution::sql::AHTOverflowPartitionIterator *iter) {
...
VM_OP_HOT void OpCountAggregateReset(noisepage::execution::sql::CountAggregate *agg) {
    agg->Reset();
}
...
VM_OP_HOT void OpCountAggregateFree(noisepage::execution::sql::CountAggregate *agg) {
    agg->~CountAggregate();
}
...
VM_OP_HOT void OpCountStarAggregateReset(noisepage::execution::sql::CountStarAggregate *agg) {
    agg->Reset();
}
...
VM_OP_HOT void OpIntegerSumAggregateReset(noisepage::execution::sql::IntegerSumAggregate *agg) {
    agg->Reset();
}
...
VM_OP_HOT void OpRealSumAggregateReset(noisepage::execution::sql::RealSumAggregate *agg) {
    agg->Reset();
}
...
VM_OP_HOT void OpRealSumAggregateFree(noisepage::execution::sql::RealSumAggregate *agg) {
...
VM_OP_HOT void OpIntegerMaxAggregateReset(noisepage::execution::sql::IntegerMaxAggregate *agg) {
    agg->Reset();
}
...
VM_OP_HOT void OpRealMaxAggregateReset(noisepage::execution::sql::RealMaxAggregate *agg) {
    agg->Reset();
}
...
VM_OP_HOT void OpRealMaxAggregateFree(noisepage::execution::sql::RealMaxAggregate *agg) {
...
VM_OP_HOT void OpDateMaxAggregateReset(noisepage::execution::sql::DateMaxAggregate *agg) {
    agg->Reset();
}
...
VM_OP_HOT void OpDateMaxAggregateFree(noisepage::execution::sql::DateMaxAggregate *agg) {
    agg->~DateMaxAggregate();
}
...
VM_OP_HOT void OpStringMaxAggregateReset(noisepage::execution::sql::StringMaxAggregate *agg) {
    agg->Reset();
}
...
VM_OP_HOT void OpIntegerMinAggregateReset(noisepage::execution::sql::IntegerMinAggregate *agg) {
    agg->Reset();
}
...
VM_OP_HOT void OpRealMinAggregateReset(noisepage::execution::sql::RealMinAggregate *agg) {
    agg->Reset();
}
...
VM_OP_HOT void OpRealMinAggregateFree(noisepage::execution::sql::RealMinAggregate *agg) {
...
VM_OP_HOT void OpDateMinAggregateReset(noisepage::execution::sql::DateMinAggregate *agg) {
    agg->Reset();
}
...
VM_OP_HOT void OpDateMinAggregateFree(noisepage::execution::sql::DateMinAggregate *agg) {
    agg->~DateMinAggregate();
}
...
VM_OP_HOT void OpStringMinAggregateReset(noisepage::execution::sql::StringMinAggregate *agg) {
    agg->Reset();
}
...
VM_OP_HOT void OpAvgAggregateReset(noisepage::execution::sql::AvgAggregate *agg) {
    agg->Reset();
}
...
VM_OP_HOT void OpAvgAggregateFree(noisepage::execution::sql::AvgAggregate *agg) {
    agg->~AvgAggregate();
}
...
#define GEN_BINARY_AGGREGATE(SQL_TYPE, AGG_TYPE)                                                                       \
    ...
    VM_OP_HOT void Op##AGG_TYPE##Reset(noisepage::execution::sql::AGG_TYPE *agg) {                                     \
        agg->Reset();                                                                                                  \
    }                                                                                                                  \
    ...
    VM_OP_HOT void Op##AGG_TYPE##Free(noisepage::execution::sql::AGG_TYPE *agg) {                                      \
        agg->~AGG_TYPE();                                                                                              \
    }
...
VM_OP_HOT void OpJoinHashTableIteratorNext(noisepage::execution::sql::JoinHashTableIterator *iter) {
    iter->Next();
}
...
VM_OP void OpSorterInit(noisepage::execution::sql::Sorter                    *sorter,
                        noisepage::execution::exec::ExecutionContext         *exec_ctx,
                        noisepage::execution::sql::Sorter::ComparisonFunction cmp_fn,
                        uint32_t                                              tuple_size);
...
VM_OP_HOT void
OpSorterAllocTupleTopK(noisepage::byte **result, noisepage::execution::sql::Sorter *sorter, uint64_t top_k) {
...
VM_OP_HOT void OpSorterIteratorNext(noisepage::execution::sql::SorterIterator *iter) {
    iter->Next();
}
...
VM_OP_WARM void OpResultBufferFinalize(noisepage::execution::exec::OutputBuffer *out) {
    out->Finalize();
}
...
VM_OP_WARM void OpAtan2(noisepage::execution::sql::Real       *result,
                        const noisepage::execution::sql::Real *arg_1,
...
VM_OP_WARM void OpRound2(noisepage::execution::sql::Real          *result,
                         const noisepage::execution::sql::Real    *v,
                         const noisepage::execution::sql::Integer *precision) {
    noisepage::execution::sql::ArithmeticFunctions::Round2(result, *v, *precision);
}

VM_OP_WARM void OpLog(noisepage::execution::sql::Real       *result,
                      const noisepage::execution::sql::Real *base,
                      const noisepage::execution::sql::Real *val) {
    noisepage::execution::sql::ArithmeticFunctions::Log(result, *base, *val);
}

VM_OP_WARM void OpPow(noisepage::execution::sql::Real       *result,
                      const noisepage::execution::sql::Real *base,
                      const noisepage::execution::sql::Real *val) {
...
VM_OP_HOT void OpAtomicAnd1(uint8_t *ret, uint8_t *dest, uint8_t val) {
    *ret = ATOMIC(and, dest, val);
}

// NOLINTNEXTLINE (clang-tidy incorrectly thinks "dest" should be a pointer to a const)
VM_OP_HOT void OpAtomicAnd2(uint16_t *ret, uint16_t *dest, uint16_t val) {
    *ret = ATOMIC(and, dest, val);
}

// NOLINTNEXTLINE (clang-tidy incorrectly thinks "dest" should be a pointer to a const)
VM_OP_HOT void OpAtomicAnd4(uint32_t *ret, uint32_t *dest, uint32_t val) {
    *ret = ATOMIC(and, dest, val);
}

// NOLINTNEXTLINE (clang-tidy incorrectly thinks "dest" should be a pointer to a const)
VM_OP_HOT void OpAtomicAnd8(uint64_t *ret, uint64_t *dest, uint64_t val) {
    *ret = ATOMIC(and, dest, val);
}

// NOLINTNEXTLINE (clang-tidy incorrectly thinks "dest" should be a pointer to a const)
VM_OP_HOT void OpAtomicOr1(uint8_t *ret, uint8_t *dest, uint8_t val) {
    *ret = ATOMIC(or, dest, val);
}

// NOLINTNEXTLINE (clang-tidy incorrectly thinks "dest" should be a pointer to a const)
VM_OP_HOT void OpAtomicOr2(uint16_t *ret, uint16_t *dest, uint16_t val) {
    *ret = ATOMIC(or, dest, val);
}

// NOLINTNEXTLINE (clang-tidy incorrectly thinks "dest" should be a pointer to a const)
VM_OP_HOT void OpAtomicOr4(uint32_t *ret, uint32_t *dest, uint32_t val) {
    *ret = ATOMIC(or, dest, val);
}

// NOLINTNEXTLINE (clang-tidy incorrectly thinks "dest" should be a pointer to a const)
VM_OP_HOT void OpAtomicOr8(uint64_t *ret, uint64_t *dest, uint64_t val) {
    *ret = ATOMIC(or, dest, val);
}
...
VM_OP_WARM void OpNpRunnersEmitInt(noisepage::execution::exec::ExecutionContext *ctx,
    ...
    noisepage::execution::sql::MiniRunnersFunctions::EmitTuples(ctx,
                                                                *num_tuples,
                                                                *num_cols,
                                                                *num_int_cols,
                                                                *num_real_cols);
...
// String functions
// ---------------------------------------------------------
VM_OP_WARM void OpChr(noisepage::execution::sql::StringVal         *result,
                      noisepage::execution::exec::ExecutionContext *ctx,
                      const noisepage::execution::sql::Integer     *n) {
    noisepage::execution::sql::StringFunctions::Chr(result, ctx, *n);
}

VM_OP_WARM void OpASCII(noisepage::execution::sql::Integer           *result,
                        noisepage::execution::exec::ExecutionContext *ctx,
...
VM_OP_WARM void OpConcat(noisepage::execution::sql::StringVal         *result,
                         noisepage::execution::exec::ExecutionContext *ctx,
                         const noisepage::execution::sql::StringVal   *inputs[],
                         const int64_t                                 num_inputs) {
    noisepage::execution::sql::StringFunctions::Concat(result, ctx, inputs, num_inputs);
}

VM_OP_WARM void OpLeft(noisepage::execution::sql::StringVal         *result,
                       noisepage::execution::exec::ExecutionContext *ctx,
                       const noisepage::execution::sql::StringVal   *str,
                       const noisepage::execution::sql::Integer     *n) {
...
VM_OP_WARM void OpLike(noisepage::execution::sql::BoolVal         *result,
                       const noisepage::execution::sql::StringVal *str,
                       const noisepage::execution::sql::StringVal *pattern) {
    noisepage::execution::sql::StringFunctions::Like(result, nullptr, *str, *pattern);
}

VM_OP_WARM void OpLength(noisepage::execution::sql::Integer           *result,
                         noisepage::execution::exec::ExecutionContext *ctx,
                         const noisepage::execution::sql::StringVal   *str) {
    noisepage::execution::sql::StringFunctions::Length(result, ctx, *str);
}

VM_OP_WARM void OpLower(noisepage::execution::sql::StringVal         *result,
                        noisepage::execution::exec::ExecutionContext *ctx,
                        const noisepage::execution::sql::StringVal   *str) {
...
VM_OP_WARM void OpRight(noisepage::execution::sql::StringVal         *result,
                        noisepage::execution::exec::ExecutionContext *ctx,
                        const noisepage::execution::sql::StringVal   *str,
                        const noisepage::execution::sql::Integer     *n) {
...
VM_OP_WARM void OpTrim(noisepage::execution::sql::StringVal         *result,
                       noisepage::execution::exec::ExecutionContext *ctx,
                       const noisepage::execution::sql::StringVal   *str) {
    noisepage::execution::sql::StringFunctions::Trim(result, ctx, *str);
}

VM_OP_WARM void OpTrim2(noisepage::execution::sql::StringVal         *result,
                        noisepage::execution::exec::ExecutionContext *ctx,
                        const noisepage::execution::sql::StringVal   *str,
                        const noisepage::execution::sql::StringVal   *chars) {
...
VM_OP_WARM void OpUpper(noisepage::execution::sql::StringVal         *result,
                        noisepage::execution::exec::ExecutionContext *ctx,
                        const noisepage::execution::sql::StringVal   *str) {
...
VM_OP_WARM void OpIndexIteratorScanKey(noisepage::execution::sql::IndexIterator *iter) {
    iter->ScanKey();
}

VM_OP_WARM void OpIndexIteratorScanAscending(noisepage::execution::sql::IndexIterator *iter,
                                             noisepage::storage::index::ScanType       scan_type,
                                             uint32_t                                  limit) {
...
#define GEN_PR_SCALAR_SET_CALLS(Name, SqlType, CppType)                                                                \
    VM_OP_HOT void OpPRSet##Name(noisepage::storage::ProjectedRow   *pr,                                               \
                                 uint16_t                            col_idx,                                          \
                                 noisepage::execution::sql::SqlType *val) {                                            \
        pr->Set<CppType, false>(col_idx, static_cast<CppType>(val->val_), val->is_null_);                              \
    }                                                                                                                  \
                                                                                                                       \
    VM_OP_HOT void OpPRSet##Name##Null(noisepage::storage::ProjectedRow   *pr,                                         \
                                       uint16_t                            col_idx,                                    \
                                       noisepage::execution::sql::SqlType *val) {                                      \
...
#define GEN_PR_SCALAR_GET_CALLS(Name, SqlType, CppType)                                                                \
    VM_OP_HOT void OpPRGet##Name(noisepage::execution::sql::SqlType *out,                                              \
                                 noisepage::storage::ProjectedRow   *pr,                                               \
                                 uint16_t                            col_idx) {                                                                   \
        /* Read */                                                                                                     \
        ...
        out->val_ = *ptr;                                                                                              \
                                                                                                                       ...
                                                                                                                       \
    VM_OP_HOT void OpPRGet##Name##Null(noisepage::execution::sql::SqlType *out,                                        \
                                       noisepage::storage::ProjectedRow   *pr,                                         \
                                       uint16_t                            col_idx) {                                                             \
        /* Read */                                                                                                     \
        bool  null = false;                                                                                            \
...
VM_OP_HOT void OpPRSetVarlen(noisepage::storage::ProjectedRow     *pr,
                             uint16_t                              col_idx,
                             noisepage::execution::sql::StringVal *val,
                             bool                                  own) {
...
VM_OP_HOT void OpPRSetVarlenNull(noisepage::storage::ProjectedRow     *pr,
                                 uint16_t                              col_idx,
                                 noisepage::execution::sql::StringVal *val,
                                 bool                                  own) {
    const auto varlen = noisepage::execution::sql::StringVal::CreateVarlen(*val, own);
...
VM_OP_HOT void
OpPRSetDateVal(noisepage::storage::ProjectedRow *pr, uint16_t col_idx, noisepage::execution::sql::DateVal *val) {
    auto pr_val = val->val_.ToNative();
    pr->Set<uint32_t, false>(col_idx, pr_val, val->is_null_);
}

VM_OP_HOT void
OpPRSetDateValNull(noisepage::storage::ProjectedRow *pr, uint16_t col_idx, noisepage::execution::sql::DateVal *val) {
    auto pr_val = val->is_null_ ? 0 : val->val_.ToNative();
    pr->Set<uint32_t, true>(col_idx, pr_val, val->is_null_);
}

VM_OP_HOT void OpPRSetTimestampVal(noisepage::storage::ProjectedRow        *pr,
                                   uint16_t                                 col_idx,
...
VM_OP_HOT void OpPRSetTimestampValNull(noisepage::storage::ProjectedRow        *pr,
                                       uint16_t                                 col_idx,
                                       noisepage::execution::sql::TimestampVal *val) {
    auto pr_val = val->is_null_ ? 0 : val->val_.ToNative();
...
VM_OP_HOT void
OpPRGetDateVal(noisepage::execution::sql::DateVal *out, noisepage::storage::ProjectedRow *pr, uint16_t col_idx) {
    // Read
...
VM_OP_HOT void
OpPRGetDateValNull(noisepage::execution::sql::DateVal *out, noisepage::storage::ProjectedRow *pr, uint16_t col_idx) {
    // Read
    bool  null = false;
...
VM_OP_HOT void OpPRGetTimestampVal(noisepage::execution::sql::TimestampVal *out,
                                   noisepage::storage::ProjectedRow        *pr,
                                   uint16_t                                 col_idx) {
    // Read
...
VM_OP_HOT void
OpPRGetVarlen(noisepage::execution::sql::StringVal *out, noisepage::storage::ProjectedRow *pr, uint16_t col_idx) {
    // Read
...
VM_OP_HOT void
OpPRGetVarlenNull(noisepage::execution::sql::StringVal *out, noisepage::storage::ProjectedRow *pr, uint16_t col_idx) {
    // Read
    bool  null = false;
...
VM_OP void OpStorageInterfaceTableUpdate(bool                                        *result,
                                         noisepage::execution::sql::StorageInterface *storage_interface,
                                         noisepage::storage::TupleSlot               *tuple_slot);

VM_OP void OpStorageInterfaceTableDelete(bool                                        *result,
                                         noisepage::execution::sql::StorageInterface *storage_interface,
                                         noisepage::storage::TupleSlot               *tuple_slot);
...
VM_OP_WARM void OpTestCatalogLookup(uint32_t                                     *oid_var,
                                    noisepage::execution::exec::ExecutionContext *exec_ctx,
                                    const uint8_t                                *table_name_str,
                                    uint32_t                                      table_name_length,
                                    const uint8_t                                *col_name_str,
...
VM_OP_WARM void OpTestCatalogIndexLookup(uint32_t                                     *oid_var,
                                         noisepage::execution::exec::ExecutionContext *exec_ctx,
                                         const uint8_t                                *index_name_str,
