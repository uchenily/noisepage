...
public:
    ...
    void SetTPL(std::string &&tpl) {
        map_["tpl"] = std::move(tpl);
    }
    /** Set the TBC in this module. */
    void SetTBC(std::string &&tbc) {
        map_["tbc"] = std::move(tbc);
    }

    /** @return The TPL in the module, if it exists. Otherwise empty string. */
    [[nodiscard]]
    std::string GetTPL() const {
        return map_.find("tpl") == map_.end() ? "" : map_.at("tpl");
    }
    /** @return The TBC in the module, if it exists. Otherwise empty string. */
    [[nodiscard]]
    std::string GetTBC() const {
        return map_.find("tbc") == map_.end() ? "" : map_.at("tbc");
    }
...
public:
    ...
    [[nodiscard]]
    const CompileTimeModuleMetadata &GetCompileTimeMetadata() const {
        return compile_time_metadata_;
    }
