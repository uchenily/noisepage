...
BENCHMARK_DEFINE_F(TPCCBenchmark, ScaleFactor4WithoutLogging)(benchmark::State &state) {
    ...
    const auto precomputed_args = PrecomputeArgs(&generator_,
                                                 txn_weights_,
    ...
    for (auto _ : state) {
        ...
        Builder          tpcc_builder{common::ManagedPointer(&block_store_),
                             common::ManagedPointer(&catalog),
                             common::ManagedPointer(&txn_manager)};
    ...
    if (only_count_new_order_) {
        ...
        for (const auto &worker_txns : precomputed_args) {
            for (const auto &txn : worker_txns) {
                if (txn.type_ == TransactionType::NewOrder)
                    num_new_orders++;
            }
        }
...
BENCHMARK_DEFINE_F(TPCCBenchmark, ScaleFactor4WithLogging)(benchmark::State &state) {
    ...
    for (auto _ : state) {
        ...
        // we need transactions, TPCC database, and GC
        log_manager_ = new storage::LogManager(noisepage::BenchmarkConfig::logfile_path.data(),
                                               num_log_buffers_,
