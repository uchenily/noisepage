...
struct DoNotOptimizeAwayNeedsIndirect {
    ...
    constexpr static bool VALUE = !std::is_trivially_copyable<Decayed>::value || sizeof(Decayed) > sizeof(int64_t)
...
static std::string ConstructTableName(execution::sql::SqlTypeId left_type,
                                      execution::sql::SqlTypeId right_type,
                                      int64_t                   num_left,
                                      int64_t                   num_right,
                                      size_t                    row,
                                      size_t                    car) {
 ...
 */
static std::string ConstructSQLClause(execution::sql::SqlTypeId left_type,
                                      execution::sql::SqlTypeId right_type,
                                      int64_t                   num_left,
                                      int64_t                   num_right,
...
static std::string
ConstructIndexScanPredicate(execution::sql::SqlTypeId key_type, int64_t key_num, int64_t lookup_size) {
...
static void GenIdxScanParameters(execution::sql::SqlTypeId                                  type_param,
                                 int64_t                                                    num_rows,
                                 int64_t                                                    lookup_size,
                                 int64_t                                                    num_iters,
    ...
    for (int i = 0; i < num_iters; i++) {
        ...
        auto    range = std::uniform_int_distribution(static_cast<uint32_t>(0),
                                                   static_cast<uint32_t>(num_regions - 1))(generator);
...
public:
    ...
    IndexScanChecker(size_t                                                  num_keys,
        ...
        if (idx_scan->GetLoIndexColumns().size() != num_keys)
            throw "Number keys mismatch";

        return plan;
    }

    std::unique_ptr<planner::AbstractPlanNode>
    IndexNLJoinChecker(std::string                                             build_tbl,
                       size_t                                                  num_cols,
                       common::ManagedPointer<transaction::TransactionContext> txn,
                       std::unique_ptr<planner::AbstractPlanNode>              plan) {
        if (plan->GetPlanNodeType() != planner::PlanNodeType::INDEXNLJOIN)
            throw "Expected IndexNLJoin";
        if (plan->GetChildrenSize() != 1)
            throw "Expected 1 child";
        if (plan->GetChild(0)->GetPlanNodeType() != planner::PlanNodeType::SEQSCAN)
            throw "Expected child IdxScan";

        auto *nlplan = reinterpret_cast<const planner::IndexJoinPlanNode *>(plan.get());
        if (nlplan->GetLoIndexColumns().size() != num_cols)
            throw "Number keys mismatch";
        ...
        if (nlplan->GetTableOid() != build_oid)
            throw "inner index does not match";

        return plan;
    }

    std::unique_ptr<planner::AbstractPlanNode>
    ChildIndexScanChecker(common::ManagedPointer<transaction::TransactionContext> txn,
                          std::unique_ptr<planner::AbstractPlanNode>              plan) {
    ...
    std::unique_ptr<planner::AbstractPlanNode>
    UpdateIndexScanChecker(common::ManagedPointer<transaction::TransactionContext> txn,
                           std::unique_ptr<planner::AbstractPlanNode>              plan) {
        if (plan->GetPlanNodeType() != planner::PlanNodeType::UPDATE)
            throw "Expected Update";
        auto *upd = reinterpret_cast<planner::UpdatePlanNode *>(plan.get());
        if (!upd->GetIndexOids().empty())
            throw "Update index oids not empty";
        if (plan->GetChild(0)->GetPlanNodeType() != planner::PlanNodeType::INDEXSCAN)
            throw "Expected IndexScan";
    ...
    std::unique_ptr<planner::AbstractPlanNode>
    JoinNonSelfChecker(std::string                                             build_tbl,
                       common::ManagedPointer<transaction::TransactionContext> txn,
                       std::unique_ptr<planner::AbstractPlanNode>              plan) {
        auto accessor = catalog_->GetAccessor(common::ManagedPointer(txn), db_oid, DISABLED);
        auto build_oid = accessor->GetTableOid(std::move(build_tbl));

        if (plan->GetPlanNodeType() != planner::PlanNodeType::HASHJOIN)
            throw "Expected HashJoin";
        if (plan->GetChild(0)->GetPlanNodeType() != planner::PlanNodeType::SEQSCAN)
            throw "Expected Left SeqScan";
        if (plan->GetChild(1)->GetPlanNodeType() != planner::PlanNodeType::SEQSCAN)
            throw "Expected Right SeqScan";
    ...
    OptimizeSqlStatement(const std::string                                   &query,
                         ...
                         const std::function<std::unique_ptr<planner::AbstractPlanNode>(
                             common::ManagedPointer<transaction::TransactionContext>,
                             std::unique_ptr<planner::AbstractPlanNode>)>    &checker
                         = std::function<std::unique_ptr<planner::AbstractPlanNode>(
                             common::ManagedPointer<transaction::TransactionContext>,
                             std::unique_ptr<planner::AbstractPlanNode>)>(PassthroughPlanChecker),
        ...
        auto exec_ctx = std::make_unique<execution::exec::ExecutionContext>(db_oid,
        ...
        auto exec_query
            = execution::compiler::CompilationContext::Compile(*out_plan,
                                                               exec_settings,
                                                               accessor.get(),
    ...
    void HandleBuildDropIndex(bool                      is_build,
                              int64_t                   tbl_cols,
                              int64_t                   num_rows,
                              int64_t                   num_key,
    ...
    void BenchmarkExecQuery(int64_t                                                    num_iters,
                            execution::compiler::ExecutableQuery                      *exec_query,
                            planner::OutputSchema                                     *out_schema,
                            bool                                                       commit,
        ...
        for (auto i = 0; i < num_iters; i++) {
            ...
            if (static_cast<size_t>(i) < param_ref.size()) {
                exec_ctx->SetParams(
                    common::ManagedPointer<const std::vector<parser::ConstantValueExpression>>(&param_ref[i]));
            }
...
void NetworkQueriesCreateIndexRunners(pqxx::work *txn) {
    ...
    std::vector<uint32_t> row_nums
        = {1, 10, 100, 200, 500, 1000, 2000, 5000, 10000, 20000, 50000, 100000, 300000, 500000, 1000000};
                ...
                for (auto row : row_nums) {
                    ...
                    for (int i = 0; i < iters; i++) {
                        ...
                        std::string create_query;
                        {
                            ...
                            auto              table_name
                                = ConstructTableName(type, execution::sql::SqlTypeId::Invalid, 15, 0, row, row);
...
BENCHMARK_DEFINE_F(ExecutionRunners, SEQ0_OutputRunners)(benchmark::State &state) {
    ...
    auto exec_query = execution::compiler::ExecutableQuery(output.str(),
    ...
    selfdriving::ExecutionOperatingUnitFeatureVector pipe0_vec;
    pipe0_vec.emplace_back(execution::translator_id_t(1),
...
void ExecutionRunners::ExecuteIndexOperation(benchmark::State *state, bool is_insert) {
    ...
    // Skip compiled
    if (noisepage::runner::ExecutionRunners::mode == execution::vm::ExecutionMode::Compiled)
        return;
    ...
    for (auto i = 0; i < num_index; i++) {
        ...
        auto equery = OptimizeSqlStatement(query.str(),
    ...
    for (int64_t iter = 0; iter < num_iters; iter++) {
        ...
        OpExecutionContextEndPipelineTracker(exec_ctx.get(),
                                             execution::query_id_t(0),
...
void ExecutionRunners::ExecuteSeqScan(benchmark::State *state) {
    ...
    {
        std::stringstream query;
        auto              cols = ConstructSQLClause(execution::sql::SqlTypeId::Integer,
                                       mix_type,
                                       num_integers,
                                       num_mix,
                                       ", ",
...
// NOLINTNEXTLINE
BENCHMARK_DEFINE_F(ExecutionRunners, SEQ1_0_SeqScanRunners)(benchmark::State &state) {
    ExecuteSeqScan(&state);
}

// NOLINTNEXTLINE
BENCHMARK_DEFINE_F(ExecutionRunners, SEQ1_1_SeqScanRunners)(benchmark::State &state) {
    ExecuteSeqScan(&state);
}
...
BENCHMARK_DEFINE_F(ExecutionRunners, SEQ2_0_IndexScanRunners)(benchmark::State &state) {
    ...
    pipe0_vec.emplace_back(execution::translator_id_t(1),
                           ...
                           0);
    pipe0_vec.emplace_back(execution::translator_id_t(1),
                           selfdriving::ExecutionOperatingUnitType::IDX_SCAN,
                           num_rows,
                           tuple_size,
                           key_num,
                           lookup_size,
    ...
    auto f
        = std::bind(&ExecutionRunners::IndexScanChecker, this, num_col, std::placeholders::_1, std::placeholders::_2);
    auto equery = OptimizeSqlStatement(query.str(),
                                       std::make_unique<optimizer::TrivialCostModel>(),
                                       std::move(units),
                                       f,
...
BENCHMARK_DEFINE_F(ExecutionRunners, SEQ2_1_IndexJoinRunners)(benchmark::State &state) {
    ...
    auto f = std::bind(&ExecutionRunners::IndexNLJoinChecker,
                       this,
                       inner_tbl,
                       key_num,
                       std::placeholders::_1,
...
void ExecutionRunners::ExecuteInsert(benchmark::State *state) {
    ...
    if (rerun_start || (num_rows > settings.warmup_rows_limit_ && settings.skip_large_rows_runs_))
        return;

    // Create temporary table schema
    std::vector<catalog::Schema::Column>                       cols;
    std::vector<std::pair<execution::sql::SqlTypeId, int64_t>> info = {
        {execution::sql::SqlTypeId::Integer,  num_ints},
    ...
    for (auto &i : info) {
        for (auto j = 1; j <= i.second; j++) {
            ...
            if (i.first == execution::sql::SqlTypeId::Integer) {
                cols.emplace_back(col_name.str(),
                                  i.first,
                                  false,
                                  noisepage::parser::ConstantValueExpression(execution::sql::SqlTypeId::Integer,
                                                                             execution::sql::Integer(0)));
            } else {
                cols.emplace_back(col_name.str(),
                                  i.first,
                                  false,
                                  noisepage::parser::ConstantValueExpression(execution::sql::SqlTypeId::Double,
...
// NOLINTNEXTLINE
BENCHMARK_DEFINE_F(ExecutionRunners, SEQ6_0_InsertRunners)(benchmark::State &state) {
    ExecuteInsert(&state);
}

// NOLINTNEXTLINE
BENCHMARK_DEFINE_F(ExecutionRunners, SEQ6_1_InsertRunners)(benchmark::State &state) {
    ExecuteInsert(&state);
}
...
void ExecutionRunners::ExecuteUpdate(benchmark::State *state) {
    ...
    selfdriving::ExecutionOperatingUnitFeatureVector pipe0_vec;
    pipe0_vec.emplace_back(execution::translator_id_t(1),
                           selfdriving::ExecutionOperatingUnitType::UPDATE,
                           car,
                           tuple_size,
                           update_keys,
                           car,
    ...
    equery = OptimizeSqlStatement(query.str(),
                                  std::move(cost),
                                  std::move(units),
                                  f,
                                  common::ManagedPointer<std::vector<parser::ConstantValueExpression>>(&params),
...
// NOLINTNEXTLINE
BENCHMARK_DEFINE_F(ExecutionRunners, SEQ7_2_UpdateRunners)(benchmark::State &state) {
    ExecuteUpdate(&state);
}
...
// NOLINTNEXTLINE
BENCHMARK_DEFINE_F(ExecutionRunners, SEQ8_2_DeleteRunners)(benchmark::State &state) {
    ExecuteDelete(&state);
}
...
BENCHMARK_DEFINE_F(ExecutionRunners, SEQ3_SortRunners)(benchmark::State &state) {
    ...
    pipe1_vec.emplace_back(execution::translator_id_t(1),
    ...
    auto              cols = ConstructSQLClause(execution::sql::SqlTypeId::Integer,
                                   ...
                                   false,
                                   "");
    auto              tbl_name = ConstructTableName(execution::sql::SqlTypeId::Integer,
                                       execution::sql::SqlTypeId::Double,
                                       tbl_ints,
                                       tbl_reals,
                                       row,
...
BENCHMARK_DEFINE_F(ExecutionRunners, SEQ4_HashJoinSelfRunners)(benchmark::State &state) {
    ...
    auto              cols = ConstructSQLClause(execution::sql::SqlTypeId::Integer,
                                   ...
                                   false,
                                   "");
    auto              predicate = ConstructSQLClause(execution::sql::SqlTypeId::Integer,
                                        execution::sql::SqlTypeId::BigInt,
                                        num_integers,
                                        num_bigints,
                                        " AND ",
...
BENCHMARK_DEFINE_F(ExecutionRunners, SEQ4_HashJoinNonSelfRunners)(benchmark::State &state) {
    ...
    auto build_tbl = ConstructTableName(execution::sql::SqlTypeId::Integer,
                                        execution::sql::SqlTypeId::BigInt,
                                        tbl_ints,
                                        tbl_bigints,
                                        build_row,
                                        build_car);
    auto probe_tbl = ConstructTableName(execution::sql::SqlTypeId::Integer,
                                        execution::sql::SqlTypeId::BigInt,
                                        tbl_ints,
                                        tbl_bigints,
                                        probe_row,
    ...
    auto f = std::bind(&ExecutionRunners::JoinNonSelfChecker,
                       this,
                       build_tbl,
                       std::placeholders::_1,
                       std::placeholders::_2);
...
BENCHMARK_DEFINE_F(ExecutionRunners, SEQ5_0_AggregateRunners)(benchmark::State &state) {
    ...
    for (auto i = 0; i < num_varchars; i++) {
        selfdriving::OperatingUnitRecorder::AdjustKeyWithType(execution::sql::SqlTypeId::Varchar,
                                                              &tuple_size,
                                                              &num_col);
    }
...
void ExecutionRunners::ExecuteCreateIndex(benchmark::State *state) {
    ...
    selfdriving::ExecutionOperatingUnitFeatureVector pipe0_vec;
    pipe0_vec.emplace_back(execution::translator_id_t(1),
                           selfdriving::ExecutionOperatingUnitType::CREATE_INDEX,
                           row,
                           tuple_size,
                           num_col,
                           car,
    ...
    if (num_threads != 0) {
        pipe0_vec.emplace_back(execution::translator_id_t(1),
                               selfdriving::ExecutionOperatingUnitType::CREATE_INDEX_MAIN,
                               row,
                               tuple_size,
                               num_col,
                               car,
...
// NOLINTNEXTLINE
BENCHMARK_DEFINE_F(ExecutionRunners, SEQ9_0_CreateIndexRunners)(benchmark::State &state) {
    ExecuteCreateIndex(&state);
}

// NOLINTNEXTLINE
BENCHMARK_DEFINE_F(ExecutionRunners, SEQ9_1_CreateIndexRunners)(benchmark::State &state) {
    ExecuteCreateIndex(&state);
}
...
void InitializeRunnersState() {
    ...
    param_map.find(settings::Param::port)->second.value_
        = parser::ConstantValueExpression(execution::sql::SqlTypeId::Integer,
                                          execution::sql::Integer(noisepage::runner::settings.port_));
...
void RunNetworkQueries(const NetworkWorkFunction &work) {
    // GC does not run in a background thread!
    {
        ...
        network_queries_cv.wait(lk, [] {
            return network_queries_ready;
        });
    }
...
void RunNetworkSequence(const NetworkWorkFunction &work) {
...

    {
        ...
        network_queries_cv.notify_one();
        network_queries_cv.wait(lk, [] {
            return network_queries_finished;
        });
    }
...
void RunBenchmarkSequence(int rerun_counter) {
    ...
    std::vector<std::string> titles = {"OUTPUT",
                                       "SCANS",
                                       "IDX_SCANS",
                                       "SORTS",
                                       "HJ",
    ...
    auto vm_modes
        = {noisepage::execution::vm::ExecutionMode::Interpret, noisepage::execution::vm::ExecutionMode::Compiled};
