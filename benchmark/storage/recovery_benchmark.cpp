    ...
    void RunBenchmark(benchmark::State *state, const LargeSqlTableTestConfiguration &config) {
        ...
        for (auto _ : *state) {
            ...
            storage::RecoveryManager recovery_manager(
                common::ManagedPointer<storage::AbstractLogProvider>(&log_provider),
                recovery_catalog,
                recovery_txn_manager,
                recovery_deferred_action_manager,
                recovery_replication_manager,
                recovery_thread_registry,
                recovery_block_store);
            ...
            // the table can't be freed until after all GC on it is guaranteed to be done. The easy way to do that is to
            // use a DeferredAction
...
BENCHMARK_DEFINE_F(RecoveryBenchmark, IndexRecovery)(benchmark::State &state) {
    ...
    for (auto _ : state) {
        ...
        for (uint16_t i = 0; i < num_indexes_; i++) {
            ...
            catalog::IndexSchema
                 index_schema({index_col}, storage::index::IndexType::BPLUSTREE, true, false, false, true, options);
            auto index_oid
        ...
        // Create and execute insert workload. We actually don't need to insert into indexes here, since we only care
        // about recovery doing it
