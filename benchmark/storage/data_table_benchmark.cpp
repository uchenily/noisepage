...
public:
    ...
    const storage::BlockLayout layout_{
        {column_size_, column_size_, column_size_}
    };
...
BENCHMARK_DEFINE_F(DataTableBenchmark, Insert)(benchmark::State &state) {
    ...
    for (auto _ : state) {
        ...
        auto               workload = [&](uint32_t id) {
            ...
            transaction::TransactionContext txn(transaction::timestamp_t(0),
...
BENCHMARK_DEFINE_F(DataTableBenchmark, SelectRandom)(benchmark::State &state) {
    storage::DataTable read_table(common::ManagedPointer<storage::BlockStore>(&block_store_),
                                  layout_,
                                  storage::layout_version_t(0));
    ...
    for (auto _ : state) {
        auto workload = [&](uint32_t id) {
            ...
            for (uint32_t i = 0; i < num_reads_ / BenchmarkConfig::num_threads; i++)
                read_table.Select(common::ManagedPointer(&txn),
                                  read_order[(rand_read_offsets[id] + i) % read_order.size()],
                                  reads_[id]);
        };
        common::WorkerPool thread_pool(BenchmarkConfig::num_threads, {});
        thread_pool.Startup();
