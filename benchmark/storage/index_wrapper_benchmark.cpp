...
public:
    ...
    storage::ProjectedRowInitializer tuple_initializer_
        = storage::ProjectedRowInitializer::Create(std::vector<uint16_t>{1},
                                                   std::vector<uint16_t>{1}); // This is a dummy
...
protected:
    ...
    void CreateIndex(const storage::index::IndexType type) {
        ...
        keycols.emplace_back("",
                             execution::sql::SqlTypeId::Integer,
                             false,
                             parser::ColumnValueExpression(CatalogTestUtil::TEST_DB_OID,
    ...
    // Table is populated with sequentially increasing keys, which will not make performance difference on hash index,
    // but may on the BPlusTree structure
    void PopulateTableAndIndex() {
        ...
        for (uint32_t i = 0; i < table_size_; i++) {
            auto *const insert_redo = insert_txn->StageWrite(CatalogTestUtil::TEST_DB_OID,
                                                             CatalogTestUtil::TEST_TABLE_OID,
                                                             tuple_initializer_);
    ...
    uint64_t RunWorkload() {
        ...
        for (uint32_t i = 0; i < table_size_; i++) {
            const uint32_t random_key
                = std::uniform_int_distribution(static_cast<uint32_t>(0),
                                                static_cast<uint32_t>(table_size_ - 1))(generator_);
