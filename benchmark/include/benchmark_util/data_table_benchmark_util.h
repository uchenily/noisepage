...
public:
     ...
     */
    LargeDataTableBenchmarkObject(const std::vector<uint16_t>      &attr_sizes,
     ...
     */
    transaction::TimestampManager *GetTimestampManager() {
     ...
     * @param submit_interval_us the interval between submitting two transactions for each thread, including the
     * execution
     * @return abort count, elapsed_ms
     */
    std::pair<uint64_t, uint64_t> SimulateOltp(uint32_t                 num_transactions,
