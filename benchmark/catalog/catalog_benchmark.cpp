...
public:
    ...
    std::pair<catalog::table_oid_t, std::vector<catalog::index_oid_t>>
    AddUserTableAndIndexes(const uint16_t num_indexes) {
        auto txn = txn_manager_->BeginTransaction();
        auto accessor = catalog_->GetAccessor(common::ManagedPointer(txn), db_, DISABLED);
    ...
    catalog::index_oid_t AddIndex(const std::unique_ptr<catalog::CatalogAccessor> &accessor,
        ...
        std::vector<catalog::IndexSchema::Column> key_cols{
            catalog::IndexSchema::Column{col.Name(),
                                         execution::sql::SqlTypeId::Integer,
                                         false, parser::ColumnValueExpression(db_, table_oid, col.Oid())}
        };
        catalog::IndexOptions options;
        auto                  index_schema
            = catalog::IndexSchema(key_cols, storage::index::IndexType::BPLUSTREE, true, true, false, true, options);
        const auto idx_oid
            = accessor->CreateIndex(accessor->GetDefaultNamespace(), table_oid, index_name, index_schema);
