...
void RandomDataTableTransaction::RandomInsert(Random *generator) {
    if (aborted_)
        return;
    auto *const              redo = txn_->StageWrite(CatalogTestUtil::TEST_DB_OID,
...
LargeDataTableBenchmarkObject::SimulateOltp(uint32_t                       num_transactions,
    ...
    uint64_t elapsed_ms;
    {
        ...
        for (uint32_t j = 0; j < thread_pool.NumWorkers(); j++) {
            thread_pool.SubmitTask([j, &workload] {
                workload(j);
            });
        }
...
void LargeDataTableBenchmarkObject::SimulateOneTransaction(noisepage::RandomDataTableTransaction *txn,
    ...
    RandomTestUtil::InvokeWorkloadWithDistribution({insert, update, select},
                                                   operation_ratio_,
                                                   &thread_generator,
                                                   txn_length_);
    txn->Finish();
