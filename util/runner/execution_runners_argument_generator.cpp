...
void ExecutionRunnersArgumentGenerator::GenArithArguments(OutputArgs                       *b,
...
void ExecutionRunnersArgumentGenerator::GenOutputArguments(OutputArgs                       *b,
                                                           const ExecutionRunnersSettings   &settings,
                                                           const ExecutionRunnersDataConfig &config) {
...
void ExecutionRunnersArgumentGenerator::GenScanArguments(OutputArgs                       *b,
                                                         const ExecutionRunnersSettings   &settings,
                                                         const ExecutionRunnersDataConfig &config) {
    auto row_nums = config.GetRowNumbersWithLimit(settings.data_rows_limit_);
    auto types
        = {execution::sql::SqlTypeId::Integer, execution::sql::SqlTypeId::Double, execution::sql::SqlTypeId::Varchar};
...
void ExecutionRunnersArgumentGenerator::GenScanMixedArguments(OutputArgs                       *b,
                                                              const ExecutionRunnersSettings   &settings,
                                                              const ExecutionRunnersDataConfig &config) {
...
void ExecutionRunnersArgumentGenerator::GenSortArguments(OutputArgs                       *b,
                                                         const ExecutionRunnersSettings   &settings,
                                                         const ExecutionRunnersDataConfig &config) {
    auto  is_topks = {0, 1};
    auto &num_cols = config.sweep_col_nums_;
...
void ExecutionRunnersArgumentGenerator::GenAggregateArguments(OutputArgs                       *b,
                                                              const ExecutionRunnersSettings   &settings,
                                                              const ExecutionRunnersDataConfig &config) {
    auto                         row_nums = config.GetRowNumbersWithLimit(settings.data_rows_limit_);
...
void ExecutionRunnersArgumentGenerator::GenJoinSelfArguments(OutputArgs                       *b,
                                                             const ExecutionRunnersSettings   &settings,
                                                             const ExecutionRunnersDataConfig &config) {
    auto &num_cols = config.sweep_col_nums_;
    auto  row_nums = config.GetRowNumbersWithLimit(settings.data_rows_limit_);
...
void ExecutionRunnersArgumentGenerator::GenJoinNonSelfArguments(OutputArgs                       *b,
                                                                const ExecutionRunnersSettings   &settings,
                                                                const ExecutionRunnersDataConfig &config) {
    auto &num_cols = config.sweep_col_nums_;
    auto  row_nums = config.GetRowNumbersWithLimit(settings.data_rows_limit_);
...
void ExecutionRunnersArgumentGenerator::GenIdxScanArguments(OutputArgs                       *b,
                                                            const ExecutionRunnersSettings   &settings,
                                                            const ExecutionRunnersDataConfig &config) {
    auto types
        = {execution::sql::SqlTypeId::Integer, execution::sql::SqlTypeId::BigInt, execution::sql::SqlTypeId::Varchar};
...
void ExecutionRunnersArgumentGenerator::GenIdxJoinArguments(OutputArgs                       *b,
                                                            const ExecutionRunnersSettings   &settings,
                                                            const ExecutionRunnersDataConfig &config) {
...
void ExecutionRunnersArgumentGenerator::GenInsertArguments(OutputArgs                       *b,
                                                           const ExecutionRunnersSettings   &settings,
                                                           const ExecutionRunnersDataConfig &config) {
    auto  types = {execution::sql::SqlTypeId::Integer, execution::sql::SqlTypeId::Double};
...
void ExecutionRunnersArgumentGenerator::GenInsertMixedArguments(OutputArgs                       *b,
                                                                const ExecutionRunnersSettings   &settings,
                                                                const ExecutionRunnersDataConfig &config) {
...
void ExecutionRunnersArgumentGenerator::GenUpdateIndexArguments(OutputArgs                       *b,
                                                                const ExecutionRunnersSettings   &settings,
                                                                const ExecutionRunnersDataConfig &config) {
    ...
    for (auto type : types) {
        for (auto idx_key_size : idx_key) {
            for (auto update_key : update_keys) {
                if (idx_key_size + update_key >= 15)
                    continue;

                for (auto row_num : row_nums) {
                    if (row_num > settings.updel_limit_)
                        continue;
...
void ExecutionRunnersArgumentGenerator::GenDeleteIndexArguments(OutputArgs                       *b,
                                                                const ExecutionRunnersSettings   &settings,
                                                                const ExecutionRunnersDataConfig &config) {
...
void ExecutionRunnersArgumentGenerator::GenCreateIndexArguments(OutputArgs                       *b,
                                                                const ExecutionRunnersSettings   &settings,
                                                                const ExecutionRunnersDataConfig &config) {
    ...
    for (auto col_dist : mixed_dist) {
        ...
        while (true) {
            for (auto row : row_nums) {
                ...
                while (car < row) {
                    args->push_back(
                        {start.first, start.second, col_dist.first, col_dist.second, row, car, varchar_mix});
                    car *= 2;
                }
