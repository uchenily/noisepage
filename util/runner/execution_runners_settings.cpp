...
void ExecutionRunnersSettings::InitializeFromArguments(int argc, char **argv) {
    ...
    if (gen_test_data.found_)
        generate_test_data_ = true;
    if (filter_info.found_)
        target_runner_specified_ = true;
    if (port_info.found_)
        port_ = port_info.int_value_;
    if (skip_large_rows_runs_info.found_)
        skip_large_rows_runs_ = true;
    if (warm_num_info.found_)
        warmup_iterations_num_ = warm_num_info.int_value_;
    if (index_warm_num_info.found_)
        index_model_warmup_iterations_num_ = index_warm_num_info.int_value_;
    if (rerun_info.found_)
        rerun_iterations_ = rerun_info.int_value_;
    if (updel_info.found_)
        updel_limit_ = updel_info.int_value_;
    if (warm_limit_info.found_)
        warmup_rows_limit_ = warm_limit_info.int_value_;
    if (create_index_small_data.found_)
        create_index_small_limit_ = create_index_small_data.int_value_;
    if (create_index_car_data.found_)
        create_index_large_cardinality_num_ = create_index_car_data.int_value_;
    if (run_limit.found_)
        data_rows_limit_ = run_limit.int_value_;
    if (batch_size.found_)
        index_model_batch_size_ = batch_size.int_value_;
    ...
    SETTINGS_LOG_INFO("Index Model Warmup Iterations ({}): {}",
                      index_warm_num_info.match_,
    ...
    SETTINGS_LOG_INFO("Create Index Small Build Limit ({}): {}",
                      create_index_small_data.match_,
                      create_index_small_limit_);
    SETTINGS_LOG_INFO("Create Index Large Cardinality Number Vary ({}): {}",
                      create_index_car_data.match_,
