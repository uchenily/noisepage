...
public:
    ...
    SchemaReader()
        : type_names_{
            { "tinyint",  execution::sql::SqlTypeId::TinyInt},
            {"smallint", execution::sql::SqlTypeId::SmallInt},
            {     "int",  execution::sql::SqlTypeId::Integer},
            {  "bigint",   execution::sql::SqlTypeId::BigInt},
            {    "bool",  execution::sql::SqlTypeId::Boolean},
            {    "real",   execution::sql::SqlTypeId::Double},
            { "decimal",   execution::sql::SqlTypeId::Double},
            { "varchar",  execution::sql::SqlTypeId::Varchar},
            ...
            *in >> index_info->index_name_ >> num_index_cols;
            ...
            for (uint32_t j = 0; j < num_index_cols; j++) {
                *in >> col_idx;
                ...
                index_info->cols_.emplace_back("index_col" + std::to_string(col_idx),
                                               table_column.Type(),
                                               table_column.Nullable(),
                                               parser::ConstantValueExpression(table_column.Type()));
            }
            ...
            if (col_type == execution::sql::SqlTypeId::Varchar) {
                *in >> varchar_size;
                cols.emplace_back(col_name,
                                  ...
                                  varchar_size,
                                  nullable,
                                  parser::ConstantValueExpression(col_type));
            } else {
