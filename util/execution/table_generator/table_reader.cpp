...
void TableReader::CreateIndexes(TableInfo *info, catalog::table_oid_t table_oid) {
    ...
    for (auto &index_info : info->indexes_) {
        ...
        catalog::IndexSchema
             tmp_schema{index_info->cols_, storage::index::IndexType::BPLUSTREE, false, false, false, false, options};
...
void TableReader::WriteIndexEntry(IndexInfo                   *index_info,
                                  storage::ProjectedRow       *table_pr,
                                  const std::vector<uint16_t> &table_offsets,
...
void TableReader::WriteTableCol(storage::ProjectedRow    *insert_pr,
                                uint16_t                  col_offset,
