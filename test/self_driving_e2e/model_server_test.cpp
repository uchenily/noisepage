...
TEST_F(ModelServerTest, OUAndInterferenceModelTest) {
    ...
    ms_manager->TrainModel(ModelType::Type::OperatingUnit,
                           methods,
                           &model_path,
                           ou_model_save_path,
                           nullptr,
    ...
    auto result
        = ms_manager->InferOUModel(OpUnitToString(selfdriving::ExecutionOperatingUnitType::OP_INTEGER_PLUS_OR_MINUS),
                                   ou_model_save_path,
                                   features);
    ...
    std::vector<std::vector<double>> interference_features(
        4,
        std::vector<double>{3021.72995431072,     9289.31287954857,  9.13346247415147,
    ...
    ms_manager->TrainInterferenceModel(interference_methods,
                                       input_path,
...
TEST_F(ModelServerTest, ForecastModelTest) {
    ...
    ModelServerFuture<std::string> future;
    ms_manager->TrainForecastModel(methods,
                                   input_path,
                                   save_path,
                                   interval,
                                   seq_length,
