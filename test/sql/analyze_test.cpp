...
public:
    PgStatisticOutputChecker(int64_t table_oid,
                             int64_t col_oid,
                             int64_t num_rows,
                             int64_t non_null_rows,
                             int64_t distinct_rows,
                             bool    topk_null,
                             bool    histogram_null)
    ...
    void CheckCorrectness() override {
        pg_stats_checker_.CheckCorrectness();
    }
...
TEST_F(AnalyzeTest, SingleColumnTest) {
    ...
    auto single_checker = PgStatisticOutputChecker(table_oid.UnderlyingValue(),
                                                   1,
                                                   1,
                                                   1,
                                                   1,
                                                   top_k_serialized_str,
    ...
    auto second_checker = PgStatisticOutputChecker(table_oid.UnderlyingValue(),
                                                   1,
                                                   2,
                                                   2,
                                                   2,
    ...
    auto null_checker = PgStatisticOutputChecker(table_oid.UnderlyingValue(),
                                                 1,
                                                 3,
                                                 2,
                                                 2,
    ...
    auto duplicate_checker = PgStatisticOutputChecker(table_oid.UnderlyingValue(),
                                                      1,
                                                      4,
                                                      3,
                                                      2,
...
TEST_F(AnalyzeTest, MultiColumnTest) {
    ...
    std::string histogram_serialized_str(reinterpret_cast<char *>(histogram_serialized.get()), histogram_size);
    auto        string_single_checker = PgStatisticOutputChecker(table_oid.UnderlyingValue(),
                                                          col_oid,
                                                          1,
                                                          1,
                                                          1,
    ...
    histogram_serialized_str = std::string(reinterpret_cast<char *>(histogram_serialized.get()), histogram_size);
    auto date_single_checker = PgStatisticOutputChecker(table_oid.UnderlyingValue(),
                                                        col_oid,
                                                        1,
                                                        1,
                                                        1,
    ...
    histogram_serialized_str = std::string(reinterpret_cast<char *>(histogram_serialized.get()), histogram_size);
    auto bool_single_checker = PgStatisticOutputChecker(table_oid.UnderlyingValue(),
                                                        col_oid,
                                                        1,
                                                        1,
                                                        1,
    ...
    for (int i = 5; i <= 8; i++) {
        col_oid = i;
        auto int_single_checker = PgStatisticOutputChecker(table_oid.UnderlyingValue(),
                                                           col_oid,
                                                           1,
                                                           1,
                                                           1,
    ...
    histogram_serialized_str = std::string(reinterpret_cast<char *>(histogram_serialized.get()), histogram_size);
    auto string_second_checker = PgStatisticOutputChecker(table_oid.UnderlyingValue(),
                                                          col_oid,
                                                          2,
                                                          2,
                                                          2,
    ...
    histogram_serialized = date_histogram.Serialize(&histogram_size);
    histogram_serialized_str = std::string(reinterpret_cast<char *>(histogram_serialized.get()), histogram_size);
    auto date_second_checker = PgStatisticOutputChecker(table_oid.UnderlyingValue(),
                                                        col_oid,
                                                        2,
                                                        2,
                                                        2,
    ...
    histogram_serialized = bool_histogram.Serialize(&histogram_size);
    histogram_serialized_str = std::string(reinterpret_cast<char *>(histogram_serialized.get()), histogram_size);
    auto bool_second_checker = PgStatisticOutputChecker(table_oid.UnderlyingValue(),
                                                        col_oid,
                                                        2,
                                                        2,
                                                        2,
    ...
    for (int i = 5; i <= 8; i++) {
        col_oid = i;
        auto int_second_checker = PgStatisticOutputChecker(table_oid.UnderlyingValue(),
                                                           col_oid,
                                                           2,
                                                           2,
                                                           2,
    ...
    histogram_serialized = string_histogram.Serialize(&histogram_size);
    histogram_serialized_str = std::string(reinterpret_cast<char *>(histogram_serialized.get()), histogram_size);
    auto string_null_checker = PgStatisticOutputChecker(table_oid.UnderlyingValue(),
                                                        col_oid,
                                                        3,
                                                        2,
                                                        2,
    ...
    histogram_serialized = date_histogram.Serialize(&histogram_size);
    histogram_serialized_str = std::string(reinterpret_cast<char *>(histogram_serialized.get()), histogram_size);
    auto date_null_checker = PgStatisticOutputChecker(table_oid.UnderlyingValue(),
                                                      col_oid,
                                                      3,
                                                      2,
                                                      2,
    ...
    histogram_serialized = bool_histogram.Serialize(&histogram_size);
    histogram_serialized_str = std::string(reinterpret_cast<char *>(histogram_serialized.get()), histogram_size);
    auto bool_null_checker = PgStatisticOutputChecker(table_oid.UnderlyingValue(),
                                                      col_oid,
                                                      3,
                                                      2,
                                                      2,
    ...
    for (int i = 5; i <= 8; i++) {
        col_oid = i;
        auto int_null_checker = PgStatisticOutputChecker(table_oid.UnderlyingValue(),
                                                         col_oid,
                                                         3,
                                                         2,
                                                         2,
    ...
    histogram_serialized = string_histogram.Serialize(&histogram_size);
    histogram_serialized_str = std::string(reinterpret_cast<char *>(histogram_serialized.get()), histogram_size);
    auto string_duplicate_checker = PgStatisticOutputChecker(table_oid.UnderlyingValue(),
                                                             col_oid,
                                                             4,
                                                             3,
                                                             2,
    ...
    histogram_serialized = date_histogram.Serialize(&histogram_size);
    histogram_serialized_str = std::string(reinterpret_cast<char *>(histogram_serialized.get()), histogram_size);
    auto date_duplicate_checker = PgStatisticOutputChecker(table_oid.UnderlyingValue(),
                                                           col_oid,
                                                           4,
                                                           3,
                                                           2,
    ...
    histogram_serialized = bool_histogram.Serialize(&histogram_size);
    histogram_serialized_str = std::string(reinterpret_cast<char *>(histogram_serialized.get()), histogram_size);
    auto bool_duplicate_checker = PgStatisticOutputChecker(table_oid.UnderlyingValue(),
                                                           col_oid,
                                                           4,
                                                           3,
                                                           2,
    ...
    for (int i = 5; i <= 8; i++) {
        col_oid = i;
        auto int_duplicate_checker = PgStatisticOutputChecker(table_oid.UnderlyingValue(),
                                                              col_oid,
                                                              4,
                                                              3,
                                                              2,
