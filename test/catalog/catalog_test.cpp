...
struct CatalogTests : public TerrierTest {
    ...
    void VerifyTablePresent(const catalog::CatalogAccessor &accessor,
    ...
    void VerifyTableAbsent(const catalog::CatalogAccessor &accessor,
                           catalog::namespace_oid_t        ns_oid,
                           const std::string              &table_name) {
        auto table_oid = accessor.GetTableOid(ns_oid, table_name);
...
TEST_F(CatalogTests, ProcTest) {
    ...
    auto proc_oid
        = accessor->CreateProcedure(procname,
                                    lan_oid,
                                    ns_oid,
                                    ...
                                    {},
                                    {},
                                    catalog::type_oid_t(static_cast<uint8_t>(execution::sql::SqlTypeId::Integer)),
                                    src,
                                    false);
...
TEST_F(CatalogTests, UserIndexTest) {
    ...
    std::vector<catalog::IndexSchema::Column> key_cols{
        catalog::IndexSchema::Column{"id",
                                     execution::sql::SqlTypeId::Integer,
                                     false, parser::ColumnValueExpression(db_, table_oid, schema.GetColumn("id").Oid())}
    };
...
TEST_F(CatalogTests, GetIndexObjectsTest) {
    ...
    for (auto i = 0; i < num_indexes; i++) {
        ...
        auto                  index_schema
            = catalog::IndexSchema(key_cols, storage::index::IndexType::BPLUSTREE, true, true, false, true, options);
        auto idx_oid = accessor->CreateIndex(accessor->GetDefaultNamespace(),
                                             table_oid,
                                             "test_table_idx" + std::to_string(i),
                                             index_schema);
        EXPECT_NE(idx_oid, catalog::INVALID_INDEX_OID);
