...
public:
    ...
    ManagedExpression Constant(const std::string &str) {
        ...
        return MakeManaged(std::make_unique<parser::ConstantValueExpression>(execution::sql::SqlTypeId::Varchar,
                                                                             string_val.first,
    ...
    ManagedExpression Constant(int32_t year, uint32_t month, uint32_t day) {
        return MakeManaged(
            std::make_unique<parser::ConstantValueExpression>(execution::sql::SqlTypeId::Date,
                                                              sql::DateVal(sql::Date::FromYMD(year, month, day))));
    }

    /**
     * Create an expression for a builtin call.
     */
    ManagedExpression Function(std::string                         &&func_name,
                               const std::vector<ManagedExpression> &args,
                               const execution::sql::SqlTypeId       return_value_type,
        ...
        return MakeManaged(std::make_unique<parser::FunctionExpression>(std::string{func_name},
                                                                        return_value_type,
                                                                        std::move(children),
                                                                        proc_oid));
    }
    ...
    ManagedExpression Star() {
        return MakeManaged(std::make_unique<parser::StarExpression>());
    }
     ...
     */
    ManagedExpression
    Operator(parser::ExpressionType op_type, execution::sql::SqlTypeId ret_type, ManagedExpression child) {
        std::vector<OwnedExpression> children;
     ...
     */
    ManagedExpression Operator(parser::ExpressionType    op_type,
                               execution::sql::SqlTypeId ret_type,
                               ManagedExpression         child1,
    ...
    ManagedAggExpression AggregateTerm(parser::ExpressionType agg_type, ManagedExpression child, bool distinct) {
        ...
        auto agg
            = MakeAggManaged(std::make_unique<parser::AggregateExpression>(agg_type, std::move(children), distinct));
