...
public:
     ...
     */
    explicit MultiChecker(std::vector<OutputChecker *> &&checkers)
...
public:
    GenericChecker(RowChecker row_checker, CorrectnessFn correctness_fn)
        : row_checker_{std::move(row_checker)}
        , correctness_fn_(std::move(correctness_fn)) {}
...
public:
     ...
     */
    explicit NumChecker(int64_t expected_count)
...
public:
     ...
     */
    SingleIntJoinChecker(uint32_t col1, uint32_t col2)
        : col1_(col1)
        , col2_(col2) {}
...
public:
     ...
     */
    explicit MultiOutputCallback(std::vector<exec::OutputCallback> callbacks)
    ...
    exec::OutputCallback ConstructOutputCallback() {
        return [this](byte *tuples, uint32_t num_tuples, uint32_t tuple_size) {
            operator()(tuples, num_tuples, tuple_size);
        };
    }
