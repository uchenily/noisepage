...
public:
    ...
    Identifier Ident(const std::string &s) {
        return Ctx()->GetIdentifier(s);
    }

    Expr *IdentExpr(Identifier ident) {
        return GetNodeFactory()->NewIdentifierExpr(empty_, ident);
    }

    Expr *IdentExpr(const std::string &s) {
        return IdentExpr(Ident(s));
    }

    Expr *BoolLit(bool b) {
        return GetNodeFactory()->NewBoolLiteral(empty_, b);
    }

    Expr *IntLit(int32_t i) {
        return GetNodeFactory()->NewIntLiteral(empty_, i);
    }

    Expr *FloatLit(float i) {
        return GetNodeFactory()->NewFloatLiteral(empty_, i);
    }
    ...
    Expr *Field(Expr *obj, Expr *field) {
        return GetNodeFactory()->NewMemberExpr(empty_, obj, field);
    }
    ...
    Expr *DeclRef(Decl *decl) {
        return IdentExpr(decl->Name());
    }

    Stmt *DeclStmt(Decl *decl) {
        return GetNodeFactory()->NewDeclStmt(decl);
    }
    ...
    Stmt *ExprStmt(Expr *expr) {
        return GetNodeFactory()->NewExpressionStmt(expr);
    }

    Expr *PtrType(Expr *base) {
        return GetNodeFactory()->NewPointerType(empty_, base);
    }
    ...
    Expr *PrimIntTypeRepr() {
        return BuiltinTypeRepr<BuiltinType::Int32>();
    }
    Expr *PrimFloatTypeRepr() {
        return BuiltinTypeRepr<BuiltinType::Float32>();
    }
    Expr *PrimBoolTypeRepr() {
        return BuiltinTypeRepr<BuiltinType::Bool>();
    }

    Expr *IntegerSqlTypeRepr() {
        return BuiltinTypeRepr<BuiltinType::Integer>();
    }
    Expr *RealSqlTypeRepr() {
        return BuiltinTypeRepr<BuiltinType::Real>();
    }
    ...
    Expr *ArrayTypeRepr(Expr *type) {
        return GetNodeFactory()->NewArrayType(empty_, nullptr, type);
    }

    Expr *ArrayIndex(Expr *arr, Expr *idx) {
        return GetNodeFactory()->NewIndexExpr(empty_, arr, idx);
    }
...
private:
    AstNodeFactory *GetNodeFactory() {
        return Ctx()->GetNodeFactory();
    }
