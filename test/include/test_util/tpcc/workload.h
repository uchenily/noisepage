...
public:
    ...
    Deck() {
        ...
        for (uint32_t i = 0; i < 10; i++)
            cards_.emplace_back(tpcc::TransactionType::NewOrder);
        for (uint32_t i = 0; i < 10; i++)
            cards_.emplace_back(tpcc::TransactionType::Payment);
    ...
    explicit Deck(const TransactionWeights &txn_weights) {
        ...
        NOISEPAGE_ASSERT(txn_weights.w_payment_ + txn_weights.w_order_status_ + txn_weights.w_delivery_
                                 + txn_weights.w_stock_level_
                             <= 100,
        ...
        cards_.reserve(23);
        for (uint32_t i = 0; i < min_new_order; i++)
            cards_.emplace_back(tpcc::TransactionType::NewOrder);
        for (uint32_t i = 0; i < min_payment; i++)
            cards_.emplace_back(tpcc::TransactionType::Payment);
        for (uint32_t i = 0; i < min_order_status; i++)
            cards_.emplace_back(tpcc::TransactionType::OrderStatus);
        for (uint32_t i = 0; i < min_delivery; i++)
            cards_.emplace_back(tpcc::TransactionType::Delivery);
...
TransactionArgs BuildNewOrderArgs(Random *const generator, const int8_t w_id, const int8_t num_warehouses) {
    ...
    for (int32_t i = 0; i < args.ol_cnt_; i++) {
        int32_t ol_i_id
            = (i == args.ol_cnt_ - 1 && args.rbk_ == 1) ? 8491138 : Util::NURand(8191, 1, 100000, generator);
...
template <class Random>
std::vector<std::vector<TransactionArgs>> PrecomputeArgs(Random *const             generator,
                                                         const TransactionWeights &txn_weights,
 ...
 */
void Workload(int8_t                                           worker_id,
              Database                                        *tpcc_db,
