...
public:
    explicit NewOrder(const Database *const db)
        ...
        , o_ol_cnt_insert_pr_offset_(
              static_cast<uint8_t>(order_insert_pr_map_.at(db->order_schema_.GetColumn(6).Oid())))
        ...
        , stock_update_pr_map_(db->stock_table_->ProjectionMapForOids(
              {s_quantity_oid_, s_ytd_oid_, s_order_cnt_oid_, s_remote_cnt_oid_}))
        ,
    ...
    // 2.4.2
    bool Execute(transaction::TransactionManager *txn_manager,
                 Database                        *db,
                 Worker                          *worker,
