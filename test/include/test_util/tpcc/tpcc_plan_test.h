...
public:
    ...
    Optimize(const std::string &query, catalog::table_oid_t tbl_oid, parser::StatementType stmt_type);

    // Optimizer on Insert
    void OptimizeInsert(const std::string &query, catalog::table_oid_t tbl_oid);

    // Optimize an Update
    void OptimizeUpdate(const std::string   &query,
                        catalog::table_oid_t tbl_oid,
    ...
    // Optimize a Delete
    void OptimizeDelete(const std::string   &query,
                        catalog::table_oid_t tbl_oid,
                        void (*Check)(TpccPlanTest                              *test,
                                      catalog::table_oid_t                       tbl_oid,
                                      std::unique_ptr<planner::AbstractPlanNode> plan));
