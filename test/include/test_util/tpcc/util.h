...
struct Util {
    ...
    template <typename T>
    static void SetTupleAttribute(const catalog::Schema        &schema,
    ...
    template <typename T>
    static void SetKeyAttribute(const catalog::IndexSchema                                     &schema,
                                const uint32_t                                                  col_offset,
    ...
    // 4.3.2.2
    template <class Random>
    static storage::VarlenEntry
    AlphaNumericVarlenEntry(const uint32_t x, const uint32_t y, const bool numeric_only, Random *const generator) {
    ...
    static storage::VarlenEntry OriginalVarlenEntry(const uint32_t x, const uint32_t y, Random *const generator) {
        ...
        const uint32_t original_index
            = std::uniform_int_distribution(static_cast<uint32_t>(0),
                                            static_cast<uint32_t>(astring.length() - 8))(*generator);
...
private:
    ...
    template <class Random>
    static std::string
    AlphaNumericString(const uint32_t x, const uint32_t y, const bool numeric_only, Random *const generator) {
