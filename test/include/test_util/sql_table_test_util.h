...
public:
    ...
    public:
        ...
        /**
         * @param insert_update_select_delete_ratio the ratio of inserts vs. updates vs. select vs. deletes in the
         * generated transaction (e.g. {0.1, 0.2, 0.6. 0.1} will be 10% inserts, 20% updates, 60% reads, and 10%
         * deletes)
        ...
        LargeSqlTableTestConfiguration Build() {
            return {builder_num_databases_,
                    builder_num_tables_,
                    builder_max_columns_,
                    builder_initial_table_size_,
                    builder_txn_length_,
                    builder_insert_update_select_delete_ratio_,
     ...
     */
    LargeSqlTableTestConfiguration(const uint16_t      num_databases,
                                   const uint16_t      num_tables,
                                   const uint16_t      max_columns,
                                   const uint32_t      initial_table_size,
                                   const uint32_t      txn_length,
                                   std::vector<double> insert_update_select_delete_ratio,
                                   const bool          varlen_allowed)
...
private:
    ...
    struct SqlTableMetadata {
        // Column oids for this table. We cache them to generate random updates. They never change because we don't make
        // ddl changes
...
private:
    ...
    template <class Random>
    void PopulateInitialTables(uint16_t             num_databases,
                               uint16_t             num_tables,
