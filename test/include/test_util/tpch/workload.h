...
public:
    ...
    Workload(common::ManagedPointer<DBMain> db_main,
     ...
     */
    void     Execute(int8_t                       worker_id,
                     uint64_t                     execution_us_per_worker,
                     uint64_t                     avg_interval_us,
                     uint32_t                     query_num,
                     execution::vm::ExecutionMode mode);
    uint32_t GetQueryNum() {
        return query_and_plan_.size();
    }

private:
    void
    GenerateTables(execution::exec::ExecutionContext *exec_ctx, const std::string &dir_name, enum BenchmarkType type);
