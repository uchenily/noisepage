...
#undef TO_INT
    ...
    template <typename Random>
    static void PopulateRandomRow(storage::ProjectedRow *const row,
        ...
        // For every column in the project list, populate its attribute with random bytes or set to null based on coin
        // flip
    ...
    // Populate a block with random tuple according to the given parameters. Returns a mapping of the tuples in each
    // slot.
    template <class Random>
    static std::unordered_map<storage::TupleSlot, storage::ProjectedRow *>
    PopulateBlockRandomly(storage::DataTable *table,
        ...
        NOISEPAGE_ASSERT(block->GetInsertHead() == layout.NumSlots(),
                         "The block should be considered full at this point");
        return result;
    }
    ...
    // Returns true iff the underlying varlen is bit-wise identical. Compressions schemes and other metadata are
    // ignored.
    ...
    static bool ProjectionListEqualDeep(const storage::BlockLayout &layout,
                                        const RowType1 *const       one,
        ...
        for (uint16_t projection_list_index = 0; projection_list_index < one->NumColumns(); projection_list_index++) {
            ...
            if (layout.IsVarlen(one_id)) {
                ...
                if (one_entry.Size() != other_entry.Size())
                    return false;
                if (memcmp(one_entry.Content(), other_entry.Content(), one_entry.Size()) != 0)
                    return false;
    ...
    template <class RowType1, class RowType2>
    static bool ProjectionListEqualShallow(const storage::BlockLayout &layout,
                                           const RowType1 *const       one,
                                           const RowType2 *const       other) {
    ...
    static storage::ProjectedRow *ProjectedRowDeepCopy(const storage::BlockLayout  &layout,
        ...
        for (uint16_t projection_list_index = 0; projection_list_index < original.NumColumns();
             projection_list_index++) {
            ...
            auto *copied_entry
                = reinterpret_cast<storage::VarlenEntry *>(result->AccessForceNotNull(projection_list_index));
     ...
     */
    static bool SqlTableEqualDeep(const storage::BlockLayout                                       &layout,
                                  common::ManagedPointer<storage::SqlTable>                         table_one,
    ...
    static void InsertTuple(const storage::ProjectedRow        &tuple,
                            const storage::TupleAccessStrategy &tested,
                            const storage::BlockLayout         &layout,
    ...
    static void CheckTupleEqualShallow(const storage::ProjectedRow        &expected,
                                       const storage::TupleAccessStrategy &tested,
                                       const storage::BlockLayout         &layout,
                                       const storage::TupleSlot            slot) {
        for (uint16_t col = storage::NUM_RESERVED_COLUMNS; col < layout.NumColumns(); col++) {
            ...
            const byte       *val_ptr
                = expected.AccessWithNullCheck(static_cast<uint16_t>(col - storage::NUM_RESERVED_COLUMNS));
     ...
     */
    static void ForceOid(catalog::Schema::Column *const col, catalog::col_oid_t oid) {
        col->SetOid(oid);
    }
    static void ForceOid(catalog::IndexSchema::Column *const col, catalog::indexkeycol_oid_t oid) {
        col->SetOid(oid);
    }
...
private:
    ...
    static storage::BlockLayout RandomLayout(const uint16_t max_cols, Random *const generator, bool allow_varlen) {
        ...
        if (allow_varlen)
            possible_attr_sizes.push_back(storage::VARLEN_COLUMN);
    ...
    static catalog::Schema *RandomSchema(const uint16_t max_cols, Random *const generator, bool allow_varlen) {
        ...
        std::vector<execution::sql::SqlTypeId> possible_attr_types{execution::sql::SqlTypeId::Boolean,
                                                                   execution::sql::SqlTypeId::SmallInt,
                                                                   execution::sql::SqlTypeId::Integer,
                                                                   execution::sql::SqlTypeId::Double};
        if (allow_varlen)
            possible_attr_types.push_back(execution::sql::SqlTypeId::Varchar);
        ...
        for (uint16_t i = 0; i < num_attrs; i++) {
            ...
            if (random_type == execution::sql::SqlTypeId::Varchar) {
                col = catalog::Schema::Column("col" + std::to_string(i),
                                              random_type,
                                              MAX_TEST_VARLEN_SIZE,
                                              false,
