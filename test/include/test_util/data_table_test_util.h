...
public:
    ...
    public:
        ...
        LargeDataTableTestConfiguration Build() {
            return {num_iterations_,
                    num_txns_,
                    batch_size_,
                    num_concurrent_txns_,
                    std::move(update_select_ratio_),
                    txn_length_,
                    initial_table_size_,
                    max_columns_,
                    varlen_allowed_};
    ...
    uint32_t NumTxns() const {
        return num_txns_;
    }
    uint32_t BatchSize() const {
        return batch_size_;
    }
    uint32_t NumConcurrentTxns() const {
        return num_concurrent_txns_;
    }
    const std::vector<double> &UpdateSelectRatio() const {
        return update_select_ratio_;
    }
    uint32_t TxnLength() const {
        return txn_length_;
    }
    uint32_t InitialTableSize() const {
        return initial_table_size_;
    }
    uint16_t MaxColumns() const {
        return max_columns_;
    }
    bool VarlenAllowed() const {
        return varlen_allowed_;
    }

    static LargeDataTableTestConfiguration Empty() {
        return Builder().Build();
    }

private:
    LargeDataTableTestConfiguration(const uint32_t      num_iterations,
                                    const uint32_t      num_txns,
                                    const uint32_t      batch_size,
                                    const uint32_t      num_concurrent_txns,
                                    std::vector<double> update_select_ratio,
                                    const uint32_t      txn_length,
                                    const uint32_t      initial_table_size,
...
public:
    ...
    transaction::timestamp_t BeginTimestamp() const {
        return start_time_;
    }

    transaction::timestamp_t CommitTimestamp() const {
        if (aborted_)
            return transaction::timestamp_t(static_cast<uint64_t>(-1));
        return commit_time_;
    }

    std::unordered_map<storage::TupleSlot, storage::ProjectedRow *> *Updates() {
        return &updates_;
    }
...
public:
     ...
     */
    LargeDataTableTestObject(const LargeDataTableTestConfiguration &config,
     ...
     * the table, either initial or the newest version last time this method is called, and the list of updates
     * committed.
