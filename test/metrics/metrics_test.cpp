...
public:
    ...
    const catalog::Schema table_schema_{{{"attribute",
...
TEST_F(MetricsTests, LoggingCSVTest) {
    for (const auto &file : metrics::LoggingMetricRawData::FILES)
        unlink(std::string(file).c_str());
    ...
    settings_manager_->SetBool(settings::Param::logging_metrics_enable,
                               true,
                               common::ManagedPointer(action_context),
                               setter_callback);
...
TEST_F(MetricsTests, TransactionCSVTest) {
    for (const auto &file : metrics::TransactionMetricRawData::FILES)
        unlink(std::string(file).c_str());
    const settings::setter_callback_fn setter_callback = MetricsTests::EmptySetterCallback;
    auto                               action_context = std::make_unique<common::ActionContext>(common::action_id_t(1));
    settings_manager_->SetBool(settings::Param::transaction_metrics_enable,
                               true,
                               common::ManagedPointer(action_context),
                               setter_callback);
...
TEST_F(MetricsTests, PipelineCSVTest) {
    // Unlink all files
    for (const auto &file : metrics::QueryTraceMetricRawData::FILES)
        unlink(std::string(file).c_str());
    ...
    auto verify_scenario = [insert_txn, this](bool enable_metric,
        ...
        if (enable_metric) {
            ...
            auto action_context = std::make_unique<common::ActionContext>(common::action_id_t(1));
            settings_manager_->SetBool(settings::Param::pipeline_metrics_enable,
                                       true,
                                       common::ManagedPointer(action_context),
                                       setter_callback);
        }
        ...
        if (update_interval) {
            ...
            settings_manager_->SetInt(settings::Param::pipeline_metrics_sample_rate,
                                      interval,
                                      common::ManagedPointer(action_context),
                                      setter_callback);
        }
...
TEST_F(MetricsTests, QueryCSVTest) {
    ...
    const settings::setter_callback_fn setter_callback = MetricsTests::EmptySetterCallback;
    auto                               action_context = std::make_unique<common::ActionContext>(common::action_id_t(1));
    settings_manager_->SetBool(settings::Param::query_trace_metrics_enable,
                               true,
                               common::ManagedPointer(action_context),
                               setter_callback);
...
TEST_F(MetricsTests, ToggleSettings) {
    ...
    settings_manager_->SetBool(settings::Param::gc_metrics_enable,
                               true,
                               common::ManagedPointer(action_context),
                               callback);
    EXPECT_EQ(action_context->GetState(), common::ActionState::SUCCESS);
    ...
    settings_manager_->SetBool(settings::Param::execution_metrics_enable,
                               true,
                               common::ManagedPointer(action_context),
                               callback);
    EXPECT_EQ(action_context->GetState(), common::ActionState::SUCCESS);
