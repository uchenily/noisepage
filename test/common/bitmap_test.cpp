...
TEST(BitmapTests, ByteMultipleCorrectnessTest) {
    ...
    for (uint32_t iter = 0; iter < num_bitmap_sizes; ++iter) {
        ...
        for (uint32_t i = 0; i < num_iterations; ++i) {
            ...
            ContainerTestUtil::CheckReferenceBitmap<common::RawBitmap>(*aligned_bitmap,
                                                                       stl_bitmap,
                                                                       num_elements_aligned);
        }
...
TEST(BitmapTests, WordUnalignedCorrectnessTest) {
    ...
    for (uint32_t iter = 0; iter < num_bitmap_sizes; ++iter) {
        ...
        // provision enough space for the bitmap elements, plus padding because we're going to make it unaligned to
        // wordsize
