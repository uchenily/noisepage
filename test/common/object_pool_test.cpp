    ...
    auto                                   workload = [&](uint32_t tid) {
        ...
        auto                              allocate = [&] {
            try {
                ...
                // Since threads are alloc and free in random order, object pool could possibly have no object to hand
                // out. When this occurs, we just do nothing. The purpose of this test is to test object pool
                // concurrently and check correctness. We just skip and do nothing. The object pool will eventually have
                // objects when other threads release objects.
            }
        };
        auto free = [&] {
            if (!ptrs.empty()) {
        ...
        auto set_reuse_limit = [&] {
            tested.SetReuseLimit(size_dist(generator));
        };

        auto set_size_limit = [&] {
            tested.SetSizeLimit(size_dist(generator));
        };
