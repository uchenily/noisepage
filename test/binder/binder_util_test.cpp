...
void TestCheckAndTryPromoteType(const execution::sql::SqlTypeId sql_type,
                                const std::string_view          valid_val,
                                const cpp_type                  peek_val,
                                const std::string_view          invalid_val) {
    auto string_val = execution::sql::ValueUtil::CreateStringVal(valid_val);
    auto cve = std::make_unique<parser::ConstantValueExpression>(execution::sql::SqlTypeId::Varchar,
                                                                 string_val.first,
    ...
    cve = std::make_unique<parser::ConstantValueExpression>(execution::sql::SqlTypeId::Varchar,
                                                            string_val.first,
                                                            std::move(string_val.second));
...
TEST_F(BinderUtilTest, VarcharToBigInt) {
    TestCheckAndTryPromoteType<int64_t>(execution::sql::SqlTypeId::BigInt,
                                        "157211544500",
                                        157211544500,
