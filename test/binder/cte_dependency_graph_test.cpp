...
static Edge MakeEdge(const PseudoVertex &src, const PseudoVertex &dst) {
    return Edge{
        Vertex{std::get<0>(src), std::get<1>(src), std::get<2>(src), std::get<3>(src)},
...
TEST_F(BinderCteDepdendencyGraphTest, ReferenceResolutionUnit5) {
    const std::string sql = "WITH RECURSIVE x(i) AS (WITH y(m) AS (SELECT 1), a(n) AS (SELECT * FROM y) SELECT * FROM "
                            "a), y(j) AS (SELECT 2) "
                            "SELECT * FROM x;";
    auto [_, select] = BinderTestUtil::ParseToSelectStatement(sql);
...
TEST_F(BinderCteDepdendencyGraphTest, ReferenceResolutionUnit6) {
    const std::string sql = "WITH RECURSIVE x(i) AS (WITH a(m) AS (SELECT * FROM y), y(n) AS (SELECT 1) SELECT * FROM "
                            "a), y(j) AS (SELECT 2) "
                            "SELECT * FROM x;";
    auto [_, select] = BinderTestUtil::ParseToSelectStatement(sql);
...
TEST_F(BinderCteDepdendencyGraphTest, CheckGraphConstructionSuccess1) {
    // Ambiguous reference
    const std::string sql
        = "WITH x(i) AS (WITH y(j) AS (SELECT 1), y(k) AS (SELECT 2) SELECT * FROM y) SELECT * FROM x;";
    auto [_, select] = BinderTestUtil::ParseToSelectStatement(sql);
    EXPECT_THROW(DependencyGraph::BuildUnchecked(select), BinderException);
}
...
TEST_F(BinderCteDepdendencyGraphTest, CheckMutualRecursion4) {
    const std::string sql = "WITH RECURSIVE x(i) AS (SELECT * FROM y), y(j) AS (SELECT * FROM z), z(k) AS (SELECT * "
