...
public:
    ...
    void RunTPCC(const bool logging_enabled, const bool metrics_enabled, const storage::index::IndexType type) {
        ...
        // Precompute all of the input arguments for every txn to be run. We want to avoid the overhead at benchmark
        // time
...
TEST_F(TPCCTests, WithoutLoggingHashIndexes) {
    RunTPCC(false, false, storage::index::IndexType::HASHMAP);
}

// NOLINTNEXTLINE
TEST_F(TPCCTests, WithoutLoggingBPlusTreeIndexes) {
    RunTPCC(false, false, storage::index::IndexType::BPLUSTREE);
}

// NOLINTNEXTLINE
TEST_F(TPCCTests, WithLogging) {
    RunTPCC(true, false, storage::index::IndexType::HASHMAP);
}

// NOLINTNEXTLINE
TEST_F(TPCCTests, WithLoggingAndMetrics) {
    RunTPCC(true, true, storage::index::IndexType::HASHMAP);
}
