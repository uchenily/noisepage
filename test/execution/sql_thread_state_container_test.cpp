...
protected:
    static void ForceCreationOfThreadStates(ThreadStateContainer *container, const uint32_t num_thread_states) {
        LaunchParallel(num_thread_states, [&](auto tid) {
            container->AccessCurrentThreadState();
        });
    }
};
...
TEST_F(ThreadStateContainerTest, SimpleContainerTest) {
    ...
    uint32_t total = 0;
    container.ForEach<uint32_t>([&total](const uint32_t *const count) {
        total += *count;
    });
    ...
    // Manually collect and add
    {
        ...
        total = std::accumulate(counts.begin(), counts.end(), 0, [](auto partial, auto *c) {
            return partial + *c;
        });
        EXPECT_EQ(input.size(), total);
    }
