...
TEST_F(AggregatorsTest, TopKBool) {
    ...
    // Proper top_k calculation using top_k
    {
        ...
        auto deserialized_top_k
            = optimizer::TopKElements<bool>::Deserialize(reinterpret_cast<const std::byte *>(json_res.GetContent()),
                                                         json_res.GetLength());
...
TEST_F(AggregatorsTest, TopKInt) {
    ...
    // Proper top_k calculation using top_k
    {
        ...
        auto deserialized_top_k = optimizer::TopKElements<decltype(Integer::val_)>::Deserialize(
            reinterpret_cast<const std::byte *>(json_res.GetContent()),
            json_res.GetLength());
...
TEST_F(AggregatorsTest, HistogramBool) {
    ...
    // Proper histogram calculation using histogram
    {
        ...
        auto deserialized_histogram
            = optimizer::Histogram<bool>::Deserialize(reinterpret_cast<const std::byte *>(json_res.GetContent()),
                                                      json_res.GetLength());
