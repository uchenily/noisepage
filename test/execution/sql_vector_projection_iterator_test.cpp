...
protected:
    ...
    struct ColData {
        ...
        ColData(std::unique_ptr<byte[]>     data,
...
protected:
    uint32_t NumTuples() const {
        return num_tuples_;
    }

    VectorProjection *GetVectorProjection() {
        return vector_projection_.get();
    }

    const ColData &ColumnData(uint32_t col_idx) const {
