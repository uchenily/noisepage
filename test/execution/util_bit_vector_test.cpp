...
BitVector<> Make(std::initializer_list<uint32_t> vals) {
    ...
    std::for_each(vals.begin(), vals.end(), [&, i = 0](auto &bval) mutable {
        bv.Set(i++, bval);
    });
    return bv;
}
...
TEST(BitVectorTest, SetRange) {
    ...
    const auto in_range = [](uint32_t v, uint32_t lo, uint32_t hi) {
        return lo <= v && v < hi;
    };
    ...
    bv.Reset();
    bv.SetRange(27, 57);
    EXPECT_TRUE(Verify(bv, [&](auto idx) {
        return in_range(idx, 27, 57);
    }));
...
TEST(BitVectorTest, SetFromBytes) {
    ...
    // Complex
    {
        ...
        EXPECT_TRUE(Verify(bv, [&](uint32_t idx) {
...
TEST(BitVectorTest, Iterate) {
    // Simple
    {
        ...
        bv.Set(99);
        bv.IterateSetBits([](auto idx) {
            EXPECT_EQ(99u, idx);
        });

        bv.Set(64);
        bv.IterateSetBits([](auto idx) {
            EXPECT_TRUE(idx == 64 || idx == 99);
        });
    }
