...
struct InputTuple {
    ...
    InputTuple(uint64_t key, uint64_t col_a)
        : key_(key)
        , col_a_(col_a) {}

    hash_t Hash() const noexcept {
        return common::HashUtil::Hash(key_);
    }
};
...
public:
    AggregationHashTableVectorIteratorTest() {
        std::vector<catalog::Schema::Column> cols = {
            {   "key",
             ...
             true, parser::ConstantValueExpression(execution::sql::SqlTypeId::BigInt)},
            {"count1",
             execution::sql::SqlTypeId::BigInt,
             true, parser::ConstantValueExpression(execution::sql::SqlTypeId::BigInt)},
            {"count2",
    ...
    static void
    PopulateAggHT(AggregationHashTable *aht, const uint32_t num_aggs, const uint32_t num_rows, uint32_t cola = 1) {
...
TEST_F(AggregationHashTableVectorIteratorTest, DISABLED_Perf) {
    ...
    for (uint32_t size : {10, 100, 1000, 10000, 100000, 1000000, 10000000}) {
        ...
        [[maybe_unused]] auto vaat_ms = Bench(4, [&]() {
            vaat_ret = 0;
            ...
            for (; iter.HasNext(); iter.Next(Transpose)) {
                ...
                tids.AddAll();
                VectorFilterExecutor::SelectLessThanVal(exec_ctx->GetExecutionSettings(),
                                                        vector_projection,
                                                        0,
                                                        GenericValue::CreateBigInt(filter_val),
                                                        &tids);
                vector_projection->SetFilteredSelections(tids);
            }
        });

        [[maybe_unused]] auto taat_ms = Bench(4, [&]() {
            taat_ret = 0;
