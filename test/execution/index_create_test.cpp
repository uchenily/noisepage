...
protected:
    void CreateIndex(catalog::table_oid_t                  table_oid,
        ...
        auto query = execution::compiler::CompilationContext::Compile(*plan_node,
                                                                      exec_ctx_->GetExecutionSettings(),
    ...
    void VerifyIndexResult(catalog::table_oid_t  table_oid,
                           const std::string    &index_name,
                           std::vector<uint32_t> col_oids,
                           uint32_t              key_num) {
        auto                sql_table = exec_ctx_->GetAccessor()->GetTable(table_oid);
        auto                index_oid = exec_ctx_->GetAccessor()->GetIndexOid(NSOid(), index_name);
        TableVectorIterator table_iter(exec_ctx_.get(),
                                       uint32_t(table_oid),
                                       col_oids.data(),
