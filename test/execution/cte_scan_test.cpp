...
TEST_F(CTEScanTest, CTEInitTest) {
    ...
    uint32_t cte_table_col_ids[4] = {exec_ctx_->GetAccessor()->GetNewTempOid(),
...
TEST_F(CTEScanTest, CTEInsertTest) {
    ...
    TableVectorIterator table_iter(exec_ctx_.get(),
                                   static_cast<catalog::table_oid_t>(999).UnderlyingValue(),
                                   cte_table_col_ids,
                                   static_cast<uint32_t>(col_oids.size()));
...
TEST_F(CTEScanTest, CTEInsertScanTest) {
    ...
    // Try to fetch the inserted values.
    // TODO(Gautam): Create our own TableVectorIterator that does not check in the catalog
    auto table_iter = new TableVectorIterator(exec_ctx_.get(),
                                              (cte_scan->GetTableOid()).UnderlyingValue(),
                                              cte_table_col_ids,
                                              static_cast<uint32_t>(col_oids.size()));
    table_iter->InitTempTable(common::ManagedPointer(cte_table), cte_scan->GetSchema());
    VectorProjectionIterator *vpi = table_iter->GetVectorProjectionIterator();
