...
TEST_F(TableVectorIteratorTest, EmptyIteratorTest) {
    ...
    TableVectorIterator     iter(exec_ctx_.get(),
...
TEST_F(TableVectorIteratorTest, ParallelScanTest) {
    ...
    auto                    table_oid = exec_ctx_->GetAccessor()->GetTableOid(NSOid(), "test_1");
    std::array<uint32_t, 4> col_oids{1, 2, 3, 4};
    TableVectorIterator::ParallelScan(table_oid.UnderlyingValue(),
                                      col_oids.data(),
                                      col_oids.size(),
                                      nullptr,
                                      exec_ctx_.get(),
    ...
    exec_ctx_->GetThreadStateContainer()->ForEach<Counter>([&](Counter *counter) {
        aggregate_tuple_count += counter->c_;
    });
