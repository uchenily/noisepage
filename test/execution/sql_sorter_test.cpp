...
void TestSortRandomTupleSize(exec::ExecutionContext *exec_ctx,
                             const uint32_t          num_iters,
                             const uint32_t          max_elems,
                             Random                 *generator) {
...
template <typename IntType, typename Random>
void TestTopKRandomTupleSize(exec::ExecutionContext *exec_ctx,
                             const uint32_t          num_iters,
                             const uint32_t          max_elems,
                             Random                 *generator) {
    std::uniform_int_distribution<IntType>  rng(std::numeric_limits<IntType>::min(),
...
struct TestTuple {
    ...
    int32_t Compare(const TestTuple<N> &other) const {
...
void TestParallelSort(exec::ExecutionContext *exec_ctx, const std::vector<uint32_t> &sorter_sizes) {
    ...
    uint32_t expected_total_size = std::accumulate(sorter_sizes.begin(), sorter_sizes.end(), 0u, [](auto p, auto s) {
        return p + s;
    });
