...
TEST_F(CompilerTest, SimpleSeqScanTest) {
    ...
    MultiChecker multi_checker{
        std::vector<OutputChecker *>{&col1_checker, &col2_checker}
    };

    // Create the execution context
    OutputStore         store{&multi_checker, seq_scan->GetOutputSchema().Get()};
    exec::OutputPrinter printer(seq_scan->GetOutputSchema().Get());
    MultiOutputCallback callback{
        std::vector<exec::OutputCallback>{store, printer}
    };
    ...
    // Run & Check
    auto executable = execution::compiler::CompilationContext::Compile(*seq_scan,
                                                                       exec_ctx->GetExecutionSettings(),
...
TEST_F(CompilerTest, SimpleSeqScanWithProjectionTest) {
    ...
    MultiChecker multi_checker{
        std::vector<OutputChecker *>{&num_checker, &col1_checker}
    };
    ...
    // Run & Check
    auto executable = execution::compiler::CompilationContext::Compile(*proj,
                                                                       exec_ctx->GetExecutionSettings(),
                                                                       exec_ctx->GetAccessor());
    executable->Run(common::ManagedPointer(exec_ctx), MODE);
    multi_checker.CheckCorrectness();
...
TEST_F(CompilerTest, SimpleIndexScanTest) {
    ...
    MultiChecker               multi_checker{
        std::vector<OutputChecker *>{&col1_checker, &num_checker}
    };
    // Create the execution context
    ...
    // Run & Check
    auto executable = execution::compiler::CompilationContext::Compile(*index_scan,
                                                                       exec_ctx->GetExecutionSettings(),
                                                                       exec_ctx->GetAccessor());
    executable->Run(common::ManagedPointer(exec_ctx), MODE);
    multi_checker.CheckCorrectness();
    ...
    auto feature_vec = pipeline->GetPipelineFeatures(execution::pipeline_id_t(1));
    auto exp_vec
        = std::vector<selfdriving::ExecutionOperatingUnitType>{selfdriving::ExecutionOperatingUnitType::IDX_SCAN,
                                                               selfdriving::ExecutionOperatingUnitType::OUTPUT};
    EXPECT_TRUE(CheckFeatureVectorEquality(feature_vec, exp_vec));
}
...
TEST_F(CompilerTest, SimpleAggregateTest) {
    ...
    MultiChecker        multi_checker{
        std::vector<OutputChecker *>{&num_checker, &sum_checker}
    };
    ...
    // Run & Check
    auto executable = execution::compiler::CompilationContext::Compile(*agg,
                                                                       exec_ctx->GetExecutionSettings(),
                                                                       exec_ctx->GetAccessor());
    executable->Run(common::ManagedPointer(exec_ctx), MODE);
    multi_checker.CheckCorrectness();
...
TEST_F(CompilerTest, CountStarTest) {
    ...
    MultiChecker               multi_checker{
        std::vector<OutputChecker *>{&num_checker, &count_checker}
    };
...
TEST_F(CompilerTest, StaticAggregateTest) {
    ...
    MultiChecker               multi_checker{
        std::vector<OutputChecker *>{&num_checker, &count_checker, &sum_checker}
    };
...
TEST_F(CompilerTest, StaticDistinctAggregateTest) {
    ...
    MultiChecker               multi_checker{
        std::vector<OutputChecker *>{&num_checker,
                                     &distinct_count_checker,
                                     &distinct_sum_checker,
                                     &count_star_checker}
    };
...
TEST_F(CompilerTest, SimpleHashJoinTest) {
    ...
    // Run & Check
    auto executable = execution::compiler::CompilationContext::Compile(*hash_join,
                                                                       exec_ctx->GetExecutionSettings(),
                                                                       exec_ctx->GetAccessor());
    executable->Run(common::ManagedPointer(exec_ctx), MODE);
    checker.CheckCorrectness();
...
TEST_F(CompilerTest, MultiWayHashJoinTest) {
    ...
    // Run & Check
    auto executable = execution::compiler::CompilationContext::Compile(*hash_join2,
                                                                       exec_ctx->GetExecutionSettings(),
                                                                       exec_ctx->GetAccessor());
    executable->Run(common::ManagedPointer(exec_ctx), MODE);
    checker.CheckCorrectness();
...
TEST_F(CompilerTest, SimpleSortTest) {
    ...
    RowChecker row_checker
        = [&num_output_rows, &curr_col1, &curr_col2, num_expected_rows](const std::vector<sql::Val *> &vals) {
              // Read cols
              auto col1 = static_cast<sql::Integer *>(vals[0]);
              auto col2 = static_cast<sql::Integer *>(vals[1]);
              ASSERT_FALSE(col1->is_null_ || col2->is_null_);
    ...
    // Run & Check
    auto executable = execution::compiler::CompilationContext::Compile(*order_by,
                                                                       exec_ctx->GetExecutionSettings(),
                                                                       exec_ctx->GetAccessor());
    executable->Run(common::ManagedPointer(exec_ctx), MODE);
    checker.CheckCorrectness();
...
TEST_F(CompilerTest, SortWithLimitAndOffsetTest) {
    ...
    int64_t    curr_col1{std::numeric_limits<int64_t>::max()};
    RowChecker row_checker
        = [&num_output_rows, &curr_col1, num_expected_rows, upper](const std::vector<sql::Val *> &vals) {
              // Read cols
              auto col1 = static_cast<sql::Integer *>(vals[0]);
...
TEST_F(CompilerTest, LimitAndOffsetTest) {
    ...
    OutputSchemaHelper                         limit_out{0, &expr_maker};
    {
        ...
        limit = builder.SetOutputSchema(std::move(schema))
                    .AddChild(std::move(seq_scan))
                    .SetLimit(100)
                    .SetOffset(100)
                    .Build();
    ...
    // Run & Check
    auto executable = execution::compiler::CompilationContext::Compile(*limit,
                                                                       exec_ctx->GetExecutionSettings(),
                                                                       exec_ctx->GetAccessor());
    executable->Run(common::ManagedPointer(exec_ctx), MODE);
    checker.CheckCorrectness();
...
TEST_F(CompilerTest, SimpleNestedLoopJoinTest) {
    ...
    // Run & Check
    auto executable = execution::compiler::CompilationContext::Compile(*nl_join,
                                                                       exec_ctx->GetExecutionSettings(),
                                                                       exec_ctx->GetAccessor());
    executable->Run(common::ManagedPointer(exec_ctx), MODE);
    checker.CheckCorrectness();
...
TEST_F(CompilerTest, SimpleIndexNestedLoopJoinTest) {
    ...
    // Run & Check
    auto executable = execution::compiler::CompilationContext::Compile(*index_join,
                                                                       exec_ctx->GetExecutionSettings(),
                                                                       exec_ctx->GetAccessor());
    executable->Run(common::ManagedPointer(exec_ctx), MODE);
    checker.CheckCorrectness();
...
TEST_F(CompilerTest, SimpleDeleteTest) {
    ...
    // Execute delete
    {
        ...
        auto                 executable = execution::compiler::CompilationContext::Compile(*delete_node,
                                                                           exec_ctx->GetExecutionSettings(),
                                                                           exec_ctx->GetAccessor());
        executable->Run(common::ManagedPointer(exec_ctx), MODE);
...
TEST_F(CompilerTest, SimpleUpdateTest) {
    ...
    // Execute update
    {
        ...
        auto                 executable = execution::compiler::CompilationContext::Compile(*update_node,
                                                                           exec_ctx->GetExecutionSettings(),
                                                                           exec_ctx->GetAccessor());
        executable->Run(common::ManagedPointer(exec_ctx), MODE);
...
TEST_F(CompilerTest, SimpleInsertTest) {
    ...
    // Execute insert
    {
        ...
        auto                 executable = execution::compiler::CompilationContext::Compile(*insert,
                                                                           exec_ctx->GetExecutionSettings(),
                                                                           exec_ctx->GetAccessor());
        executable->Run(common::ManagedPointer(exec_ctx), MODE);
    ...
    OutputSchemaHelper                         index_scan_out{0, &expr_maker};
    {
        ...
        index_scan
            = builder.SetTableOid(table_oid1)
                  .SetColumnOids({col1_oid, col2_oid, col3_oid, col4_oid, col5_oid, col6_oid, col7_oid, col8_oid})
                  .SetIndexOid(index_oid1)
                  .AddLoIndexColumn(catalog::indexkeycol_oid_t(1),
                                    expr_maker.PVE(execution::sql::SqlTypeId::Varchar, 0))
                  .AddHiIndexColumn(catalog::indexkeycol_oid_t(1),
                                    expr_maker.PVE(execution::sql::SqlTypeId::Varchar, 1))
                  .SetScanPredicate(nullptr)
                  .SetOutputSchema(std::move(schema))
                  .SetScanType(planner::IndexScanType::AscendingClosed)
                  .SetScanLimit(0)
