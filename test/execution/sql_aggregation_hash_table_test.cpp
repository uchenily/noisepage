...
struct InputTuple {
    ...
    explicit InputTuple(uint64_t key, uint64_t col_a)
        : key_(key)
        , col_a_(col_a) {}
...
public:
    ...
    void SetUp() override {
        ...
        agg_table_ = std::make_unique<AggregationHashTable>(exec_ctx_->GetExecutionSettings(),
                                                            exec_ctx_.get(),
                                                            sizeof(AggTuple));
    }

    AggregationHashTable *AggTable() {
        return agg_table_.get();
    }
...
TEST_F(AggregationHashTableTest, OverflowPartitonIteratorTest) {
    ...
    struct TestEntry : public HashTableEntry {
        ...
        TestEntry()
            : HashTableEntry()
            , data_{} {}
        TestEntry(uint32_t key, uint32_t val)
            : HashTableEntry()
            , data_{key, val} {}
    };
