...
struct BuildRow {
    ...
    // Constructor.
    explicit BuildRow(uint64_t key)
        : BuildRow(key, 0, 0, 0) {}
    // Constructor.
    ...
    // Hash.
    hash_t Hash() const {
        return common::HashUtil::HashCrc(key_);
    }
};
...
TEST_F(JoinHashTableVectorProbeTest, SimpleJoinProbe) {
    ...
    for (uint32_t i = 0; i < num_44_dups - 1; i++)
        rows.emplace_back(44);
    ...
    // Test: INNER-join should find all matches.
    {
        ...
        for (probe.Init(&input); probe.Next(&input); num_iters++) {
            ...
            match_filter->ForEach([&](uint64_t i) {
                ...
                auto build_key
                    = reinterpret_cast<HashTableEntry **>(matches->GetData())[i]->PayloadAs<BuildRow>()->key_;
