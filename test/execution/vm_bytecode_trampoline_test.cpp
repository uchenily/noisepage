...
TEST_F(BytecodeTrampolineTest, CodeGenComparisonFunctionSorterTest) {
...

    {
        struct S {
            int32_t a_, b_, c_, d_;
            S(int32_t a, int32_t b, int32_t c, int32_t d)
                : a_(a)
                , b_(b)
                , c_(c)
                , d_(d) {}
    ...
    fun compare(a: *S, b: *S) -> bool { return a.c < b.c })";
        ...
        // Try to sort using the generated comparison function
        ips4o::sort(elems.begin(), elems.end(), [compare](const auto &a, const auto &b) {
            return compare(&a, &b);
        });

        // Verify
        EXPECT_TRUE(std::is_sorted(elems.begin(), elems.end(), [](const auto &a, const auto &b) {
            return a.c_ < b.c_;
        }));
    }
}
