...
TEST_F(BloomFilterTest, Comprehensive) {
    ...
    for (auto prob_success : {0.00, 0.25, 0.50, 0.75, 1.00}) {
        ...
        [[maybe_unused]] double fpr = (actual_found - expected_found) / static_cast<double>(lookups.size());
        [[maybe_unused]] double probes_per_sec
            = static_cast<double>(lookups.size()) / timer.GetElapsed() * 1000.0 / 1000000.0;
        EXECUTION_LOG_TRACE("p: {:.2f}, {} M probes/sec, FPR: {:2.4f}, (expected: {}, actual: {})",
                            prob_success,
                            probes_per_sec,
                            fpr,
                            expected_found,
