...
TEST_F(SemaDeclTest, DuplicateStructFields) {
  ...
  // Test case 1 should fail.
        { true,
         "Struct has duplicate 'a' fields", GenFile({
 ...
 // Test case 2 should fail.
        { true,
         "Struct has duplicate 'a' with different types", GenFile({
 // The single struct declaration in the file.
 DeclStruct(Ident("s"),
 {
 GenFieldDecl(Ident("a"), PrimIntTypeRepr()),           // a: int
 ...
 // Test case 3 should be fine.
        {false,
         "Struct has only unique fields and should pass type-checking", GenFile({
 // The single struct declaration in the file.
 DeclStruct(Ident("s"),
 {
