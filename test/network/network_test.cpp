...
protected:
    ...
    void SetUp() override {
        ...
        catalog_ = new catalog::Catalog(common::ManagedPointer(txn_manager_),
...
TEST_F(NetworkTests, SimpleQueryTest) {
    try {
        pqxx::connection c(fmt::format("host=127.0.0.1 port={0} user={1} sslmode=disable application_name=psql",
                                       port_,
