...
TEST(PlanNodeTest, HashJoinPlanTest) {
    ...
    auto cmp_expression = std::make_unique<parser::ComparisonExpression>(parser::ExpressionType::COMPARE_EQUAL,
...
TEST(PlanNodeTest, AggregatePlanTest) {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        parser::AggregateExpression *other_aggr
            = dynamic_cast<parser::AggregateExpression *>(aggr_term->Copy().release());
        auto other_schema = PlanNodeTest::BuildOneColumnSchema("col1", execution::sql::SqlTypeId::Integer);
        ...
        case 2: {
            ...
            delete other_aggr;
            other_aggr
                = new parser::AggregateExpression(parser::ExpressionType::AGGREGATE_MAX, std::move(children), false);
            break;
        }
        case 3:
            other_schema = PlanNodeTest::BuildOneColumnSchema("XXXX", execution::sql::SqlTypeId::Integer);
