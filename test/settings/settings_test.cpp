...
TEST_F(SettingsTests, BasicTest) {
    ...
    // Integer
    auto record_buffer_segment_size
        = static_cast<uint16_t>(settings_manager_->GetInt(Param::record_buffer_segment_size));
    EXPECT_GE(record_buffer_segment_size, 0);
    settings_manager_->SetInt(Param::record_buffer_segment_size,
                              9999,
...
TEST_F(SettingsTests, ParamCallbackTest) {
    ...
    settings_manager_->SetInt(Param::record_buffer_segment_size,
                              static_cast<int32_t>(new_buffer_pool_size),
                              common::ManagedPointer(action_context),
                              setter_callback);
...
TEST_F(SettingsTests, LogManagerSettingsTest) {
    ...
    auto               action_context = std::make_unique<common::ActionContext>(action_id);
    setter_callback_fn setter_callback = SettingsTests::EmptySetterCallback;
    settings_manager_->SetInt64(Param::wal_num_buffers,
                                new_num_buffers,
                                common::ManagedPointer(action_context),
                                setter_callback);
    ...
    settings_manager_->SetInt(Param::wal_serialization_interval,
                              new_serializatio_interval,
                              common::ManagedPointer(action_context2),
                              setter_callback2);
