...
TEST_F(QueryTraceLogging, BasicLogging) {
    ...
    for (size_t i = 0; i < qids.size(); i++) {
        common::thread_context.metrics_store_->RecordQueryText(
            catalog::db_oid_t{db_oids[i]},
            execution::query_id_t{qids[i]},
            texts[i],
    ...
    auto select_count = [util](const std::string &query, size_t target) {
        ...
        auto     to_row_fn = [&row_count](const std::vector<execution::sql::Val *> &values) {
            row_count++;
        };

        execution::exec::ExecutionSettings settings{};
        bool                               result = util->ExecuteDML(query,
                                       nullptr,
                                       nullptr,
                                       to_row_fn,
                                       nullptr,
    ...
    auto check_freqs = [task_manager, &qids, &totals, &timestamps](size_t num_interval) {
        ...
        task_manager->AddTask(std::make_unique<task::TaskDML>(catalog::INVALID_DATABASE_OID,
                                                              "SELECT * FROM noisepage_forecast_frequencies",
                                                              std::make_unique<optimizer::TrivialCostModel>(),
                                                              std::move(params),
...

    {
        ...
        auto func = [&val, qids, texts, db_oids, &parameters](const std::vector<execution::sql::Val *> &values) {
            ...
            EXPECT_TRUE(reinterpret_cast<execution::sql::Integer *>(values[0])->val_ == db_oids[qid]
                        && "Invalid db_oid");
        ...
        execution::exec::ExecutionSettings settings{};
        bool                               result = util->ExecuteDML("SELECT * FROM noisepage_forecast_texts",
                                       nullptr,
                                       nullptr,
                                       func,
                                       nullptr,
...

    {
        ...
        auto                        func = [&row_count, qids, &seen](const std::vector<execution::sql::Val *> &values) {
            EXPECT_TRUE(reinterpret_cast<execution::sql::Integer *>(values[0])->val_ == 30
                        && "Flush timestamp is invalid");
        ...
        execution::exec::ExecutionSettings settings{};
        bool                               result = util->ExecuteDML("SELECT * FROM noisepage_forecast_parameters",
                                       nullptr,
                                       nullptr,
                                       func,
                                       nullptr,
