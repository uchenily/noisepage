...
public:
    ...
    template <class Random>
    storage::TupleSlot InsertRandomTuple(const transaction::timestamp_t    timestamp,
                                         Random                           *generator,
    ...
    template <class Random>
    storage::TupleSlot InsertRandomTuple(transaction::TransactionContext  *txn,
                                         Random                           *generator,
                                         storage::RecordBufferSegmentPool *buffer_pool) {
        // generate a random redo ProjectedRow to Insert
        auto *redo_buffer = common::AllocationUtil::AllocateAligned(redo_initializer_.ProjectedRowSize());
    ...
    template <class Random>
    bool RandomlyUpdateTuple(const transaction::timestamp_t    timestamp,
        ...
        std::vector<storage::col_id_t> update_col_ids
            = StorageTestUtil::ProjectionListRandomColumns(layout_, generator);
    ...
    const std::vector<storage::TupleSlot> &InsertedTuples() const {
    ...
    for (uint32_t iteration = 0; iteration < num_iterations; ++iteration) {
        ...
        for (const auto &inserted_tuple : tested.InsertedTuples()) {
            ...
            const storage::ProjectedRow *ref
                = tested.GetReferenceVersionedTuple(inserted_tuple, transaction::timestamp_t(1));
...
TEST_F(DataTableTests, SimpleVersionChain) {
    ...
    for (uint32_t iteration = 0; iteration < num_iterations; ++iteration) {
        ...
        for (uint32_t i = 0; i < num_versions; i++) {
            ...
            EXPECT_TRUE(
                StorageTestUtil::ProjectionListEqualShallow(tested.Layout(), reference_version, stored_version));
        }
    }
}
...
TEST_F(DataTableTests, WriteWriteConflictUpdateFails) {
    ...
    for (uint32_t iteration = 0; iteration < num_iterations; ++iteration) {
        ...
        EXPECT_TRUE(
            tested.RandomlyUpdateTuple(transaction::timestamp_t(UINT64_MAX), tuple, &generator_, &buffer_pool_));
        ...
        std::vector<storage::col_id_t> all_col_ids = StorageTestUtil::ProjectionListAllColumns(tested.Layout());
        storage::ProjectedRow         *stored
            = tested.SelectIntoBuffer(tuple, transaction::timestamp_t(UINT64_MAX), &buffer_pool_);
        const storage::ProjectedRow *ref
            = tested.GetReferenceVersionedTuple(tuple, transaction::timestamp_t(UINT64_MAX));
...
TEST_F(DataTableTests, InsertNoWrap) {
    ...
    for (uint32_t iteration = 0; iteration < num_iterations; ++iteration) {
        ...
        transaction::timestamp_t timestamp(0);
        auto                    *txn = new transaction::TransactionContext(timestamp,
                                                        timestamp,
...
TEST_F(DataTableTests, SlotIteraterSingleThreadedTest) {
    ...
    for (uint32_t iteration = 0; iteration < num_iterations; ++iteration) {
        ...
        for (uint64_t i = 0; i < max_tuples_inserted; i++) {
            ...
            auto it = tested.GetTable().begin();
            for (uint64_t num_slots = 0; it != tested.GetTable().end() && num_slots < size; num_slots++)
                it++;
            EXPECT_EQ(slot, *it);
        }
