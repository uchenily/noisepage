...
void BasicNodeInitPushBackTest() {
    ...
    auto node = BPlusTree<int, TupleSlot>::ElasticNode<BPlusTree<int, TupleSlot>::KeyNodePointerPair>::Get(
        size,
        BPlusTree<int, TupleSlot>::NodeType::LeafType,
        depth,
        item_count,
        p1,
        p2);
...
void InsertElementInNodeRandomTest() {
    ...
    for (int key = 0; key < item_count; key++) {
        ...
        while (positions.find(k) != positions.end())
            k = (k + 1) % (node->GetSize() + 1);
...
void FindLocationTest() {
    ...
    while (node->GetSize() < node->GetItemCount()) {
        ...
        EXPECT_EQ(node->InsertElementIfPossible(
                      p,
                      static_cast<BPlusTree<int, TupleSlot>::InnerNode *>(node)->FindLocation(k, bplustree)),
                  true);
    }
...
void StructuralIntegrityTestWithRandomInsertAndDelete() {
    ...
    for (unsigned i = 0; i < 10; i++) {
        ...
        EXPECT_EQ(bplustree->StructuralIntegrityVerification(*newkeys.begin(),
                                                             *newkeys.rbegin(),
                                                             &newkeys,
                                                             bplustree->GetRoot()),
                  true);
...
void LargeStructuralIntegrityVerificationTest() {
    ...
    for (unsigned i = 0; i < 1000; i++) {
        ...
        EXPECT_EQ(bplustree->StructuralIntegrityVerification(*keys_copy.begin(),
                                                             *keys_copy.rbegin(),
                                                             &keys_copy,
                                                             bplustree->GetRoot()),
                  true);
...
TEST_F(BPlusTreeTests, MultiThreadedInsertTest) {
     ...
     */
    std::function<bool(const int64_t)> predicate = [](const int64_t slot) -> bool {
        return false;
    };
    ...
    for (uint32_t i = 0; i < num_threads_; i++) {
        thread_pool_.SubmitTask([i, &workload] {
            workload(i);
        });
    }
...
TEST_F(BPlusTreeTests, MultiThreadedDeleteTest) {
     ...
     */
    auto predicate = [](const int64_t slot) -> bool {
        return false;
    };
    const int key_num = 1000 * 1000;
