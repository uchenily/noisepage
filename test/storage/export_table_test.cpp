...
#define EXPORT_TEST_PYSCRIPT                                                                                           \
    ...
    "pa_table = pa.ipc.open_stream('" EXPORT_TEST_EXPORT_TABLE_NAME "').read_next_batch()\n"                           \
...
struct ExportTableTest : public ::noisepage::TerrierTest {
    ...
    void CheckContent(std::ifstream                                                         &csv_file,
                ...
                for (int j = 0; j < read_row->NumColumns(); j++) {
                    ...
                    if (data == nullptr) {
                        ...
                        if (layout.IsVarlen(col_id)) {
                            ...
                            EXPECT_EQ(content_len, bytes.size() - 2) << "Value of row " << i << " column " << j
                                                                     << " should have length " << content_len << "\n";
                            ...
                            for (int k = 0; k < static_cast<int>(content_len); ++k) {
                                EXPECT_EQ(bytes[k + 1], content[k])
                                    ...
                                    << static_cast<uint8_t>(content[k]) << " instead of "
                                    << static_cast<uint8_t>(bytes[k + 1]) << "\n";
                            }
                        } else {
                            ...
                            EXPECT_TRUE(std::fabs(1 - (std::stof(integer) + 1e-6) / (true_integer + 1e-6)) < 1e-6)
                                << "Value of row " << i << " column " << j << " should be " << true_integer
                                << " instead of " << integer << "\n";
                        }
                    }
                }
...
TEST_F(ExportTableTest, ExportDictionaryCompressedTableTest) {
    ...
    storage::DataTable table(common::ManagedPointer<storage::BlockStore>(&block_store_),
                             layout,
    ...
    if (::testing::Test::HasFailure()) {
        ...
        STORAGE_LOG_WARN(
            "Error happened, see detailed exported data file {}. You may use the same seed suffix to debug.",
            error_csv_file_name);
    } else {
        unlink(EXPORT_TEST_EXPORT_TABLE_NAME);
    }
