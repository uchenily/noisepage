...
public:
    ...
    template <typename Random>
    std::vector<int64_t>
    FillProjectedRowWithRandomCompactInts(const IndexMetadata &metadata, storage::ProjectedRow *pr, Random *generator) {
        std::uniform_int_distribution<int64_t> rng(std::numeric_limits<int64_t>::min(),
                                                   std::numeric_limits<int64_t>::max());
        const auto                            &key_schema = metadata.GetSchema();
        const auto                            &oid_offset_map = metadata.GetKeyOidToOffsetMap();
    ...
    void BasicOps(Index *const index) {
        ...
        columns.emplace_back("attribute",
     ...
     */
    template <typename Random>
    bool ModifyRandomColumn(const IndexMetadata   &metadata,
        ...
        if (coin(*generator)) {
            ...
            for (uint32_t i = 0; i < column; i++) {
                offset = static_cast<uint16_t>(offset
                                               + execution::sql::GetSqlTypeIdSize(key_schema.GetColumns()[i].Type()));
            }
    ...
    void SetGenericKeyFromString(const IndexMetadata &metadata,
 ...
 */
template <uint8_t KeySize>
bool CompactIntsFromProjectedRowEq(const IndexMetadata         &metadata,
                                   const storage::ProjectedRow &pr_A,
 ...
 */
template <uint8_t KeySize>
bool CompactIntsFromProjectedRowCmp(const IndexMetadata         &metadata,
                                    const storage::ProjectedRow &pr_A,
                                    const storage::ProjectedRow &pr_B) {
    auto key_a = CompactIntsKey<KeySize>();
    auto key_b = CompactIntsKey<KeySize>();
...
TEST_F(IndexKeyTests, GenericKeyInlineVarlenComparisons) {
    ...
    *reinterpret_cast<VarlenEntry *>(pr->AccessForceNotNull(0)) = data;
    ...
    data
        = VarlenEntry::CreateInline(reinterpret_cast<byte *>(johnathan), static_cast<uint32_t>(std::strlen(johnathan)));
    *reinterpret_cast<VarlenEntry *>(pr->AccessForceNotNull(0)) = data;
    key1.SetFromProjectedRow(*pr, metadata, 1);
...
TEST_F(IndexKeyTests, GenericKeyBuilderTest) {
    ...
    const std::vector<execution::sql::SqlTypeId> generic_key_types{execution::sql::SqlTypeId::Boolean,
                                                                   execution::sql::SqlTypeId::TinyInt,
                                                                   execution::sql::SqlTypeId::SmallInt,
                                                                   execution::sql::SqlTypeId::Integer,
                                                                   execution::sql::SqlTypeId::BigInt,
