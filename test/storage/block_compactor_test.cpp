...
public:
    explicit ProjectedRowDeepEqual(const storage::BlockLayout &layout)
        : layout_(layout) {}
...
public:
    explicit ProjectedRowDeepEqualHash(const storage::BlockLayout &layout)
        : layout_(layout) {}
...
TEST_F(BlockCompactorTest, CompactionTest) {
    ...
    for (uint32_t iteration = 0; iteration < repeat; iteration++) {
        ...
        txn_manager.Commit(txn,
                           ...
                           nullptr); // Commit: will be cleaned up by GC
        delete[] buffer;
        ...
        for (uint32_t i = 0; i < num_tuples; i++) {
            ...
            for (uint16_t offset = 0; offset < read_row->NumColumns(); offset++) {
                ...
                if (!varlen->IsInlined()) {
                    ...
                    EXPECT_EQ(std::memcmp(varlen->Content(),
                                          arrow_column.Values() + arrow_column.Offsets()[i],
                                          varlen->Size()),
                              0);
                }
            }
        ...
        for (storage::col_id_t varlen_col : layout.Varlens()) {
            ...
            for (uint32_t code = 1; code < arrow_column.OffsetsLength() - 1; code++) {
                std::string_view prev(
                    reinterpret_cast<char *>(arrow_column.Values() + arrow_column.Offsets()[code - 1]),
        ...
        for (uint32_t i = 0; i < num_tuples; i++) {
            ...
            for (uint16_t offset = 0; offset < read_row->NumColumns(); offset++) {
                ...
                if (!varlen->IsInlined()) {
                    ...
                    EXPECT_EQ(std::memcmp(varlen->Content(),
                                          arrow_column.Values() + arrow_column.Offsets()[dict_code],
                                          varlen->Size()),
                              0);
                }
            }
