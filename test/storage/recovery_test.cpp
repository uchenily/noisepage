...
protected:
    ...
    void DropDatabase(transaction::TransactionContext         *txn,
                      common::ManagedPointer<catalog::Catalog> catalog,
    ...
    catalog::table_oid_t CreateTable(transaction::TransactionContext                 *txn,
        ...
        auto col = catalog::Schema::Column("attribute",
                                           execution::sql::SqlTypeId::Integer,
                                           false,
    ...
    void DropTable(transaction::TransactionContext                 *txn,
                   common::ManagedPointer<catalog::DatabaseCatalog> db_catalog,
    ...
    catalog::index_oid_t CreateIndex(transaction::TransactionContext                 *txn,
        ...
        auto index_oid
            = db_catalog->CreateIndex(common::ManagedPointer(txn), ns_oid, index_name, table_oid, index_schema);
    ...
    void DropIndex(transaction::TransactionContext                 *txn,
                   common::ManagedPointer<catalog::DatabaseCatalog> db_catalog,
    ...
    void DropNamespace(transaction::TransactionContext                 *txn,
                       common::ManagedPointer<catalog::DatabaseCatalog> db_catalog,
                       const catalog::namespace_oid_t                   ns_oid) {
        EXPECT_TRUE(db_catalog->DeleteNamespace(common::ManagedPointer(txn), ns_oid));
    }

    storage::RedoBuffer &GetRedoBuffer(transaction::TransactionContext *txn) {
        return txn->redo_buffer_;
    }
    ...
    void ShutdownAndRestartSystem() {
        ...
        db_main_->GetTransactionLayer()->GetDeferredActionManager()->FullyPerformGC(
            db_main_->GetStorageLayer()->GetGarbageCollector(),
            log_manager_);
    ...
    void RunTest(const LargeSqlTableTestConfiguration &config) {
        ...
        for (auto &database : tested->GetTables()) {
            ...
            for (auto &table_oid : database.second) {
                ...
                auto original_sql_table
                    = catalog_->GetDatabaseCatalog(common::ManagedPointer(original_txn), database_oid)
                ...
                EXPECT_TRUE(StorageTestUtil::SqlTableEqualDeep(original_sql_table->table_.layout_,
                                                               original_sql_table,
                                                               recovered_sql_table,
                                                               tested->GetTupleSlotsForTable(database_oid, table_oid),
                                                               recovery_manager.tuple_slot_map_,
                                                               txn_manager_.Get(),
        ...
        // the table can't be freed until after all GC on it is guaranteed to be done. The easy way to do that is to use
        // a DeferredAction
...
TEST_F(RecoveryTests, UnrecoverableTransactionsTest) {
    ...
    // prevent it from doing so since it never finishes, and by deleting the gc thread during "shutdown", the previous
    // txn will never get cleaned up.
    db_main_->GetGarbageCollectorThread()->StopGC();
    db_main_->GetGarbageCollectorThread()->StartGC();

    // Create a ton of databases to make a transaction flush redo record buffers. In theory we could do any change, but
    // a create database call will generate a ton of records. Importantly, we don't commit the txn.
    ...
    // Simulate the system "crashing" (i.e. unrecoverable_txn has not been committed). We guarantee persist of log
    // records because the purpose of the test is how we handle these unrecoverable records showing up during recovery
    ShutdownAndRestartSystem();
...
TEST_F(RecoveryTests, DoubleRecoveryTest) {
    ...
    for (auto &database : tested->GetTables()) {
        ...
        for (auto &table_oid : database.second) {
            ...
            EXPECT_TRUE(StorageTestUtil::SqlTableEqualDeep(GetBlockLayout(original_sql_table),
                                                           original_sql_table,
                                                           recovered_sql_table,
                                                           tested->GetTupleSlotsForTable(database_oid, table_oid),
                                                           recovery_manager.tuple_slot_map_,
    ...
    // Contrary to other tests, we clean up the recovery catalog and gc thread here because the secondary_log_manager is
    // a local object. Setting the appropriate variables to nullptr will allow the TearDown code to run normally.
    ...
    RecoveryManager secondary_recovery_manager(common::ManagedPointer<AbstractLogProvider>(&secondary_log_provider),
                                               secondary_recovery_catalog,
                                               secondary_recovery_txn_manager,
                                               secondary_recovery_deferred_action_manager,
                                               DISABLED,
                                               secondary_recovery_thread_registry,
                                               secondary_recovery_block_store);
