...
TEST_F(ProjectedRowTests, Nulls) {
    ...
    for (uint32_t iteration = 0; iteration < num_iterations; ++iteration) {
        ...
        // generator a binary vector and set nulls according to binary vector. For null attributes, we set value to be
        // 0.
...
TEST_F(ProjectedRowTests, CopyProjectedRowLayout) {
    ...
    for (uint32_t iteration = 0; iteration < num_iterations; iteration++) {
        ...
        for (uint16_t i = 0; i < row->NumColumns(); i++) {
            ...
            uintptr_t offset
                = reinterpret_cast<uintptr_t>(row->AccessForceNotNull(i)) - reinterpret_cast<uintptr_t>(row);
