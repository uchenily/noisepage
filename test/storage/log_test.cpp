...
protected:
    ...
    void SetUp() override {
        ...
        db_main_ = noisepage::DBMain::Builder()
                       .SetWalFilePath(LOG_TEST_LOG_FILE_NAME)
                       .SetUseLogging(true)
                       .SetUseGC(true)
                       .Build();
    ...
    void TearDown() override {
        log_manager_->Start(); // all of the tests stop the LogManager, but DBMain's teardown logic expects it to still
                               // be running, so we'll just restart it
        // Delete log file
        unlink(LOG_TEST_LOG_FILE_NAME);
    }
    ...
    storage::LogRecord *ReadNextRecord(storage::BufferedLogReader *in) {
        ...
        if (record_type == storage::LogRecordType::COMMIT) {
            ...
            // is_read_only argument is set to false, because we do not write out a commit record for a transaction if
            // it is not read-only.
            return storage::CommitRecord::Initialize(buf,
                                                     txn_begin,
                                                     txn_commit,
                                                     nullptr,
                                                     nullptr,
        ...
        NOISEPAGE_ASSERT(record_type == storage::LogRecordType::REDO,
                         "Unknown record type during test deserialization");
        ...
        // Get an in memory copy of the record's null bitmap. Note: this is used to guide how the rest of the log file
        // is read in. It doesn't populate the delta's bitmap yet. This will happen naturally as we proceed
        // column-by-column.
...
TEST_F(WriteAheadLoggingTests, LargeLogTest) {
    ...
    for (auto *txn : result.first)
        txns_map[txn->BeginTimestamp()] = txn;
