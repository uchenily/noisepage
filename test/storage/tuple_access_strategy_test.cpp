...
public:
    ...
    std::pair<const storage::TupleSlot, storage::ProjectedRow *> &
    TryInsertFakeTuple(const storage::BlockLayout                                      &layout,
...
TEST_F(TupleAccessStrategyTests, MemorySafety) {
    ...
    for (uint32_t i = 0; i < repeat; i++) {
        ...
        StorageTestUtil::CheckInBounds(StorageTestUtil::IncrementByBytes(lower_bound, last_column_size),
                                       lower_bound,
                                       upper_bound);
    }
}
...
TEST_F(TupleAccessStrategyTests, Alignment) {
    ...
    for (uint32_t i = 0; i < repeat; i++) {
        ...
        for (uint16_t j = 0; j < layout.NumColumns(); j++) {
            ...
            auto              alignment
                = layout.AttrSize(col_id) > 8 ? 8 : layout.AttrSize(col_id); // no need to align above 8 bytes
