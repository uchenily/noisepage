    ...
    for (uint32_t iteration = 0; iteration < num_iterations; ++iteration) {
        ...
        for (uint32_t i = 0; i < chain_size; ++i) {
            ...
            storage::BlockLayout layout
                = StorageTestUtil::RandomLayoutNoVarlen(common::Constants::MAX_COL, &generator_);
            ...
            // get data table
            storage::DataTable data_table(common::ManagedPointer<storage::BlockStore>(&block_store_),
                                          layout,
            ...
            const std::vector<storage::col_id_t> col_ids
                = StorageTestUtil::ProjectionListRandomColumns(layout, &generator_);
        ...
        for (auto record : record_list)
            delete[] reinterpret_cast<byte *>(record);
    }
}
...
TEST_F(DeltaRecordTests, UndoGetProjectedRow) {
    ...
    for (uint32_t iteration = 0; iteration < num_iterations; ++iteration) {
        ...
        storage::UndoRecord     *record
            = storage::UndoRecord::InitializeUpdate(record_buffer, time, slot, &data_table, *redo);
