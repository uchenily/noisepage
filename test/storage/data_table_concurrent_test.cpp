...
public:
    FakeTransaction(const storage::BlockLayout       &layout,
                    ...
                    const transaction::timestamp_t    start_time,
                    const transaction::timestamp_t    txn_id,
...
TEST_F(DataTableConcurrentTests, ConcurrentInsert) {
    ...
    for (uint32_t iteration = 0; iteration < num_iterations; iteration++) {
        ...
        storage::DataTable   tested(common::ManagedPointer<storage::BlockStore>(&block_store_),
                                  layout,
        ...
        for (auto &fake_txn : fake_txns) {
            for (auto slot : fake_txn->InsertedTuples()) {
                ...
                EXPECT_TRUE(
                    StorageTestUtil::ProjectionListEqualShallow(layout, fake_txn->GetReferenceTuple(slot), select_row));
            }
        }
...
TEST_F(DataTableConcurrentTests, ConcurrentUpdateOneWriterWins) {
    ...
    for (uint32_t iteration = 0; iteration < num_iterations; iteration++) {
        ...
        storage::DataTable   tested(common::ManagedPointer<storage::BlockStore>(&block_store_),
                                  layout,
                                  storage::layout_version_t(0));
        FakeTransaction      insert_txn(layout,
                                   &tested,
                                   null_ratio_(generator_),
                                   transaction::timestamp_t(0),
                                   transaction::timestamp_t(1),
                                   &buffer_pool_);
        ...
        for (uint64_t thread = 0; thread < num_threads; thread++)
            ...
            fake_txns.emplace_back(
                std::make_unique<FakeTransaction>(layout,
                                                  &tested,
                                                  null_ratio_(generator_),
                                                  transaction::timestamp_t(2),
                                                  transaction::timestamp_t(static_cast<uint64_t>(-thread - 1)),
                                                  &buffer_pool_));
