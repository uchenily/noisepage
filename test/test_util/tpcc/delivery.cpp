...
// 2.7.4
bool Delivery::Execute(transaction::TransactionManager *const txn_manager,
                       Database *const                        db,
                       Worker *const                          worker,
    ...
    for (int8_t d_id = 1; d_id <= 10; d_id++) {
        ...
        *reinterpret_cast<int32_t *>(new_order_key_hi->AccessForceNotNull(no_o_id_key_pr_offset_))
            = 10000000; // max O_ID

        db->new_order_primary_index_->ScanAscending(*txn,
                                                    storage::index::ScanType::Closed,
                                                    3,
                                                    new_order_key_lo,
                                                    new_order_key_hi,
                                                    1,
        ...
        *reinterpret_cast<int8_t *>(order_line_key_hi->AccessForceNotNull(ol_number_key_pr_offset_))
            = 15; // max OL_NUMBER

        index_scan_results.clear();
        db->order_line_primary_index_->ScanAscending(*txn,
                                                     storage::index::ScanType::Closed,
                                                     4,
                                                     order_line_key_lo,
                                                     order_line_key_hi,
        ...
        for (const auto &order_line_slot : index_scan_results) {
            ...
            NOISEPAGE_ASSERT(ol_amount >= 0.01 && ol_amount <= 9999.99,
                             "Invalid ol_amount read from the Order Line table.");
        ...
        *reinterpret_cast<int32_t *>(customer_key->AccessForceNotNull(c_id_key_pr_offset_)) = o_c_id;
        ...
        auto *const customer_update_redo
            = txn->StageWrite(db->db_oid_, db->customer_table_oid_, customer_pr_initializer_);
