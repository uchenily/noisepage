...
Database *Builder::Build(const storage::index::IndexType index_type) {
    ...
    auto new_order_primary_index_schema = Schemas::BuildNewOrderPrimaryIndexSchema(new_order_schema,
    ...
    auto order_secondary_index_schema = Schemas::BuildOrderSecondaryIndexSchema(order_schema,
                                                                                storage::index::IndexType::BPLUSTREE,
                                                                                db_oid,
                                                                                order_table_oid);
    auto order_line_primary_index_schema
        = Schemas::BuildOrderLinePrimaryIndexSchema(order_line_schema,
                                                    storage::index::IndexType::BPLUSTREE,
                                                    db_oid,
    ...
    const catalog::index_oid_t warehouse_primary_index_oid = accessor->CreateIndex(accessor->GetDefaultNamespace(),
                                                                                   warehouse_table_oid,
                                                                                   "WAREHOUSE PRIMARY",
                                                                                   warehouse_primary_index_schema);
    const catalog::index_oid_t district_primary_index_oid = accessor->CreateIndex(accessor->GetDefaultNamespace(),
                                                                                  district_table_oid,
                                                                                  "DISTRICT PRIMARY",
                                                                                  district_primary_index_schema);
    const catalog::index_oid_t customer_primary_index_oid = accessor->CreateIndex(accessor->GetDefaultNamespace(),
                                                                                  customer_table_oid,
                                                                                  "CUSTOMER PRIMARY",
                                                                                  customer_primary_index_schema);
    const catalog::index_oid_t customer_secondary_index_oid = accessor->CreateIndex(accessor->GetDefaultNamespace(),
                                                                                    customer_table_oid,
                                                                                    "WAREHOUSE SECONDARY",
                                                                                    customer_secondary_index_schema);
    const catalog::index_oid_t new_order_primary_index_oid = accessor->CreateIndex(accessor->GetDefaultNamespace(),
                                                                                   new_order_table_oid,
                                                                                   "NEW ORDER PRIMARY",
                                                                                   new_order_primary_index_schema);
    const catalog::index_oid_t order_primary_index_oid = accessor->CreateIndex(accessor->GetDefaultNamespace(),
                                                                               order_table_oid,
                                                                               "ORDER PRIMARY",
                                                                               order_primary_index_schema);
    const catalog::index_oid_t order_secondary_index_oid = accessor->CreateIndex(accessor->GetDefaultNamespace(),
                                                                                 order_table_oid,
                                                                                 "ORDER SECONDARY",
                                                                                 order_secondary_index_schema);
    const catalog::index_oid_t order_line_primary_index_oid = accessor->CreateIndex(accessor->GetDefaultNamespace(),
                                                                                    order_line_table_oid,
                                                                                    "ORDER LINE PRIMARY",
                                                                                    order_line_primary_index_schema);
    const catalog::index_oid_t item_primary_index_oid = accessor->CreateIndex(accessor->GetDefaultNamespace(),
                                                                              item_table_oid,
                                                                              "ITEM PRIMARY",
                                                                              item_primary_index_schema);
    const catalog::index_oid_t stock_primary_index_oid = accessor->CreateIndex(accessor->GetDefaultNamespace(),
                                                                               stock_table_oid,
                                                                               "STOCK PRIMARY",
    ...
    if (index_type == storage::index::IndexType::HASHMAP) {
        ...
        NOISEPAGE_ASSERT(district_index->Type() == storage::index::IndexType::HASHMAP,
                         "Constructed the wrong index type.");
        NOISEPAGE_ASSERT(customer_index->Type() == storage::index::IndexType::HASHMAP,
                         "Constructed the wrong index type.");
        ...
        NOISEPAGE_ASSERT(new_order_index->Type() == storage::index::IndexType::BPLUSTREE,
                         "Constructed the wrong index type.");
        NOISEPAGE_ASSERT(order_index->Type() == storage::index::IndexType::HASHMAP,
                         "Constructed the wrong index type.");
        ...
        NOISEPAGE_ASSERT(stock_index->Type() == storage::index::IndexType::HASHMAP,
                         "Constructed the wrong index type.");
        ...
        NOISEPAGE_ASSERT(new_order_index->Type() == storage::index::IndexType::BPLUSTREE,
                         "Constructed the wrong index type.");
        NOISEPAGE_ASSERT(order_index->Type() == storage::index::IndexType::BPLUSTREE,
                         "Constructed the wrong index type.");
        NOISEPAGE_ASSERT(order_secondary_index->Type() == storage::index::IndexType::BPLUSTREE,
                         "Constructed the wrong index type.");
        NOISEPAGE_ASSERT(order_line_index->Type() == storage::index::IndexType::BPLUSTREE,
                         "Constructed the wrong index type.");
        NOISEPAGE_ASSERT(item_index->Type() == storage::index::IndexType::BPLUSTREE,
                         "Constructed the wrong index type.");
        NOISEPAGE_ASSERT(stock_index->Type() == storage::index::IndexType::BPLUSTREE,
                         "Constructed the wrong index type.");
    ...
    return new Database(item_schema,
                        warehouse_schema,
                        stock_schema,
                        district_schema,
                        customer_schema,
                        ...
                        district_index,
                        customer_index,
                        customer_secondary_index,
                        new_order_index,
                        order_index,
                        order_secondary_index,
                        order_line_index,
                        ...
                        warehouse_primary_index_oid,
                        district_primary_index_oid,
                        customer_primary_index_oid,
                        customer_secondary_index_oid,
                        new_order_primary_index_oid,
                        order_primary_index_oid,
                        order_secondary_index_oid,
                        order_line_primary_index_oid,
                        item_primary_index_oid,
