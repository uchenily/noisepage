...
// 2.8.2
bool StockLevel::Execute(transaction::TransactionManager *const txn_manager,
                         Database *const                        db,
                         Worker *const                          worker,
    ...
    *reinterpret_cast<int8_t *>(order_line_key_hi->AccessForceNotNull(ol_number_key_pr_offset_)) = 15; // max OL_NUMBER
    ...
    for (const auto &order_line_tuple_slot : index_scan_results) {
        ...
        select_result = db->order_line_table_->Select(common::ManagedPointer(txn),
                                                      order_line_tuple_slot,
                                                      order_line_select_tuple);
        ...
        if (item_counts.count(ol_i_id) > 0)
            continue; // don't look up items we've already checked
