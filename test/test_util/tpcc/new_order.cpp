...
bool NewOrder::Execute(transaction::TransactionManager *const txn_manager,
    ...
    *reinterpret_cast<uint64_t *>(order_insert_tuple->AccessForceNotNull(o_entry_d_insert_pr_offset_))
        = args.o_entry_d_;
    ...
    *reinterpret_cast<int32_t *>(order_secondary_key->AccessForceNotNull(o_c_id_secondary_key_pr_offset_)) = args.c_id_;
    ...
    for (const auto &item : args.items_) {
        ...
        *reinterpret_cast<int32_t *>(item_key->AccessForceNotNull(0)) = item.ol_i_id_;
        ...
        const auto i_price
            = *reinterpret_cast<double *>(item_select_tuple->AccessWithNullCheck(i_price_select_pr_offset_));
        const auto i_data = *reinterpret_cast<storage::VarlenEntry *>(
            item_select_tuple->AccessWithNullCheck(i_data_select_pr_offset_));
        ...
        *reinterpret_cast<int8_t *>(stock_key->AccessForceNotNull(s_w_id_key_pr_offset_)) = item.ol_supply_w_id_;
        ...
        select_result
            = db->stock_table_->Select(common::ManagedPointer(txn), index_scan_results[0], stock_select_tuple);
        NOISEPAGE_ASSERT(select_result, "Stock table doesn't change (no new entries). All lookups should succeed.");
        const auto s_quantity = *reinterpret_cast<int16_t *>(stock_select_tuple->AccessWithNullCheck(
            stock_select_pr_offsets_[args.d_id_ - 1].s_quantity_select_pr_offset_));
        const auto s_dist_xx = *reinterpret_cast<storage::VarlenEntry *>(stock_select_tuple->AccessWithNullCheck(
            stock_select_pr_offsets_[args.d_id_ - 1].s_dist_xx_select_pr_offset_));
        ...
        auto *const stock_update_redo
            = txn->StageWrite(db->db_oid_, db->stock_table_oid_, stock_update_pr_initializer_);
        ...
        *reinterpret_cast<int32_t *>(order_line_insert_tuple->AccessForceNotNull(ol_o_id_insert_pr_offset_))
            = d_next_o_id;
        *reinterpret_cast<int8_t *>(order_line_insert_tuple->AccessForceNotNull(ol_d_id_insert_pr_offset_))
            = args.d_id_;
        *reinterpret_cast<int8_t *>(order_line_insert_tuple->AccessForceNotNull(ol_w_id_insert_pr_offset_))
            = args.w_id_;
        *reinterpret_cast<int8_t *>(order_line_insert_tuple->AccessForceNotNull(ol_number_insert_pr_offset_))
            = ol_number;
        ...
        *reinterpret_cast<double *>(order_line_insert_tuple->AccessForceNotNull(ol_amount_insert_pr_offset_))
        ...
        if (s_dist_xx.Size() <= storage::VarlenEntry::InlineThreshold()) {
            *reinterpret_cast<storage::VarlenEntry *>(
                order_line_insert_tuple->AccessForceNotNull(ol_dist_info_insert_pr_offset_))
                = s_dist_xx;
