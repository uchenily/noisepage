...
// 2.5.2
bool Payment::Execute(transaction::TransactionManager *const txn_manager,
                      Database *const                        db,
                      Worker *const                          worker,
    ...
    *reinterpret_cast<int8_t *>(warehouse_key->AccessForceNotNull(0)) = args.w_id_;
    ...
    const auto w_name = *reinterpret_cast<storage::VarlenEntry *>(
        warehouse_select_tuple->AccessWithNullCheck(w_name_select_pr_offset_));
    const auto w_ytd
        = *reinterpret_cast<double *>(warehouse_select_tuple->AccessWithNullCheck(w_ytd_select_pr_offset_));
    ...
    *reinterpret_cast<int8_t *>(district_key->AccessForceNotNull(d_w_id_key_pr_offset_)) = args.w_id_;
    ...
    const auto d_name = *reinterpret_cast<storage::VarlenEntry *>(
        district_select_tuple->AccessWithNullCheck(d_name_select_pr_offset_));
    ...
    *reinterpret_cast<double *>(district_update_redo->Delta()->AccessForceNotNull(0)) = d_ytd + args.h_amount_;
    ...
    if (!args.use_c_last_) {
        ...
        *reinterpret_cast<int8_t *>(customer_key->AccessForceNotNull(c_w_id_key_pr_offset_)) = args.w_id_;
        ...
        auto *const customer_name_key
            = customer_name_key_pr_initializer.InitializeRow(worker->customer_name_key_buffer_);
        ...
        if (index_scan_results.size() > 1) {
            ...
            for (const auto &tuple_slot : index_scan_results) {
                ...
                sorted_index_scan_results.emplace(
                    std::string(reinterpret_cast<const char *const>(c_first.Content()), c_first.Size()),
                    tuple_slot);
            }
    ...
    const auto c_credit = *reinterpret_cast<storage::VarlenEntry *>(
        customer_select_tuple->AccessWithNullCheck(c_credit_select_pr_offset_));
    const auto c_data = *reinterpret_cast<storage::VarlenEntry *>(
        customer_select_tuple->AccessWithNullCheck(c_data_select_pr_offset_));
