...
void WorkloadCached::LoadTPCCQueries(const std::vector<std::string> &txn_names) {
    ...
    for (auto &txn_name : txn_names) {
        ...
        for (auto &query : sqls_.find(txn_name)->second) {
            ...
            auto exec_query = std::make_unique<execution::compiler::ExecutableQuery>(*plan_node,
...
void WorkloadCached::Execute(int8_t                       worker_id,
                             uint32_t                     num_precomputed_txns_per_worker,
...
void WorkloadCached::InitNewOrder() {
    ...
    vec.emplace_back("INSERT INTO \"ORDER\" (O_ID, O_D_ID, O_W_ID, O_C_ID, O_ENTRY_D, O_OL_CNT, O_ALL_LOCAL) VALUES "
                     "(1,2,3,4,0,6,7)");
...
void WorkloadCached::InitOrderStatus() {
    ...
    std::vector<std::string> &vec = curr.first->second;
    vec.emplace_back("SELECT O_ID, O_CARRIER_ID, O_ENTRY_D FROM \"ORDER\" WHERE O_W_ID=1 AND O_D_ID=2 AND O_C_ID=3 "
                     "ORDER BY O_ID DESC "
                     "LIMIT 1");
    vec.emplace_back(
