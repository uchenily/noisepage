...
SimulationResult LargeDataTableTestObject::SimulateOltp(uint32_t num_transactions, uint32_t num_concurrent_txns) {
    ...
    for (RandomDataTableTransaction *txn : txns)
        (txn->aborted_ ? aborted : committed).push_back(txn);
...
void LargeDataTableTestObject::CheckReadsCorrect(std::vector<RandomDataTableTransaction *> *commits) {
    ...
    for (RandomDataTableTransaction *txn : *commits)
...
void LargeDataTableTestObject::UpdateSnapshot(RandomDataTableTransaction *txn,
                                              TableSnapshot              *curr,
...
VersionedSnapshots
LargeDataTableTestObject::ReconstructVersionedTable(std::vector<RandomDataTableTransaction *> *txns) {
