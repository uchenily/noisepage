...
Workload::Workload(common::ManagedPointer<DBMain> db_main,
    ...
    auto exec_ctx = execution::exec::ExecutionContext(db_oid_,
...
void Workload::GenerateTables(execution::exec::ExecutionContext *exec_ctx,
                              const std::string                 &dir_name,
                              enum BenchmarkType                 type) {
    // TPCH table names;
    static const std::vector<std::string>
        tpch_tables{"part", "supplier", "partsupp", "customer", "orders", "lineitem", "nation", "region"};
    ...
    for ([[maybe_unused]] const auto &table_name : *tables) {
        [[maybe_unused]] auto table_dir = dir_name + table_name;
        [[maybe_unused]] auto num_rows
            = table_reader.ReadTable(dir_name + table_name + ".schema", table_dir.append(kind));
...
void Workload::Execute(int8_t                       worker_id,
                       uint64_t                     execution_us_per_worker,
    ...
    for (uint32_t i = 0; i < total_query_num; ++i)
        index[i] = i;
