...
void RandomSqlTableTransaction::RandomUpdate(Random *generator) {
    ...
    storage::TupleSlot updated;
    {
        ...
        if (sql_table_metadata->inserted_tuples_.empty())
            return;
    ...
    // will serialize the txns by getting the tuple and quickly doing the operation on the tuple immedietly after.
    // Adding an expensive call (Like GetTable) will help in having the OS interleave the threads more.
    auto sql_table_ptr = test_object_->catalog_->GetDatabaseCatalog(common::ManagedPointer(txn_), database_oid)
                             ->GetTable(common::ManagedPointer(txn_), table_oid);
        ...
        for (const auto &database : inserted_tuples_) {
            for (const auto &table : database.second) {
                auto &metadata = test_object_->tables_[database.first][table.first];
                {
                    ...
                    metadata->inserted_tuples_.insert(metadata->inserted_tuples_.end(),
                                                      table.second.begin(),
                                                      table.second.end());
                }
            }
...
LargeSqlTableTestObject::LargeSqlTableTestObject(const LargeSqlTableTestConfiguration &config,
    ...
    PopulateInitialTables(config.num_databases_,
                          config.num_tables_,
                          config.max_columns_,
                          config.initial_table_size_,
                          config.varlen_allowed_,
                          block_store,
                          generator_);
}
...
void LargeSqlTableTestObject::SimulateOneTransaction(noisepage::RandomSqlTableTransaction *txn, uint32_t txn_id) {
    ...
    auto insert = [&] {
        txn->RandomInsert(&thread_generator);
    };
    auto update = [&] {
        txn->RandomUpdate(&thread_generator);
    };
    auto select = [&] {
        txn->RandomSelect(&thread_generator);
    };
    auto remove = [&] {
        txn->RandomDelete(&thread_generator);
    };

    RandomTestUtil::InvokeWorkloadWithDistribution({insert, update, select, remove},
                                                   insert_update_select_delete_ratio_,
                                                   &thread_generator,
                                                   txn_length_);
    txn->Finish();
}
...
template <class Random>
void LargeSqlTableTestObject::PopulateInitialTables(uint16_t             num_databases,
                                                    uint16_t             num_tables,
    ...
    for (uint16_t db_idx = 0; db_idx < num_databases; db_idx++) {
        ...
        for (uint16_t table_idx = 0; table_idx < num_tables; table_idx++) {
            ...
            auto  table_oid = db_catalog_ptr->CreateTable(common::ManagedPointer(initial_txn_),
                                                         namespace_oid,
                                                         "table" + std::to_string(table_idx),
                                                         *schema);
