...
SSBQuery::SSBMakeExecutableQ1Part1(const std::unique_ptr<catalog::CatalogAccessor> &accessor,
    ...
    {
        auto lo_orderdate
            = expr_maker.CVE(lo_schema.GetColumn("lo_orderdate").Oid(), execution::sql::SqlTypeId::Integer);
        ...
        std::vector<catalog::col_oid_t> lo_col_oids = {lo_schema.GetColumn("lo_quantity").Oid(),
                                                       lo_schema.GetColumn("lo_discount").Oid(),
                                                       lo_schema.GetColumn("lo_extendedprice").Oid(),
                                                       lo_schema.GetColumn("lo_orderdate").Oid()};
...
SSBQuery::SSBMakeExecutableQ1Part2(const std::unique_ptr<catalog::CatalogAccessor> &accessor,
    ...
    execution::compiler::test::OutputSchemaHelper d_seq_scan_out{0, &expr_maker};
    {
        ...
        std::vector<catalog::col_oid_t> d_oids
            = {d_schema.GetColumn("d_datekey").Oid(), d_schema.GetColumn("d_yearmonthnum").Oid()};
...
SSBQuery::SSBMakeExecutableQ2Part1(const std::unique_ptr<catalog::CatalogAccessor> &accessor,
    ...
    execution::compiler::test::OutputSchemaHelper s_seq_scan_out{0, &expr_maker};
    {
        ...
        std::vector<catalog::col_oid_t> s_oids
            = {s_schema.GetColumn("s_suppkey").Oid(), s_schema.GetColumn("s_region").Oid()};
    ...
    execution::compiler::test::OutputSchemaHelper lo_seq_scan_out{1, &expr_maker};
    {
        ...
        std::vector<catalog::col_oid_t> lo_col_oids = {lo_schema.GetColumn("lo_revenue").Oid(),
                                                       lo_schema.GetColumn("lo_suppkey").Oid(),
                                                       lo_schema.GetColumn("lo_partkey").Oid(),
                                                       lo_schema.GetColumn("lo_orderdate").Oid()};
        // Make output schema.
        lo_seq_scan_out.AddOutput("lo_orderdate", lo_orderdate);
...
SSBQuery::SSBMakeExecutableQ2Part2(const std::unique_ptr<catalog::CatalogAccessor> &accessor,
    ...
    execution::compiler::test::OutputSchemaHelper p_seq_scan_out{0, &expr_maker};
    {
        ...
        std::vector<catalog::col_oid_t> p_oids
            = {p_schema.GetColumn("p_partkey").Oid(), p_schema.GetColumn("p_brand1").Oid()};
        // Make the predicate: p_brand1 between 'MFGR#2221' and 'MFGR#2228'
        auto predicate = expr_maker.ComparisonBetween(p_brand1,
                                                      expr_maker.Constant("MFGR#2221"),
...
SSBQuery::SSBMakeExecutableQ3Part1(const std::unique_ptr<catalog::CatalogAccessor> &accessor,
    ...
    execution::compiler::test::OutputSchemaHelper lo_seq_scan_out{1, &expr_maker};
    {
        ...
        std::vector<catalog::col_oid_t> lo_col_oids = {lo_schema.GetColumn("lo_revenue").Oid(),
                                                       lo_schema.GetColumn("lo_suppkey").Oid(),
                                                       lo_schema.GetColumn("lo_custkey").Oid(),
                                                       lo_schema.GetColumn("lo_orderdate").Oid()};
        // Make output schema.
        lo_seq_scan_out.AddOutput("lo_orderdate", lo_orderdate);
...
SSBQuery::SSBMakeExecutableQ3Part2(const std::unique_ptr<catalog::CatalogAccessor> &accessor,
    ...
    execution::compiler::test::OutputSchemaHelper d_seq_scan_out{0, &expr_maker};
    {
        ...
        auto d_year = expr_maker.CVE(d_schema.GetColumn("d_year").Oid(), execution::sql::SqlTypeId::Integer);
        std::vector<catalog::col_oid_t> d_oids
            = {d_schema.GetColumn("d_datekey").Oid(), d_schema.GetColumn("d_year").Oid()}; // Make output schema.
        d_seq_scan_out.AddOutput("d_datekey", d_datekey);
        d_seq_scan_out.AddOutput("d_year", d_year);
        // Make predicate: d_year >= 1992 and d_year <= 1997
        auto predicate = expr_maker.ComparisonBetween(d_year, expr_maker.Constant(1992), expr_maker.Constant(1997));
...
SSBQuery::SSBMakeExecutableQ3Part3(const std::unique_ptr<catalog::CatalogAccessor> &accessor,
    ...
    execution::compiler::test::OutputSchemaHelper c_seq_scan_out{0, &expr_maker};
    {
        auto c_custkey = expr_maker.CVE(c_schema.GetColumn("c_custkey").Oid(), execution::sql::SqlTypeId::Integer);
        auto c_city = expr_maker.CVE(c_schema.GetColumn("c_city").Oid(), execution::sql::SqlTypeId::Varchar);
        std::vector<catalog::col_oid_t> c_oids
            = {c_schema.GetColumn("c_custkey").Oid(), c_schema.GetColumn("c_city").Oid()};
    ...
    execution::compiler::test::OutputSchemaHelper s_seq_scan_out{0, &expr_maker};
    {
        ...
        auto s_city = expr_maker.CVE(s_schema.GetColumn("s_city").Oid(), execution::sql::SqlTypeId::Varchar);
        std::vector<catalog::col_oid_t> s_oids
            = {s_schema.GetColumn("s_suppkey").Oid(), s_schema.GetColumn("s_city").Oid()};
    ...
    execution::compiler::test::OutputSchemaHelper lo_seq_scan_out{1, &expr_maker};
    {
        ...
        auto lo_suppkey = expr_maker.CVE(lo_schema.GetColumn("lo_suppkey").Oid(), execution::sql::SqlTypeId::Integer);
        auto lo_revenue = expr_maker.CVE(lo_schema.GetColumn("lo_revenue").Oid(), execution::sql::SqlTypeId::Integer);
        std::vector lo_col_oids = {lo_schema.GetColumn("lo_orderdate").Oid(),
                                   lo_schema.GetColumn("lo_custkey").Oid(),
                                   lo_schema.GetColumn("lo_suppkey").Oid(),
                                   lo_schema.GetColumn("lo_revenue").Oid()};
        // Make output schema.
        lo_seq_scan_out.AddOutput("lo_orderdate", lo_orderdate);
        lo_seq_scan_out.AddOutput("lo_custkey", lo_custkey);
        lo_seq_scan_out.AddOutput("lo_suppkey", lo_suppkey);
...
SSBQuery::SSBMakeExecutableQ3Part4(const std::unique_ptr<catalog::CatalogAccessor> &accessor,
    ...
    execution::compiler::test::OutputSchemaHelper d_seq_scan_out{0, &expr_maker};
    {
        ...
        std::vector d_oids = {d_schema.GetColumn("d_datekey").Oid(),
                              d_schema.GetColumn("d_year").Oid(),
...
SSBQuery::SSBMakeExecutableQ4Part1(const std::unique_ptr<catalog::CatalogAccessor> &accessor,
    ...
    execution::compiler::test::OutputSchemaHelper p_seq_scan_out{0, &expr_maker};
    {
        ...
        std::vector<catalog::col_oid_t> p_col_oids
            = {p_schema.GetColumn("p_partkey").Oid(), p_schema.GetColumn("p_mfgr").Oid()};
    ...
    execution::compiler::test::OutputSchemaHelper lo_seq_scan_out{1, &expr_maker};
    {
        ...
        auto lo_revenue = expr_maker.CVE(lo_schema.GetColumn("lo_revenue").Oid(), execution::sql::SqlTypeId::Integer);
        auto lo_supplycost
            = expr_maker.CVE(lo_schema.GetColumn("lo_supplycost").Oid(), execution::sql::SqlTypeId::Integer);
        std::vector lo_col_oids = {lo_schema.GetColumn("lo_orderdate").Oid(),
                                   lo_schema.GetColumn("lo_custkey").Oid(),
                                   lo_schema.GetColumn("lo_suppkey").Oid(),
                                   lo_schema.GetColumn("lo_partkey").Oid(),
                                   lo_schema.GetColumn("lo_revenue").Oid(),
                                   lo_schema.GetColumn("lo_supplycost").Oid()};
        // Make output schema.
        lo_seq_scan_out.AddOutput("lo_orderdate", lo_orderdate);
        lo_seq_scan_out.AddOutput("lo_custkey", lo_custkey);
        lo_seq_scan_out.AddOutput("lo_suppkey", lo_suppkey);
...
SSBQuery::SSBMakeExecutableQ4Part2(const std::unique_ptr<catalog::CatalogAccessor> &accessor,
    ...
    execution::compiler::test::OutputSchemaHelper c_seq_scan_out{0, &expr_maker};
    {
        ...
        std::vector<catalog::col_oid_t> c_col_oids
            = {c_schema.GetColumn("c_custkey").Oid(), c_schema.GetColumn("c_region").Oid()};
        // Make the predicate: c_region = 'AMERICA'
        auto predicate = expr_maker.ComparisonEq(c_region, expr_maker.Constant("AMERICA"));
        // Make output schema.
        c_seq_scan_out.AddOutput("c_custkey", c_custkey);
