...
protected:
    ...
    void CreateTable1() {
        ...
        auto real_column_stats
            = std::make_unique<ColumnStats<execution::sql::Real>>(db_oid_,
                                                                  table1_oid_,
                                                                  table1_real_col_oid_,
                                                                  10,
                                                                  8,
                                                                  8,
                                                                  std::make_unique<TopKElements<double>>(),
                                                                  std::make_unique<Histogram<double>>(),
                                                                  execution::sql::SqlTypeId::Double);

        // Values and frequencies.
        std::vector<std::pair<double, int>> real_vals = {
            {3, 2},
            {4, 2},
            {5, 2},
            {0, 1},
        ...
        // INTEGER type column.
        auto integet_col_stats
            = std::make_unique<ColumnStats<execution::sql::Integer>>(db_oid_,
                                                                     table1_oid_,
                                                                     table1_int_col_oid_,
                                                                     10,
                                                                     8,
                                                                     8,
                                                                     std::make_unique<TopKElements<int64_t>>(),
                                                                     std::make_unique<Histogram<int64_t>>(),
                                                                     execution::sql::SqlTypeId::Integer);

        // Values and frequencies.
        std::vector<std::pair<int64_t, int>> int_vals = {
            {3, 2},
            {4, 2},
            {5, 2},
            {0, 1},
    ...
    void CreateTable3() {
        ...
        for (int i = 1; i <= 1000; ++i)
            real_col_stats->GetTopK()->Increment(static_cast<float>(i), i);
        // Construct histogram.
    ...
    void CreateTable4() {
        // BOOLEAN column.
        auto bool_col_stats
            = std::make_unique<ColumnStats<execution::sql::BoolVal>>(db_oid_,
                                                                     table4_oid_,
                                                                     table4_bool_col_oid_,
                                                                     100,
                                                                     80,
                                                                     80,
                                                                     std::make_unique<TopKElements<bool>>(),
                                                                     std::make_unique<Histogram<bool>>(),
    ...
    void CreateTable5() {
        ...
        auto date_col_stats = std::make_unique<ColumnStats<execution::sql::DateVal>>(
            db_oid_,
            table5_oid_,
            table5_date_col_oid_,
            5,
            5,
            4,
            std::make_unique<TopKElements<execution::sql::Date>>(),
            std::make_unique<Histogram<execution::sql::Date>>(),
        ...
        auto timestamp_col_stats = std::make_unique<ColumnStats<execution::sql::TimestampVal>>(
            db_oid_,
            table5_oid_,
            table5_timestamp_col_oid_,
            5,
            5,
            4,
            std::make_unique<TopKElements<execution::sql::Timestamp>>(),
            std::make_unique<Histogram<execution::sql::Timestamp>>(),
    ...
    void CreateTable6() {
        ...
        auto varchar_col_stats = std::make_unique<ColumnStats<execution::sql::StringVal>>(
            db_oid_,
            table6_oid_,
            table6_varchar_col_oid_,
            2,
            2,
            2,
            std::make_unique<TopKElements<storage::VarlenEntry>>(),
            std::make_unique<Histogram<storage::VarlenEntry>>(),
    ...
    void CreateTable7() {
        // Empty column
        auto empty_col_stats
            = std::make_unique<ColumnStats<execution::sql::Integer>>(db_oid_,
                                                                     table7_oid_,
                                                                     table7_empty_col_oid_,
                                                                     0,
                                                                     0,
                                                                     0,
                                                                     std::make_unique<TopKElements<int64_t>>(),
...
TEST_F(SelectivityUtilTests, TestFloatLessThan) {
    ...
    std::unique_ptr<parser::ConstantValueExpression> const_value_expr_ptr
        = std::make_unique<parser::ConstantValueExpression>(execution::sql::SqlTypeId::Double,
                                                            execution::sql::Real(6.f));

    // Create a value condition to pass to SelectivityUtil.
    ValueCondition value_condition(table1_real_col_oid_,
                                   "",
    ...
    // TEST PART 2
    const_value_expr_ptr = std::make_unique<parser::ConstantValueExpression>(execution::sql::SqlTypeId::Double,
                                                                             execution::sql::Real(3.f));

    // Create a value condition to pass to SelectivityUtil.
    value_condition = ValueCondition(table1_real_col_oid_,
                                     "",
                                     parser::ExpressionType::COMPARE_LESS_THAN,
                                     std::move(const_value_expr_ptr));
...
TEST_F(SelectivityUtilTests, TestIntegerLessThan) {
    ...
    std::unique_ptr<parser::ConstantValueExpression> const_value_expr_ptr
        = std::make_unique<parser::ConstantValueExpression>(execution::sql::SqlTypeId::Integer,
                                                            execution::sql::Integer(6));
    // Create a value condition to pass to SelectivityUtil.
    ValueCondition value_condition(table1_int_col_oid_,
                                   "",
                                   parser::ExpressionType::COMPARE_LESS_THAN,
                                   std::move(const_value_expr_ptr));
    // Compute selectivity for col < 6.
...
TEST_F(SelectivityUtilTests, TestTinyIntLessThan) {
    // Create a constant value expression to pass to ValueCondition.
    std::unique_ptr<parser::ConstantValueExpression> const_value_expr_ptr
        = std::make_unique<parser::ConstantValueExpression>(execution::sql::SqlTypeId::TinyInt,
                                                            execution::sql::Integer(6));
    // Create a value condition to pass to SelectivityUtil.
    ValueCondition value_condition(table1_int_col_oid_,
                                   "",
...
TEST_F(SelectivityUtilTests, TestTinyIntegerEqual) {
    ...
    // Create a constant value expression to pass to ValueCondition.
    const_value_expr_ptr = std::make_unique<parser::ConstantValueExpression>(execution::sql::SqlTypeId::Integer,
                                                                             execution::sql::Integer(0));

    // Create a value condition to pass to SelectivityUtil.
    value_condition = ValueCondition(catalog::col_oid_t(3),
                                     "",
                                     parser::ExpressionType::COMPARE_EQUAL,
                                     std::move(const_value_expr_ptr));
...
TEST_F(SelectivityUtilTests, TestFloatEqual) {
    ...
    std::unique_ptr<parser::ConstantValueExpression> const_value_expr_ptr
        = std::make_unique<parser::ConstantValueExpression>(execution::sql::SqlTypeId::Double,
                                                            execution::sql::Real(1.0));
    // Create a value condition to pass to SelectivityUtil.
    ValueCondition value_condition(table2_real_col_oid_,
                                   "",
                                   parser::ExpressionType::COMPARE_EQUAL,
                                   std::move(const_value_expr_ptr));
    ...
    // Create a constant value expression to pass to ValueCondition.
    const_value_expr_ptr = std::make_unique<parser::ConstantValueExpression>(execution::sql::SqlTypeId::Double,
                                                                             execution::sql::Real(5.0));
    // Create a value condition to pass to SelectivityUtil.
    value_condition = ValueCondition(table2_real_col_oid_,
                                     "",
                                     parser::ExpressionType::COMPARE_EQUAL,
                                     std::move(const_value_expr_ptr));
    ...
    for (int i = 1; i <= 1000; ++i) {
        ...
        const_value_expr_ptr
            = std::make_unique<parser::ConstantValueExpression>(execution::sql::SqlTypeId::Double,
                                                                execution::sql::Real(static_cast<float>(i)));
        // Create a value condition to pass to SelectivityUtil.
        value_condition = ValueCondition(table3_real_col_oid_,
                                         "",
                                         parser::ExpressionType::COMPARE_EQUAL,
                                         std::move(const_value_expr_ptr));
...
TEST_F(SelectivityUtilTests, TestIntegerLessThanEqual) {
    ...
    // TEST PART 2
    const_value_expr_ptr = std::make_unique<parser::ConstantValueExpression>(execution::sql::SqlTypeId::Integer,
                                                                             execution::sql::Integer(3));

    // Create a value condition to pass to SelectivityUtil.
    value_condition = ValueCondition(table1_int_col_oid_,
                                     "",
                                     parser::ExpressionType::COMPARE_LESS_THAN_OR_EQUAL_TO,
                                     std::move(const_value_expr_ptr));
    // Compute selectivity for col <= 3.
...
TEST_F(SelectivityUtilTests, TestBoolEqual) {
    ...
    // Create a value condition to pass to SelectivityUtil.
    ValueCondition value_condition(table4_bool_col_oid_,
                                   "",
                                   parser::ExpressionType::COMPARE_EQUAL,
                                   std::move(const_value_expr_ptr));
    ...
    // Create a value condition to pass to SelectivityUtil.
    value_condition = ValueCondition(table4_bool_col_oid_,
                                     "",
                                     parser::ExpressionType::COMPARE_EQUAL,
                                     std::move(const_value_expr_ptr));
...
TEST_F(SelectivityUtilTests, TestDateEqual) {
    std::unique_ptr<parser::ConstantValueExpression> const_value_expr_ptr
        = std::make_unique<parser::ConstantValueExpression>(
            execution::sql::SqlTypeId::Date,
            execution::sql::DateVal(execution::sql::Date::FromYMD(1995, 8, 6)));
    ValueCondition value_condition(table5_date_col_oid_,
                                   "",
                                   parser::ExpressionType::COMPARE_EQUAL,
                                   std::move(const_value_expr_ptr));
...
TEST_F(SelectivityUtilTests, TestTimestampEqual) {
    ...
    ValueCondition value_condition(table5_timestamp_col_oid_,
                                   "",
                                   parser::ExpressionType::COMPARE_EQUAL,
                                   std::move(const_value_expr_ptr));
    double         res = SelectivityUtil::ComputeSelectivity(table_stats_5_, value_condition);
...
TEST_F(SelectivityUtilTests, TestVarcharEqual) {
    ...
    ValueCondition value_condition(catalog::col_oid_t(1),
                                   "",
                                   parser::ExpressionType::COMPARE_EQUAL,
                                   std::move(const_value_expr_ptr));
...
TEST_F(SelectivityUtilTests, TestEmptyEqual) {
    ...
    ValueCondition value_condition(table6_varchar_col_oid_,
                                   "",
                                   parser::ExpressionType::COMPARE_EQUAL,
                                   std::move(const_value_expr_ptr));
