...
public:
    ...
    void SetUp() final {
        ...
        task_manager_->AddTask(
            std::make_unique<task::TaskDDL>(catalog::db_oid_t(0), "CREATE TABLE t (a INT)", nullptr));
    ...
    void LoadInsert() {
        for (size_t i = 0; i < NUM_VALUE; i++) {
            ...
            task_manager_->AddTask(std::make_unique<task::TaskDML>(catalog::db_oid_t(0),
...
TEST_F(TaskManagerTests, InvalidStatements) {
    ...
    // Binding error
    task_manager_->AddTask(
        std::make_unique<task::TaskDDL>(catalog::db_oid_t(0), "CREATE INDEX idxx on tt (a)", nullptr));
    task_manager_->AddTask(std::make_unique<task::TaskDML>(catalog::db_oid_t(0),
